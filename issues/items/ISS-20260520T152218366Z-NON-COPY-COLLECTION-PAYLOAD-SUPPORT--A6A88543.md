...
- subagent 監査で、`InitializeEmpty` / `MoveOut` / `ReplaceReturnOld` が payload value-flow evidence を持たず state transition だけを行える残件を確認したため、[ISS-20260521T010410090Z-COLLECTION-SLOT-OWNER-TRANSFER-LIFEC-3C1056B2](./ISS-20260521T010410090Z-COLLECTION-SLOT-OWNER-TRANSFER-LIFEC-3C1056B2.md) を追加した。次段階では slot state transition と実際の payload consume / materialize proof を統合する。
