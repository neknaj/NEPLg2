...
`codegen_wasm::generate_wasm` は `Result<CodegenResult, Vec<Diagnostic>>` を返す形になり、unsupported signature、raw wasm parse error、missing return、unknown variable / function / intrinsic、string literal table mismatch、field selector mismatch、aggregate lower 非対応を `Diagnostic` として返します。
LLVM backend も `LlvmCodegenError::CodegenDiagnostic` 経由で raw body mismatch、unknown variable / function / function value、unsupported intrinsic、lowering invariant failure を diagnostic error として返します。
production codegen 経路に残る明示 `panic!` / `unwrap()` / `expect()` はなく、現在の `expect()` は `codegen_llvm.rs` の内部 test に限られます。
