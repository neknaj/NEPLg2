...
- 2026-06-02 の RPN Resource op inventory checkpoint では、cache を使わない compile で同じ Resource op tree を繰り返し走査していた箇所を統合した。`ResourceFunctionOpInventory` により通常 summary dependency、function-value call dependency、raw-init direct trigger を 1 回の再帰走査で収集し、RPN proof-disabled stage 中央値では `resource_static_check` が `main=1753ms` から `branch=1695ms` へ下がった。ただし proof-backed no-stage wall-clock は `main=905.280ms`、`branch=927.817ms` で通常 base 改善とはまだ言えないため、この issue は open のまま維持する。
- 同 checkpoint で、`parse_u128_radix_digits_from` の `Branch` / `Loop` / `Match` は `incoming_paths=0` でも約 `90ms` を占め、支配点が path replay ではなく control op 内部の table clone / merge と線形探索にあることを確認した。branch / loop state move 化、Copy 型 exact initialized fast path は実測で改善しなかったため採用しない。次は touched table を制御部分木ごとに分ける effect mask、または `CellTable` / alias table の stable place index 化を設計する。
