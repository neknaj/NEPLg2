...
- 残件: block / lambda / borrow / pipe argument expression checking、generic instantiation inference、trait solving、明示 `@ident` の検査結果から DefId 付き `FnValue` HIR へ lower する処理、indirect call、`memo_call` Phase 1 境界、cross-arena serialized canonical key / fingerprint、nested generic binder depth と stable binder identity は未実装のため、この issue は open のまま維持する。
