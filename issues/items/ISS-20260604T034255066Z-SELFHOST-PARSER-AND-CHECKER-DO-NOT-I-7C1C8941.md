...
- subagent design review `019eb594-fd38-7442-91ec-39bbfc618066`: 同名 `add` overload のまま `1 |> add add 2 3` を success にすると nested call overload narrowing まで含むため、今回 slice では outer target を overloaded `add`、inner callable を単一候補 `sum` に分ける方針を推奨。これに従い、`1 |> add sum 2 3` で source-backed nested suffix の `CheckedExpr` topology を固定し、同名 overloaded inner call narrowing は残件に残した。
