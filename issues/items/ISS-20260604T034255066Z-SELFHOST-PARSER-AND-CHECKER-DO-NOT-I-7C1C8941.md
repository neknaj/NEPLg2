...
- 2026-07-14: current-VFS Matchのsingle `NamedValue` scrutineeをprefix rangeからlatest scope binding、assigned DefId、value type evidence、同一arena recordへ通し、actual `SelfhostTypeId`を得るchecker境界を追加した。選択armはcontext-owned exact 3 segment projectionだけを使う。export originには既存constructor attach経路を追加し、再登録やsynthetic Named割当をせず、name、exact kind/arity、宣言名span identityが一致する同じ宣言のnominal IDだけを再利用してNamed / Applied base nominalをparser enum definitionへ照合してから既存member gateへ渡す。同名・同arityでもorigin spanが異なるconstructorはtypedに拒否する。公開compositeはsource/token/range/span/TypeId/nominal/raw nameを受けない。一方、scope/value evidence/arena/constructor tableと選択FunctionDeclのco-productionはcaller preconditionであり、production authorityではない。direct call、arm-derived expected-type retry、reference deref、payload bind、網羅性とarm body型統一は未完了である。
...
- 2026-07-14: qualified pattern spellingのalias・enum name・member tail spanからtarget export origin、query SourceText、enum member checkerを一つのnonproduction consumer checkpoint内で接続した。aliasはcurrent graph/VFSのimport recordへ再結合するが、残るspanの同一pattern AST所属はactual expression parser接続までcaller preconditionでありsemantic authorityではない。public入口はcontext / export entry / nominal / TypeId / raw visible nameを受けず、成功ownerがsame-session originとchecked identityを同時に保持する。actual Match scrutineeとchecked tree / HIR / Resource輸送は後続である。
- 2026-07-14: Rustと同じ任意長`Ident (:: Ident)*`をordered segment span ownerへparseし、optional bindと消費後token cursorを同時生成するMatch variant pattern構文evidenceを追加した。raw path spellingは保存せず、exactly 3 segment projectionはdirect alias subset adapterに限定する。current VFS lexer / actual Match arm parser / semantic consumer接続は後続である。
- 2026-07-14: enclosing bodyの`match` BlockIntroからRust互換outer layoutを検証し、variant armごとのordered pattern、bind、colon、body envelopeを同時生成するarm segmenterを追加した。move-only pattern ownerはCopy arm metadataと共有segment bufferへ正規化する。scalar/wildcardとcurrent VFS/actual scrutinee checker接続は後続である。
...
- 2026-06-11: HIR `Call` callee identity follow-up として、`SelfhostHirCallExpr` が表示名だけでなく `SelfhostDefId`、callee function type、effect、monomorphic `type_arg_count` を保持するようにした。`lower/hir/direct_call.nepl` は legacy direct-call result 経路では reducer-selected candidate table entry、checked-tree 経路では `DirectCall` node 内の `SelfhostCallableCandidate` から HIR call payload を作り、source token / scope lookup / callable candidate collection を再実行しない。`name` は diagnostic / dump / backend symbol 用に残すが、monomorphize、Resource IR、indirect call、`memo_call` が参照する authority は DefId / type / effect payload である。generic instantiation の stable type-argument range はまだ無いため、`selfhost_hir_expr_call` は `type_arg_count` を引数に取らず 0 に固定し、`Unique` など `NoneRequired` 以外の candidate は `GenericCallIdentityUnsupported` として拒否する。
...
- 2026-06-11: binary / octal i32 literal slice として、self-host lexer が `0b` / `0B` と `0o` / `0O` を 1 個の `IntLiteral` token として切り出すようにした。`check/expr/literal_payload.nepl` は `SelfhostLiteralI32RadixPlan` で radix 2 / 8 / 10 / 16 と digit body range を token-local に決め、`string::to_i32_radix` へ接頭辞除去済み body だけを渡す。`add 0b1010 0o12` は stage1 smoke で `I32Literal(10)` を 2 引数ぶん保存することを確認し、HIR lowering は source token や literal lexeme を再読しない。suffix と `i32` / `f32` 固定を越える defaulting はこの slice では広げず、未知の alphabetic radix marker は引き続き `I32RadixUnsupported` で fail-closed にする。
...
- 2026-06-11: source-backed nested overload boundary を `SelfhostNestedArgumentBoundary` として整理した。通常 call の引数位置では `OuterContinuation` を渡し、borrowed probe が inner candidate の `next_index` を得た後、その cursor から outer candidate の残り parameter が source-less matcher で満たせる候補だけを一意化する。pipe left では `FinalRange` を渡し、pipe operator 直前までで単一値を作る候補だけを許すため、pipe target suffix を left nested overload の continuation として誤用しない。stage1 smoke は `outer add 1 2 3` の outer continuation success、duplicate overload の `OverloadAmbiguous`、`add 1 2 |> use 3` の pipe-left final-range success と checked tree topology を確認する。source-backed finisher は選択後に 1 回だけ実行し、借用 probe の `next_index` と実消費 cursor がずれた場合は `InternalInvariant` として fail-closed にする。
...
- 2026-06-11: selfhost `memo_call @pure_named_func` Phase 1 の compiler-known identity gate を追加した。`lower/hir/memo_call.nepl` は resolver / stdlib registry が source identity 確認後に渡す trusted identity と reducer-selected callee candidate の `DefId` を比較し、名前文字列や source token を primitive 判定に使わない。`memo_call @func` は checked argument が `FunctionValue` 1 個で、call result type が source function candidate の callable type と一致し、既存 memoized function value gate が DefId あり / monomorphic / Pure と認める場合だけ `MemoizedFunctionValue` HIR leaf になる。同名 user `memo_call`、即時適用、checked argument 個数不一致、通常値 argument、result type mismatch、generic / impure source function は typed error で fail-closed にする。この checkpoint は trusted identity gate であり、`MemoKey` / `MemoValue`、private cache SourceCapability、Resource IR `PrivateCache` proof、sealed backend representation はまだ実装しない。
- 2026-06-11: selfhost compiler-known primitive identity registry を追加した。`builtins/prelude/compiler_known.nepl` は `SelfhostCompilerKnownPrimitiveKind::MemoCall` と `SelfhostCompilerKnownPrimitiveIdentity { kind, module_path, symbol, def_id }` を持ち、typecheck / HIR / Resource IR / backend が同じ primitive authority を共有するための registry 境界になる。`lower/hir/memo_call.nepl` は lowering-local identity type を持たず、shared identity の kind が `MemoCall` で `DefId` が一致する場合だけ accepted path に進む。prelude registry は `lower/hir` / `hir` / `check/expr` に依存せず、`module_path` / `symbol` は監査 metadata に留める。source hash / policy hash materialization、`MemoKey` / `MemoValue`、private cache proof、sealed backend representation はまだ実装しない。
- 2026-06-12: `core/ty/ty/memo_trait.nepl` を追加し、selfhost 側の `MemoKey` / `MemoValue` Phase 1 predicate を `SelfhostTypeArena` record だけで判定する fail-closed 境界として固定した。主 API は `Result unit SelfhostMemoTraitRejectKind` を返し、`unit` / `bool` / `i32` / `u8` / `char` を key/value として、`f32` を value として受理し、`f32` key、`str`、function、named、applied、parameter、missing type record、`I64` / `F64` は enum reason 付きで拒否する。`Named` / `Applied` は field layout / trait evidence が selfhost TypeArena に無いため、Rust 実装の structural aggregate acceptance をこの checkpoint では再現しない。
- 2026-06-12: `core/ty/ty/memo_trait.nepl` に `SelfhostMemoTraitEvidenceTable` と evidence-aware predicate を追加した。これは aggregate proof solver ではなく、後続の layout / trait solver が作った `Result unit SelfhostMemoTraitRejectKind` payload を `Named` / `Applied` だけが消費する session-local 境界である。primitive、function、parameter、missing record は evidence で上書きできず、証拠なし aggregate は従来どおり fail-closed に残る。
- 2026-06-12: `core/ty/ty/memo_trait_producer.nepl` を追加し、後続 solver が作る `SelfhostMemoTraitAggregateProof` を `SelfhostMemoTraitEvidenceRecord` へ変換する producer gate を分離した。`SelfhostMemoTraitAggregateProof` は `SelfhostTypeId`、field layout summary、Copy / Drop / Eq / Hash proof status、cache escape hazard classification、key/value の `Result unit SelfhostMemoTraitRejectKind` を持つ。accepted record の候補になるのは `Named` / `Applied` のみで、primitive、function、parameter、missing type record は `SelfhostMemoTraitEvidenceProduceRejectKind` で拒否する。さらに `Named` / `Applied` であっても、field layout missing、invalid field range、generic argument unsubstituted、cycle limit reached、operation proof missing / impure / unknown、cache reference escape、external handle、owner token、public mutable state、unknown hazard は producer 側で typed reject になる。これは field layout / trait solver そのものではなく、trusted proof summary を session-local consumer table へ渡す前の fail-closed gate である。
- 2026-06-12: `core/ty/ty/memo_trait_proof_store.nepl` を追加し、canonical type key と solver policy identity で `MemoKey` / `MemoValue` aggregate proof を索引する store boundary を作った。store record は session-local `SelfhostTypeId` を持たず、lookup 時に現在 arena の TypeId を一時 canonical key へ再投影し、cross-arena canonical equality と policy 一致を確認してから existing producer gate へ戻す。canonical key は同じだが policy が違う record は探索を止めず stale proof として記録し、期待 policy の record が最後まで見つからなかった場合だけ `PolicyMismatch` を返す。producer rejection は payload まで比較する。stale policy、missing proof、unsupported proof kind、primitive fake proof は typed error で fail-closed になる。これは serialized proof artifact writer / reader ではなく、proof store と session-local evidence table の責務境界である。
- 2026-06-12: `core/ty/ty/memo_trait_policy.nepl` を追加し、proof store policy identity を typed source identity と typed rule identity へ分離した。`SelfhostMemoTraitProofStorePolicy` は raw `trait_source_hash` / `rule_hash` を持たず、`MemoKey` / `MemoValue` source identity set と solver rule identity を保持する。proof store はこの policy module を import し、canonical key と typed policy が一致する proof だけを再利用する。trait definition table から source identity を実生成する registry はまだ未接続であり、次の `MemoKey` / `MemoValue` 残件に残す。
- 2026-06-12: `core/ty/ty/memo_trait_source.nepl` を追加し、prepared `MemoKey` / `MemoValue` trusted source identity registry を proof store policy へ接続した。proof store は `selfhost_memo_trait_trusted_source_identity_set_current_result` から source set を受け取り、source kind constructor や source identity constructor を stage0 内で直接呼ばない。これは full trait definition table materializer ではなく、current prepared fingerprint を typed registry boundary に集約する checkpoint である。
- 2026-06-12: `memo_trait_source.nepl` の current source identity を typed definition source record から `Result` で materialize する境界へ進めた。expected kind mismatch と signature missing は enum error で拒否し、proof store stage0 は `selfhost_memo_trait_trusted_source_identity_set_current_result` から policy source set を取得する。full trait definition table scan と public surface hash 生成は引き続き後続 slice で扱う。
- 2026-06-12: `memo_trait_source.nepl` の current source registry を `SelfhostMemoTraitDefinitionSourceTable` 経由に変更し、missing key、missing value、duplicate key、duplicate value、key source rejected、value source rejected を typed registry error として分離した。per-kind current source identity helper は private smoke helper に戻し、proof store policy 用 source set は table-backed current registry Result API からだけ取得する。full trait definition table scan、public surface hash、stable trait definition key 由来の source record producer は引き続き後続 slice で扱う。
- 2026-06-12: `core/ty/ty/memo_trait_layout.nepl` を追加し、`MemoKey` / `MemoValue` aggregate proof の type constructor layout evidence を `SelfhostNamedTypeId` keyed table と `SelfhostTypeId` field record で表す境界へ分離した。`Named` は arity 0 layout のみ、`Applied` は constructor identity と型引数数一致および未解決 parameter 不在を要求する。field range、owner mismatch、field index mismatch、missing field type、generic field、missing layout は typed error で fail-closed にする。これは Copy / Drop / Eq / Hash proof や recursive aggregate solver ではなく、producer gate に渡す field evidence 入力の検証境界である。
- 残件: lambda / borrow / source-backed pipe suffix の lambda など複合式 success path、generic instantiation inference、trait solving、numeric suffix の言語仕様化、defaulting beyond current Rust fixed `i32` / `f32`、indirect call、`memo_call` の private cache proof / backend representation、`MemoKey` / `MemoValue` aggregate layout / trait evidence solver 入力、cross-arena serialized canonical key / fingerprint、stable nominal key、nested generic binder depth と stable binder identity は未実装のため、この issue は open のまま維持する。
...
2026-06-11 pipe trailing block candidate checkpoint:
...
2026-07-14 Match checked scrutinee root checkpoint:

- actual leading Match contextのscrutinee rangeを既存body-line reducerへ渡し、expectedなしのchecked tree rootからactual TypeIdを取得してqualified exported enum member gateへ接続した。
...
- actual VFSからchecked rootとmember identityまでのruntime gateは、固定direct callだけを持つ独立fixtureで約29秒完走した。direct / ascription / pipe / ambiguityを集約したfixtureはcompiler state探索を増幅するため恒久suiteへ置かず、ascription / pipe / ambiguityは既存reducer component runtimeとcontractで維持する。Rust同様のarm-derived expected type推論とrollback/retryは次sliceである。
