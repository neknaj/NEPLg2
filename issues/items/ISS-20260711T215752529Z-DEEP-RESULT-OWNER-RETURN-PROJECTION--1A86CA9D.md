...
- `resource_ir_owner_check_returns_deep_multi_owner_aggregate_through_result`は、同時に生存する2個のowner fieldをそれぞれ内部enumへ置き、外側Resultの選択payloadから返すdistinct-authority topologyの通過基準をowner checkとnormal compileで固定した。parameter root、aggregate type、または外側Resultだけを根拠にこの2 authorityを同一視してはならない。production誤報の最小再現では、単一registered source内の複数deep leafが明示的alias contractで同じauthorityへ収束するという仮説と、flat group化を拒否するpaired controlを次に検証する。
