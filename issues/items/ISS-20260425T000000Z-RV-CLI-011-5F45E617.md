...
full dual backend verification は、まず `tests` / `stdlib` に分け、さらに各 suite を `--shard 1/4` から `--shard 4/4` までの 4 shard に分割します。
各 shard は `tests-dual-tests-shard-N.json` / `tests-dual-stdlib-shard-N.json` を artifact として upload し、Pages final bundle で `tests-dual-tests.json` / `tests-dual-stdlib.json` へ merge します。
...
2026-06-01 の GitHub Actions run `26728316260` では、以前の `tests` / `stdlib` 2 shard だけではそれぞれ 18 分の外側 timeout に達し、`exit code 137` で失敗しました。
そのため、`nodesrc/tests.js` に汎用 `--shard INDEX/TOTAL` を追加し、元の doctest case 集合を stable sort してから SHA-256 hash で分割するようにしました。
`--runner all --llvm-all --strict-dual` では shard 後の同じ case 集合から wasm / LLVM case を派生するため、shard 境界によって `missing llvm counterpart result` が発生しません。
空 shard は、分割の結果として正当な 0 件 report を出力します。
...
- `node nodesrc/test_tests_js_shard_policy.js`
- `node nodesrc/test_ci_llvm_dual_shard_job.js`
- `node nodesrc/test_merge_doctest_json.js`
- `node nodesrc/tests.js -i tests/stdlib/io.n.md --no-tree -o tmp\io-shard-1of2.json --shard 1/2 -j 1`
- `node nodesrc/tests.js -i tests/stdlib/io.n.md --no-tree -o tmp\io-shard-2of2.json --shard 2/2 -j 1`
- `node nodesrc/merge_doctest_json.js -o tmp\io-shards-merged.json tmp\io-shard-1of2.json tmp\io-shard-2of2.json` (`total=6`, `passed=6`)
- `git diff --check`
