...
Split the Vec sort public facade from raw implementation helpers. Keep safe Vec sort APIs and observers in the canonical facade, move raw MemPtr traversal helpers behind explicit sort/raw or implementation submodules, remove the sort_i32 public adapter, and update focused tests/source policies to reject raw helper re-export.
...
- raw unchecked access は `alloc/collections/vec/sort/raw/access`、raw quick-sort traversal は `sort/raw/quick`、raw heap helper は `sort/raw/heap` へ分離した。
- `sort_i32` は互換 alias を残さず削除した。raw slice sort が必要な implementation boundary は `sort/raw/quick` の `sort_slice_quick` を明示 import する。
