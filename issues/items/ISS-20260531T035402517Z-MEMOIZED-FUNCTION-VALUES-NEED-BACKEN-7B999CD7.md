...
2026-07-11: Resource IR固有Place inventory ownerを追加し、Return Placeをsame request / graphの登録済みPlaceだけに制限した。root/projection/type、escape classification、actual lowering co-productionは未実装である。
...
2026-07-12: Rust `ResourceOp`全24 top-level variantをfallbackなしのkind tagとして保持する専用operation-kind inventoryを追加した。legacy classifier / span ownerと分離し、scopeは全ownerのcount一致後にlegacy、span、kindのidentity / dense ordinalを順に検査する。stage0もkind ownerを別生成してborrow後に解放する。nested Branch / Loop / Match topology、variant payload / endpoint、actual lowering co-productionは未実装で、originは`ResourceIrInventoryValidated`を維持する。
...
2026-07-12: `ResourceOp::Read`専用private sparse payload ownerを追加した。proof key / graph / dense ordinalと、graph identityを内包するsource / output Place handleを保持し、kind ownerとのmerge-cursorで欠落・異種混入、identity / ordinal、各handleのwrong graph / missing membershipをfield固有typed errorへ拒否する。異なるsource / output handleを保持するpositive fixtureも固定した。scopeはExpr / DeclareLocal / Read payloadを順に検査しても非production originを維持する。Branch / Loop / Match topologyはblock-aware nested ownerと再帰ordinal authorityを先に設計しないままflat ownerへ固定できないため撤回し、actual co-production、残る21 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::Assign`専用private sparse payload ownerを追加した。proof key / graph / dense ordinalと、graph identityを内包するtarget / value Place handleを独立に保持し、kind ownerとのmerge-cursorで欠落・異種混入、identity / ordinal、各handleのwrong graph / missing membershipをfield固有typed errorへ拒否する。scopeとstage0 lifecycleへownerを接続しても非production originを維持する。actual co-production、assignment semantics、残る20 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::Borrow`専用private sparse payload ownerを追加した。proof key / graph / dense ordinal、graph-bound source / output、専用Shared / Unique kind、synthetic boolを保持する。merge-cursor、identity / ordinal、両Placeのwrong graph / missing membershipをexact errorへ拒否し、kindとsyntheticの全4組合せをstage0でtransportする。scope lifecycleへ接続後も非production originを維持する。actual co-production、borrow semantics、残る19 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::Move`専用private sparse payload ownerを追加した。proof key / graph / dense ordinalとgraph-bound source / outputを独立に保持し、merge-cursor、identity / ordinal、両Placeのwrong graph / missing membershipをexact errorへ拒否する。scope lifecycleへ接続後も非production originを維持する。このsliceは構造transportだけであり、actual co-production、move semantics、残る18 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::Drop { place, span }`専用private sparse payload ownerを追加した。proof key / graph / dense ordinalとgraph-bound Placeを保持し、kind ownerとのmerge-cursor、identity / ordinal、wrong graph / missing membershipをexact errorへ拒否する。scope lifecycleへ接続後も非production originを維持する。このsliceは構造transportだけであり、destructor execution / no-escape proof、actual co-production、残る17 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::EndScope { locals, result, span }`をprivate sparse header ownerとdense local endpoint ownerへ分離した。headerのkey / graph / operation ordinal / checked locals range / optional resultと、Vec順を保持するlocal ordinal / Placeをscopeで照合し、missing / unexpected、identity / ordinal、range overflow、dense gap / excess、全Placeのgraph / membershipをtyped rejectionする。Rust Vecで許されるPlace重複は拒否しない。destructor execution、drop elaboration、no-escape、actual co-production、残る16 payload、nested topologyは未完成である。

2026-07-12: `ResourceOp::CollectionStorageRelocate { old_storage, new_storage, span }`をkind tag 17専用のprivate sparse ownerへ接続した。proof key / graph / operation ordinalと独立graph-bound old/new storage Placeをscopeで照合し、missing / unexpected、identity / ordinal、endpoint membership / wrong graphをtyped rejectionする。raw realloc proof、canonical owner-cell、slot state rekey、proof consumption、actual co-production、残る15 payloadは未完成である。

2026-07-13: `ResourceOp::CollectionSlotDropTraversal { storage, initialized_count, expected_ty, span }`をkind tag 18専用のprivate sparse ownerへ接続した。proof key / graph / operation ordinal、独立graph-bound storage / initialized-count Place、同じborrowed TypeArenaに属するnonnegative expected TypeIdをscopeで照合し、missing / unexpected、identity / ordinal、各endpointとtype membershipをexact typed rejectionする。drop traversal execution、initialized rangeの全称証明、slot element type照合、actual co-production、残る14 payloadは未完成である。

2026-07-13: `ResourceOp::CollectionSlotTransformRange { source_storage, source_initialized_count, output_storage, output_initialized_count, expected_ty, span }`をkind tag 19専用のprivate sparse ownerへ接続した。proof key / graph / operation ordinal、4個の独立graph-bound Place、同じborrowed TypeArenaに属するnonnegative expected TypeIdをscopeで照合し、missing / unexpected、identity / ordinal、各endpointとtype membershipをexact typed rejectionする。transform semantics、source/output rangeの全称証明、rollback、actual co-production、残る13 payloadは未完成である。

2026-07-13: `ResourceOp::RawAddressAlias { source, target, kind, span }`をkind tag 13専用のprivate sparse ownerへ接続した。2個の独立graph-bound Placeと`Transparent` / `InternalHelper` / `OwnerTokenConstruct`をlosslessに保持し、missing / unexpected、identity / ordinal、各endpointのmembership / graph mismatchをexact typed rejectionする。alias safety / provenance semantics / source capability / actual co-productionと残る12 payloadは未完成である。
...
2026-07-13: Matchのactual declaration authority前提として、constructor namespace seed + 1 sourceだけを受け、内部lex / parseした全AST itemをsource順に1回走査するone-shot module producerを追加した。返却sessionはsource/tokens、dense enum definitions、span-only variants、constructor tableを所有し、外部AST/token/itemや返却後appendを受けない。seedはstruct/importのnamespace prefixでありparser origin証明ではない。generic bounds、invalid payload introducer、variant/name UTF-8境界、name copy OOMをtypedに分け、definition全体spanとconstructor name診断spanを分離した。TypeArena、import/alias解決、checker-resolved `(nominal_id, variant_ordinal)`、checked tree/HIR Match、Resource sidecar、reference Deref、cross-session stable key、production originは未接続である。
2026-07-13: `ResourceOp::RawMemory { operation, output, args, span }`専用のprivate sparse headerとdense arg endpoint ownerを追加した。Rust `RawMemoryOp`全13種、same-graph output、checked args range、operation/arg ordinal付きPlace endpointを保持し、重複argsを許可する。これは非productionの構造検査でありraw semantics/capability/co-productionは未証明、残る8 payloadとnested topologyは未完成である。

2026-07-13: `ResourceOp::CallEffect { effect, span }`専用private sparse ownerを追加した。Rust `EffectOp`全10 variant、UserCall名の専用canonical symbol role、surface Pure / Impure、RawMemoryOp、private operation / region、ExternalIo / Nondet / Unknownをclosed enumで区別し、全named variantをstage0で輸送する。PrivateEffectRegionIdはi64上のu32全域を保持して0も受理し、範囲外regionとunknown canonical nameをtyped rejectionする。effect semantics、region freshness / masking、no-escape、actual co-production、残る7 payloadは未完成である。

2026-07-13: `ResourceOp::FunctionValue`専用private sparse headerとdense type-argument ownerを追加した。output Place、独立canonical roleの表示name / identity symbol、optional DefId、function TypeId、surface effect、Plain / Memoized、closed operation EffectOp、ordered type argsをlosslessに保持し、各owner membershipをexact typed rejectionする。Rust loweringが現在nameとsymbolへ同じ値を渡す事実をidentity同一性には昇格させない。resolution、memo namespace、effect整合、actual co-production、残る6 payloadは未完成である。

2026-07-13: `ResourceOp::Call`専用private sparse header、source-order argument Place owner、target固有ordered type-argument ownerを追加した。Rust `ResourceCallTarget`のBuiltin / User / Traitをvariant-nativeに保持し、targetごとのcanonical symbol role、Trait self TypeId、User / Trait type args、output / args Place、closed `EffectOp`をscopeで検査する。sparse / dense ownerのmissing / unexpected、identity / ordinal、range gap / excess、symbol / TypeArena / Place membershipはfield固有typed rejectionで固定する。このnonproduction transportはdirect-call resolution、trait dispatch、signature / effect semantics、actual HIR-to-Resource loweringとowner tableのco-productionを証明せず、production originを発行しない。残るpayloadはIndirectCall / Construct / Branch / Loop / Matchの5件である。

2026-07-13: `ResourceOp::IndirectCall`専用private sparse header、source-order parameter TypeId owner、source-order argument Place ownerを追加した。output / callee、params / result、args、closed `EffectOp`をRust payload shapeのまま保持し、TypeArena / same-graph Place membershipとsparse / dense ownerのmissing / unexpected、identity / ordinal、range gap / excessをfield固有typed rejectionへ分類する。0件、複数、重複parameter / argument、projected Place、同一output / calleeを許容する。このnonproduction transportはfunction-value alias resolution、callback signature / effect semantics、actual loweringとのco-productionを証明せず、production originを発行しない。残るpayloadはConstruct / Branch / Loop / Matchの4件である。

2026-07-13: `ResourceOp::Construct`専用private sparse header、source-order input Place owner、Struct / Tupleのsource-order field-offset ownerを追加した。Rust `AggregateKind`のEnum / Struct / Tupleをvariant-nativeに保持し、enum name / variant / struct nameを別canonical symbol roleへ結合する。inputsとoffsetsは独立Vecとして0件、複数、重複、非単調offsetを保持し、Enumにoffsetを混入した場合だけ拒否する。sparse / dense missing / unexpected、identity / ordinal、range gap / excess / overflow / negative narrowing、same-graph Place / symbol membershipをfield固有typed rejectionへ分類する。arity / layout / type semantics、actual co-production、production originは未完成であり、残るpayloadはBranch / Loop / Matchの3件である。

2026-07-13: Branch / Loop / Match payloadの前提として、ResourceFunction.blocks直下と各control operation内の`Vec<ResourceOp>`を表せるblock-awareなprivate nodes / scopes / edges standalone validatorを追加した。global preorder node ordinal、exactly-one-parent、root forest、closed Branch / Loop / Match role、dense edge / sibling / preorder、exclusive subtree、cross-block / orphan / cycle形状 / overflow / count mismatchをtyped rejectionし、nested Branch、nonempty / empty Loop、0 / multiple-arm Match、control相互nestをruntime matrixで実validatorへ通す。scope idはowner内denseとし、異なるparent group間の割当順は問わず、同一operation parent内のrole / dense arm順だけを要求する。ただし現行central inventory / materializerはflat operation rangeをauthorityとしており、`root_operation_scope_id`はstandalone fixture用の予約fieldである。次はrecursive materializerからtopology / kind / span / sparse payloadをco-produceしてcentral scopeへ接続し、その後Branch condition/value fact / payloadとLoop / Match payloadへ進む。actual lowering co-productionとproduction originも未完成である。

2026-07-13: recursive topology ownerをcentral inventory scopeの必須borrowへ接続した。topology-native coreはActualWalker operation tableを引数に取らず、operation domainを`topology.nodes.len`だけから定める。legacy tableのcount / identity検査は旧stage0用compat helperへ分離し、その一致をResourceOp co-productionへ昇格させない。span / kind ownerをdense検査した後にfunction root scopeとnodes / scopes / edgesをcross-owner検査し、その後だけ既存sparse payload ownerを同じglobal ordinalで検査する。block validatorはflat operation accumulatorを持たずblock / terminator構造だけを検査する。全scopeはedgeが0件でもOperation parentの実在、parent preorder、owning blockを検査する。flat fixtureは負first / countとchecked-add overflowをowner生成前に拒否してcleanupし、2-block positiveは各rootとblock 0内nested Branch descendantのglobal forest continuityをcentral wrapperへ通す。成功originは`ResourceIrInventoryValidated`のままで、actual ResourceOp co-production、Branch condition / value / recursive condition fact、Loop / Match payload、production originは未完成である。

2026-07-13: `ResourceOp::Branch`のoutput / condition / then value / else valueとoptional recursive `ResourceConditionFact`を、topology global ordinal上のprivate sparse ownerとしてcentral authorityへ接続した。then / else operation列はBranch headerへ重複保持せずrecursive topologyを唯一のauthorityにする。fact ownerは全6 unary、I32Relation全6 op、Any / Allをvariant-nativeに保持し、node / ordered edgeのidentity、dense ordinal、operation owner、Place membership、exclusive rangeを検査する。reachabilityとincoming countだけではRust再帰treeを一意に復元できなかったため、rootがowner範囲先頭で、各direct child subtreeがglobal preorderの次nodeからsibling順に連続し、範囲全体を消費する再帰cursor検査を追加した。stage0はNone、空Any / All、nested / repeated、same / projected Place、複数global ownerを受理し、header / Place / fact root / node / edge / orphan / two-parent / cycle形状 / trailing / noncanonical preorderをtyped exact rejectionする。成功originは`ResourceIrInventoryValidated`のままで、actual co-production、Branch control-flow semantics、Loop / Match payload、production originは未完成である。

2026-07-13 review follow-up: condition fact rootをBranch headerからBranch / Loop共通のgeneric root binding ownerへ移し、二重authorityを除いた。global kind走査はBranch / Loopにexactly 1件のbindingを要求し、その他kindへのbinding / fact、Noneとnodeの競合、全node / edge exhaustionをgeneric validatorで一元検査する。Branch sparse validatorはheader / 4 Placeだけを担当する。同owner内Branch + Loop fact positiveとbinding / non-control / cross-owner edge negative、逐次allocation failure cleanupをstage0へ固定した。

2026-07-13: `ResourceOp::Loop`のcondition Placeだけをglobal ordinal sparse headerへ追加し、condition / body operationsはrecursive topology、condition factはBranch / Loop共有generic ownerだけをauthorityにした。central authorityをConstruct→Branch→Loop→generic fact順へ拡張し、2 block上のBranch 21 / Loop 22、empty child scopes、両header、Branch fact / Loop None bindingを同じ実callへ通した。gap / excess、identity / ordinal、Place missing / graph mismatchとtopology parent欠落はtyped errorで拒否する。Loop convergence semanticsとactual co-productionは未実装で、次のpayload sliceはMatchである。

2026-07-13: `ResourceOp::Match`のoutput / scrutinee / borrow-target headerと、source順のpattern / optional bind metadata / value / span arm ownerをglobal ordinalへ追加した。arm operationsはrecursive topologyのMatchArm scopeだけをauthorityにし、header range、arm owner operation / relative ordinal、topology parent / dense arm ordinal / exact countをcentral validatorで三者照合する。全pattern、Owned / Borrowed mutable bit、独立Option、Place / canonical symbol / span membershipとrange gap / excess / overflowをtyped matrixで固定した。actual ResourceFunctionからのowner co-production、pattern exhaustiveness、arm type join、production originは未実装である。

2026-07-13 Match Variant membership prerequisite: 現TypeArenaのNamed / Applied recordとTypeConstructorTableはnominal id / type argument / arityだけを持ち、enum宣言種別とvariant列のauthorityを持たないため、canonical symbol membershipだけをenum member証明へ昇格させない。非production sidecarは`SelfhostNamedTypeId`とproducer-resolved declaration member identityのdense rangeだけを所有し、concrete TypeId / canonical type key / raw spellingを偽って束ねない。central compatibility pathはtopology / Match payload構造の検査後にowner全体を閉じ、`Place.ty → TypeArena Named / Applied → nominal definition → variant member`を照合し、wrong owner、missing definition、definition / variant重複、range破損をtyped rejectionする。Match bind source nameは入力に含めない。actual checker / Resource materializerはqualified / alias source spellingと解決済みmember identityを分離して正規化し、TypeArena projection / constructor table / membership ownerのsame-session producer origin、reference-to-enumのDeref projection、cross-session canonical type keyをco-produceする必要があるが未実装である。
