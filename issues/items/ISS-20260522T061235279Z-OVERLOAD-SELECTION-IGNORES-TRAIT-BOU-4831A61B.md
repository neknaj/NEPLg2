...
Filter overload candidates against their instantiated type parameter bounds before signature deduplication. Reject only definitely-unsatisfied concrete bounds; keep unresolved generic candidates pending. Do not use stdlib allowlists or string-based special cases.
...
- 具体型に対して明らかに満たせない bound を持つ候補だけを候補から落とし、未確定型変数の bound は pending のまま保持する。
