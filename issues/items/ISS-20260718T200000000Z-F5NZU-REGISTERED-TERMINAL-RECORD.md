...
The production helper and source policy are implemented, but an actual composite F5nzt-to-F5mu runtime fixture causes nonlinear resource analysis and exceeds a 300-second compile timeout at about 1.8 GB RSS. The F5nzt 1023 control fixture must be compiled with CLI `--test-mode`; with that contract restored it resolves the nested test binding and reaches resource analysis, but still exceeds 360 seconds after the effect-boundary stage. Integration remains blocked until the control and composite fixtures compile and run within the normal test gate.
