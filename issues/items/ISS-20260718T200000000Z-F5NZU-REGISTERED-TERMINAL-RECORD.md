...
The production helper and source policy are implemented, but an actual composite F5nzt-to-F5mu runtime fixture causes nonlinear resource analysis and exceeds a 300-second compile timeout at about 1.8 GB RSS. The F5nzt 1023 control fixture must be compiled with CLI `--test-mode`; with that contract restored it compiles in about 339 seconds and its generated Wasm reports evidence 1023 with zero failures. Owner-return summaries take about 173 seconds, dominated by one-pass enumeration of deeply nested Result owner leaves. A cycle-sensitive concrete-subtype projection cache passed focused owner-summary tests but still timed out the 1023 control at 300 seconds after reaching effect-boundary analysis, so it was removed. Integration remains blocked until the control and composite fixtures compile and run within the normal test gate.
