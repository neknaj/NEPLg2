...
- complete leaf entry checkpoint として、cache value を個別 stable value の dedup 可能な `Vec` から、順序と重複を保持する `ResourceSummaryStableDropTraversalForallLeafEntry` に変更した。function summary 全体が top-level `DropTraversal + ForallInitializedRange` だけで、return facts が空で、summary dependency と `IndirectCall` がない場合だけ store/hit 候補にする。依存あり caller、partial summary surface、duplicate value の multiplicity について regression を追加した。
- complete leaf entry preseed checkpoint として、保存済みの dependency-free complete leaf entry を fixed-point worklist 前に replay し、対象関数を初期 pending から外すようにした。`resource_summary_value_replay_hits` / `resource_summary_value_replayed_ops` は実 replay 時だけ増え、通常 recompute された replay 対象 op は `resource_summary_value_recomputed_ops` へ分けて記録する。
- semantic source key checkpoint として、compiled-output cache key の source 部分を raw text から lexer token stream 由来の `compiled_source_cache_key_part` へ差し替えた。ordinary comment / doc comment / span-only change は compiled output key に入れず、indent / newline / directive / raw text / literal payload は保持する。lexer diagnostic がある source では raw text key へ戻す。
- RPN same-session 実測では、base compile が `compile_ms=9013`、ordinary comment-only edit が `compile_ms=2`、doccomment text edit が `compile_ms=1`、code edit が `compile_ms=8347` だった。コメント増加を compile cache miss にしない境界は入ったが、code edit は引き続き Resource summary value reuse が必要である。
- raw-init param facts checkpoint として、dependency-free complete leaf の `RawCellInitializationParamCell` / simple `RawCellReleaseParamRequirement` を stable mirror store 候補にした。ただし RPN 実測では `raw_init_param_facts_stores=0` / `bypasses=225` であり、nominal-heavy stdlib summary が `Named` / `Struct` / `Enum` stable type key 未対応で候補化できない可能性を別 issue `ISS-20260528T110220373Z-RESOURCE-SUMMARY-CACHE-NEEDS-QUALIFI-08D1AA04` に分離した。
- raw-init preseed は実際に fixed-point worklist を skip する経路なので、追加 regression で保存済み summary surface の再投影一致と、function body / source policy / signature type boundary の変更時 miss を固定した。subagent review で指摘された raw body key については、本文が `ResourceFunction` に残らないため `RawBody` を含む body hash は引き続き拒否する。
- subagent review の指摘により、`ResourceFunction.name` に入る重複定義用 `__def<file>_<start>_<end>` span component を Resource summary function identity key から外した。function body hash と source capability policy hash は維持し、SourceMap / Span に依存しない identity へ近づける。
