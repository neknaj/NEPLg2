...
- `PreparedProgram` に `ResourceSummaryCacheNamespaceKey` を追加した。key は target / profile / typed public signature hash / dependency public surface hash option から決定的に作るが、現時点では Resource IR summary value の hit / store は行わない。Web / Node prewarm hot path でも dependency aggregate を同期計算せず、`TypeId` / `Span` / `SourceMap` / typed HIR / Resource IR body / diagnostic span / codegen fragment は key に保存しない。
