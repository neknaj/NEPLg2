...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native の全体 wall-clock を 0.5 秒未満へ近づけるため、bundled stdlib `.neplmeta` / `.neplproof` preseed、typecheck/interface artifact、bootstrap proof generation 短縮、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では persistent `.neplproof` codec、native disk proof cache、summary/pass snapshot preseed、owner obligation pass-level snapshot により RPN proof-backed `resource_static_check` median は約 `0.34-0.39s` まで下がり、shallow arity path snapshot により RPN cold base no-stage median は `905.472ms` になった。CLI stage 計測では `loader_load=370.406ms`、`check_pipeline=542.679ms`、`resource_typecheck=144ms`、`resource_static_check=369ms`、`.neplproof` read は約 `0.6-0.7ms` だったため、次は stdlib body merge / dependency typecheck を `.neplmeta` / typed interface artifact で削る。local condition memo / parameter condition 重複削減 / owner summary relevance filter / i32 scalar proof 無効化 / merge済みModule cache store clone除去は RPN 実測で改善しなかったため、主経路にしない
