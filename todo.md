...
- Web Playground の `platforms/gui/web` runtime bridge を NEPL/Wasm が生成した `DrawCommand` stream へ接続し、生成 command を `web/src/gui-preview/host-bridge.ts` の decode 境界経由で floating GUI window surface へ渡す
...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base `compile_ms=9283` / native `resource_static_check=7053ms` 級を 0.5 秒未満へ近づけるため、persistent / bundled `.neplproof` preseed と stdlib prechecked artifact の設計を実装へ落とす
