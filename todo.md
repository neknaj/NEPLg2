...
- `ISS-20260604T034255066Z-SELFHOST-PARSER-AND-CHECKER-DO-NOT-I-7C1C8941` の次 slice として、reducer が `SelfhostCheckedExprTree` を生成し、`NestedDirectCall` / `BlockResult` summary argument を tree node id payload へ接続する。`TypedExpression` / summary payload に残っている argument は source 再読なしで HIR expression tree へ lowering できる形に分解する
