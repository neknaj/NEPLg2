...
- `ISS-20260601T193116311Z-NEPLMETA-TRAIT-IMPL-MATERIALIZER-NEEDED-D3A0C2F1` と `ISS-20260531T111205690Z-BINARY-INTERMEDIATE-ARTIFACTS-NEEDED-1C570649` に沿って、materialized compile report で `body_missing_fallbacks_delta_sum` が増える surface を `.neplobj` candidate として抽出し、bundled stdlib `.neplmeta` preseed と `.neplobj` / `.nepllink` の実装順序を決める
