...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native `resource_static_check` が 4-6 秒台に残っている状態を 0.5 秒未満へ近づけるため、actual `.neplproof` preseed を native `--check` cold path に接続し、stdlib-heavy proof の事前検査済み artifact、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では local condition memo / parameter condition 重複削減 / owner summary relevance filter は RPN 実測で改善しなかったため、主経路にしない
