...
- Web Playground の `GuiWebEvent` action / pointer down-up-cancel / keyboard / single-scalar text input checkpoint を、pointer move coalescing、IME composition / multi-scalar text、window / timer / lifecycle variant、session id formalization へ拡張する
- Mandelbrot progressive rendering を TS 側 simulation ではなく NEPL app の update loop で処理する
...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native `resource_static_check` が 3-4 秒台に残っている状態を 0.5 秒未満へ近づけるため、persistent / bundled `.neplproof` artifact を native `--check` の proof-backed wrapper へ接続し、stdlib-heavy proof の事前検査済み artifact、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では local condition memo / parameter condition 重複削減 / owner summary relevance filter は RPN 実測で改善しなかったため、主経路にしない
