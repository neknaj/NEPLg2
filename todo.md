...
2026-04-26 NEPLg2 Self-host
  ...
  - `ISS-20260604T034255066Z-SELFHOST-PARSER-AND-CHECKER-DO-NOT-I-7C1C8941` の次 slice として、float literal の `f32` checked payload / HIR `F32Literal`、負数 literal の `Minus + IntLiteral/FloatLiteral` consume-width、numeric suffix の言語仕様化、`NestedDirectCall` / `BlockResult` の checked tree payload を設計し、`TypedExpression` / summary payload に残っている argument を source 再読なしで HIR expression tree へ lowering できる形に分解する
