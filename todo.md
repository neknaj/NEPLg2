...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native の全体 wall-clock を 0.5 秒未満へ近づけるため、bundled stdlib `.neplmeta` / `.neplproof` preseed、typecheck/interface artifact、bootstrap proof generation 短縮、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では persistent `.neplproof` codec、native disk proof cache、summary/pass snapshot preseed、owner obligation pass-level snapshot により RPN proof-backed `resource_static_check` median は専用 proof cache 追加確認で約 `0.37s` まで下がったが、native CLI wall-clock はまだ約 `0.9-1.0s` である。local condition memo / parameter condition 重複削減 / owner summary relevance filter / i32 scalar proof 無効化は RPN 実測で改善しなかったため、主経路にしない
