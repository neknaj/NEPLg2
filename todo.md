...
- `ISS-20260601T151600000Z-NEPLMETA-CALLABLE-ARITY-MISMATCH-BLOCK-5C8E2B91` に沿って、field accessor / callable reject 細分化後に残る `type.public_surface.materializer.callable.arity_mismatch` の対象 entry と root cause を特定する
- `ISS-20260601T193116311Z-NEPLMETA-TRAIT-IMPL-MATERIALIZER-NEEDED-D3A0C2F1` と `ISS-20260531T111205690Z-BINARY-INTERMEDIATE-ARTIFACTS-NEEDED-1C570649` に沿って、field accessor blocker 解消後の `body_missing_candidate_surfaces_delta_sum` を実測し、`.neplobj` stable link symbol / selected callable body hash / generic instantiation hash の schema を実装する
