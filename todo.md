...
- `ISS-20260604T034255066Z-SELFHOST-PARSER-AND-CHECKER-DO-NOT-I-7C1C8941` の次 slice として、escaped string decode、numeric suffix / radix / defaulting、`NestedDirectCall` / `BlockResult` の checked tree payload を設計し、`TypedExpression` / summary payload に残っている argument を source 再読なしで HIR expression tree へ lowering できる形に分解する
