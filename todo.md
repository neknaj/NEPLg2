...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native の全体 wall-clock を 0.5 秒未満へ近づけるため、bundled stdlib `.neplmeta` / `.neplproof` preseed、typecheck/interface artifact、bootstrap proof generation 短縮、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では persistent `.neplproof` codec、native disk proof cache、summary/pass snapshot preseed、owner obligation pass-level snapshot により RPN proof-backed `resource_static_check` median は約 `0.34-0.39s` まで下がったが、RPN cold base no-stage wall-clock median は `965.283ms` である。CLI stage 計測では `loader_load=421.186ms`、`check_pipeline=539.710ms`、`resource_typecheck=130ms`、`resource_static_check=379ms`、`.neplproof` read は約 `0.7-0.9ms` だったため、次は stdlib body merge / dependency typecheck を `.neplmeta` / typed interface artifact で削る。local condition memo / parameter condition 重複削減 / owner summary relevance filter / i32 scalar proof 無効化は RPN 実測で改善しなかったため、主経路にしない
