...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native `resource_static_check` を 0.5 秒未満へ近づけるため、bundled stdlib `.neplproof` preseed、owner obligation pass-level snapshot、owner return summary stable mirror、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template を実装へ落とす。2026-06-02 follow-up では persistent `.neplproof` codec / native disk proof cache は実装済みだが、RPN proof-backed median はまだ約 1.02 秒である。local condition memo / parameter condition 重複削減 / owner summary relevance filter / i32 scalar proof 無効化は RPN 実測で改善しなかったため、主経路にしない
