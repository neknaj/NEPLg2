...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native `resource_static_check=3793ms` median を 0.5 秒未満へ近づけるため、actual `.neplproof` preseed を native `--check` cold path に接続し、stdlib-heavy proof の事前検査済み artifact、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template、source 単位 i32 condition query memo を実装へ落とす
