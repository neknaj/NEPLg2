...
- `ISS-20260524T225852366Z-PER-PROGRAM-COMPILE-TIME-EXCEEDS-DEF-189918C5` に沿って、RPN cold base native `resource_static_check=3878ms` median を 0.5 秒未満へ近づけるため、ByteBuilder / StringBuilder 系 i32 scalar condition proof、`dealloc_raw` / `apply_op` / Stack owner flow の Resource proof template、persistent / bundled `.neplproof` preseed と stdlib prechecked artifact の設計を実装へ落とす
