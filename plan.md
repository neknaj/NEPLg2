...
### 文字列・バイト列
文字列は UTF-8 保証されたイミュータブルな値です。
純粋永続値 (pure persistent value) であり、所有権やライフタイムを気にせず共有できます（メモリ回収はコンパイラの領域推論により自動で行われます）。
文字の連結などの操作には、一意可変な状態である `StringBuilder` を使用します。
任意のバイト列を扱う場合は、所有権管理される `ByteBuf` を用います。

文字列リテラルは、単行のものと複行のものを用意します。
複行文字列リテラル(multi line string literal)は `mlstr:` に続き、インデントを1つ増やして`##: `の後ろに文字列を書きます。
複行文字列リテラルは、行間に`\n`を挿入し、末尾には挿入しない。
