...
module parser は `SelfhostParserRawMode` と `SelfhostParserTokenAction` を enum として持ち、raw text の pending/active state や token action を match で扱っている。lexer 側も `caca505d` で `SelfhostLexerRawMode` enum へ移行し、raw mode model は parser と揃った。

module AST は import directive など S1/S2 に必要な item を保持し、import graph が parser 内部へ直接依存しすぎない境界を作っている。

## 問題とリスク

lexer の raw block state は `SelfhostLexerRawMode` enum へ移行済みである。`lex_token_pending_raw_mode`、`lex_raw_mode_is_active`、`lex_raw_kind` は typed raw mode を受け渡し、numeric sentinel や fallback kind mapping へ戻らない regression が追加されている。

directive 分類のうち `#indent` prefix は `str_starts_with_at` を使うよう改善された。今後 directive 種類が増える場合は、finite classifier と source policy へ寄せるのが望ましい。
...
- `ISS-20260507T151236784Z-SELFHOST-LEXER-RAW-MODES-AND-DIRECTI-B080723B`
  - `caca505d` で fixed。`SelfhostLexerRawMode` enum と directive string helper の回帰を監視する。
...
| `syntax/lexer.nepl` | Rust token parity に必要な char/raw/indent/directive がある。raw mode は `SelfhostLexerRawMode` enum 化済み。 | source policy と parity fixture で回帰監視する。 |
...
| lexer parity tests | Rust/selfhost parity harness と raw mode enum source policy がある。 | fixture を拡張する。 |

## 推奨対応

- lexer raw state の enum model を維持し、unexpected raw mode が token kind fallback に戻らないようにする。
- directive classifier を拡張する場合は keyword classifier と同じ設計へ移し、source policy で deep nested directive chain の再導入を拒否する。
