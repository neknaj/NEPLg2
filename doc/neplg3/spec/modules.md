...
NEPLg3 のモジュールシステムは **ファイル** と **モジュール** を完全に直交させる。3 層に分離する。
...
`<cond_expr>` は compile-time に評価される条件式であり、現在のコア仕様では `target` / `profile` 述語を主に使う。`#if[target=...]` のような角括弧記法は 2.0 系の表記であり、NEPLg3 の正仕様には含めない。
