...
NEPLg3 は単なるアプリケーション言語ではなく、様々な言語・DSL（JSON / Markdown / CSV / カスタム DSL）を構築・処理するための**言語プラットフォーム**として設計されている。
...
- 場所: `/stdlib`（移行期間は `/stdlib-g3` を並行開発）
- 責務: 実際のプラットフォーム実装・パーサツールキット・DSL 実装・**セルフホストコンパイラ**（`stdlib/neplg3`）。
...
`#if` は NEPLg3 では角括弧メタ構文ではなく、`#if <cond_expr>:` という前置ディレクティブとして扱う。`target "wasm"` や `profile "debug"` のような compile-time 条件式を後ろに置き、ブロック全体へ作用させる。
...
最終目標は NEPLg3 コンパイラを NEPLg3 で書くこと（`stdlib/neplg3`）。
