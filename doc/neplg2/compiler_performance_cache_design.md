...
この MVP は summary 再投影をまだ行わない。hit した stable value を現在 compile の `CollectionSlotLifecycleSummaryOp` へ戻すには、`ResourceSummaryStableTypeKey` から現在の `TypeId` への逆投影、stable place / offset / i32 operand / drop proof の再構築、固定点 worklist への安全な差し込みが必要になるためである。したがって現 checkpoint の hit は「同じ key/value が session 内に既に存在し、再利用可能だった」ことを統計として観測する段階に限定する。実際の summary replay は次段階の設計対象に残す。

map key は namespace hash、function identity、function body hash、type parameter boundary hash、generic type argument hash、source capability policy hash、summary kind/version を含む。map value は `ResourceSummaryStableDropTraversalForallValue` だけであり、`ResourceFunction`、Resource IR body、既存 summary struct 全体、`TypeId`、`Span`、`SourceMap`、typed HIR、diagnostic span、raw alias graph、`RawCellInitializationFunctionSummary` 全体は保存しない。同じ関数内に複数の top-level `DropTraversal + ForallInitializedRange` が現れても上書きしないよう、key ごとに複数 stable value を保持する。
