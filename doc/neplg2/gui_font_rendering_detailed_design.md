...
Bare では embedded blob table を provider とする。Blob table が未設定の環境では filesystem probing を行わず unsupported を返す。open / byte length / read / close は同じ immutable snapshot handle に結合し、length取得後のpayload差し替えを許さない。Headless / offscreen tests は borrowed canonical fixture path `str` とowned `ByteBuf`を明示的に渡し、request path exact match、`SfntOnly`、non-empty、expected hashの順に検査する。fixture用の別owned pathは作らず、host font APIには依存しない。両providerのsuccess sourceは`EmbeddedBlob`とする。
...
F5nq は `GuiFontRegisteredFaceTableEntry` を借用し、table membership 済み face から 1 code point の glyph id を解決する。entry は `GuiFontRegisteredFace` owner と Copy record を同時に保持するため、F5nq は entry record だけを盲信しない。F5nr で table layer に追加する `gui_font_registered_face_table_entry_validate` が face owner から record を再導出し、entry record と全 field を一度だけ照合する。F5nq と後続 lookup はこの validator を共有する。
...
F5nq の public lookup は `&GuiFontRegisteredFaceTableEntry` を受け取り、entry owner を消費しない。success は `GuiFontRegisteredFaceGlyphMapping` の Copy evidence だけを返す。failure は entry owner recovery ではなく、同じ record / code point / lower `GuiSfntParseError` / `GuiFontRegisteredFaceTableEntryValidationError` を Copy evidence として返す。旧 `table_error Option GuiFontRegisteredFaceTableRegisterErrorKind` field は F5nr migration で削除し、validation error accessor も新しい shared validation payload を返す。これにより `InvalidFaceProjection` と `RecordMismatch` の分類を glyph lookup error へ写した後も失わない。caller は entry owner を保持したまま、別 code point lookup、hmtx / glyf lookup、または後続 owner close を選べる。
...
## Font registered face horizontal metric lookup boundary

F5nr は table entry validation を table layer の共有責務へ移し、registered face owner と F5nq mapping から raw font-unit horizontal metric を解決する。validator は entry を借用し、face owner 由来 record の再導出失敗を `InvalidFaceProjection`、entry record との全 field 不一致を `RecordMismatch` とする Copy error evidence を返す。成功時は canonical derived record を返し、bytes owner は entry に残す。

```text
GuiFontRegisteredFaceTableEntryValidationError:
    kind GuiFontRegisteredFaceTableEntryValidationErrorKind
    record GuiFontRegisteredFaceRecord
    table_error Option GuiFontRegisteredFaceTableRegisterErrorKind

GuiFontRegisteredFaceGlyphHorizontalMetric:
    mapping GuiFontRegisteredFaceGlyphMapping
    metric GuiSfntHorizontalMetric

GuiFontRegisteredFaceHorizontalMetricLookupError:
    kind GuiFontRegisteredFaceHorizontalMetricLookupErrorKind
    record GuiFontRegisteredFaceRecord
    mapping GuiFontRegisteredFaceGlyphMapping
    validation_error Option GuiFontRegisteredFaceTableEntryValidationError
    parse_error Option GuiSfntParseError
...
public lookup は `&GuiFontRegisteredFaceTableEntry` と `&GuiFontRegisteredFaceGlyphMapping` を受ける。処理順序は entry validation、canonical record と mapping record の照合、entry face から bytes 借用、canonical record から selected face index 取得、mapping から glyph 取得、`gui_sfnt_lookup_horizontal_metric` の 1 回呼び出しで固定する。mapping mismatch branch は hmtx parser へ進まない。success evidence と error evidence は Copy であり、entry owner を保持しない。
...
## Registered contour span index and indexed path boundary

F5nwはregistered evidenceの脱落を防ぐため、既存public `gui_font_registered_face_simple_glyph_collected_owner_take_collection`を削除する。registered index adapterはF5nu completed ownerのprivate constructor / field transfer authorityを持つ同じ`registered_face_simple_glyph_collection` moduleに置き、evidenceとcollectionを同時にlower indexへ移す。startはentry validation、mapping record、canonical `glyf` record、source glyph、topology、collection capacity / item count / storage shapeをallocation前に照合する。failureは元のcompleted ownerを回収可能にする。

canonical provenanceは独立した`registered_face_simple_glyph_provenance` moduleに置く。`GuiFontRegisteredFaceSimpleGlyphProvenance`はcanonical record、evidence、sourceを持つCopy success value、`GuiFontRegisteredFaceSimpleGlyphProvenanceError`は`EntryValidationFailed`、`EvidenceRecordMismatch`、`SourceTableMismatch`、`SourceGlyphMismatch`とoptional table validation errorを持つCopy typed errorである。reader start / stepはshared resultを既存reader error kindへ明示的にmappingし、cursor source identityだけをreader moduleで追加検証する。registered index startはshared errorを保持し、allocationせず元completed ownerを返す。

index startはcompleted ownerを消費し、shared provenanceとcollection shapeを検証した後だけprivate field transferでF5nv lower startを呼ぶ。lower start failureはlower errorからcollectionを回収し、同じevidenceからprivate completed ownerを再構成する。builder ownerはevidenceとlower builderを、completed indexed ownerはevidenceとlower indexed ownerを束ねる。point stepとcompletionのlower errorもregistered owner-bearing errorへ包み、borrowed metadata、consuming owner recovery、freeを持たせる。ownerとowner-bearing errorはaggregate constructorをprivateにし、ownerは`Clone` / `Copy`を実装しない。raw lower owner、collection、span storageをconsuming accessorで外へ分離しない。

existing `sfnt/glyf`内のpoint / edge / curve / event関数は、checked-span coreと旧full-scan adapterへ根本から分割する。旧adapterはspan discoveryを一度だけ行ってshared coreへ渡し、`sfnt/glyf_indexed_path`はindexed spanを同じcoreへ渡す。checked-span point coreは`collection + span + local index`、edge coreは同じspanをstart / end pointへ渡し、curve coreとevent coreも同じspanをnested callへ伝播する。coreはspan lookupを行わない。これにより検証authorityを複製せず、`glyf_indexed_path -> glyf`の一方向依存を保つ。

上位`registered_face_simple_glyph_indexed_path`はregistered indexed owner、sink policy、private `GuiFontRegisteredFaceSimpleGlyphIndexedPathState`を同じ`GuiFontRegisteredFaceSimpleGlyphIndexedPathOwner`に所有させる。stateは`PendingContour contour_index`、`ActiveContour span cursor`、`Completed`である。startはlookupを行わず`PendingContour 0`を作る。stepはpath ownerだけを消費し、`PendingContour`でだけindex lookupをexactly once行い、`ActiveContour`では保存済みspanを再利用する。checked-span coreはcursor contour indexとspan contour indexをevent lookup前に比較し、異なるcontourの組を`CursorContourMismatch`で拒否する。contour stepをexisting pure sink-step projectionへ渡し、final eventのtail actionまで確定する。次contourがあれば`PendingContour`、最後なら`Completed`へ遷移する。foreign cursorはprivate state constructorとownership transitionにより構築不能である。lookup / point / edge / curve / event failureはpath ownerを回収可能にする。budget APIはowner-bearing terminal variantへ分岐せず、`GuiFontRegisteredFaceSimpleGlyphIndexedPathBudgetStep`に唯一のpath ownerとCopy `GuiFontRegisteredFaceSimpleGlyphIndexedPathBudgetStatus`を保持する。callerはstatusを借用で読んだ後、同じresultからownerをexactly once回収する。

budgeted traversalはowner stateの`Completed`を先に判定する。completedならbudget値に関係なくownerを保持するcompleted terminalを返し、`PendingContour` / `ActiveContour`かつbudget 0ならlookup / collection readなしで同じownerを保持するbudget-exhausted terminalを返す。positive budgetはstepを一度だけ呼び、成功したsink stepだけがbudgetを1減らす。off-curve開始、連続off-curve、quadratic lookahead、single-point contour、final event、`KeepOpen`、`EmitCloseAfterFinalEvent`はexisting pure policyと同じ結果を返す。lookup failureは`GuiSfntSimpleGlyphContourSpanIndexLookupError`を保持する専用typed errorとし、旧full-scan errorへ変換しない。
...
F5nxr scan ownerはF5nxq writer、quadratic subdivision config、phase/cell/sample/edge/quadratic/join progressを表すCopy cursorを保持する。side edgeはnested F5nxp sourceの`Vec<GuiSfntSimpleGlyphRenderStrokeSideEdgeProjection>`、joinはF5nxp geometry Vecからexact count/len/cap検査付きで読む。legacy F5lb ownerはauthority型が異なるため利用しない。

Quadratic projectionはsource quadraticを`0..N`のbounded parameterで評価し、endpoint normalを同じparameterで補間する。Leftはnormalを加算し、Rightはnormalを減算してdirected ordinalを`N-i`へ反転する。crossingはcount加算ではなくparityで畳み、巨大edge/join countによるi32 overflowを避ける。sample座標はwide arithmeticからf32へ変換し、整数sampleは±2^24内で厳密変換し、edge、join、quadratic intermediateはfiniteかつ同範囲として検査する。startはedge/join countを各4096以下、`sample_scale^2 * (edge_count * N + join_count * 2 + 1)`をi64で事前検査して1,048,576以下に固定する。pollはterminal-firstのbudget 0/1だけを受理し、旧recursive drainは持たない。
...
bounded pollはowner invariantとterminalをbudgetより先に検査する。非terminalの1 stepは一つのchord、connector、sample commit、またはcell pushだけを進める。cell push成功後にcell indexとwritten countがともに1進んだことを要求する。push/completion failureはwriterをscan ownerへ戻し、exact fullだけをcompleted raw coverage ownerへ進める。
...
F5nxb public startはregistered indexed ownerをborrowしてcollection capacityを取得した後、そのownerをF5nxa startへmoveする。任意のF5nxa ownerを受けるpublic startは作らない。これによりmid-stream action ownerへzero apply stateを再付与する経路を閉じる。F5nxaはprivate NeedSinkStep / TailPendingを公開せず、Copy progress kind Active / Completedだけを公開する。progress mappingはNeedSinkStepとTailPendingがActive、CompletedだけがCompletedである。F5nxbはsink packet、lower path progress、contour indexからcompletionを再導出しない。

F5nxb summary ownerはlower action owner、canonical capacity、Copy apply state、private phaseを持つ。phase payloadはRunning Option last-status、Rejected reason、Completed concrete last-statusである。terminal phaseとlast statusを別fieldにしないため、Rejected / Completed sealでmissing terminal status分岐は存在しない。summary metadataはCopy projectionとして読めるが、continuation authorityではなく、F5nxc allocation APIはsummary metadata単体を受けない。旧ConsumerConsumeSummaryのadvanceと旧F5am capacity付きoutcomeはnew registered chainへ持ち込まない。

Running stepはF5nxa action stepを最大1回呼び、packet actionをshared pure apply helperへexactly once渡す。更新statusがRejectedなら更新済みsummaryをcommitしてphase Rejectedへ移る。reject後のlower action ownerはTailPendingのままsealed owner内に残り、resume accessorを公開しない。reject以外ではlower progressを読み、Completedならphase Completed、それ以外はRunningとする。final Tailを適用したstep resultはAppliedを返すが、返却ownerはすでにCompleted phaseであり、次回budget queryはCompletedを返す。

budget statusはApplied apply-status、Rejected reason、Completed、StepBudgetExhaustedである。Rejected / Completedをbudget 0より先に判定し、Running budget 0はcounter、last status、lower ownerを変更しない。summary step lower failureはlower outer kind、path kind、lookup error、contour-step errorをborrowで読んだ後にaction ownerを回収し、変更前summary / phaseと再結合する。

summary step error kindはLowerActionFailed、AlreadyRejected、AlreadyCompletedである。LowerActionFailedはoptional lower action kind、path kind、span lookup error、contour-step errorを持ち、metadataを全て読んでからlower ownerをtakeする。terminal direct-step errorはlower metadataをNoneとし、same summary ownerを保持する。

completed sealとrejected sealはsummary ownerを消費するchecked transitionである。seal target kindはCompleted / Rejected、actual progress kindはRunning / Rejected / CompletedのCopy enumで表す。phase mismatch errorはtarget、actual、original summary ownerを保持する。completed ownerとrejected ownerはlast apply statusを非optionalで保持し、rejected ownerはreject reasonも非optionalで保持する。terminal ownerはsummary / capacity / apply state / concrete last status / reject reasonのCopy projectionとfreeだけを公開し、action/path/indexed/collection/storage takeを公開しない。F5nxcはcompleted ownerを分解せずnested authorityとして所有し、allocation failureでも同じcompleted authorityを回収可能にする。

F5nxcのpublic startはActionSummaryCompletedOwnerとborrowed OutlineStorageLimitだけを受ける。startはcompleted ownerをborrowしてcanonical capacityをCopyした後、低層の`gui_sfnt_simple_glyph_outline_storage_alloc`をexactly once呼ぶ。低層allocatorはcapacity shape、limit、scalar slot count、Vec allocationをtyped Resultで分ける既存F5b primitiveであり、再利用してよい。旧F5am outcome、F5an storage owner / terminal / allocation error、collection-backed summary projectionはauthorityとして再利用しない。

allocation successでは`RegisteredIndexedOutlineStorageOwner`がcompleted ownerと`GuiSfntSimpleGlyphOutlineStorage`をnested ownershipで保持する。limitはallocation policyであり、success continuationに必要なauthorityではないためowner fieldへ複製しない。allocation failureでは`RegisteredIndexedOutlineStorageAllocError`が元completed ownerとCopy lower allocation errorを保持する。errorはlower kind、capacity、limit、capacity checkのborrowed Copy projection、single take completed owner、freeを公開する。lower metadataをtake後に読めると仮定せず、利用側はmetadata read-before-takeを守る。

limit precedenceはcontour -> point -> edge -> commandである。contour first-failureはlater point / edge / command値を見ずContourCapacityExceeded、point first-failureはlater edge / command値を見ずPointCapacityExceeded、edge first-failureはlater command値を見ずEdgeCapacityExceeded、先行3項目が通ったcommand failureだけがCommandCapacityExceededになる。各first-failing fieldについてpositive-low、zero、negativeを同じtyped rejectionへ写し、0以下をunlimitedにしない。exact contour 2 / point 4 / edge 4 / command 8だけがcontrolled fixtureのsuccess boundaryである。

success ownerはcanonical capacity、apply state、concrete last status、storage capacity、scalar slot count / len / cap、typed full invariant checkをborrowed projectionとして公開する。invariant error kindはCanonicalCapacityShapeInvalid、GlyphMismatch、ContourCountMismatch、PointCountMismatch、EdgeCountMismatch、PathCommandPairCountMismatch、PathCommandCountMismatch、ScalarSlotCountOverflow、ScalarSlotCountMismatch、StorageNotEmpty、StorageSlotCapacityMismatchを区別する。checkはこの順にfail-closedで進み、Valid branchだけを成功とする。owner constructor、completed owner take、storage take、storage ref、raw collection / action / path splitは公開しない。freeはallocated storageを先に閉じ、その後completed summary authorityを閉じる。

F5nxdはF5nxc owner全体をinput authorityとして受ける。F5nxd開始時にtyped F5nxc invariantを再検査し、invalidならsame F5nxc ownerを持つerrorを返す。indexed sourceのCopy lookupはF5nxd実装時に各nested ownerのowning moduleへborrowed owner-bound forwarding operationを追加し、最上位F5nxc ownerからspan / item valueだけを返す。storage mutationはF5nxc owning moduleへconsuming owner-bound operationを追加し、successではreconstructed/next outer owner、failureではsame outer ownerとlower metadataを返す。generic callbackやraw pair returnでcompleted authority、indexed owner、collection、storageを分割しない。F5nxc phaseの状態責務はallocationまでであり、F5nxc owning moduleはF5nxd向けno-split forwarding/mutation boundaryだけを追加提供する。endpoint cursorとslot population stateはF5nxdが所有し、command、stroke、raster、platform presentationへ進まない。

F5nxd startはF5nxc `Valid`を確認してからchecked ContourEndpoint cursorを作る。success ownerはF5nxc ownerとprivate stateをnested ownershipで保持する。stateは`Active cursor previous_endpoint`または`Completed cursor last_endpoint`であり、startは`Active cursor None`を作る。start errorはoriginal F5nxc owner、typed invariant invalid kindまたはcursor errorを保持し、metadata read-before-take、single take、freeを提供する。

borrowed span forwardingはregistered indexed ownerからindexed path owner、indexed action owner、action summary owner、completed summary owner、F5nxc ownerの順に各owning moduleへ追加する。各層は直下ownerをborrowし、span countまたは`Result GuiSfntSimpleGlyphContourSpan GuiSfntSimpleGlyphContourSpanIndexLookupError`だけを返す。raw indexed/path/action/summary owner、collection refを公開しない。item forwardingはPointXを扱うF5nxeまで追加しない。

F5nxc owning moduleにはcontour endpoint push専用のconsuming operationを追加する。operationはouter owner、checked cursor、typed endpoint slot、previous endpointを受け、privateにcompleted ownerとstorageを分解してlower F5e pushを1回呼ぶ。successではreturned storageからouter ownerを再構築し、next cursor/previous endpointと束ねる。failureではlower metadataをすべて読んでからstorageを回収し、same outer owner、rejected endpoint、lower kindを持つerrorへ正規化する。raw completed/storage pair、storage take/ref、generic mutation callbackは公開しない。

F5nxd開始後はF5nxc empty invariantを再利用しない。next contour indexはcursor nextから導出し、別fieldとして重複保持しない。phase invariant invalid kindのprecedenceはCanonicalCapacityShapeInvalid、StorageCapacityMismatch、StorageShapeMismatch、SpanCountMismatch、CursorInvalid、CursorRegionMismatch、CursorCapacityMismatch、StorageLengthMismatch、ActiveCursorAtEnd、ActivePreviousUnexpected、ActivePreviousMissing、ActivePreviousOutOfRange、ActivePreviousSpanLookupFailed、ActivePreviousSpanMismatch、CompletedCursorNotAtEnd、CompletedLastEndpointOutOfRange、CompletedLastEndpointMismatchとする。start順序はF5nxc invariant、span count/capacity contour count、cursor construction、F5nxd invariantとする。

各predicateは次に固定する。StorageCapacityMismatchはcanonical/storage capacityのglyph、contour count、point count、edge count、path command pair count、path command countのいずれかの不一致である。StorageShapeMismatchはstorage scalar slot countがcanonical checked countと異なるか、storage capが同countと異なる場合である。SpanCountMismatchはforwarded span countとcanonical contour countの不一致である。CursorInvalidはcursor well-formed check failure、CursorRegionMismatchはregionがContourEndpointでない場合である。CursorCapacityMismatchはcanonical capacityからchecked再導出したContourEndpoint cursorとregion/start/endのいずれかが異なる場合であり、nextは比較しない。private lower predicateへ依存せず、public checked cursor construction結果をexpected authorityとして比較する。StorageLengthMismatchはstorage lenとcursor nextの不一致である。

ActiveCursorAtEndはcursor next >= cursor endである。ActivePreviousUnexpectedはcursor next == 0かつpreviousがSome、ActivePreviousMissingはcursor next > 0かつpreviousがNoneである。ActivePreviousOutOfRangeはprevious < 0またはprevious >= canonical point countである。ActivePreviousSpanLookupFailedはindex `cursor next - 1`のowner-bound lookup failure、ActivePreviousSpanMismatchはそのspan end point indexとpreviousの不一致である。CompletedCursorNotAtEndはcursor next != cursor endまたはcursor end != canonical contour count、CompletedLastEndpointOutOfRangeはlast endpoint < 0またはlast endpoint >= canonical point count、CompletedLastEndpointMismatchはlast endpoint != canonical point count - 1である。

stepはstate、span lookup、endpoint derivation、owner-bound pushの順で進む。lower indexed lookup自身がglyph/index/range/count identityを検証するため、F5nxdはspan identityを重複検査しない。最終endpoint push callは`Pushed endpoint`を返し、返却ownerのstateはCompletedになる。次回budget callがbudget値に関係なくCompletedを返す。

F5nxc contour endpoint push success payloadはouter owner、cursor、nonoptional previous endpointを保持する。error payloadはouter owner、cursor、rejected endpoint、previous endpoint、`GuiSfntSimpleGlyphContourEndpointPushErrorKind`、optional `GuiSfntSimpleGlyphOutlineRegionPushErrorKind`、optional `StdErrorKind`を保持する。successはcursor/previous/take owner、errorは全metadata/take owner/freeを公開する。owner-bearing payloadはClone/Copyを実装しない。

budget drainはCompleted判定、budget 0以下、最大1 stepの順を守る。0以下ではlookup/pushを呼ばずStepBudgetExhaustedを返す。checked completed sealはCompleted stateとphase invariant `Valid`の場合だけ、running owner全体をnested ownershipで保持するcompleted endpoint ownerを返す。seal errorはsame running owner、actual progress、typed invariant checkを保持し、Active mismatchをinvariant failureより先に分類する。F5nxdはcompleted endpoint ownerで停止し、PointX cursor開始はF5nxeの責務とする。

budget resultは唯一のrunning ownerとCopy statusを保持し、single take ownerとfreeを公開する。running owner freeはnested F5nxc ownerをexactly once freeし、budget freeは保持ownerのrunning freeへexactly once委譲する。completed owner freeもnested running ownerをexactly once freeする。seal errorのtake ownerとfreeは排他的であり、freeはsame running ownerをexactly once閉じる。どのfree pathもcompleted authorityやstorageを個別に取り出さず、F5nxc owner freeだけを使う。

F5nxeはF5nxd completed endpoint ownerを唯一のsource authorityとして、`Active PointXCursor | Completed PointXCursor`のprivate stateを持つ。point indexやcommitted countを別fieldとして重複保持せず、logical indexは`cursor.next - cursor.start`から導出する。start stageのfail-fast順序はF5nxd completed phase invariant、owner-bound collection item countとcanonical point countの一致、checked PointX cursor construction、F5nxe phase invariantとする。canonical capacity/storage shapeは最後のphase invariant stage内で固定順位により評価する。先行stage失敗時は後続stageを評価しない。point countが0の場合はchecked cursorがstart時点でendにあるためCompleted stateを作り、空でない場合だけActive stateを作る。

F5nxe phase invariant invalid kindのprecedenceは`CanonicalCapacityShapeInvalid`、`EndpointCompletionInvariantInvalid`、`StorageCapacityMismatch`、`StorageShapeMismatch`、`CollectionItemCountMismatch`、`CursorInvalid`、`CursorRegionMismatch`、`CursorCapacityMismatch`、`StorageLengthMismatch`、`ActiveCursorAtEnd`、`CompletedCursorNotAtEnd`とする。この順位はphase invariant evaluator内部だけに適用し、start stageのfail-fast順序を置換しない。StorageLengthMismatchは`storage len != contour count + cursor.next - cursor.start`、CursorCapacityMismatchはcanonical capacityから再導出したPointX cursorとのregion/start/end不一致でありnextは比較しない。ActiveCursorAtEndはnext >= end、CompletedCursorNotAtEndはnext != endまたはend-start != canonical point countである。

start error kindは`EndpointCompletionInvariantInvalid | CollectionItemCountMismatch | CursorStartFailed | PhaseInvariantInvalid`とする。payloadは元F5nxd completed owner、endpoint invariant check、expected/actual item count、optional cursor error、phase invariant checkを持ち、該当しないmetadataはtyped neutral valueで保持する。全metadata accessorはborrowed Copyであり、single take completed ownerとsingle freeを提供する。start error constructorはprivateとし、各stageは自分のkindだけを構築する。

borrowed item forwardingはregistered indexed owner、indexed path owner、indexed action owner、action summary owner、completed summary owner、F5nxc outline storage owner、F5nxd running owner、F5nxd completed ownerの順に各owning moduleへ追加する。各層は直下ownerを借用し、`Result GuiSfntSimpleGlyphOutlinePointStreamItem GuiSfntSimpleGlyphOutlinePointStreamItemCollectionReadError`だけを返す。最下層だけがcollection readをexactly once呼び、途中層はcollection refやnested ownerを返さない。sealed ownerはcollectionのcapacity/count/len/capとregistered provenanceの結合を保証するが、Copy item payloadのglyph/index/kindはreadごとにF5nxeが検査する。

F5nxc owning moduleにはPointX push専用consuming operationを追加する。operationはF5nxc owner、checked PointX cursor、typed PointX slotを受け、privateにcompleted authorityとstorageを分けてlower pushをexactly once呼ぶ。successはnext cursorを読み、returned storageを回収してouter ownerを再構築する。failureはlower PointX kind、rejected slot、optional region kind、optional storage push kindをstorage takeより先に読み、returned storageとsame completed authorityからouter ownerを再構築する。raw pair、storage take/ref、generic callbackは公開しない。

F5nxe stepはstate確認、logical index導出、owner-bound item read、item glyph/index/kind検証、PointX slot構築、owner-bound pushの順で進む。identity failureはrejected itemとsame ownerを保持し、pushへ進まない。push failureはlower metadataを全て読んだ後、F5nxc owner、F5nxd running owner、F5nxd completed owner、F5nxe ownerの順に内側から再構築し、失敗前cursorを保持する。lower error storage len、reconstructed cursor next、再導出logical indexはfailure前とexactly一致し、phase invariantがValidでなければならない。budgetはCompleted、remaining steps 0以下、最大1 point commitの順であり、read/validation/push failureをsuccessful operationに数えない。

F5nxe start/step/seal errorとbudget resultはowner-bearing move-only payloadとし、borrowed metadata accessor、single take owner、single freeを提供する。step error kindは`AlreadyCompleted | ItemReadFailed | ItemGlyphMismatch | ItemIndexMismatch | ItemKindMismatch | PointXPushFailed`を区別する。checked sealはActive mismatchをinvariant failureより先に分類し、CompletedかつValidの場合だけF5nxfへ渡すcompleted ownerを作る。F5nxeはPointY cursorを開始しない。

F5nxfはF5nxe completed ownerをnested authorityとして保持し、PointY cursor以外の進捗fieldを重複保持しない。phase invariantはPointX completion、canonical/storage shape、collection item count、PointY cursor、`contour_count + point_count + logical progress`のstorage長、Active/Completed終端を固定順で検査する。stepはowner-bound item read、glyph/index/kind検証、typed PointY slot、owner-bound PointY pushの順であり、lower failure metadataを全て読んでからF5nxe completed ownerを再構築する。checked completed sealまではEdge cursorやindexed span lookupを開始しない。

F5nxgはF5nxf checked completed ownerと`PendingContour | ActiveContour | Completed` stateだけを保持する。Pending payloadはcontour index、expected start、Edge cursorを持ち、owner-bound indexed span lookupをexactly once行う。Active payloadはchecked span、expected start、Edge cursorを持ち、同じcontour内ではlookupを繰り返さずexisting checked-span edge coreへlocal indexを渡す。成功時だけtyped Edge slotのcontour owner scalarをpushし、contour終端で次のPendingまたはCompletedへ遷移するため、全体計算量はO(point + contour)、追加領域はO(1)である。budget result、error、completed ownerはmove-only owner-bearing payloadであり、borrowed status/kind、single take owner、freeを提供する。endpoint marker/full scan、raw collection/storage split、command以降のAPIは公開しない。

F5nxhはF5nxg checked completed Edge ownerと`PendingContour | ActiveContour | Completed` stateを一体所有する。Pending payloadはcontour index、expected start、PathCommandTag cursorを持ち、contourごとにowner-bound indexed span lookupをexactly once行う。Active payloadはchecked span、expected start、cursorを保持するため、同じcontourの2 event per edgeでspan lookupを繰り返さない。logical command indexから`edge_index = index / 2`と`event_slot_ordinal = index % 2`を導出し、checked-span event coreの結果をstable tag scalarへ射影してowner-bound pushをexactly once呼ぶ。contour終端では次のPendingまたはCompletedへ遷移し、全体計算量はO(command + contour)、追加領域はO(1)である。

startはEdge completion invariant、checked cursor construction、phase invariantの順にfail-fastする。stepはterminal、budget、span、checked event、event kindからstable tagへのexact projection、pushの順で、failureは失敗前stateとsame registered authorityを保持する。F5nxhはcaller supplied tagやstored tagを照合する段階ではなく、checked event payloadのkindだけをtag/slot/scalarのsourceにする。lower metadataはowner回収前に読む。owner-bearing start error、step error、budget result、seal error、completed ownerはClone/Copyにせず、borrowed metadata accessor、single take、single freeを対称に提供する。checked completed sealだけをF5nxiのcommand value / cursor / stream preparationへ渡し、raw collection/storage split、caller supplied span/event/tag、full scan、fallback、後続stroke/raster/render/platform APIを公開しない。

F5nxiのstart inputはchecked completed PathCommandTag ownerだけである。private ownerはそのauthority、`next_index / end_index` cursor、`total_count / MoveTo / LineTo / QuadraticTo / SkipNoSegment / last_path_command_index` summaryを一体所有する。stepはterminal、budget、registered command value lookup、value index/tag identity、kind別summary加算の順に進む。contour spanはowner-bound lookupでcontourごとに再利用し、全drainをO(command + contour)に保つ。failureはlookup metadataをborrowしてからsame authorityと失敗前cursor/summaryを回収する。
...
F5nxlのactual metric側はF5ku completed ownerをopaqueに保つ。public projectionはsealed count / len / cap検査後のowner-bound indexed Copy metricだけで、plan owner、metric Vec、constructor、takeを公開しない。production checkpoint bridgeはcompleted legacy writerからF5kq request、F5kr plan、F5ks cursor、F5ku drainを順に開始し、path command count + 1を上限として完走する。失敗時はprivate段階ownerを公開error payloadに保持し、callerにはkind借用とsingle freeだけを許す。registered transitionは同じcompleted sinkからsourceとwriterを一度だけ移し、sourceをcheckpointまたはlower errorと一体所有する。join ownerはF5nxk provenance ownerとactual ownerを同時に所有し、metric index、variant、curve座標、stroke widthを照合してからだけcursorをcommitする。registered authorityにはstroke widthがないため、checked curveからwidthを推測してmetricを再生成してはならない。

F5nxl join startはF5nxk completed ownerのowning moduleでだけnested source contour / sink completedを消費する。provenance Vecを保持したままsinkを既存registered actual checkpoint transitionへ渡し、成功時はそのsource+checkpoint ownerとprovenanceを再結合する。開始時はprovenance metric / line / quadratic countとactual summary、actual total / len / capを照合する。build入力paintのstrokeを事前にCopyし、checkpoint成功後にpositive canonical widthとしてjoin ownerへ保存する。build failureとpost-build mismatchはprivate enum payloadで区別し、どちらもprovenanceとlower authorityをsingle freeで閉じる。

F5nxm projection ownerはF5nxl join ownerだけをnested fieldに持つ。stepはF5nxl stepを一度だけ呼び、Completedとbudget exhaustionをそのまま射影し、JoinedだけをF5kx-neutral helperへ渡す。helperはvariant別LineTo / QuadraticToで既存provenance guardを通し、既存metric constructorとgeometry builderを呼ぶ。公開Copy geometryはF5ky相当の後続が再読込や再計算を行わないよう、全topology provenance、source座標、normal/tangent source、offset端点を保持する。
...
deterministic allocation failure testはNEPLg2 `#test` modeだけで有効なF5nxc test entryを使う。production startとtest entryはprivate allocation-result finalizerを共有し、test entryはcompleted authorityのcanonical capacityからexact limitを内部生成し、対応する`ScalarSlotStorageAllocFailed` lower errorを構築してfinalizerへ渡す。caller supplied limitは受けないため、CapacityRejectedにすべき入力へ`Some Fits`を付けた矛盾状態を構築できない。production buildではtest entryはactive statement集合に存在せず、production public API、allocator callback、runtime branch、owner layoutを増やさない。

test entryは`#test`の直後1 statementでなければならない。canonical capacityからexact limitを作り、capacity check payloadは`Some Fits canonical_capacity`をexactに構築する。production startとtest entryはそれぞれprivate finalizerをexactly once呼び、success owner / allocation error wrapperを構築できる場所はfinalizerだけとする。normal non-test compileではtest entry identifierはpublic surfaceに存在しない。
...
## Std compositor tile RLE present-frame bridge boundary

F5mq is the std-side compositor tile RLE present-frame bridge after F5mp. It consumes caller supplied `SurfaceId` plus `GuiRgba8888CompositorTileRlePacketOwner`, copies compositor metadata and the packet descriptor first, verifies the metadata frame id against the packet descriptor frame id, creates `FrameId` through `frame_id_result`, extracts the lower `GuiRgba8888RowTileRlePacketOwner`, and calls `gui_rgba8888_row_tile_rle_present_frame_prepare` exactly once.

The success owner is `GuiRgba8888CompositorTileRlePresentFrameOwner`, which stores the lower `GuiRgba8888RowTileRlePresentFrameOwner` plus compositor metadata. It exposes a metadata付き descriptor wrapper, `GuiRgba8888CompositorTileRlePresentFrameDescriptor`, that stores lower `GuiRgba8888RowTileRlePresentDescriptor` plus compositor metadata and is Clone / Copy. Descriptor accessors delegate to lower F5cn descriptor accessors for surface, frame, expected run count, and expected pixel count.

The error path follows Avicenna plan review approval. F5mq must read metadata frame id and packet descriptor frame id before consuming the compositor packet owner. Frame id mismatch is rejected as `FrameIdMismatch` before lower owner extraction. If the matching metadata frame id cannot become a `FrameId` through `frame_id_result`, the bridge returns `FrameIdInvalid` and keeps the original compositor packet owner. Lower F5cn failures keep the lower packet owner, so F5mq reads `kind` and `category` before consuming the lower error and wraps the recovered lower packet owner with the copied metadata. This is compositor packet owner recovery, not present-frame recovery and not encoded/payload recovery.

F5mq provides `owner_finish_packet` as the explicit abort helper from success present owner back to metadata付き compositor packet owner. `prepare_error_finish_packet` returns the recovery compositor packet owner. `prepare_error_free` delegates to F5mp compositor packet owner free. `owner_free` delegates to lower `gui_rgba8888_row_tile_rle_present_frame_owner_free` and wraps lower encoded finish kind as `GuiRgba8888CompositorTileRleEncodedFinishErrorKind::EncodedFinishFailed`. F5mp does not need a public lower-packet extraction helper for this bridge.
...
## Std compositor tile RLE present run step bridge boundary

F5ms is the std-side compositor tile RLE present run step bridge after F5mr. It consumes `GuiRgba8888CompositorTileRlePresentRunCursorOwner`, copies compositor metadata before moving the lower run cursor owner, and calls lower F5co `gui_rgba8888_row_tile_rle_present_run_cursor_step` exactly once.

The success terminal is `GuiRgba8888CompositorTileRlePresentRunStep`, which stores the lower `GuiRgba8888RowTileRlePresentRunCursorStepResult` plus metadata-wrapped next `GuiRgba8888CompositorTileRlePresentRunCursorOwner`. F5ms does not create a new result enum; it preserves lower `RunReady` / `Completed` as a Copy value and exposes metadata, next record index, and total run count through the wrapped next owner.

Lower F5co step failure keeps a lower run cursor owner. F5ms reads lower kind / category before consuming the lower error, then wraps the recovered lower owner with the copied metadata as `GuiRgba8888CompositorTileRlePresentRunCursorOwner`. This is run cursor owner recovery, not present-frame recovery, command cursor recovery, or host recovery.

`step_finish_present_frame` and `step_error_finish_present_frame` delegate to F5mr `owner_finish_present_frame` and preserve its F5mq rewrap `Result`. `step_free` and `step_error_free` delegate to F5mr `owner_free`. F5ms intentionally does not provide `finish_encoded`, `finish_payload`, or `finish_entry` helpers.
...
F5nb is the std layer compositor tile RLE present host executor validation boundary after F5na. It mirrors lower F5cy without projecting compositor actions to lower row-tile actions. The input authority is a F5mz `GuiRgba8888CompositorTileRlePresentHostExecutionAction` and a F5na `GuiRgba8888CompositorTileRlePresentHostExecutionReport`. The validation order is fixed as support check, report action read, then full action equality. Unsupported support failure returns `UnsupportedAction`; mismatched reports return `ReportActionMismatch`.

The support enum is intentionally nonempty. `GuiRgba8888CompositorTileRlePresentHostExecutorSupport` can represent Window, Offscreen, Device, and their nonempty combinations, but cannot represent supports-nothing. Full action identity includes target x record variant, Window id for Window actions, descriptor surface and frame ids, expected run and pixel counts, all compositor packet descriptor scalar fields, compositor metadata fields, and run offset/count/RGBA channels. A matching failed report is still a valid association because F5nb validates that the executor result belongs to the expected action, not whether the executor succeeded.
...
## Std layer compositor tile RLE present host action executor session boundary

F5nh introduces the std layer compositor tile RLE present host action executor session boundary. It sits above F5ne / F5ng and below any actual Web, native, bare, or headless platform loop. The purpose is not to run a scheduler or perform platform work; it is to give executor wrappers a typed request / complete surface around one F5ne one-shot driver pending.

The session state is either `Ready driver` or `Completed`. `Completed` is terminal: requesting it returns the `Completed` request result and performs no lower call. `Ready` consumes the state, reads F5ne `pending_action &driver`, and creates `GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionPending` with the driver owner and a borrowed-derived expected action value. The pending value is owner-bearing and does not implement Clone or Copy.

F5nh complete is outcome-only complete. The actual executor does not report action identity back to this API. Instead, `gui_rgba8888_compositor_tile_rle_present_host_action_executor_session_complete` reads the pending expected action, wraps that action with the caller supplied `Result unit GuiError` by calling F5ng attempt construction, consumes the driver owner, and calls F5ng `attempt_driver_step` exactly once. This keeps action identity authority in the pending owner and prevents platform wrappers from choosing a different action at completion time.
...
The Web bridge maps raw status `0` to `Result::Ok unit`; negative status values become typed `GuiError`, including `GuiError::Unsupported`. The `gui_web_compositor_host_executor_step` function reads F5nh pending action, executes that action once, and returns through the F5ni bridge into F5nh outcome-only complete. The default `nodesrc/run_test.js` imports remain fail-closed for stdlib doctests. This is explicitly no video memory fallback: the bridge does not reuse legacy stdout transport, does not call Canvas / DOM directly, and does not run a scheduler, queue, timer wait, present loop, fallback, or silent no-op path.

## Web compositor host import actual implementation

F5nj connects the Web worker imports to a full-frame video memory surface backing. `begin` validates the descriptor, supports Window target only, acquires one `GuiVideoMemorySurface` write slot, and attaches it to the caller supplied frame id as compositor-owned state. `run` requires contiguous tile-local `run_pixel_offset` values and writes RGBA8888 bytes directly into that slot. `end` validates run / pixel totals for the current packet, tracks batch / tile completion, and publishes the write slot only after every metadata batch and tile is complete.

The actual Web implementation is still no video memory fallback: video memory is the formal Web pixel backing for this host import. It does not draw through Canvas / DOM, does not post its own presenter message, and reaches the main thread only through the existing `gui_video_memory_present` ack path after publish. Offscreen / Device targets return unsupported instead of being routed to Window. The compositor ABI has no title payload, so the worker uses a deterministic title policy derived from the surface handle rather than an empty-string fallback.

F5nk adds partial dirty slot preservation/copy without changing the Web pixel backing. Each Web host surface record owns a committed snapshot as the baseline authority. Full publish copies the whole write slot into that committed snapshot. Partial begin requires the committed snapshot, copies it into the newly acquired write slot, and then run writes only the metadata row range. If no committed snapshot exists, partial begin and raw rect publish fail closed with invalid command status; the worker never acquires a presenter read slot to use a published slot as a baseline.
...
The F5nn native follow-up uses dedicated `nepl_gui_native.font_resource_open`, `font_resource_byte_len`, `font_resource_read_bytes`, and `font_resource_close` imports. Open binds canonical path bytes and the decode-policy tag to one snapshot session handle; len, read, and close use that same handle. Close consumes and invalidates the handle regardless of its returned finalization status, so the guest calls it exactly once and never retries it. The host owns configured resource-root resolution, containment checks, and exact file lookup. The guest accepts read success only when the written length equals the snapshot length, closes the session, validates the expected hash, and then seals the result as `FileSystem`. Every failure closes the live handle and/or storage before returning the typed provider error. OS font discovery and display names never become resource identity.
...
F5nxv consumes the F5nxu registered resource owner as the sole lifetime and metadata authority. It reads the stored Copy record, rederives the expected record from the nested F5nxt reservation through the same invariant-bearing projection, and compares mask id, rect, paint, width, height, cell count, and alpha max before command construction. A non-positive id, nested F5nxs shape/storage failure, rect/shape mismatch, or record mismatch is a typed owner-bearing error. Table mutation states are not valid prepare states and are mapped to an explicit unexpected-state error if shared validation exposes one.

Only after equality succeeds may preparation call `render_command_alpha_mask_rect`. The resulting Copy `RenderCommand` and the registered resource owner are stored in one non-Clone/non-Copy sealed prepared owner. The owner exposes derived metadata needed for diagnostics, but no raw command accessor, borrowed command accessor, arbitrary callback receiving the command, or split consuming accessor. The error path never stores a command and permits recovery or freeing of the original registered resource owner.

SourceOver remains prior evidence embodied by the F5nxt reservation; F5nxv does not claim to reread a blend field that F5nxu does not store. Preparation proves neither host upload nor backend renderability. It performs no command stream emission, table lookup/register/push, alpha storage copy, pixel read/write, software drain, DrawTarget/RenderTarget call, platform/host/backend call, fallback, shadow, tile/bitmap transport, or compositor drain. F5nxw must consume the complete prepared owner so the command cannot outlive its mask resource.

## Registered stroke packed alpha-mask software drain-start boundary

F5nxw consumes the complete F5nxv sealed prepared owner and a generic `GuiRgba8888SoftwareSurfaceOwner`. The resulting non-Clone/non-Copy drain owner keeps the prepared owner, surface owner, and cell index together. It does not expose the internal Copy command, alpha storage, registered resource, or surface through a split consuming accessor.

Start validates in fail-closed order: the F5nxv stored/expected metadata and nested F5nxs shape/storage invariants; the internal command is `AlphaMaskRect` and exactly matches mask id, rect, and paint; the software surface width, height, stride, byte length, and storage invariants; checked rect right/bottom and containment in the surface; and exact alpha cell count for the command rectangle. Only then does it create a drain owner with cell index zero. SourceOver remains prior F5nxt evidence rather than a value reconstructed at F5nxw.
...
### F5nyi registered direct compositor RLE completed-count bridge

F5nyi lives in `stroke_compositor_tile_rle_count_completed.nepl`. It invokes public F5nyh exactly once, preserves its outer start and middle step errors, and only on step success finishes the step owner and invokes existing F5mh exactly once. The inner result retains either F5mh's completed error with its original count-owner recovery or the metadata-bearing completed count evidence. A zero budget reaches F5mh as Pending and returns `CountNotCompleted`; sufficient budget returns positive total-run and cursor-completion evidence. The boundary stops before F5mi encoding, writer/storage transport, packets, and presentation.
...
### F5nym registered direct compositor RLE storage bridge

F5nym adapterはF5nylの六層result graphを再包装し、writer-plan success branchだけでF5ml storage prepareを呼ぶ。F5mlが保持するmetadata、total run count 1、encoded byte count 12、cursor 0/16とstorage authorityを第七層へ置く。有効なsmall capacityではallocation failureをfixtureで強制せず、既存F5ml error/recovery regressionへ委譲する。production graphはfixture-freeで、F5mm write cursorより前で停止する。

### F5nyn registered direct compositor RLE write-cursor bridge

F5nyn adapterはF5nymの七層result graphを再包装し、storage success branchだけでF5mm write cursor startを呼ぶ。F5mmが保持するmetadata、total run count 1、encoded byte count 12、cursor 0/16、written run / byte count 0とwriter authorityを第八層へ置く。有効なproduction storageではstart failureをfixtureで強制せず、既存F5mm error/recovery regressionへ委譲する。production graphはfixture-freeで、F5mn write stepより前で停止する。

### F5nyo registered direct compositor first RLE write-step bridge
...
### F5nyq registered direct compositor RLE encoded seal bridge
...
### F5nys registered direct compositor RLE present-frame bridge

F5nys adapterはF5nyrの十二層result graphを再包装し、packet successとopaque typed SurfaceIdだけをF5mq present-frame preparationへexactly once移送する。present successまたはpacket ownerを保持するprepare errorを第十三層へ置き、SurfaceId/FrameId validationやowner recoveryを再実装しない。production-derived fixtureはmetadata付きpresent descriptorのsurface/frame/expected countsとowner freeをevidence 31で検証する。production graphはfixture-freeで、F5mr run cursor、record reader、host executionより前で停止する。

### F5nyt registered direct compositor RLE present run-cursor bridge

F5nyt adapterはF5nysの十三層result graphを再包装し、present-frame successだけをF5mr run-cursor startへexactly once移送する。run-cursor successまたはrecoverable present-frame ownerを保持するstart errorを第十四層へ置き、metadata preservation、lower start、rewrap recoveryを再実装しない。production-derived fixtureはmetadata、initial record index 0、total run count 1とowner freeをevidence 7で検証する。production graphはfixture-freeで、F5ms run step、command cursor、record reader、host executionより前で停止する。

### F5nyu registered direct compositor RLE first run-step bridge

F5nyu adapterはF5nytの十四層result graphを再包装し、run-cursor successだけをF5ms run-step-oneへexactly once移送する。typed step successまたはrecoverable run-cursor ownerを保持するstep errorを第十五層へ置き、lower step、result projection、next-owner recoveryを再実装しない。production-derived fixtureはfirst `RunReady`、metadata、next index 1、total count 1とstep freeをevidence 7で検証する。production graphはfixture-freeで、second step、F5mt command cursor、record reader、host executionより前で停止する。

### F5nyv registered direct compositor RLE completed-step bridge
...
### F5nyx registered direct compositor RLE command-cursor start bridge
...
### F5nyz registered direct compositor RLE Run step bridge

F5nyz adapterはF5nyyの十九層result graphを再包装し、BeginFrame step successだけをF5mt step-finish-ownerへexactly once渡し、回収したcontinuationをF5mt stepへexactly once移送する。second-step successまたはcommand-cursor ownerを回収できるstep errorを第二十層へ置く。productionで許可するF5mt APIは`step_finish_owner`と`step`だけであり、この順序を変えない。productionはBeginFrame result、Run result、descriptor、phaseを読まず、F5mtのRunPending branchだけがF5msを進めてtyped `Run`を構築する。fixtureはcanonical metadata 263/16/3/1、surface 7、frame 263、expected run/pixel count 1/16、run offset/count 0/16、color 11/22/33/44を511として積み上げ、`RunPending`だけに512を与え、owner free後の合計1023を確認する。F5nyzはF5nzaの`EndFrame` step、F5mu record projection、host executionより前で停止する。

### F5nza registered direct compositor RLE EndFrame step bridge

F5nza adapterはF5nyzの二十層result graphを再包装し、Run step successだけをF5mt step-finish-ownerへexactly once渡し、回収したcontinuationをF5mt stepへexactly once移送する。third-step successまたはcommand-cursor ownerを回収できるstep errorを第二十一層へ置く。productionのF5mt API allowlistは`step_finish_owner`、`step`の順だけとし、result、descriptor、phase、EndFrame payloadを読まない。fixtureはEndFrame payloadとcanonical descriptor双方のmetadata 263/16/3/1、surface 7、frame 263、expected run/pixel count 1/16を接続したaccumulatorで63、`Completed`だけを64として積み上げ、owner free後の合計127を確認する。F5nzaはF5nzbのterminal `Completed` step、F5mu record projection、host executionより前で停止する。

### F5nzb registered direct compositor RLE terminal Completed bridge
...
F5nzd adapterはF5nyyの十九層result graphを同じ順序で再包装し、BeginFrame step successをborrowしてF5mu projectionへexactly once渡す。private constructorはmove-only stepとCopy resultを一つのownerに封入し、public surfaceはfrom-step、borrowed result accessor、consuming take-step、freeだけを提供する。freeはtake-stepと既存F5mt step-freeを各1回使う。production adapterはprojection variant、command、descriptor、phaseを読まず、F5mv、host、scheduler APIを参照しない。fixtureは全Err authorityを既存free APIで閉じ、successだけでtyped `Record(BeginFrame)`、canonical descriptor、元stepのBeginFrame、finish-owner後の`RunPending`を検証する。
...
The affine wrapper retains either the complete F5nyf error or complete F5mf start error. Borrowed diagnostics precede consuming recovery; recovery returns completed, dirty-owner, entry, range, storage, plan, or payload authority. A valid F5nyf payload cannot honestly reach an F5mf start failure, so runtime does not forge one and relies on the existing F5mf regression plus source policy. F5nyg does not duplicate lower cursor/count starts or enter F5mg count steps, completed count, encoding, presentation, transport, host/platform/backend work, or fallback.
...
### F5nzk registered BeginFrame host action executor session request

F5nzk consumes one F5nzj owner and moves the registered command continuation and F5ne driver pending in the same transition. The driver enters existing F5nh `executor_session_start` and `executor_session_request` exactly once each. The reachable `Action` result becomes a move-only registered owner that co-locates the continuation with the F5nh session pending; expected action observation delegates to the F5nh borrowed accessor. The type-level `Completed` result immediately after `Ready` start is represented as an owner-bearing `UnexpectedCompleted` value that retains the continuation rather than synthesizing a driver or outcome. No session-pending-only consuming accessor is exposed. The slice stops before executor outcome construction, F5nh completion, F5ng/F5nf, actual executor invocation, and platform presentation.
...
F5nzt inspects the F5nzs schedule phase before consuming its move-only owner. Completed alone transfers formal parts once, recovers the EndFrame continuation through the existing F5mt finish-owner API exactly once, and invokes the existing F5mt step API exactly once. Success co-locates the updated loop state and opaque terminal step; lower failure retains the same state and owner-bearing F5mt error, while a non-Completed phase retains the original step and state as PhaseExpected recovery. Public borrowed diagnostics expose only boolean predicates and delegated Copy lower diagnostics; a consuming recovery enum returns either the phase step/state or lower state/error authority. The natural F5nzs fixture always produces Completed and its terminal step cannot enter the lower run-step failure path, so no synthetic runtime error owner is constructed; source policy verifies both recovery variants. Production does not inspect terminal result, descriptor, cursor phase, F5mu projection, host request, or platform execution.
