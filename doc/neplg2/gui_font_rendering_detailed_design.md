...
partial dirty unsupported until slot preservation/copy is intentional. Current write slots do not copy pixels from the previous published frame, so accepting a metadata row range smaller than the full frame would publish undefined pixels outside that range. F5nj therefore rejects non-full metadata row ranges as unsupported until a slot preservation or copy contract is added.
