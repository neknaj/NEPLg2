...
- source / token backed owner 入口で単一 `left |> named_target suffix...` と `left |> %fn ... named_target suffix...` を `named_target left suffix...` 相当の `DirectCall` checked tree へ正規化し、左辺と右辺 suffix の checked argument payload 順序を保持する接続。ascribed target の同名 overload narrowing は callable type equality で 0 / 1 / 複数一致を分類する。source-less reducer と pipe chain / lambda target / borrow target / 引数式先読みを伴う advanced target narrowing は fail-closed 残件として維持する
