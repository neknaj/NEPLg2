...
stage0 matrixはcondition factなし、全6 unary、全6 relation、空Any / All、nested / repeated fact、同一 / projected Place、同じowner内のBranch / Loop bindingをpositiveとしてcentral structural validatorへ通す。header、4 Place、binding missing / unexpected / identity / ordinal / None競合、non-control fact、node / edge identity・ordinal・owner・range、root、sibling、leaf child、orphan、two-parent、cycle形状、trailing、fact Place、canonical preorderのnegativeは具体的なtyped variantとordinalまで照合する。central authorityはdense kind / span ownerとtopologyを先に検査し、その後Branch header、Loop header、generic condition fact ownerの順でborrowして検査する。actual loweringとのowner co-production、conditionのcontrol-flow意味、then / else value join、production `ResourceIrEnumerated` originは未実装である。次はMatch arm payloadへ進む。

central fixtureは2 blockのglobal operation 0..22、ordinal 21のBranchとordinal 22のLoop node、empty BranchThen / Else / LoopCondition / LoopBody scopeを構築し、全既存sparse ownerとnonempty Branch / Loop header、Branch root binding / fact node、Loop None bindingを同じauthority callへ渡す。positiveはoperation count 23と`ResourceIrInventoryValidated` originを確認し、negativeはblock rangeとtopology / kind domainの不一致を同じcentral callで拒否する。fixtureだけはlegacy walker table countをResourceOp authorityにしないためcompatibility bridgeを通さず、topology-native central coreを直接検査する。
...
central authorityはdense span / kind、topology、Construct、Branch、Loop、generic condition factの順に検査する。compatibility wrapperは空Loop ownerもallocate / borrow / freeする。central control fixtureはBranch 21とLoop 22を共存させ、4個のempty child scope、両header、Branchのnonempty fact root、LoopのNone bindingを同じauthority callへ通す。block rangeを1件短くしたnegativeは存在しないLoop parent scopeを中央topology validatorで拒否する。独立matrixはLoop Noneと実EqZero root bindingに加え、header gap / excess、identity / ordinal、condition missing / graph mismatchを実validatorへ通す。全fact variantのlossless tree表現は共有generic matrixを再利用する。

この境界はLoop収束、condition truth semantics、body effect、actual loweringとのowner co-productionを証明せず、成功originは非productionの`ResourceIrInventoryValidated`を維持する。次はMatch arm pattern / bind metadata / value payloadへ進む。
