...
- lambda / borrow / source-backed advanced pipe target argument expression success path を含む prefix call reduction stack。source-backed `left |> named_target suffix...`、`left |> %fn ... named_target suffix...`、`left |> named_target suffix... |> named_target suffix...` は checked tree 付き direct call として接続済みで、注釈無し pipe target の一意 source-backed suffix は `%T literal` と `NamedValue` まで checked argument payload へ接続済み
