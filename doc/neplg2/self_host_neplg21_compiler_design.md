...
各block inventory recordはRust `ResourceBlock.span`のfile id / byte start / byte endをselfhost非負i32範囲で保持し、block key / graph / ordinal / nonnegative ID / unique ID確定後かつoperation range検査前に1回だけ妥当性を検査する。invalid IDとduplicate IDはinvalid spanより優先する。非空、empty、dummy spanを受理し、invalid spanはblock ordinalと元spanを保持して拒否する。function spanとblock spanのsame-file / containmentは要求せず、異なるfile idのpositive fixtureも受理する。actual u32 narrowingとsame-file / containmentを要求しないmulti-file source-policy aggregation provenanceは後続であり、originは`ResourceIrInventoryValidated`のままである。

各block recordはさらにRust `ResourceTerminator::Return / Unreachable / RawBody`各variant固有のspanを独立して保持する。block spanとoperation rangeを検査し、dense terminator ordinalを確定した後、payload解釈前にterminator spanを1回だけ構造検査する。非空、empty、dummyとblockとは異なるfile idを受理し、負file id、負start、end-before-startはblock ordinalと元span付き`TerminatorSpanInvalid`へ拒否する。block spanとのsame-file / containmentは要求しない。Rust u32上位域、source text境界、actual producer provenanceとoperation spanは後続であり、この構造検査だけではproduction originへ昇格しない。

Rust `ResourceOp`全24 variantが持つspanは、legacy HIR classifierのoperation recordへ混ぜず、key、graph id、dense operation ordinal、spanだけを持つ専用inventory ownerへ保存する。scopeはblock inventoryのoperation総数をlegacy owner、span owner、kind ownerの順に照合し、全count一致後にlegacy identity / ordinal、span identity / ordinal / shape、kind identity / ordinalの順で検査する。非空、empty、dummyとblockとは異なるfile idを受理し、same-file / containmentは要求しない。これはselfhost非負i32範囲の構造検査であり、Rust u32上位域、source text境界、multi-file source-policy aggregation provenance、nested Branch / Loop / Match topology、operation endpoint、actual loweringとのco-productionを証明しない。従って成功originは`ResourceIrInventoryValidated`のままで、synthetic fixture spanから`ResourceIrEnumerated`を発行しない。

Rust `ResourceOp`のtop-level 24 variantは、legacy HIR classifierとspan ownerのいずれにも混ぜず、key、graph id、dense operation ordinal、排他的kind tagを持つ専用inventory ownerへ保存する。scopeはblock inventoryで確定したoperation countをlegacy owner、span owner、kind ownerの全てと照合し、legacy identity / ordinal、span identity / ordinal / shape、kind identity / ordinalの順で検査する。全24 tagはfallbackなしでroundtripするが、Branch / Loop / Match内部の再帰operation列、各variant payload、endpointはまだ列挙しない。独立ownerの一致はactual loweringとのco-productionを証明しないため、成功originは`ResourceIrInventoryValidated`を維持する。
...
Rust `ResourceFunction.blocks[].ops`と、`ResourceOp::Branch`の`then_ops` / `else_ops`、`Loop`の`condition_ops` / `body_ops`、`Match`の各arm `ops`を単一のprivate `nodes / scopes / edges` ownerへ投影できるstandalone validator modelを先に固定する。node ordinalは将来のselfhost recursive materializerが割り当てる全block共通preorderであり、Rust modelにordinal fieldが存在するという主張ではない。現行central inventory / stage0 materializerは引き続きflat `first_operation_ordinal / operation_count`をauthorityとして使い、このtopology owner、global node ordinal、nested sparse payloadをco-produceしない。
...
現時点では`first_operation_ordinal / operation_count`がcentral authorityで、`root_operation_scope_id`はstandalone fixture用予約値に過ぎない。stage0はnested BranchのThen / Else Exprと後続root Expr、nonempty / empty Loop、0-arm / multiple dense-arm Match、control operation相互nestをpositiveにし、node / scope / edge gap、role欠落 / 重複、wrong parent、cross-block、two-parent、cycle形状、subtree range、overflow、kind / span count、orphan、identityをnegativeに固定する。次の明示sliceはrecursive materializerがtopology / kind / span / sparse payloadを同じglobal ordinalでco-produceし、central scopeをflat authorityからroot scopeへ切り替える接続である。その後にBranch condition/value fact / payloadとLoop / Match payloadを接続する。このsliceはnested payloadのcentral connection、control-flow semantics、actual lowering co-production、production originを発行しない。
