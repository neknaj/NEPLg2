...
stage0 matrixはcondition factなし、全6 unary、全6 relation、空Any / All、nested / repeated fact、同一 / projected Place、同じowner内のBranch / Loop bindingをpositiveとしてcentral structural validatorへ通す。header、4 Place、binding missing / unexpected / identity / ordinal / None競合、non-control fact、node / edge identity・ordinal・owner・range、root、sibling、leaf child、orphan、two-parent、cycle形状、trailing、fact Place、canonical preorderのnegativeは具体的なtyped variantとordinalまで照合する。central authorityはtopologyとdense kind / span ownerを先に検査し、その後Branch header、generic condition fact ownerの順でborrowして検査する。actual loweringとのowner co-production、conditionのcontrol-flow意味、then / else value join、Loop固有header、production `ResourceIrEnumerated` originは未実装である。次はLoop固有headerを追加し、その後Match arm payloadへ進む。

central fixtureは2 blockのglobal operation 0..21、ordinal 21のBranch node、empty BranchThen / Else scopeを構築し、全既存sparse ownerとnonempty Branch header / root binding / fact nodeを同じauthority callへ渡す。positiveはoperation count 22と`ResourceIrInventoryValidated` originを確認し、negativeはblock rangeとtopology / kind domainの不一致を同じcentral callで拒否する。fixtureだけはlegacy walker table countをResourceOp authorityにしないためcompatibility bridgeを通さず、topology-native central coreを直接検査する。
