...
2026-06-05 trailing block argument checkpoint では、`BlockIntro` を flat prefix item に混ぜず、source-backed owner 入口へ optional trailing block evidence として渡す境界を追加した。`add 1:` のように head prefix の引数が不足し、残る parameter を nested body が満たす位置では、現 checkpoint は block body を仮に `unit` などとして受理せず `TrailingBlockArgumentUnsupported` を返す。`add 1 2:` のように parameter を消費し終えた後に block が残る場合は `UnexpectedTrailingBlockArgument` として拒否する。これにより、block argument 未対応を `PartialApplicationRejected` や raw arity error へ潰さず、次 slice の block body result checker が接続するための typed boundary を固定した。
...
- block body result checker、lambda / borrow / pipe argument expression checking を含む prefix call reduction stack
...
- BlockIntro 専用 owner 入口で `head` prefix と `body` envelope を分け、末尾 block argument を `TrailingBlockArgumentUnsupported` / `UnexpectedTrailingBlockArgument` として typed fail-closed にする接続
