...
block terminator inventoryはRust `Option<Place>`に対応するtyped payloadを保持する。`Return`はNoneまたはsame graph identity付き非負idのResource Placeを持て、専用Place inventory ownerのsame request / graph recordに存在する場合だけ受理する。このdense PlaceId inventoryはRustに同型tableがあるという主張ではなく、後続materializerがRustの`Place { root, projections, ty }`構造値をcanonicalizeするためのselfhost固有modelである。Place recordはRust `PlaceRoot`のLocal / Temporary / I32Constant / Return / Storage / Unknownをfallbackなしで保持する。現段階のLocal payloadはRust `Local(String)`やstable symbol authorityに未結合な非zero opaque identityであり、Temporary / Storageの負identityとLocal placeholder 0だけをstructural rejectionする。UnknownはRust sentinelを保持するためinventoryでは受理するが、`ResourceIrEnumerated`発行前に拒否またはunsupportedへ明示分類する。typeは非負`SelfhostTypeId`を同じrecordへ保持し、scopeがborrowした同じ`SelfhostTypeArena`内にrecordが実在することまで検査する。canonical type keyとactual lowering由来arena authorityはまだ証明しない。`Unreachable`と`RawBody`にPlace payloadが付く形、負のPlace id、block inventoryと異なるgraph identity、未登録Placeは拒否する。projectionとescape classificationはまだ扱わない。

Place projectionは巨大proof gate内のflat tagged recordへ押し込まず、`resource_ir_place_projection.nepl`のvariant-native modelとして分離する。Rust `PlaceProjection` 5 variantと`ResourceOffset` 6 variantをnested enumで保持し、signed i64 offsetを縮退させない。Rustの`Box<Place>`は現段階では同一function owner内Placeへのopaque index link、EnumPayload Stringはopaque variant identityであり、usize payloadは非負i32範囲に限定する。このmodelはstructural transportだけで、Place inventory ownerへのdense projection列接続、recursive link membership、variant stable symbol結合、usize範囲外rejection、Unknownのunsupported分類は次段で行う。
