...
- lambda / borrow / pipe chain / advanced pipe target argument expression checking を含む prefix call reduction stack。単一の source-backed `left |> named_target suffix...` と `left |> %fn ... named_target suffix...` は checked tree 付き direct call として接続済み
...
- source / token backed owner 入口で単一 `left |> named_target suffix...` と `left |> %fn ... named_target suffix...` を `named_target left suffix...` 相当の `DirectCall` checked tree へ正規化し、左辺と右辺 suffix の checked argument payload 順序を保持する接続。source-less reducer と pipe chain / lambda target / borrow target / overload narrowing を含む advanced target は fail-closed 残件として維持する
