...
2026-06-05 checked argument payload checkpoint では、`check/expr/argument_payload.nepl` を追加し、1 つの実引数式を検査した結果を `SelfhostCheckedArgument` として保持する境界を作った。`TypedExpression` は literal / named value / ascribed value のように expression type だけで十分な引数を表し、`FunctionValue` は `@ident` が選んだ `SelfhostCallableCandidate` と use-site span を保持する。`NestedDirectCall` は nested named call が direct call として縮約されたことを保持し、`BlockResult` は末尾 block body が外側 parameter の expected type を満たしたことを保持する。`SelfhostExprArgumentOwnedMatch`、`SelfhostCallReduceOwnedResult`、`SelfhostExpressionLineCheckSuccess` はこの checked argument payload を owner として運び、`next_index` だけに戻らない。`check/expr` は HIR を直接作らず、`@ident` 由来の `FunctionValue(candidate)` を後続の `lower/hir/function_value.nepl` へ渡せる証拠として保存する。現 checkpoint では payload list を HIR direct call tree に消費する処理はまだ未接続であり、nested call の内側 argument tree 全体も `NestedDirectCall` summary に留める。次の slice では checked argument list を HIR lowering の direct call / indirect call 境界へ渡し、`FunctionValue(candidate)` を `SelfhostHirExprPayload::FnValue` へ変換する。

2026-06-05 HIR direct call checked argument lowering checkpoint では、`lower/hir/direct_call.nepl` を追加した。この module は `SelfhostCallReduceResult::DirectCall`、`Vec SelfhostCheckedArgument`、candidate table を受け取り、HIR module に argument child expression と parent call expression を追加する。callee は `SelfhostReducedCall.candidate_index` で candidate table から得るため、prefix token spelling や scope lookup を lowering で再実行しない。現 checkpoint で HIR child にできる argument kind は `FunctionValue(candidate)` だけであり、これは `lower/hir/function_value.nepl` の `selfhost_hir_expr_fn_value_from_candidate` を通って `SelfhostHirExprPayload::FnValue` になる。`TypedExpression`、`NestedDirectCall`、`BlockResult` は、literal / value / nested tree / block tree の lowerable payload がまだ足りないため `UnsupportedArgumentKind` として fail-closed にする。`SelfhostExpressionLineCheckSuccess` を borrow する facade も用意したが、success owner は caller が閉じる。これにより、checked argument payload transport は HIR direct call の最初の消費点まで接続された。

2026-06-05 unit argument HIR lowering checkpoint では、`unit` literal argument を `SelfhostCheckedArgumentKind::UnitValue` として `TypedExpression` から分離した。`unit` は値 payload を持たず、checker が item kind と expected type の一致を確認した時点で HIR lowering に必要な情報が `value_type` と `span` に揃う。そのため `lower/hir/direct_call.nepl` は source token や lexeme を再読せず `UnitValue` を HIR の unit expression child に変換できる。数値、bool、char、string literal は実際の値 payload と literal parser、`NamedValue` は DefId-linked variable identity、`NestedDirectCall` と `BlockResult` は内側の checked tree がまだ不足しているため、引き続き fail-closed にする。
