...
`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` に、module-private traversal source table と complete traversal authority から public coverage handoff pair を作る producer boundary を追加した。complete authority は request root、body root、body module fingerprint、graph id を持ち、source record の proof key root / body fingerprint / graph id と照合する。follow-up では production reader context path も接続し、`actual_traversal_body_resolution_lookup_result` が module に対して検査した body root と、`actual_traversal_body_adapter_sources_from_request_context_result` が返す source table owner を同じ coverage producer へ渡すようにした。request root を public coverage handoff の body root として流用しない。
...
source policy は `nodesrc/test_selfhost_memo_call_backend_private_cache_proof_gate_contract.js` で更新した。complete authority / handoff pair の backend-private 化、public API での authority / pair 露出禁止、`SourceRejected` から内部 bridge error payload を公開しないこと、empty source rejection、PrivateCache / PrivateState effect operation から no-escape を推測しないこと、mixed absence+escape が `EffectObservedMayEscape` に畳まれること、source record と complete authority の fingerprint / request root / graph id 照合を固定している。follow-up では resolver-bound authority 作成、source-derived witness 前の production source owner mapping、reader-context neutral body の `33` と PrivateCache effect body の `53` も固定した。
...
`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、resolver-bound reader context から作った 1 つの source table owner を、coverage handoff pair と source-derived fresh witness request-evidence gate の両方に渡す backend-private boundary を追加した。

coverage handoff pair は borrowed source owner から先に作り、成功した場合だけ同じ owner を `actual_traversal_bundle_source_derived_witness_result` へ移す。coverage 側で拒否した場合は source owner をこの boundary で閉じ、witness 側の拒否は既存 `SelfhostMemoCallBackendPrivateCacheRegionProofProducerErrorKind` に包む。これにより coverage と request-evidence が別 source table を読む authority drift を防ぐ。

public stage0 summary は accepted request/proof count、coverage pair code、PrivateCache effect body の typed rejection だけを返す。neutral body は same-owner path で coverage pair code `33`、request/proof `1/1` になる。PrivateCache effect source は coverage 側では unsupported handoff に分類されるが、fresh witness 側では no-escape proof に昇格しない。

この checkpoint は full Resource IR traversal ではない。actual fresh-region witness table、no-escape authority、GraphInput、Resource proof table、checker proof table、PrivateCache / PrivateState effect mask、sealed backend representation、backend bytes、prechecked artifact、`.neplobj` / `.neplproof` artifact key は作らない。`EffectObservedNoEscape` は引き続き actual traversal 由来 fresh witness / no-escape authority が同じ body identity に束ねられた後だけ発行する。

source policy は `nodesrc/test_selfhost_memo_call_backend_private_cache_proof_gate_contract.js` で固定した。combined coverage/witness owner と gate summary の public exposure 禁止、combined owner の Clone / Copy 禁止、coverage before witness の順序、coverage failure 時の source cleanup、stage0 witness fixture / GraphInput / lower proof / backend / effect mask / artifact 合成禁止、reader context から resolver-derived coverage authority と単一 source owner を作る順序を検査する。

残件:

- full Resource IR / HIR lowering traversal が、accepted / escaping / observation / unsupported source vocabulary と actual fresh witness / no-escape authority を同じ resolver-bound body identity で発行する。
...
新しい `actual_traversal_body_reader_fresh_witness_authority_bundle_from_request_context_result` は、`actual_traversal_body_reader_sources_from_request_context_result` が返す source table owner だけを authority として受け取り、`actual_traversal_fresh_witness_authority_bundle_from_sources_result` へ move する。`FreshWitnessAuthorityBundle` は source table owner を内包するため、source table を別 field として二重所有しない。body-reader no-escape request-context helper はこの boundary へ直接委譲し、`actual_traversal_body_reader_events_from_request_context_result`、`actual_walker_event_split_result`、`actual_traversal_body_adapter_sources_from_request_context_output_result` を fresh witness authority の主経路にしない。

accepted / escaping / observation / unsupported source vocabulary は reader source output 由来の同一 body identity に寄り、fresh witness authority も同じ source owner から作られる。external witness metadata、`RegionFreshWitnessStatus` fixture、split-output fixture、stage0 witness wrapper、`ActualTraversalBundle`、source-derived request-evidence bundle、request-evidence gate、GraphInput、Resource proof table push、checker proof table、PrivateCache / PrivateState effect mask、backend bytes、sealed representation、`.neplobj` / `.neplproof` artifact key は作らない。

この checkpoint も full Resource IR traversal ではない。source output は HIR body reader source vocabulary であり、actual Resource IR / HIR lowering traversal が独立した Resource walker event と fresh-region witness table を実発行したわけではない。残件は、full Resource IR / HIR lowering traversal が accepted / escaping / observation / unsupported source vocabulary、actual fresh witness / no-escape authority、coverage handoff を同じ resolver-bound body identity で発行し、PrivateCache / PrivateState effect mask 実体、sealed backend representation、artifact stable key projectionへ渡す production boundary である。

## 2026-06-22 selfhost reader fresh witness authority producer checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、body-reader no-escape coverage path から caller supplied witness fingerprint / graph / ordinal / status を外し、resolver-bound reader source owner から same-source candidate 由来の fresh witness authority bundle を作る backend-private producer を追加した。

新しい `actual_traversal_fresh_witness_authority_bundle_from_sources_result` は、source owner から region proof table を作り、candidate checker を通した後、source table を直接読み直して closed wrapper accepted pair に限定する。許可するのは 2 record ちょうど、ordinal 0 の `PrivateCacheStoragePlace` root、ordinal 1 の `CloneOutOwnedValueEdge` support、from/to place 0、candidate と同じ key / graph の場合だけである。この guard を通った candidate だけから fresh witness table owner を作る。candidate extraction 後は region proof table を閉じ、shape guard / candidate / witness 作成失敗では source owner も閉じる。成功時だけ source owner と witness owner を `SelfhostMemoCallBackendPrivateCacheActualTraversalFreshWitnessAuthorityBundle` へ移す。

body-reader no-escape helper は `actual_traversal_body_reader_events_from_request_context_result`、`actual_walker_event_split_result`、`actual_traversal_body_adapter_sources_from_request_context_output_result` の後、この producer だけを呼ぶ。`actual_traversal_fresh_witness_authority_bundle_stage0_with_sources_result`、`RegionFreshWitnessStatus` fixture、root/support ordinal 引数、witness fingerprint mismatch fixture は body-reader path から外した。witness missing / rejected / authority mismatch の taxonomy は既存の actual no-escape coverage stage0 と operation-classified smoke に残し、body-reader stage0 は accepted same-source authority と HIR body problem source rejection だけを公開する。

この checkpoint も full Resource IR traversal ではない。accepted source は HIR body reader finalizer の wrapper source pair であり、fresh witness authority はその same-source candidate 由来である。actual Resource IR / HIR lowering traversal が独立に fresh witness authority を実発行したわけではない。`ActualTraversalBundle`、source-derived request-evidence bundle、request-evidence gate、GraphInput、Resource proof table push、checker proof table、PrivateCache / PrivateState effect mask、backend bytes、sealed representation、`.neplobj` / `.neplproof` artifact key は作らない。

残件:

- full Resource IR / HIR lowering traversal が、accepted / escaping / observation / unsupported source vocabulary と actual traversal-owned fresh witness authority bundle を same resolver-bound body identity で発行する。今回の same-source candidate 由来 producerは、external witness fixture を外す境界であり、Resource IR walker 本体の代替ではない。
- production coverage handoff producer から no-escape coverage authority boundary と checker-layer upper orchestration へ、fixture ではなく full traversal output 由来の public evidence を渡す。
- Resource summary hash invalidation、artifact policy hash、PrivateCache / PrivateState effect mask 実体、sealed memoized backend representation、Wasm / LLVM bytes、`.neplobj` / `.neplproof` stable key projectionへ接続する。

## 2026-06-22 selfhost body-reader no-escape coverage authority checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、resolver-bound HIR body reader source plan を event producer、unified event split、context-bound split-output adapter、actual traversal-owned fresh witness authority bundle の順に通し、no-escape coverage authority boundary へ渡す backend-private smoke を追加した。

新しい path は `actual_traversal_private_effect_coverage_authority_from_reader_context_result` で coverage complete authority を作ってから、同じ reader context / resolution table で body reader events を作る。split output owner から source owner へ戻す処理は `actual_traversal_body_adapter_sources_from_request_context_output_result` に集約し、walker input / observation owner cleanup と context-bound source validation を重複実装しない。成功した source owner だけを `SelfhostMemoCallBackendPrivateCacheActualTraversalFreshWitnessAuthorityBundle` に渡す。

public stage0 summary は accepted pair code、missing / rejected witness、witness authority mismatch、HIR body `PrivateCache` effect rejection、FnValue / MemoizedFunctionValue observation rejection、pure call unsupported rejection の compact no-escape Result だけを返す。neutral body は wrapper source pair から pair code `13` に到達するが、これは executable backend plan ではない。PrivateCache effect、function identity observation、pure call unsupported は accepted wrapper source に混ぜず、typed no-escape rejection として固定する。

この checkpoint は full Resource IR traversal ではない。accepted source は HIR body reader finalizer の wrapper source plan であり、actual Resource IR / HIR lowering traversal が accepted source / fresh witness authority を実発行したことを意味しない。`ActualTraversalBundle`、source-derived witness helper、request-evidence gate、GraphInput、Resource proof table push、checker proof table、PrivateCache / PrivateState effect mask、backend bytes、sealed representation、`.neplobj` / `.neplproof` artifact key は作らない。
...
2026-06-22 follow-up では、production reader output と operation producer bridge が読む source authority を `actual_traversal_body_reader_sources_from_request_context_result` に一本化した。output path はこの source plan を `actual_traversal_body_reader_output_from_context_sources_result` に渡し、context-bound validation、operation table projection、`actual_walker_operation_classifier_append_records_loop`、`actual_walker_event_split_result` を通して owner-bearing output を得る。source / operation / event / split failure は availability error と bridge error の双方で typed に保ち、key / graph mismatch を seed mismatch とは分ける。
...
この checkpoint は fresh witness authority 発行を producer-owned source lifecycle に近づけるものであり、full Resource IR graph walker 完了ではない。underlying source vocabulary はまだ resolver-bound HIR body reader 由来である。request-evidence gate、GraphInput / proof table push、PrivateCache / PrivateState effect mask、sealed memoized backend representation、Wasm / LLVM fragment、`.neplobj` / `.neplproof` artifact key はまだ作らない。残件は actual Resource IR graph walker 本体が source / fresh witness / coverage を実発行すること、PrivateCache / PrivateState effect mask 実体、sealed backend representation、artifact stable key projectionである。

## 2026-06-23 selfhost production source producer boundary checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、`ResourceLoweringTraversalProduced` output の source owner 生成を `actual_traversal_resource_lowering_producer_sources_from_body_root_result` に分離した。この helper は request-context で resolver 済みの body root だけを受け取り、body root lookup を繰り返さない。
...
## 2026-06-23 selfhost resource-lowering traversal authority entry checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、producer traversal output owner から fresh witness authority bundle と producer authority output へ進む module-private 境界を追加した。`actual_traversal_resource_lowering_producer_traversal_output_into_fresh_witness_authority_bundle_result` は walker input / observation owner を collector-owned source table へ戻す処理を helper 内に閉じ、producer 専用 source-to-witness helper へ move する。source collection failure は `Stage0SourceRejected` に写し、source table owner を caller に返さない。

`actual_traversal_resource_lowering_producer_traversal_output_into_authority_output_result` は traversal output が保持する context / body root と coverage authority を照合し、成功した場合だけ traversal output 由来 fresh witness authority bundle を producer authority output へ束ねる。coverage validation failure では traversal output の walker input / observation owner を閉じる。`actual_traversal_resource_lowering_producer_authority_output_from_body_root_result` は source table を直接取り出さず、この traversal-output authority helper へ owner を渡すだけになった。

この checkpoint も full Resource IR graph walker 完了ではない。underlying source vocabulary はまだ resolver-bound HIR body reader 由来であり、request-evidence gate、GraphInput / proof table push、PrivateCache / PrivateState effect mask、sealed memoized backend representation、Wasm / LLVM fragment、`.neplobj` / `.neplproof` artifact key はまだ作らない。残件は actual Resource IR graph walker 本体が accepted / escaping / observation / unsupported source と fresh witness / coverage authority を同じ resolver-bound body identity へ実発行すること、PrivateCache / PrivateState effect mask 実体、sealed backend representation、artifact stable key projectionである。

## 2026-06-23 selfhost resource-lowering traversal coverage owner checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、`ActualTraversalResourceLoweringProducerTraversalOutput` が coverage authority も保持するようにした。これにより、producer traversal output owner は context、coverage authority、body root、walker input owner、observation table owner を同じ resolver-bound body identity で束ねる。coverage authority は split output conversion 時に context / body root から作り、source-only producer output や producer authority output の body-root helper が別途再生成しない。

`actual_traversal_resource_lowering_producer_traversal_output_into_output_result` を追加し、source-count / origin smoke 用の `ActualTraversalResourceLoweringProducerOutput` も traversal output owner から派生するようにした。`actual_traversal_resource_lowering_producer_traversal_output_into_authority_output_result` は coverage authority を引数で受け取らず、traversal output owner から取り出して identity を再検査する。body-root source-only helper と body-root authority helper は、いずれも traversal output owner へ委譲するだけになった。

この checkpoint も full Resource IR graph walker 完了ではない。underlying source vocabulary はまだ resolver-bound HIR body reader 由来であり、request-evidence gate、GraphInput / proof table push、PrivateCache / PrivateState effect mask、sealed memoized backend representation、Wasm / LLVM fragment、`.neplobj` / `.neplproof` artifact key はまだ作らない。残件は actual Resource IR graph walker 本体が accepted / escaping / observation / unsupported source と fresh witness / coverage authority を同じ resolver-bound body identity へ実発行すること、PrivateCache / PrivateState effect mask 実体、sealed backend representation、artifact stable key projectionである。

## 2026-06-23 selfhost resource-lowering source vocabulary authority checkpoint

`stdlib/neplg2/core/codegen/memo_call_backend_private_cache_proof_gate.nepl` で、producer source table から accepted / escaping / observation / unsupported の count を持つ `ActualTraversalProducerSourceVocabulary` を作る module-private summary を追加した。summary は source owner ではなく copyable value であり、`PrivateCacheEffectOperation` / `PrivateStateEffectOperation` は no-escape proof ではないため unsupported count に入れる。

`actual_traversal_resource_lowering_producer_traversal_output_from_sources_result` は context-bound source validation 後、source owner を split output へ move する前に vocabulary summary を作り、producer traversal output owner に束ねる。source-only producer output と producer authority output も同じ summary を保持するため、fresh witness authority bundle は accepted source owner だけでなく、その producer が観測した source vocabulary と同じ resolver-bound body identity に紐づく。summary の fold は wildcard fallback を使わず、escaping source、observation source、unsupported observation、`ResourceIrTraversalUnavailable` を accepted count へ混ぜない。
...
各block inventory recordはRust `ResourceBlock.span`のfile id / byte start / byte endをselfhost非負i32範囲で保持し、block key / graph / ordinal / nonnegative ID / unique ID確定後かつterminator構造検査前に1回だけ妥当性を検査する。invalid IDとduplicate IDはinvalid spanより優先する。非空、empty、dummy spanを受理し、invalid spanはblock ordinalと元spanを保持して拒否する。function spanとblock spanのsame-file / containmentは要求せず、異なるfile idのpositive fixtureも受理する。actual u32 narrowingとsame-file / containmentを要求しないmulti-file source-policy aggregation provenanceは後続であり、originは`ResourceIrInventoryValidated`のままである。

各block recordはさらにRust `ResourceTerminator::Return / Unreachable / RawBody`各variant固有のspanを独立して保持する。block identityとspanを検査し、dense terminator ordinalを確定した後、payload解釈前にterminator spanを1回だけ構造検査する。operation列との結合は後続のtopology root forest検査が担い、flat rangeをblock validatorのauthorityにしない。非空、empty、dummyとblockとは異なるfile idを受理し、負file id、負start、end-before-startはblock ordinalと元span付き`TerminatorSpanInvalid`へ拒否する。block spanとのsame-file / containmentは要求しない。Rust u32上位域、source text境界、actual producer provenanceとoperation spanは後続であり、この構造検査だけではproduction originへ昇格しない。

Rust `ResourceOp`全24 variantが持つspanは、legacy HIR classifierのoperation recordへ混ぜず、key、graph id、dense operation ordinal、spanだけを持つ専用inventory ownerへ保存する。scopeはrecursive topologyのnode数をspan ownerとkind ownerのglobal ordinal domainにし、span identity / ordinal / shape、kind identity / ordinal、topology cross-ownerの順で検査する。非空、empty、dummyとblockとは異なるfile idを受理し、same-file / containmentは要求しない。これはselfhost非負i32範囲の構造検査であり、Rust u32上位域、source text境界、multi-file source-policy aggregation provenance、operation endpoint、actual loweringとのco-productionを証明しない。従って成功originは`ResourceIrInventoryValidated`のままで、synthetic fixture spanから`ResourceIrEnumerated`を発行しない。

Rust `ResourceOp`の24 variantは、legacy HIR classifierとspan ownerのいずれにも混ぜず、key、graph id、dense global operation ordinal、排他的kind tagを持つ専用inventory ownerへ保存する。scopeはrecursive topologyのnode数をspan ownerとkind ownerの両方に照合し、span identity / ordinal / shape、kind identity / ordinal、各nodeのkind / span結合を検査する。全24 tagはfallbackなしでroundtripし、Branch / Loop / Match内部の再帰operation列もtopology nodeとして同じordinal domainに入るが、各control variant payloadとendpointはまだ列挙しない。独立ownerの一致はactual loweringとのco-productionを証明しないため、成功originは`ResourceIrInventoryValidated`を維持する。

`ResourceOp::Expr`はoperation kind ownerへpayloadを混ぜず、Expr operationだけを持つsparse ownerへ分離する。recordはkey、graph id、operation ordinal、Rust `ResourceExprKind`全18 variant、output Place link、expression TypeIdを保持し、`LayoutSizeOf`だけは対象TypeIdもvariant payloadとして保持する。scopeはkind ownerとmerge-cursorで照合し、Exprのpayload欠落とnon-Exprへのpayload混入を拒否する。accepted payloadは同じPlace ownerのoutput membership、同じTypeArenaのexpression TypeId、output Placeとの型一致、`LayoutSizeOf`対象型のarena membershipを要求する。巨大scope checkerの型推論量を増やさないため、このmerge検査とauthority生成は既存legacy / span / kind検査後のprivate helperへ分離する。残る23 operation payload、Expr variant内部のPlace / call target / branch topology、actual materializer co-productionは未完成であり、originは`ResourceIrInventoryValidated`のままである。

`ResourceOp::DeclareLocal`もoperation kind ownerと分離したprivate sparse payload ownerで保持する。recordはkey、graph id、operation ordinal、宣言Place、opaque source-name identity、mutable flag、optional initializer Placeを持つ。ただしRustの`source_name: String`自体は保持しないため、このsliceはDeclareLocal payloadのlossless表現ではない。opaque identityはsource textやdiagnostic文字列のauthorityではなく、stage0で`PlaceRoot::Local`と同じ値へ結合する非zero function-local投影に留まる。scopeは宣言Placeがsame graphのprojectionを持たないLocal rootで、そのidentityが一致することを要求する。initializerがある場合もsame graphのPlace inventory membershipを要求する。kind ownerとのmerge-cursorはDeclareLocal payload欠落とnon-DeclareLocal payload混入を拒否する。canonical local-name authority、Stringとの結合、actual HIR-to-Resource co-productionは未完成なのでproduction originは発行しない。

`ResourceOp::Read`はspan ownerとkind ownerに加え、source / outputを専用private sparse payload ownerに保持する。各Place handleはResource graph identityを内包し、record自身のproof key / graph / operation ordinalを検査した後、source、outputの順でsame-graph Place inventory membershipを要求する。kind ownerとのmerge-cursorはReadだけにpayloadを1件要求し、欠落とnon-Read ordinalへの混入を拒否する。sourceとoutputは別handleであり、同一視やimplicit copyをしない。このinventoryはactual materializer由来ではないため、検証後も`ResourceIrInventoryValidated`だけを発行する。

function inventoryはRust `ResourceFunction.span`を`SelfhostSourceSpan`のfile id / byte start / byte endとして保持する。ただしRustのu32全域ではなくselfhostの非負i32範囲に限定し、actual adapterでのu32上位域rejectionは後続とする。非空rangeとempty rangeを受理し、負file id、負start、end-before-startはheader検査の`FunctionSpanInvalid`で元span値を保持したまま拒否する。source text length、UTF-8境界、stable file identity、block / terminator / operation spanとの包含やsame-file関係は証明しない。span妥当性だけからproduction provenanceは発行せず、originは引き続き`ResourceIrInventoryValidated`である。

Rustの`ResourceFunction`はordered `type_params: Vec<TypeId>`、`result: TypeId`、surface `effect: Effect`、`entry_block: ResourceBlockId`、`blocks: Vec<ResourceBlock>`を持ち、各blockは独立した`id: ResourceBlockId`、`ops: Vec<ResourceOp>`、必須の`ResourceTerminator`を持つ。selfhost側でもtype parameter owner、result TypeId、surface Pure / Impure、entry block ID、block tableを同じfunction inventory ownerへ保持する。declaration側type parameter ownerはcall occurrence側proof keyの`type_arg_count`とは別authorityであり、count一致から同一視しない。ordered ownerは全要素の非負index、scopeがborrowした同じTypeArena内のrecord実在、`Parameter` record variant、非負binder depth/index、単一function binderの`depth=0 / parameter_index=ordinal`を順に検査し、dangling TypeId、primitive等の異種record、重複・並べ替えbinderをgeneric未対応へ丸めない。negative bindingはsafe arena APIが投入前に拒否し、validator側のinvalid taxonomyはunchecked/raw arenaへのdefense-in-depthである。selfhost `Parameter`はRust `TypeKind::Var`全情報ではなくbinder参照shapeだけを投影するため、Rust側のbound/unbound Optionやcopy/clone/drop capabilityを検査済みとは主張しない。現行memo backendはmonomorphic requestだけを受理するため、全要素のshape確認後に非空inventoryをtyped unsupported rejectionする。actual lowering由来unbound evidence、canonical binder identity、arena authorityは後続に残す。Rust surface effectは内部`SelfhostEffectKind`と同一domainではないため専用enumで保持し、memo request proofが受理するPureと一致する場合だけPlace走査へ進む。resultも同じarena内のrecord実在までを検査するが、canonical type keyは後続に残す。block recordはtable走査用dense ordinalとは別にblock IDを保持し、全IDが非負かつ一意で、entry IDに一致するrecordが実在する場合だけscope検査を続ける。各blockはrecursive topologyの`BlockOps` root scopeをexactに指し、block ordinal順のroot forestが全nodeを隙間なく覆う。flat `first_operation_ordinal / operation_count`は旧stage0 fixture shapeとの互換fieldであり、central block validatorはoperation authorityとして検査しない。blockごとに1件のdense terminator ordinalとterminator kindを保持し、RawBody terminatorはpayloadなしの単一tagへ縮退させず、Rust `RawBodyKind::Wasm / LlvmIr`を排他的payloadとして保持する。

`resource_ir_inventory_scope_authority_result`は空inventoryを拒否し、recursive topologyのroot forest、nodes / scopes / edges、kind / span owner、実装済みsparse payload ownerがsame key / graph / dense global ordinalで整合する場合だけ`ResourceIrInventoryValidated` scopeを発行する。legacy actual-walker operation tableはこのtopology-native authorityの入力ではなく、旧stage0 compatibility wrapperだけがtable shapeを別途検査する。この中間originはcoverage transport後もproduction validatorで拒否する。scopeのexpected event countはbody header 1件とtopology node数から定め、legacy event producer側のcountはcompatibility boundaryでのみ照合する。terminatorはeventに偽装せずinventory coverageとして検査する。

このcheckpointはactual provenanceの前段構造検査に留まり、`ResourceIrEnumerated`は発行しない。HIR lowering結果から`ResourceFunction` inventory、recursive topology、kind / span / sparse payload ownerを同一materializerからco-produceする本体、nested operationの意味分類、terminator return placeのescape event化は未実装である。Branch condition / value / recursive condition fact payloadはtopologyのglobal ordinalへ接続済みであり、次は同じfact ownerをLoopへ再利用してからactual co-productionへ進む。

block terminator inventoryはRust `Option<Place>`に対応するtyped payloadを保持する。`Return`はNoneまたはsame graph identity付き非負idのResource Placeを持て、専用Place inventory ownerのsame request / graph recordに存在する場合だけ受理する。このdense PlaceId inventoryはRustに同型tableがあるという主張ではなく、後続materializerがRustの`Place { root, projections, ty }`構造値をcanonicalizeするためのselfhost固有modelである。Place recordはRust `PlaceRoot`のLocal / Temporary / I32Constant / Return / Storage / Unknownをfallbackなしで保持する。現段階のLocal payloadはRust `Local(String)`やstable symbol authorityに未結合な非zero opaque identityであり、Temporary / Storageの負identityとLocal placeholder 0をstructural rejectionする。UnknownはRust sentinelとしてmodelに残すが、通常rootとして意味を推測せずinventory scopeでtyped unsupported rejectionにする。typeは非負`SelfhostTypeId`を同じrecordへ保持し、scopeがborrowした同じ`SelfhostTypeArena`内にrecordが実在することまで検査する。canonical type keyとactual lowering由来arena authorityはまだ証明しない。`Unreachable`と`RawBody`にPlace payloadが付く形、負のPlace id、block inventoryと異なるgraph identity、未登録Placeは拒否する。escape classificationはまだ扱わない。

Place projectionは巨大proof gate内のflat tagged recordへ押し込まず、`resource_ir_place_projection.nepl`のvariant-native modelとして分離する。Rust `PlaceProjection` 5 variantと`ResourceOffset` 6 variantをnested enumで保持し、signed i64 offsetを縮退させない。Place inventory recordの`projection_count`とPlace順prefix sumで別ownerのprojection範囲を確定し、各projection recordのrequest key / graph / Place id / ordinal、variant固有payloadを全検査する。`StorageOffset`内の`Symbolic` / `ScaledSymbolic` / `Offset` / `ScaledOffset`が持つrecursive Place linkは同じPlace ownerの実在recordへ解決できる場合だけ受理する。ResourceOffset::Unknownはsentinelとしてmodelに残すが、通常offsetとして意味を推測せずinventory scopeでtyped unsupported rejectionにする。Rustの`Box<Place>`はまだopaque index linkで、usize payloadは非負i32範囲に限定する。EnumPayloadはraw i32ではなく非zeroのtyped stable variant-symbol evidenceを持ち、同じsymbol authority内のidentity比較を専用関数へ集約する。ただしRust `String` spellingのcanonical intern、enum type keyとのmembership、serialized symbol authorityはactual materializerに未接続であり、lossless String保持やproduction provenanceは主張しない。usize範囲外rejectionとactual loweringとのco-productionは次段で行う。
...
2026-07-15 generic Match arm inference checkpointでは、先頭direct variant armのenum constructor arityからarena-local fresh inference variableを生成し、`Applied<Enum, fresh...>` を `MatchArmDerived` expected型としてscrutinee retryへ渡す。候補resultとの照合は候補ごとの所有binding tableで行い、同じfresh variableの再出現を同一具体型へ制約する。tableは候補ごとに空から開始して必ず閉じるため、失敗候補の束縛は次候補へ漏れない。未解決variableはcanonical key、substitution、memo trait/layout/producer/recursive aggregateへ流出させずtyped errorで拒否する。現段階ではpayload型へのsubstitution、pattern bind、arm body型統一、一般callのgeneric inferenceは未接続である。
...
### 2026-07-13 Resource Call payload

`ResourceOp::Call { output, target, args, effect, span }`はkind tag 10専用のprivate sparse headerと、source orderのargument Placeおよびtarget固有type argumentを持つ2つのdense endpoint ownerへ分ける。`ResourceCallTarget`は`Builtin { name }`、`User { name, type_args }`、`Trait { application, method, self_ty }`をvariant-nativeに保持する。builtin name、user name、trait application identity、trait method identityは異なるcanonical symbol roleで包み、表示文字列の一致をtarget identityの代用にしない。Traitの`self_ty`とUser / Traitのordered type argumentsは同じborrowed TypeArenaのmembershipに結合する。

headerはproof key、graph、operation ordinal、same-graph output Place、checked argument range、variant-native target、closed operation `EffectOp`を持つ。argument endpointはoperation / argument ordinalとgraph-bound Placeを、type-argument endpointはoperation / type-argument ordinalとTypeIdを保持する。Rustの`Vec`と同じく0件、複数件、重複Place / TypeIdを許す。Builtinはtype argument ownerを消費せず、同operationにendpointがあればtyped rejectionする。scopeはkind ownerと3 ownerをmergeし、missing / unexpected、identity / ordinal、range overflow / gap / excess、canonical symbol、Place graph / membership、TypeArena membershipをfield固有errorで検査する。この境界はdirect-call resolution、trait dispatch、callee signatureとargs / outputのtype整合、effect summary適用、actual loweringとowner tableのco-productionを証明しない。成功originは非productionの`ResourceIrInventoryValidated`を維持し、残るoperation payloadはIndirectCall、Construct、Branch、Loop、Matchの5件である。

### 2026-07-13 Resource IndirectCall payload

`ResourceOp::IndirectCall { output, callee, params, result, args, effect, span }`はkind tag 11専用のprivate sparse header、source orderのparameter TypeId owner、source orderのargument Place ownerへ分ける。headerはproof key、graph、operation ordinal、独立したoutput / callee Place、parameter / argumentのchecked range、result TypeId、closed operation `EffectOp`を保持する。paramsとresultは同じborrowed TypeArenaへ、output、callee、argsはsame-graph Place inventoryへ結合する。

Rustの`Vec`と同じくparameter / argumentは0件、複数件、同一TypeId / Placeの重複を許す。outputとcalleeも同一Placeを許容し、manual ownerがproducer固有alias規則を推測しない。scopeはkind ownerと3 ownerをmergeし、header / dense endpointのmissing / unexpected、identity / ordinal、range overflow / gap / excess、TypeArena membership、Place graph / membership、closed effect payloadをfield固有errorへ拒否する。stage0はzero range、multiple / duplicate endpoint、projection付きPlace、同一output / calleeのpositiveに加え、start gap、checked-add overflow、負countを含む全拒否経路を実validatorへ通し、全ownerの後にTypeArenaを解放する。stage0 fixture責務はmodel / validator ownerとは別のprivate source partに置き、proof-gate anchorだけが`@merge`する。この境界はfunction-value alias resolution、callback signatureとparams / result / argsの整合、effect summary適用、actual loweringとのco-productionを証明しない。成功originは非productionの`ResourceIrInventoryValidated`を維持し、残るoperation payloadはConstruct、Branch、Loop、Matchの4件である。

### 2026-07-13 Resource Construct payload

`ResourceOp::Construct { output, kind, inputs, span }`はkind tag 20専用のprivate sparse header、source orderのinput Place owner、Struct / Tupleだけが消費するsource orderのfield-offset ownerへ分ける。`AggregateKind`は`Enum { name, variant }`、`Struct { name, field_offsets }`、`Tuple { field_offsets }`をvariant-nativeに保持し、enum type名、enum variant名、struct type名を異なるcanonical symbol roleで包む。Rust `usize` offsetはproducerで0以上かつselfhost i32上限以下へchecked narrowingされた値だけをownerへ渡す。

inputsとfield offsetsは独立したRust `Vec`であるため、0件、複数件、重複、offsetの非単調順序を保持し、manual transportはarity、input countとoffset countの一致、layout、type、enum payload cardinalityを推測しない。Enumはfield-offset ownerを消費せず、offset混入だけをtyped rejectionする。scopeはkind ownerと3 ownerをmergeし、missing / unexpected、identity / ordinal、range overflow / gap / excess、canonical symbol、same-graph output / input Place membership、offset narrowingをfield固有errorへ拒否する。stage0はzero、duplicate / non-monotonic offset、複数Enum inputのpositiveと全rejection familyを実validatorへ通す。この境界はconstructor resolution、aggregate arity / layout / type semantics、actual loweringとのowner co-productionを証明しない。成功originは非productionの`ResourceIrInventoryValidated`を維持し、残るoperation payloadはBranch、Loop、Matchの3件である。

### 2026-07-13 Resource recursive operation topology prerequisite

Rust `ResourceFunction.blocks[].ops`と、`ResourceOp::Branch`の`then_ops` / `else_ops`、`Loop`の`condition_ops` / `body_ops`、`Match`の各arm `ops`を単一のprivate `nodes / scopes / edges` ownerへ投影する。node ordinalはselfhost recursive materializerが割り当てる全block共通preorderであり、Rust modelにordinal fieldが存在するという主張ではない。central inventory scopeはこのtopologyを必須borrowとし、node数をoperation / kind / span / sparse payload全ownerのglobal ordinal domainにする。

scopeは親をblock ordinalまたはoperation node ordinalとして保持し、`BlockOps`、`BranchThen`、`BranchElse`、`LoopCondition`、`LoopBody`、dense `MatchArm`を排他的roleで区別する。scope idはowner内で0からdenseとするが、異なるparentのscope group間にglobal割当順は要求せず、同じoperation parentの中だけrole順（Branch Then / Else、Loop Condition / Body、Match arm 0..N）を固定する。0-arm Matchはarm scopeを作らず、空Then / Else / Condition / Body / armは対応scopeを作ってrangeを空にする。edge rangeはdirect childだけを数え、node / scopeのexclusive subtree rangeがdescendantを表す。standalone validatorはfixture block recordの予約済み`root_operation_scope_id`をexactに引き、blockごとのroot 1件、全nodeのincoming edge 1件、global edge rangeの非重複dense被覆、dense sibling、closed control role、blockを跨がない親子、canonical preorder、subtree不足 / 過剰、range overflowを拒否する。root forestもblock ordinal順に0から全nodeを隙間なく覆う。

block recordの`root_operation_scope_id`はBlockOps scopeをexactに指す。`first_operation_ordinal / operation_count`はlegacy stage0 fixture shapeとの互換fieldに留め、central scopeはこれをoperation authorityに使わない。topology-native central scopeはActualWalker operation tableを入力に取らず、block / terminator構造、topology node数、span / kindのdense owner、topology cross-owner、実装済みsparse payloadを同じglobal ordinalで検査する。旧stage0だけが別compat wrapperでlegacy tableのshapeを検査し、その結果をResourceOp co-productionやcentral authorityへ昇格させない。

stage0のflat互換topologyはactual walker table長から推測せず、typed function block fixtureのrangeからroot scope / node / edgeを生成してownerを中央validatorへ渡し、検査後に1回だけ解放する。rangeはfirst / countの非負性とchecked-add上限を確かめてから生成する。nested matrixは2 blockの各rootとblock 0内Branch descendantをglobal forestとして連結するpositive、empty scopeの存在しないOperation parent、負first、count加算overflowを含むnegativeをcentral cross-owner wrapperへ通す。次の明示sliceはBranch condition / value / recursive condition fact payloadをこのglobal ordinalへ接続し、その後Loop / Match payloadへ進む。control-flow semantics、actual lowering co-production、production originはまだ発行しない。

### 2026-07-13 Resource Branch payload and condition fact owner

`ResourceOp::Branch`はglobal operation ordinalに対するsparse headerとして、output、condition、then value、else valueの4個のsame-graph Place linkを保持する。condition fact rootはBranch headerへ重複保持せず、Branch / Loopそれぞれにexactly 1件あるgeneric root binding ownerの`Option<i32>`だけをauthorityにする。then / else operation列もrecursive topologyの`BranchThen` / `BranchElse` scopeだけをauthorityにする。kind ownerとのmerge cursorはBranch 1件ごとのheaderを要求し、欠落、non-Branchへの混入、proof key / graph identity、operation ordinalをtyped rejectionする。4個のPlaceは順序を保ったまま個別にinventory membershipを検査し、同一Placeやprojection付きPlaceを許容する。

condition factはBranch / Loopで共有する独立ownerであり、Rust `ResourceConditionFact`のEqZero、NeZero、Positive、NonPositive、Negative、NonNegative、I32Relationの6演算、Any、Allをfallbackなしで保持する。generic validatorはglobal operation kindを全走査し、Branch / Loopにexactly 1件のroot bindingを要求し、それ以外へのbindingやfact nodeを拒否する。nodeは全owner共通dense ordinal、owning global operation ordinal、kind、parent-local edge rangeを持ち、edgeはparent、sibling ordinal、childを保持する。1 operationのnode範囲はrootから始まり、direct childはglobal preorderの次nodeでなければならず、各child subtreeをsibling順に排他的かつ連続して消費し、最後にowner範囲全体を覆う。このcanonical preorder契約により、incoming countと到達性だけを満たす任意graphやsibling subtreeの並べ替えを受理せず、Rustの再帰treeを一意に復元できる。Any / Allは空child rangeをNoneと区別して保持し、leafへのchild混入、orphan、two-parent、cycle形状、trailing edgeもtyped rejectionする。

stage0 matrixはcondition factなし、全6 unary、全6 relation、空Any / All、nested / repeated fact、同一 / projected Place、同じowner内のBranch / Loop bindingをpositiveとしてcentral structural validatorへ通す。header、4 Place、binding missing / unexpected / identity / ordinal / None競合、non-control fact、node / edge identity・ordinal・owner・range、root、sibling、leaf child、orphan、two-parent、cycle形状、trailing、fact Place、canonical preorderのnegativeは具体的なtyped variantとordinalまで照合する。central authorityはdense kind / span ownerとtopologyを先に検査し、その後Branch header、Loop header、Match header / arm、generic condition fact ownerの順でborrowして検査する。actual loweringとのowner co-production、conditionのcontrol-flow意味、then / else value join、production `ResourceIrEnumerated` originは未実装である。

central fixtureは2 blockのglobal operation 0..23、ordinal 21のBranch、22のLoop、23のMatch node、empty BranchThen / Else / LoopCondition / LoopBody / MatchArm scopeを構築し、全既存sparse ownerとnonempty Branch / Loop / Match header、ordered Match arm、Branch root binding / fact node、Loop None bindingを同じauthority callへ渡す。positiveはoperation count 24と`ResourceIrInventoryValidated` originを確認し、negativeはblock rangeとtopology / kind domainの不一致を同じcentral callで拒否する。fixtureだけはlegacy walker table countをResourceOp authorityにしないためcompatibility bridgeを通さず、topology-native central coreを直接検査する。

### 2026-07-13 Resource Loop payload

`ResourceOp::Loop`はglobal operation ordinalに対するprivate sparse headerとしてcondition Placeだけを保持する。`condition_ops` / `body_ops`はrecursive topologyの`LoopCondition` / `LoopBody` scope、`condition_fact`はBranchと共有するgeneric root bindingとfact node / edge ownerだけをauthorityにし、headerへrange、root、spanを重複保持しない。kind mergeはLoopごとにheader exactly 1件を要求し、gap / excess、key / graph identity、global ordinalをfield固有errorへ拒否する。condition Placeはsame-graph membershipを検査する。

central authorityはdense span / kind、topology、Construct、Branch、Loop、Match、generic condition factの順に検査する。compatibility wrapperは空Loop / Match ownerもallocate / borrow / freeする。central control fixtureはBranch 21、Loop 22、Match 23を共存させ、各empty child scope、各header、Match arm、Branchのnonempty fact root、LoopのNone bindingを同じauthority callへ通す。block rangeを1件短くしたnegativeはMatch operationだけを除き、存在しないMatch parentを持つscope 6を中央topology validatorで拒否する。独立Loop matrixはNoneと実EqZero root bindingに加え、header gap / excess、identity / ordinal、condition missing / graph mismatchを実validatorへ通す。全fact variantのlossless tree表現は共有generic matrixを再利用する。

この境界はLoop収束、condition truth semantics、body effect、actual loweringとのowner co-productionを証明せず、成功originは非productionの`ResourceIrInventoryValidated`を維持する。

### 2026-07-13 Resource Match payload

`ResourceOp::Match`はglobal operation ordinalに対するprivate sparse headerとしてoutput、scrutinee、`scrutinee_is_borrow_target`、ordered arm ownerのchecked rangeを保持する。各armはRust `ResourceMatchArm`のsource順を保ち、Variant / IntLiteral / BoolLiteral / Wildcard pattern、独立した`bind_local` / `bind_source_name` / `bind_mode` Option、value Place、arm spanを持つ。Variant spellingとbind source nameは異なるcanonical symbol roleで包み、`ResourceMatchBindMode::Borrowed { is_mut }`のboolを失わない。三つのbind OptionにRust型が持たないall-or-none制約は追加しない。

armの`ops`はheaderやarm recordへrangeとして重複保持せず、recursive topologyのparent Operation + dense `MatchArm(ordinal)` scopeだけをauthorityにする。Match validatorはheader range、global arm cursor、arm recordのowning operation / relative ordinalに加えて、topologyをborrowして同じoperation parentに属するMatchArm scopeのrole、dense ordinal、exact countを照合する。これによりarm payload 2件とscope 1件のようなcross-owner不一致を受理しない。output、scrutinee、bind local、valueはfield別のsame-graph Place membership、pattern / bind source nameはcanonical symbol membership、arm spanはvalid spanとして検査する。

stage0 matrixは0 arm、複数arm、全pattern、Owned / Borrowed mutable / immutable、borrow-target true、三つのbind Optionの独立形、projected Place、i32上限、bool falseをpositiveとして実validatorへ通す。sparse header gap / excess、identity / ordinal、各Placeのmissing / wrong graph、arm rangeの負start / 負count / checked-add overflow / end超過 / trailing endpoint、arm identity / ordinal、canonical symbol、span、topology count / ordinal / role不一致をtyped errorとexact ordinalで拒否する。central authorityはBranch 21、Loop 22、Match 23と一つのMatchArm scope / recordを同じ24-operation domainで検査する。
...
checker向けmember lookupはraw qualifierを解釈せず、name/import resolverが既に選んだnominal ID、enum session、利用側`SelfhostSourceText`とmember-tail spanを受ける。query spanはSourceTextのfile ID、source byte長、UTF-8境界へexactに結合し、範囲をclampするslice APIは使わない。宣言spanもsession source、definition file、UTF-8境界へ個別に結合した上で、異なるfile IDを許した二つのsource owner間byte比較を行う。一致時だけdefinition-local ordinalからmember IDを発行する。同じspellingが複数enumにあっても渡されたnominal owner内だけを検索する。

checker connectorはscrutinee TypeIdをTypeArena membershipへ戻し、Namedならrecord nominal、Appliedならbase constructor nominalを得る。resolver済みqualifier ownerがある場合はmember lookupより先にscrutinee nominalと比較し、不一致を`OwnerMismatch`として拒否する。これによりRust現行checkerがprefixを捨てて受理する`WrongEnum::Ok`をselfhost側ではfail closedにする。これは意図的な先行修正であり、Rust checkerにも同じowner検査を戻す必要がある。connectorはraw spellingを成功payloadへ残さず、scrutinee TypeId、ResolvedEnumMemberId、diagnostic spanだけをchecked evidenceにする。
...
public enum origin connectorはcaller supplied export tableを受けず、current graph/VFS/orderからpublic export closureを内部再構築する。visible lookupから得たorigin module/file/DefId/name spanを同じVFS sourceのfile ID付きparse / hoist / enum materializeに通し、definition bindingから導出したnominalを同じproducer内でTypeArenaへ追加する。入力arenaはprimitive / parameter / functionのみを許し、既存Named / Applied recordがある場合は別sessionの数値nominal衝突を排除できないため拒否する。返却contextとCopy witnessの値単体はcapabilityではなく、authorityはproducerの成功control-flowと、後続consumerがcurrent graph/VFS/orderへ再validationする境界にある。ここで保証するはsame-checker-session identityだけで、source path / kind / arity / definition hashを含むcross-session stable nominal keyは別stageで付与する。
...
single `NamedValue` scrutineeのchecker接続では、context-owned rangeをflat prefixへ変換し、exactly one NamedValueであることを確認してからcurrent source spellingをlatest scope bindingへ解決する。assigned DefIdに対応するvalue type evidenceが同一arenaの既存recordを指す場合だけactual TypeIdとする。export originのactual経路はこのarenaへ型を追加せず、parser enumのname、exact constructor kind/arity、宣言名span identityを既存constructor tableへ照合して同じ宣言のnominal IDだけを再利用し、NamedまたはAppliedのbase nominalとdefinitionを検査してから既存member lookupへ渡す。公開compositeはgraph/VFS/order/pathとfunction/segment/arm ordinal、scope/value evidence、arena/constructor ownerだけを受け、range/span/TypeId/nominal/raw nameをcaller authorityにしない。ただしscope/value evidence/arena/constructor ownerが選択FunctionDeclと同じchecker phase由来であることは現段階ではcaller preconditionであり、module checker environmentとのco-productionまではproduction authorityとしない。direct callのchecked-tree rootとarm-derived expected-type retryは接続済みである。reference deref、generic payload substitution、bind、重複・網羅性、arm body型統一は後続段階である。
...
leading Match contextが所有するscrutinee rangeを既存body-line checkerへ渡し、`expected = None`の1回の検査で得たchecked treeのroot nodeからactual `SelfhostTypeId`を再取得する。actual VFSからmember identityまでのruntime gateはdirect callに加え、whole-scrutinee明示ascription `%Choice i32 make` を通常のascription projectionとcall reductionへ通し、rootとmemberが同じApplied typeを保持する。`%Choice bool make` はactual connectorのtyped `ExpectedTypeMismatch`として拒否し、ascriptionを無視する退行を検出する。pipe gateは`1 |> make`を同じbody-line入口へ渡し、単一`make : fn i32 Choice<i32>`ではroot/memberのApplied typeを保持する。同じcallable typeを持つ異なるDefIdをscopeとsignature tableへ2件登録した場合はtyped `PipeTargetAmbiguous`を要求する一方、外部candidate vectorは意図的に1件のままにする。これによりpipe RHSがscope/signature再収集を迂回し外部vectorをauthorityにする退行を検出する。このfixtureのconstructor tableはsynthetic preseedであり、qualified importからconstructorを構築するactual name-resolution originまでは証明しない。root nodeと型recordがそれぞれchecked treeと同じarenaに存在しない場合はtyped errorとしてfail closedにする。
...
Rust checkerは最初のexpectedなし検査が失敗した場合、arm variant patternのqualified nameからexpected enum typeを導出し、diagnostic/type contextのcheckpointをrollbackしてscrutineeを再検査する。selfhost側もexact import/export originからarm-derived expected typeを作り、候補ごとにTypeArenaとchecked treeをforkしてgeneric state、arity、source-backed argument、trailing block、result inference、root node追加を一つのtransactionとして再試行する。候補固有不一致はrollbackし、成功1件だけをcommit、0件は`OverloadNoCandidate`、複数成功は`OverloadAmbiguous`とする。checked Match tree、arm body型統一、payload bind、網羅性、HIR / Resource輸送は後続境界である。
### Match generic declaration origin と宣言全体 range

module parser は通常式に残る旧 angle 構文を拒否する一方、enum / struct / trait / impl / function の宣言keyword境界へ直結し、`.Ident`から始まってnested trait applicationを含めangleが釣り合うgeneric binderだけをresolverが検証するtoken evidenceとして通す。boundのsemantic妥当性はmodule parserで先取りせず後段へ残す。宣言位置にあるだけのconcrete angle、signature後のbinder、unmatched angleは許可しない。header range の終端計算も同じ contextual 判定を使い、generic binder と末尾 colon を欠落させない。
