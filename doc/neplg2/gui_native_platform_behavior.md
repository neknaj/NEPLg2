...
- `NativeWindowHostLoopLinuxOnlyPlatformWaitBackend` と `build_native_window_host_loop_platform_wait_backend_from_selection_with_linux_api` により、cfg Linux sys API を generic platform wait owner の Linux helper から構築できる。F5if 以降は helper と cfg Linux entry が explicit event source capability を要求し、observed-input-only source を raw API 呼び出し前に `LinuxEventSourceSupportFailed` として拒否する。ただし CLI selection、runner dispatch、minifb wait path にはまだ接続していない。
