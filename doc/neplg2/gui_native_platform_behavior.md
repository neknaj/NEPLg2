...
- cfg Linux では `NativeWindowHostLoopLinuxSelectorTimerFdSysApi` により、actual `epoll` / `timerfd` / `eventfd` syscall を raw API implementation として使える。host-event-only wait は host event fd readiness と eventfd drain だけを成功にする。ただし eventfd producer、generic platform wait helper、CLI selection、runner dispatch にはまだ接続していない。
