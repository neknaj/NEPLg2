...
- F5jz 以降は nonzero Atom ID owner と request-specific `InternAtom` reply packet parser を持つ。ただし current reader からの request / reply correlation、`WM_PROTOCOLS` / `WM_DELETE_WINDOW` meaning assignment、actual `ChangeProperty` registration は未接続である。
- F5ka 以降は optional WM protocol InternAtom batch を持つ reader が `CreateWindow -> InternAtom batch -> MapWindow` の順序で partial write できる。これは actual WM protocol registration ではなく、reply correlation、Atom meaning assignment、`ChangeProperty`、`ClientMessage` decode は引き続き未接続である。
