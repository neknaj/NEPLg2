...
- F5ka 以降は optional WM protocol InternAtom batch を持つ reader が `CreateWindow -> InternAtom batch -> MapWindow` の順序で partial write できる。F5kb 以降はその accepted sequence から `WM_PROTOCOLS` / `WM_DELETE_WINDOW` InternAtom reply を generic reply へ縮約する前に相関でき、F5kc 以降は parsed Atom ID を semantic slot へ割り当てる。F5kd 以降は `CreateWindow -> InternAtom batch -> InternAtom replies / Atom IDs -> ChangeProperty WM_PROTOCOLS -> MapWindow` の順に進め、F5ke 以降は registration accepted 後の `ClientMessage` を close observation へ写せる。F5kf 以降は default event mask が StructureNotify / Exposure も購読し、それらに対応する event packet を明示 event kind として保持する。F5kg 以降は default event mask が KeyPress / KeyRelease も購読し、raw keycode を保持する。F5kh 以降は X11 raw `state` key/button mask も保持する。
