...
- F5jz 以降は nonzero Atom ID owner と request-specific `InternAtom` reply packet parser を持つ。ただし current reader からの request / reply correlation、`WM_PROTOCOLS` / `WM_DELETE_WINDOW` meaning assignment、actual `ChangeProperty` registration、MapWindow scheduling relocation は未接続である。
