...
F5jz では、X11 `InternAtom` reply の fixed 32 byte packet から nonzero Atom ID を取り出す pure parser と owner を追加する。`NativeWindowLinuxX11AtomId` は zero を typed error として拒否し、`NativeWindowLinuxX11InternAtomReply` は sequence と Atom ID owner を保持する。parser は reply response type、length units `0`、offset 8 の little-endian atom id を検査する。F5jz は request-specific reply payload boundary であり、current reader の generic reply drain、request / reply correlation、accepted write progress、`WM_PROTOCOLS` / `WM_DELETE_WINDOW` の意味付け、`ChangeProperty`、`ClientMessage` decode、MapWindow scheduling、runner dispatch、support gate `Ok` 化、fallback、synthetic readiness は扱わない。

F5ka では、reader が optional `WM_PROTOCOLS` / `WM_DELETE_WINDOW` `InternAtom` batch owner を保持し、batch configured path では setup 後の window request write を `CreateWindow -> InternAtom batch -> MapWindow` の順に進める。これは MapWindow を batch 前に送らないための最小 scheduling split である。batch writer は partial write offset を保持し、would-block は retryable error、hard failure / zero write / overflow は failed state にする。F5ka は reply parse / correlation、Atom ID meaning assignment、`ChangeProperty`、`ClientMessage` decode、runner dispatch、support gate `Ok` 化、fallback、synthetic readiness は扱わない。

F5kb では、F5ka の InternAtom batch request sequence を reader-owned plan として保持し、X11 reply packet が届いた時点で sequence correlation を行う。matched sequence の reply は full packet を F5jz parser に渡し、`WM_PROTOCOLS` / `WM_DELETE_WINDOW` request correlation と parsed Atom ID owner を持つ typed observation error として返す。matched packet の parse failure は lower parser error と correlation を保持する。unmatched reply は generic `ServerReplyReceived` のままにする。reply body drain が必要な packet でも full packet を pending state に保持し、stream sync を壊さない。F5kb は Atom ID meaning assignment、actual `ChangeProperty` registration、`ClientMessage` decode、runner dispatch、support gate `Ok` 化、fallback、synthetic readiness は扱わない。

F5kc では、F5kb の correlated `InternAtom` reply を reader-owned assignment state に接続し、`WM_PROTOCOLS` property Atom と `WM_DELETE_WINDOW` protocol Atom を semantic slot として保持する。両方が揃った時だけ completed Atom owner を返し、actual `ChangeProperty` registration の前提を型で表す。parse failure、unmatched reply、duplicate assignment は silent success にせず、duplicate は typed error として既存 slot を上書きしない。F5kc は actual `ChangeProperty` registration、`WM_DELETE_WINDOW` `ClientMessage` decode、runner dispatch、support gate `Ok` 化、fallback、synthetic readiness は扱わない。

F5kd では、F5kc で揃った `WM_PROTOCOLS` / `WM_DELETE_WINDOW` Atom owner を使い、top-level window に対する X11 `ChangeProperty` request を reader-owned partial-write path へ接続する。request は 28 byte / length 7 units で、property は `WM_PROTOCOLS`、type は predefined `ATOM`、format は `32`、data item は `WM_DELETE_WINDOW` 1 個だけである。reader は `CreateWindow -> InternAtom batch -> InternAtom replies / Atom IDs -> ChangeProperty WM_PROTOCOLS -> MapWindow` の順序を守り、registration request が accepted write boundary を越えるまで MapWindow を送らない。would-block は state を保持し、write failure / zero write / overflow / request build failure は registration failed state にして fail closed する。F5kd は actual `ChangeProperty` registration write boundary であり、`WM_DELETE_WINDOW` `ClientMessage` decode、StructureNotify / Expose decode、keyboard / IME、Wayland concrete decoding、runner dispatch、support gate `Ok` 化、fallback、synthetic readiness は扱わない。

F5ke では、accepted registration context を持つ X11 window について、`WM_DELETE_WINDOW` `ClientMessage` を close request observation へ decode する。event type は raw response byte ではなく send-event bit を落とした type で判定し、raw `33` と raw `161` の両方を `ClientMessage` として扱う。decode は format `32`、window id、message type `WM_PROTOCOLS` Atom、data32[0] `WM_DELETE_WINDOW` Atom を accepted context と照合し、どれかが違う場合は typed mismatch error にする。Atom assignment や registration request owner の存在だけでは context を作らず、registration write state が `Ready` の場合だけ close decode を許す。F5ke は close observation decode boundary であり、StructureNotify / Expose、keyboard / IME、Wayland concrete decoding、Linux runner / CLI dispatch、support gate `Ok` 化、fallback、silent no-op、synthetic readiness は扱わない。

F5kf では、X11 top-level window の default event mask に `ExposureMask` と `StructureNotifyMask` を追加し、購読した `ConfigureNotify`、`MapNotify`、`Expose` を platform-neutral event kind として保持する。`NativeWindowEventPumpEventKind` は `Poll`、`CloseRequested`、`WindowResized`、`PointerMotion`、`PointerButton`、`WindowMapped`、`RedrawRequested` を分け、`NativeWindowLinuxWindowEventSourceObservation` から `NativeWindowEventPumpSnapshot`、backend loop outcome、host action まで event evidence を落とさずに運ぶ。`ConfigureNotify` は `WindowResized` として current size を更新し、`MapNotify` は `WindowMapped`、`Expose` は `RedrawRequested` として previous size / mouse state を保持する。compatibility builder は close、resize、pointer button だけを推論し、pointer sample があるだけでは `PointerMotion` を合成しない。F5kf は X11 event evidence boundary であり、keyboard / IME、Wayland concrete decoding、Linux runner / CLI dispatch、support gate `Ok` 化、fallback redraw、silent no-op、synthetic readiness は扱わない。

F5kg では、X11 top-level window の default event mask に `KeyPressMask` と `KeyReleaseMask` を追加し、購読した `KeyPress` / `KeyRelease` を raw keyboard event evidence として保持する。`NativeWindowEventPumpEventKind` は `Keyboard` を持ち、`NativeWindowKeyboardEvent` は `Pressed` / `Released` と raw X11 keycode を保持する。raw keycode は keysym、layout 済み文字列、IME composition、shortcut command ではない。`KeyPress` / `KeyRelease` は send-event bit を落とした event type で判定し、raw keycode `0` は typed error として拒否する。keyboard evidence は observation から snapshot、backend loop outcome、host action まで optional payload として伝播する。F5kg は raw keyboard event evidence boundary であり、IME / text input、keysym / layout / modifier mapping、shortcut policy、Wayland concrete keyboard decoding、Linux runner / CLI dispatch、support gate `Ok` 化、fallback text、silent no-op、synthetic readiness は扱わない。
...
F5nn native runner follow-upでは`nepl-cli --run --gui-font-resource-root DIRECTORY`を唯一のroot authorityとし、wasmi storeが所有する1つのhostへ4 importを接続する。未指定時は`Unsupported`であり、compile-onlyはfilesystemへ触れない。macOS / Windows secure backendと後続pipelineは未完了である。

F5nn macOS secure-open follow-upでは、canonical root自身を`O_DIRECTORY | O_NOFOLLOW`で開き、open前に取得したexpected metadataとopened descriptorのdevice/inode identityを照合してcheck-open差し替えを拒否する。そのdirectory handleを起点にcanonical relative pathの各segmentを`openat` + `O_NOFOLLOW`で順に開く。中間segmentは`O_DIRECTORY`を要求し、保持中のparent fdからだけ次segmentを解決する。最終segmentは`O_NOFOLLOW | O_NONBLOCK`で開き、FIFO等の非regular fileによるopen blockを防いでからmetadataで拒否する。Linux root/final openにも同じ検証と`O_NONBLOCK`を適用する。Windows secure backendは未完了である。
