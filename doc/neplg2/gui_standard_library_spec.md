...
```text
web/src/gui-preview
    Web Playground の floating GUI window layer、typed preview command DTO、Canvas2D adapter
...
Preview の描画 data は `web/src/gui-preview/commands.ts` の `fill-rect` / `text-run` command union と `rgba8888` 相当の color struct で表す。`renderer.ts` は command stream と `mandelbrot` / `life` / `counter` metrics union だけを生成し、`CanvasRenderingContext2D`、DOM element、CSS color string、`null | undefined`、optional metric field を持たない。Canvas 固有の色変換、font、text align、scale / viewport 計算は `canvas-renderer.ts` に閉じ込める。
...
| `web/src/gui-preview` | Web Playground display smoke backend | editor panel layout の上に floating GUI window layer を描画し、Mandelbrot / Life / Counter preview を typed command DTO 経由で Canvas2D adapter へ表示する。window manager は source / move / window mode / dock 状態を union で表し、`commands.ts` / `renderer.ts` は Canvas / DOM 型、`null | undefined`、optional metric field を持たない。formal `DrawCommand` host bridge は未実装 |
