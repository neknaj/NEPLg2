...
## F5kg Native Linux X11 raw keyboard event evidence boundary

F5kg は X11 raw keyboard packet を platform-neutral event evidence へ写す boundary である。`NativeWindowEventPumpEventKind` は `Keyboard` を持ち、`NativeWindowKeyboardEvent` は `Pressed` / `Released` と raw X11 keycode を保持する。raw keycode は layout 済み文字列、keysym、shortcut、IME text ではない。

default event mask は F5kf の mask に `KeyPress` と `KeyRelease` を追加し、`KeyPress | KeyRelease | ButtonPress | ButtonRelease | PointerMotion | Exposure | StructureNotify` とする。`KeyPress` / `KeyRelease` は send-event bit を落とした event type で判定する。raw keycode `0` は typed observation error として拒否し、empty keyboard event や no-op として成功させない。

keyboard evidence は `NativeWindowLinuxWindowEventSourceObservation`、`NativeWindowEventPumpSnapshot`、backend loop outcome、host action へ optional payload として伝播する。compatibility builder は keyboard を推論せず `None` とし、X11 concrete decode だけが `Some NativeWindowKeyboardEvent` を作る。
...
`alloc/gui/font/sfnt/glyf` の F5ly dirty-owner bridge は、fill / shadow software drain completed owner が持つ single `DirtyRegion` を `dirty_regions_push_region_checked dirty_regions_empty dirty` で `DirtyRegionSet` に変換してから、`GuiRgba8888SoftwareSurfaceDirtyOwner` へ surface ownership を渡す。dirty aggregation は completed owner の `finish_surface` より前に行い、失敗時は fill / shadow 別の owner-bearing bridge error が元 completed owner を保持する。これは 2D compositor drain 本体ではなく、F5bu bitmap frame prepare、row byte copy、tile / RLE、host present、platform API、fallback へ進まない。

`alloc/gui/render2d/compositor_frame_entry` の F5lz compositor frame entry は、F5ly 後の `GuiRgba8888SoftwareSurfaceDirtyOwner` を `GuiRgba8888CompositorFrameEntryOwner` へ正規化する。entry prepare は `gui_rgba8888_bitmap_frame_prepare`、`gui_rgba8888_row_batch_plan_prepare`、`gui_rgba8888_row_batch_cursor_start` を順に呼び、`frame_id` / `max_rows_per_batch` が不正でも ownerless config error へ逃がさず lower owner-bearing error から dirty owner を回収する。row plan / cursor start 失敗では、frame / plan に保持された dirty metadata を surface owner より先に読み、`GuiRgba8888SoftwareSurfaceDirtyOwner` へ戻してから error に保持する。success metadata は row batch cursor start の前に plan から Copy value として読み、entry owner は cursor owner と metadata を保持する。これは row batch drain、row byte storage、tile / RLE、std present、host import、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

`alloc/gui/render2d/compositor_batch_drain` の F5ma compositor batch drain は、F5lz entry owner を F5bx `gui_rgba8888_row_batch_drain_budget` へ 1 回だけ委譲する scheduler continuation boundary である。entry metadata は entry の cursor を取り出す前に Copy し、`GuiRgba8888CompositorBatchDrainTerminal` / `GuiRgba8888CompositorBatchDrainError` の owner-bearing wrapper に保持する。terminal / error から entry owner を再構成するときも、lower terminal / error を消費する前に metadata を読む。F5ma は row batch cursor status / next batch を再実装せず、row batch range、row byte storage、tile / RLE、std present、host import、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

`alloc/gui/render2d/compositor_batch_range` の F5mb compositor batch range は、F5lz entry owner を `gui_rgba8888_row_batch_cursor_next_batch` と `gui_rgba8888_row_batch_range_prepare` へ 1 回ずつ通し、`GuiRgba8888CompositorBatchRangeOwner` として lower row range owner と copied entry metadata を束ねる bridge である。error は lower owner-bearing error を保持せず、kind / category を読んだ後で lower cursor または batch owner を `GuiRgba8888CompositorFrameEntryOwner` へ正規化し、`kind/category/entry` を持つ `GuiRgba8888CompositorBatchRangeError` として返す。complete cursor は `CursorNextBatchFailed` の owner-bearing error で回収できる。F5mb は row byte storage、tile / RLE、std present、host import、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

`alloc/gui/render2d/compositor_byte_storage` の F5mc compositor byte storage は、F5mb range owner を `gui_rgba8888_row_byte_storage_prepare` へ 1 回だけ通し、`GuiRgba8888CompositorByteStorageOwner` として lower copied row byte storage owner と copied entry metadata を束ねる bridge である。prepare error は lower owner-bearing error を保持せず、kind / category を読んだ後で lower range owner を `GuiRgba8888CompositorBatchRangeOwner` へ正規化し、prepare error は `kind/category/range` を持つ。finish error は lower finish kind を読んだ後で returned cursor を `GuiRgba8888CompositorFrameEntryOwner` へ正規化し、finish error は `kind/category/entry` を持つ。`owner_free` は finish failure と finish 成功後の entry free failure を分ける。F5mc は checked byte count / byte read 以外の raw storage accessor、row tile / RLE、std present、host import、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

`alloc/gui/render2d/compositor_tile_plan` の F5md compositor tile plan は、F5mc byte storage owner を `gui_rgba8888_row_tile_plan_prepare` へ 1 回だけ通し、`GuiRgba8888CompositorTilePlanOwner` として lower row tile plan owner と copied entry metadata を束ねる bridge である。prepare error は lower owner-bearing error を保持せず、kind / category を読んだ後で lower byte storage owner を `GuiRgba8888CompositorByteStorageOwner` へ正規化し、prepare error は `kind/category/storage` を持つ。descriptor access は `descriptor_at` を lower `gui_rgba8888_row_tile_plan_descriptor_at` に委譲して storage-relative descriptor metadata だけを返す。finish error は F5mc byte storage finish error の kind / category を読んだ後で returned entry owner を保持し、finish error は `kind/category/entry` を持つ。`owner_free` は finish failure と finish 成功後の entry free failure を分ける。F5md は direct `gui_rgba8888_row_byte_storage_validate_authority`、row tile storage ref、byte read、tile payload / RLE、std present、host import、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no RLE / host present の bridge である。

`alloc/gui/render2d/compositor_tile_payload` の F5me compositor tile payload は、F5md tile plan owner を `gui_rgba8888_row_tile_payload_prepare` へ 1 回だけ通し、`GuiRgba8888CompositorTilePayloadOwner` として lower row tile payload owner と copied entry metadata を束ねる bridge である。prepare error は lower owner-bearing error を保持せず、kind / category を読んだ後で lower tile plan owner を `GuiRgba8888CompositorTilePlanOwner` へ正規化し、prepare error は `kind/category/plan` を持つ。descriptor / plan metadata は lower checked helper に委譲し、tile-relative byte read は lower `gui_rgba8888_row_tile_payload_byte_at` に委譲する。finish error は F5md tile plan finish error の kind / category を読んだ後で returned entry owner を保持し、finish error は `kind/category/entry` を持つ。`owner_free` は finish failure と finish 成功後の entry free failure を分ける。F5me は row tile storage ref、direct row byte storage API、RLE、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no RLE / host present の bridge である。

`alloc/gui/render2d/compositor_tile_rle_count` の F5mf compositor tile RLE count は、F5me tile payload owner を `gui_rgba8888_row_tile_rle_cursor_start` と `gui_rgba8888_row_tile_rle_count_start` へ順に 1 回ずつ通し、`GuiRgba8888CompositorTileRleCountOwner` として lower row tile RLE count owner と copied entry metadata を束ねる bridge である。start error は lower cursor/count owner-bearing error を保持せず、kind / category を読んだ後で lower payload owner または lower cursor owner を `GuiRgba8888CompositorTilePayloadOwner` へ正規化し、start error は `kind/category/payload` を持つ。accumulated run count、cursor next pixel index、cursor status は lower count owner helper に委譲する。finish error は F5me tile payload finish error の kind / category を読んだ後で returned entry owner を保持し、finish error は `kind/category/entry` を持つ。`owner_finish_payload` は metadata を Copy してから lower count owner を消費する。`owner_free` は finish failure と finish 成功後の entry free failure を分ける。F5mf は row tile RLE drain、count step、completed count、encode / writer / storage / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no drain / encode / present の compositor tile RLE count bridge である。

`alloc/gui/render2d/compositor_tile_rle_count_step` の F5mg compositor tile RLE count step は、F5mf count owner を `gui_rgba8888_row_tile_rle_count_step_budget` へ 1 回だけ渡し、lower step status と metadata 付き next count owner を `GuiRgba8888CompositorTileRleCountStep` として返す bridge である。step error は lower count error の kind / category / accumulated run count / cursor next pixel index を読んだ後で lower cursor を payload owner へ戻し、metadata 付き `GuiRgba8888CompositorTilePayloadOwner` を保持する。lower count error では fake continuation count owner を作らない。success finish / free は F5mf count owner finish / free に委譲し、error finish / free は count owner が存在しないため F5me payload finish / free に委譲する。F5mg は direct drain、completed count、encode / writer / storage / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no completed / encode / present の compositor tile RLE count step bridge である。

`alloc/gui/render2d/compositor_tile_rle_count_completed` の F5mh compositor tile RLE completed count は、F5mf/F5mg count owner を `gui_rgba8888_row_tile_rle_count_completed_prepare` へ 1 回だけ渡し、lower completed owner と metadata を `GuiRgba8888CompositorTileRleCountCompletedOwner` として束ねる exact capacity evidence bridge である。completed error は lower completed error の kind / category / total run count / cursor next pixel index を読んだ後で lower error の original count owner を metadata 付き `GuiRgba8888CompositorTileRleCountOwner` へ戻し、error は `kind/category/total_run_count/cursor_next_pixel_index/count` を持つ。success / error の finish / free はいずれも F5mf の count owner finish / free に委譲する。F5mh は count step、direct drain、encode / writer / storage / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no count step / encode / present の compositor tile RLE completed count bridge である。

`alloc/gui/render2d/compositor_tile_rle_encode_seed` の F5mi compositor tile RLE encode seed は、F5mh completed owner を `gui_rgba8888_row_tile_rle_encode_seed_prepare` へ 1 回だけ渡し、lower seed owner と metadata を `GuiRgba8888CompositorTileRleEncodeSeedOwner` として束ねる payload seed bridge である。seed error は lower seed error の kind / category / total run count を読んだ後で lower error の completed owner を metadata 付き `GuiRgba8888CompositorTileRleCountCompletedOwner` へ戻し、error は `kind/category/total_run_count/completed` を持つ。success finish / free は F5me payload finish / free に委譲し、error finish / free は F5mh completed owner finish / free に委譲する。F5mi は cursor restart、count step、direct drain、encode cursor / writer / storage / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no cursor restart / encode / present の compositor tile RLE encode seed bridge である。

`alloc/gui/render2d/compositor_tile_rle_encode_cursor` の F5mj compositor tile RLE encode cursor は、F5mi seed owner を `gui_rgba8888_row_tile_rle_encode_cursor_start` へ 1 回だけ渡し、lower ready cursor owner と metadata を `GuiRgba8888CompositorTileRleEncodeCursorOwner` として束ねる ready cursor bridge である。cursor error は lower cursor error の kind / category / total run count / start kind を読んだ後で lower start error payload を metadata 付き `GuiRgba8888CompositorTilePayloadOwner` へ戻し、error は `kind/category/total_run_count/start_kind/payload` を持つ。success / error の finish / free はいずれも F5me payload finish / free に委譲する。F5mj は lower cursor / lower error を公開 recovery payload にせず、direct cursor start、count step、direct drain、writer / storage / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no writer / storage / packet / present の compositor tile RLE encode cursor bridge である。

`alloc/gui/render2d/compositor_tile_rle_writer_plan` の F5mk compositor tile RLE writer plan は、F5mj ready cursor owner を `gui_rgba8888_row_tile_rle_writer_plan_prepare` へ 1 回だけ渡し、lower writer plan owner と metadata を `GuiRgba8888CompositorTileRleWriterPlanOwner` として束ねる encoded byte count capacity bridge である。writer plan error は lower writer plan error の kind / category / total run count を読んだ後で lower ready cursor owner を metadata 付き `GuiRgba8888CompositorTileRleEncodeCursorOwner` へ戻し、error は `kind/category/total_run_count/ready` を持つ。success finish / free は lower writer plan owner を payload owner へ戻して F5me payload finish / free に委譲し、error finish / free は F5mj ready cursor owner 経由で F5me payload finish / free に委譲する。F5mk は lower writer plan error を公開 recovery payload にせず、direct cursor start、count step、direct drain、storage / write step / encoded / packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no storage / packet / present の compositor tile RLE writer plan bridge である。

`alloc/gui/render2d/compositor_tile_rle_storage` の F5ml compositor tile RLE storage は、F5mk writer plan owner を `gui_rgba8888_row_tile_rle_storage_prepare` へ 1 回だけ渡し、lower storage owner と metadata を `GuiRgba8888CompositorTileRleStorageOwner` として束ねる encoded storage allocation bridge である。storage prepare error は lower storage prepare error の kind / category / total run count / encoded byte count を読んだ後で lower writer plan owner を metadata 付き `GuiRgba8888CompositorTileRleWriterPlanOwner` へ戻し、error は `kind/category/total_run_count/encoded_byte_count/plan` を持つ。success `owner_finish_payload` は lower storage finish cursor を通して lower payload を metadata 付き payload owner へ戻し、storage finish error は lower cursor から復元した payload owner を保持する。`owner_free` は lower storage owner free に委譲し、lower finish kind を compositor storage finish kind に包む。storage finish と payload finish は error domain が異なるため、F5ml は `owner_finish_entry` を作らない。prepare error finish / free は F5mk writer plan owner recovery から F5me payload finish / free に委譲する。F5ml は lower storage prepare error を公開 recovery payload にせず、direct write cursor start、write step、encoded seal、packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no write / encoded / packet / present の compositor tile RLE storage bridge である。

`alloc/gui/render2d/compositor_tile_rle_write_cursor` の F5mm compositor tile RLE write cursor は、F5ml storage owner を `gui_rgba8888_row_tile_rle_write_cursor_start` へ 1 回だけ渡し、lower write cursor owner と metadata を `GuiRgba8888CompositorTileRleWriteCursorOwner` として束ねる write cursor start bridge である。start error は lower write cursor start error の kind / category を読んだ後で lower storage owner を metadata 付き `GuiRgba8888CompositorTileRleStorageOwner` へ戻し、error は `kind/category/storage` を持つ storage owner recovery である。success owner は total run count、encoded byte count、written run count、written byte count、cursor next pixel index、cursor pixel count を lower write cursor accessor に委譲し、start 直後の written counts は 0 である。success `owner_finish_payload` は lower write cursor finish cursor を通して lower payload を metadata 付き payload owner へ戻し、write cursor finish error は lower cursor から復元した payload owner を保持する。`owner_free` は lower write cursor owner free に委譲し、lower finish kind を compositor write cursor finish kind に包む。start error free は F5ml storage owner free に委譲する。storage finish と payload finish は error domain が異なるため、F5mm は `owner_finish_entry` を作らない。F5mm は lower write cursor start error を公開 recovery payload にせず、write step、encoded seal、packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no step / encoded / packet / present の compositor tile RLE write cursor bridge である。

`alloc/gui/render2d/compositor_tile_rle_write_step` の F5mn compositor tile RLE write step は、F5mm write cursor owner を `gui_rgba8888_row_tile_rle_write_cursor_step_one` へ 1 回だけ渡し、lower write step status と metadata 付き next write cursor owner を `GuiRgba8888CompositorTileRleWriteStep` として返す bridge である。success は `WroteRun` または `Completed` status を Copy value として保持し、total run count、encoded byte count、written run count、written byte count、cursor next pixel index、cursor pixel count を next write cursor owner から読めるようにする。lower write step error は lower kind / category を読んだ後で lower write cursor owner を metadata 付き `GuiRgba8888CompositorTileRleWriteCursorOwner` へ戻し、error は write cursor owner recovery である。F5mn は lower write step error を公開 recovery payload にせず、success / error の `finish_payload` と `free` は F5mm write cursor owner finish payload / free に委譲する。encoded completion ではなく recovery / abort helper なので、storage finish と payload finish を混ぜる `finish_entry` は作らない。F5mn は encoded seal、packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no encoded / packet / present の compositor tile RLE write step bridge である。

`alloc/gui/render2d/compositor_tile_rle_encoded` の F5mo compositor tile RLE encoded seal は、F5mn/F5mm write cursor owner を `gui_rgba8888_row_tile_rle_encoded_seal` へ 1 回だけ渡し、lower sealed encoded owner と metadata を `GuiRgba8888CompositorTileRleEncodedOwner` として束ねる compositor tile RLE encoded seal bridge である。success owner は total run count、encoded byte count、cursor next pixel index、cursor pixel count を lower encoded accessor に委譲し、lower encoded owner を直接返さない。lower encoded seal error は lower kind / category を読んだ後で lower write cursor owner を metadata 付き `GuiRgba8888CompositorTileRleWriteCursorOwner` へ戻し、error は write cursor owner recovery である。F5mo は lower encoded seal error を公開 recovery payload にせず、seal error finish / free は F5mm write cursor owner finish payload / free に委譲する。success `owner_finish_payload` は lower encoded finish cursor から metadata 付き payload owner を復元し、finish error も lower cursor を payload owner へ戻す。`owner_free` は lower encoded owner free に委譲し、lower finish kind を compositor encoded finish kind に包む。F5mo は `gui_rgba8888_row_tile_rle_encoded_tile_descriptor_checked` / `gui_rgba8888_row_tile_rle_encoded_tile_plan_metadata_checked`、packet、tile payload direct byte reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no packet / present / raw byte の compositor tile RLE encoded seal bridge である。

`alloc/gui/render2d/compositor_tile_rle_packet` の F5mp compositor tile RLE packet bridge は、F5mo encoded owner を `gui_rgba8888_row_tile_rle_packet_prepare` へ 1 回だけ渡し、lower packet owner と metadata を `GuiRgba8888CompositorTileRlePacketOwner` として束ねる bridge である。success は lower packet owner を直接返さず、`GuiRgba8888CompositorTileRlePacketDescriptor` に lower `GuiRgba8888RowTileRlePacketDescriptor` と metadata を束ねる metadata付き descriptor wrapper を返す。descriptor scalar accessor は lower packet descriptor accessor に委譲する。lower packet prepare error は lower kind / category を読んだ後で lower encoded owner を metadata 付き `GuiRgba8888CompositorTileRleEncodedOwner` へ戻し、error は encoded owner recovery である。F5mp は lower packet prepare error を公開 recovery payload にせず、`owner_finish_encoded` と `prepare_error_finish_encoded` は encoded owner recovery のみを返す。finish_payload / finish_entry は F5mp に置かない。`prepare_error_free` は F5mo encoded owner free に委譲し、`owner_free` は lower packet owner free に委譲して lower encoded finish kind を compositor encoded finish kind に包む。F5mp は `gui_rgba8888_row_tile_rle_encoded_tile_descriptor_checked` / `gui_rgba8888_row_tile_rle_encoded_tile_plan_metadata_checked` を直接呼ばず、packet record reader、row byte storage、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない no packet record / present / raw byte の compositor tile RLE packet bridge である。

`std/gui/compositor_tile_present` の F5mq compositor tile RLE present-frame bridge は、F5mp の `GuiRgba8888CompositorTileRlePacketOwner` と caller supplied `SurfaceId` を lower `gui_rgba8888_row_tile_rle_present_frame_prepare` へ 1 回だけ渡し、lower present owner と metadata を `GuiRgba8888CompositorTileRlePresentFrameOwner` として束ねる bridge である。surface id は compositor metadata にはないため caller supplied typed `SurfaceId` を受け、surface raw validation と geometry validation は F5cn に委譲する。F5mq は lower packet owner を取り出す前に metadata frame id と packet descriptor frame id を読み、不一致を `FrameIdMismatch` として owner-bearing error にし、一致した frame id だけを `frame_id_result` で `FrameId` に変換する。`frame_id_result` の failure は `FrameIdInvalid` として original compositor packet owner を保持する。

F5mq の lower F5cn error wrapper は lower kind / category を読んでから lower packet owner を回収し、metadata 付き `GuiRgba8888CompositorTileRlePacketOwner` へ戻す。success wrapper は `owner_finish_packet` で lower present owner から compositor packet owner を復元できる。`owner_free` は lower present owner free に委譲し、lower encoded finish kind を compositor encoded finish kind に包む。F5mq は packet record reader、`tile_present_run_cursor`、`tile_present_command_cursor`、`tile_present_host_command`、`tile_present_run_span`、host import、dispatch、scheduler、platform API、video memory、Canvas / DOM / minifb、`surface_id_raw` / `surface_id_result`、raw memory、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない no cursor / record / host / platform の compositor tile RLE present-frame bridge である。

F5mq の `gui_rgba8888_compositor_tile_rle_present_frame_rewrap` は、後続 bridge が lower `GuiRgba8888RowTileRlePresentFrameOwner` と copied metadata を metadata 付き `GuiRgba8888CompositorTileRlePresentFrameOwner` へ戻す唯一の public helper である。rewrap は lower present descriptor frame id と metadata frame id を照合し、不一致は `GuiRgba8888CompositorTileRlePresentFrameRewrapErrorKind::FrameIdMismatch` として owner-bearing error にする。error は compositor present-frame owner を保持するため、caller は finish packet / free を選べる。

`std/gui/compositor_tile_present_run_cursor` の F5mr compositor tile RLE present run cursor bridge は、F5mq success owner を lower F5co `gui_rgba8888_row_tile_rle_present_run_cursor_start` へ 1 回だけ渡し、lower run cursor owner と metadata を `GuiRgba8888CompositorTileRlePresentRunCursorOwner` として束ねる bridge である。start は metadata-before-owner-move を守る。lower start error は lower kind / category を読んでから lower present owner を回収し、F5mq rewrap helper で metadata 付き compositor present-frame owner へ戻す。rewrap failure は `PresentFrameRewrapFailed` として start error に保持する。F5mr は F5mq present owner direct constructor を使わない。

F5mr success owner は metadata、next record index、total run count を lower F5co accessor に委譲して返す。`owner_finish_present_frame` は lower run cursor finish present を通して lower present owner を取り出し、F5mq rewrap helper の `Result` で compositor present-frame owner へ戻す。`owner_free` は lower run cursor owner free に委譲し、lower encoded finish kind を compositor encoded finish kind に包む。F5mr は `tile_present_command_cursor`、run cursor step、packet record reader、host import、dispatch、scheduler、platform API、video memory、Canvas / DOM / minifb、raw memory、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない no command / step / record / host / platform の compositor tile RLE present run cursor bridge である。

`std/gui/compositor_tile_present_run_step` の F5ms compositor tile RLE present run step bridge は、F5mr run cursor owner を lower F5co `gui_rgba8888_row_tile_rle_present_run_cursor_step` へ 1 回だけ渡し、lower step result と metadata 付き next run cursor owner を `GuiRgba8888CompositorTileRlePresentRunStep` として束ねる bridge である。step は metadata-before-owner-move を守り、success では lower result を Copy value として読んでから lower next owner を回収し、metadata 付き `GuiRgba8888CompositorTileRlePresentRunCursorOwner` へ戻す。

F5ms lower step error は lower kind / category を読んでから lower run cursor owner を回収し、metadata 付き run cursor owner を保持する owner-bearing error に正規化する。success / error の finish present-frame は F5mr `owner_finish_present_frame` に委譲し、F5mq rewrap `Result` を潰さない。success / error free は F5mr `owner_free` に委譲する。F5ms は `tile_present_command_cursor`、packet record reader、host import、dispatch、scheduler、platform API、video memory、Canvas / DOM / minifb、raw memory、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない no command / record / host / platform の compositor tile RLE present run step bridge である。

`std/gui/compositor_tile_present_command_cursor` の F5mt compositor tile RLE present command cursor は、F5mr/F5ms を authority として metadata 付き command stream を作る bridge である。start は F5mq present-frame owner から descriptor を Copy してから F5mr `gui_rgba8888_compositor_tile_rle_present_run_cursor_start` を 1 回だけ呼び、F5mr run cursor owner、descriptor、phase を `GuiRgba8888CompositorTileRlePresentCommandCursorOwner` に束ねる。command は `GuiRgba8888CompositorTileRlePresentCommand::BeginFrame`、`Run`、`GuiRgba8888CompositorTileRlePresentCommand::EndFrame` であり、BeginFrame / EndFrame は compositor present descriptor を保持する。

F5mt step は BeginPending で BeginFrame、RunPending で F5ms `gui_rgba8888_compositor_tile_rle_present_run_step_one`、Completed phase で terminal Completed を返す。F5ms `RunReady` は Run command、F5ms `Completed` は同じ public step の EndFrame へ写し、次 owner を Completed phase にする。start / step error は kind / category を読んでから owner を回収する owner-bearing error である。finish present-frame / free は F5mr owner helper に委譲し、F5mq rewrap `Result` を潰さない。F5mt は lower `tile_present_command_cursor`、`tile_present_host_command`、host import、packet record reader、raw storage、platform API、video memory、Canvas / DOM / minifb、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない no lower-command / host-command / record / host / platform の compositor tile RLE present command cursor である。

`std/gui/compositor_tile_present_host_command` の F5mu compositor tile RLE present host-command record は、F5mt command cursor step の public accessor だけから metadata 付き host-command record を作る pure projection である。`GuiRgba8888CompositorTileRlePresentHostCommandRecord` は `BeginFrame`、`RunRecord`、`EndFrame` を持ち、`GuiRgba8888CompositorTileRlePresentHostCommandStepResult` は `Record` と terminal `Completed` を enum で分ける。RunRecord は `GuiRgba8888CompositorTileRlePresentFrameDescriptor` と `GuiRgba8888RowTileRleRun` の両方を保持し、does not flatten to kind plus optional run。これは compositor metadata を lower descriptor へ落とさず、不正な record 状態を表現不能にするためである。

F5mu は F5mt の `gui_rgba8888_compositor_tile_rle_present_command_cursor_step_descriptor` と `gui_rgba8888_compositor_tile_rle_present_command_cursor_step_result` を authority とし、F5mt step 内部 owner / result field を直接読まない。step continuation owner を消費せず、advance / finish / free を行わない。F5mu does not bypass F5mt。lower `std/gui/tile_present_host_command`、lower `tile_present_command_cursor`、F5mr/F5ms/F5co run cursor、packet record reader、raw storage、host import、dispatch、scheduler、virtual drain、run-span、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない no lower-host-command / host-import / platform の compositor host-command record boundary である。

`std/gui/compositor_tile_present_virtual_drain` の F5mv compositor tile RLE present virtual drain は、F5mu compositor host-command records only を受け取る headless / test 用 boundary である。`GuiRgba8888CompositorTileRlePresentVirtualDrain` は phase、optional surface / frame、optional `GuiRgba8888CompositorFrameEntryMetadata`、expected / seen count を保持する。metadata is part of the drain authority であり、BeginFrame で固定した metadata と RunRecord / EndFrame descriptor metadata の full equality を検査する。descriptor frame と metadata frame id の内部不一致も typed error として返す。

F5mv は RunRecord で `run_pixel_offset == seen_pixel_count` を要求し、gap / overlap / reorder を拒否する。EndFrame は expected run count と expected pixel count が完了している場合だけ Ended になる。F5mv does not reuse lower F5cq/F5cs。lower `std/gui/tile_present_host_command` や `std/gui/tile_present_virtual_drain` へ落とすと compositor metadata を検査できなくなるためである。F5mv は presentation target ではなく、host import、dispatch、scheduler、run-span、platform API、raw storage、Vec、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない no lower-host-command / virtual-drain / host / platform の compositor virtual drain boundary である。

`std/gui/compositor_tile_present_schedule` の F5mw std layer compositor tile RLE present schedule boundary は、F5mu compositor host-command record stream を host continuation / scheduler の前で deterministic slice budget に区切る。`GuiRgba8888CompositorTileRlePresentScheduleState` は F5mv virtual drain state と slice-local command / pixel counters だけを保持する。stream validation is delegated to F5mv virtual drain であり、metadata is part of the F5mv drain authority のまま、schedule layer は metadata copy や独自の Begin / Run / End validator を持たない。

F5mw の `Yield means exact slice budget` は、valid record を消費した後に command budget または pixel budget へちょうど到達したときだけ `Yield` を返す契約である。over-budget is a typed error であり、single RunRecord pixel budget 超過、total budget 超過、counter overflow、lower F5mv failure は previous schedule state を持つ typed error で返す。`resume_slice` は F5mv drain を保持し、slice counters だけ reset する。F5mw does not bypass F5mv and does not reuse lower F5cq/F5cs。lower F5ct / F5cs / F5cq、host import、dispatch、real scheduler、timer、queue、platform API、raw storage、Vec、video memory、Canvas / DOM / minifb、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_import` の F5mx std layer compositor tile RLE present host continuation request boundary は、F5mu compositor host-command record を `GuiRgba8888CompositorTileRlePresentHostImportRequest` に包む。`GuiRgba8888CompositorTileRlePresentHostImportTarget` は Window / Offscreen / Device の 3 target だけを持ち、Headless is not a presentation target である。request は target と original `GuiRgba8888CompositorTileRlePresentHostCommandRecord` を保持し、metadata 付き compositor descriptor を lower record に落とさない。

F5mx は host capability の `FormatRgba8888`、surface kind、windowing capability、default window を検査する。WindowPixel では `has_windowing` と `default_window` の `Some` を要求し、OffscreenPixel は Offscreen、DevicePixel は Device にする。Headless / TextGrid / unsupported surface kind、wrong color format、window missing は `GuiError::Unsupported` であり、fallback / silent no-op はない。F5mx consumes F5mu compositor host-command record only で、F5mx does not depend on F5mw and does not execute host imports。F5mw schedule state には依存しない。F5mx does not reuse lower F5cr/F5cq。lower F5cr / F5cq / F5ct / F5cs、compositor cursor、virtual drain、schedule、dispatch、host execution action、raw storage、Vec、platform API、video memory、Canvas / DOM / minifb、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_dispatch` の F5my std layer compositor tile RLE present scheduled dispatch boundary は、F5mw schedule state と F5mx request construction を接続する。`GuiRgba8888CompositorTileRlePresentDispatchState` は `GuiRgba8888CompositorTileRlePresentScheduleState` だけを保持する。F5mw before F5mx を守り、schedule step が成功した場合だけ host import request を作る。

F5my success は `GuiRgba8888CompositorTileRlePresentDispatchReadyRequest` を持つ `RequestReady request plus post phase` である。post phase は F5mw phase の Continue / Yield / Completed を写した値であり、exact-budget `Yield` や terminal `Completed` でも request を落とさない。F5mw error と F5mx error は previous dispatch state を保持する。F5mx request construction error では updated schedule state を採用せず、`HostImportRequestFailed` と `Option::Some host_error` を返す。

F5my does not execute host imports and does not call F5mv directly。F5mv authority と metadata validation は F5mw/F5mv に残す。F5my は lower row-tile dispatch / schedule / drain / host import、host execution action、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_execution` の F5mz std layer compositor tile RLE present host execution action boundary は、F5mx request accessor だけで `GuiRgba8888CompositorTileRlePresentHostImportRequest` を読み、`GuiRgba8888CompositorTileRlePresentHostExecutionAction` に変換する。action は Window / Offscreen / Device と BeginFrame / RunRecord / EndFrame の flat target x record action であり、Window action payload は `WindowId` と compositor descriptor / run record を保持する。

F5mz の Begin / End action は `GuiRgba8888CompositorTileRlePresentFrameDescriptor` を保持し、Run action は `GuiRgba8888CompositorTileRlePresentHostCommandRunRecord` を保持する metadata-preserving boundary である。F5mz consumes F5mx requests, not F5my ready requests。F5mz does not depend on F5my and does not depend on F5mw/F5mv。F5mz does not execute host imports。F5mx が request construction と capability validation を終えているため、F5mz は `Result` を追加しない。lower row-tile host execution / import / command、F5my dispatch、F5mw schedule、F5mv virtual drain、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_execution_report` の F5na std layer compositor tile RLE present host execution report boundary は、F5mz action と executor-supplied `Result unit GuiError` を `GuiRgba8888CompositorTileRlePresentHostExecutionReport` に束ねる。report は action context and executor outcome を同じ value に保持する metadata-preserving envelope であり、`GuiRgba8888CompositorTileRlePresentHostExecutionReportKind` は `Succeeded` / `Failed GuiError` だけを持つ。

F5na is not actual execution and not pending completion。F5na does not depend on F5my / F5mw / F5mv and does not execute host imports。F5mx request bridge は F5mz action decoding を 1 回だけ呼び、request construction や capability validation は行わない。`report_outcome` は completion boundary へ渡す original `Result unit GuiError` を復元する。lower row-tile host execution / import / command / report、F5my dispatch、F5mw schedule、F5mv virtual drain、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_executor` の F5nb std layer compositor tile RLE present host executor validation boundary は、actual Web / native / bare executor の手前で target support と returned report association を検査する。`GuiRgba8888CompositorTileRlePresentHostExecutorSupport` は Window、Offscreen、Device とその非空の組み合わせだけを表し、empty support variant を持たない。`GuiRgba8888CompositorTileRlePresentHostExecutorError` は `UnsupportedAction` / `ReportActionMismatch`、category、expected action、reported action option を保持する typed error である。

F5nb の `validate_report_for_action` は support check、report action read、full action equality の順に進む。full action identity は target x record variant、Window id、surface、frame、expected counts、packet descriptor、compositor metadata、run offset、run count、RGBA channel を含む。matching action の failed report は association として valid なので、この layer では拒否しない。F5nb is not actual execution and does not execute host imports。F5my / F5mw / F5mv、F5mx request construction、lower row-tile host / dispatch / schedule / virtual paths、raw storage、Vec、host API、platform API、queue、timer、scheduler、DOM / Canvas / minifb、video memory、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_dispatch_loop` の F5nc std layer compositor tile RLE present dispatch loop outcome boundary は、F5my の scheduled dispatch output と host executor outcome を one-shot pending completion で接続する。`GuiRgba8888CompositorTileRlePresentDispatchLoopPendingRequest` は previous state、next state、request、post phase を保持し、Clone / Copy を持たない。`complete_request consumes pending` ため、同じ host outcome を二重に反映しない。Err outcome は `HostImportExecutionFailed` と previous state を返し、Ok outcome は post phase に従って Continue / Yield / Completed に next state を入れる。F5nc は F5my dispatch step only を authority とし、F5mw schedule、F5mx request construction、F5mv virtual drain、lower row-tile path、raw storage、Vec、queue、timer、scheduler、platform API、DOM / Canvas / minifb、video memory、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_report_loop_bridge` の F5nd std layer compositor tile RLE present host report loop bridge boundary は、F5nc pending request から request を読み、F5mz action decoding で expected action を作り、F5nb validate_report_for_action で support と full metadata identity を validation before completion として検査し、成功時だけ F5na report_outcome を F5nc completion に渡す。`GuiRgba8888CompositorTileRlePresentHostReportLoopBridgeError` は `ExecutorValidationFailed %GuiRgba8888CompositorTileRlePresentHostExecutorError` または `LoopCompletionFailed %GuiRgba8888CompositorTileRlePresentDispatchLoopError`、category、state を保持する。validation failure では pending previous state を返し、completion は呼ばない。matching failed report は loop completion error として `HostImportExecutionFailed` に進む。F5nd は F5my / F5mw / F5mx / F5mv direct call、lower row-tile host / dispatch / schedule / virtual path、raw storage、Vec、host API、platform API、queue、timer、scheduler、DOM / Canvas / minifb、video memory、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_execution_driver` の F5ne std layer compositor tile RLE present host execution driver boundary は、F5nc pending request を actual executor が読む action と one-shot pending completion value に束ねる。`GuiRgba8888CompositorTileRlePresentHostExecutionDriverPending` は pending と F5mz action を所有し、Clone / Copy を持たない。`prepare` は F5nc pending request accessor、F5mz action decoding、driver pending construction の順に進む。`pending_action` は executor 用 action を借用で公開し、`complete_outcome` は stored action と executor-supplied `Result unit GuiError` から F5na report を作り、F5nd bridge に validation before completion と F5nc completion を委譲する。F5ne は F5nc `complete_request`、F5nb `validate_report_for_action`、F5mx request construction を直接呼ばない。actual host import、lower row-tile host / dispatch / schedule / virtual path、raw storage、Vec、host API、platform API、queue、timer、scheduler、DOM / Canvas / minifb、video memory、fallback、silent no-op は提供しない。

`std/gui/compositor_tile_present_host_action_sink` の F5nf std layer compositor tile RLE present host action sink boundary は、actual executor が返した executor-supplied outcome を F5mz action と一緒に `GuiRgba8888CompositorTileRlePresentHostActionSinkStep` へ包む。step construction の前に F5nb `require_supported` を呼び、unsupported target は `UnsupportedAction` と typed category を持つ error として返す。F5nf sink does not manufacture success であり、std layer では `Result::Ok unit` や fallback success を作らない。F5nf sink は F5ne driver pending ownership、F5ne completion、F5na report construction、F5nd bridge、F5mx request construction、actual platform execution、lower row-tile path、raw storage、queue、timer、scheduler、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_sink_driver` の F5nf std layer compositor tile RLE present host action sink driver boundary は、F5nf sink step と F5ne one-shot driver completion を接続する。`gui_rgba8888_compositor_tile_rle_present_host_action_sink_driver_step` は driver pending から action を借用で読み、caller が渡した executor-supplied outcome を F5nf sink step に渡す。sink rejection では completion を呼ばず、`SinkRejected` の owner-bearing error として original driver pending を返す。sink success では同じ outcome を F5ne `complete_outcome` に渡し、`GuiRgba8888CompositorTileRlePresentHostActionSinkDriverStep` として sink step と completion を返す。driver completion failure では pending は既に消費済みなので、F5ne driver error と sink step だけを `DriverCompletionFailed` に保持する。F5nf driver does not manufacture executor outcome。std layer で `Result::Ok unit` や synthetic `Result::Err` を作らず、actual executor の `Result unit GuiError` をそのまま流す。F5nf driver は F5nc direct completion、F5nd direct bridge、F5nb direct validation、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、raw storage、queue、timer、scheduler、platform API、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_attempt_driver` の F5ng std layer compositor tile RLE present host action attempt driver boundary は、actual Web / native / bare / headless executor が返した action attempt と F5ne driver pending の action identity を比較してから、F5nf action sink driver へ outcome を渡す。`GuiRgba8888CompositorTileRlePresentHostActionAttempt` は attempted action と executor-supplied outcome だけを持つ Copy value である。`gui_rgba8888_compositor_tile_rle_present_host_action_attempt_driver_step` は driver pending から expected action を借用で読み、attempt action と F5nb full action equality で比較する。一致しない場合は F5nf sink driver を呼ばず、`AttemptActionMismatch` の owner-bearing error として expected action、attempted action、`GuiError::InvalidCommand` category、original driver pending を返す。一致した場合だけ、attempt outcome を F5nf `gui_rgba8888_compositor_tile_rle_present_host_action_sink_driver_step` に委譲する。F5ng does not manufacture executor outcome。`Result::Ok unit` や synthetic `Result::Err` を作らず、attempt に含まれる `Result unit GuiError` だけを流す。F5ng は F5nf sink direct call、F5ne direct completion、F5nd direct bridge、F5nb direct validation、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、raw storage、queue、timer、scheduler、platform API、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_executor_session` の F5nh std layer compositor tile RLE present host action executor session boundary は、F5ne one-shot driver pending を actual Web / native / bare / headless executor が扱う session request / complete surface に包む。`GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionState` は `Ready driver` または `Completed` で、`Completed` request は lower call なしで terminal result を返す。`Ready` request は F5ne pending action を借用で読み、`GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionPending` に driver owner と borrowed-derived expected action value を保存する。complete は outcome-only complete であり、actual executor から action identity を受け取らない。F5nh は pending の expected action と caller supplied `Result unit GuiError` から F5ng attempt を作り、F5ng attempt driver を 1 回だけ呼ぶ。success は F5ng completion accessor で dispatch loop completion を読み、failure は F5ng category accessor と lower F5ng error を owner-bearing complete error に保持する。F5nh does not manufacture executor outcome。`Result::Ok unit` や synthetic `Result::Err` を作らず、caller supplied outcome だけを流す。F5nh は F5nf sink / sink driver direct call、F5ne direct completion、F5nd direct bridge、F5nb direct validation、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、raw storage、queue、timer、scheduler、platform API、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_platform_bridge` の F5ni std layer compositor tile RLE present host action platform bridge boundary は、F5nh session pending から borrowed pending accessor で expected action を読み、target / record kind / descriptor を platform executor 用に投影する。`GuiRgba8888CompositorTileRlePresentHostActionPlatformTarget` は Window / Offscreen / Device、`GuiRgba8888CompositorTileRlePresentHostActionPlatformRecordKind` は Begin / Run / End を表す。complete helper は outcome-only complete を保ち、platform executor が返した `Result unit GuiError` を F5nh complete へ戻すだけである。この std helper は action identity、report、attempt、raw host status、platform host import を作らず、does not manufacture platform outcome。F5ng / F5nf / F5ne lower completion、F5nd bridge、F5nb validation、F5na report construction、F5mx request construction、scheduler、queue、timer、DOM / Canvas / minifb、video memory、fallback、silent no-op は提供しない。

`platforms/gui/web/compositor_host_executor` の F5ni Web compositor host action executor backend bridge は、F5ni std bridge から得た expected action を `nepl_gui_web.compositor_tile_present_begin/run/end` host import ABI の begin / run / end いずれか 1 回に渡す。raw status `0` は success、負 status は `GuiError::Unsupported` / `InvalidCommand` / `ResourceExhausted` / `BackendFailure` などに写し、最後に F5ni bridge / F5nh outcome-only complete へ戻す。`nodesrc/run_test.js` の default Web import は stdlib doctest 用の fail-closed boundary として `GuiError::Unsupported` 相当を返す。これは no video memory fallback の境界であり、legacy stdout transport、Canvas / DOM 直接描画、queue、timer、scheduler、present loop、silent no-op は行わない。
...
F5nk の partial dirty slot preservation/copy は、Web host surface record の committed snapshot を正式な baseline authority として扱う。full publish は slot 全体を committed snapshot に保存し、partial begin は snapshot を新しい write slot へ copy してから metadata row range の RLE run を書く。baseline が無い partial begin と raw rect publish は `InvalidCommand` 相当へ fail-closed にし、published slot を presenter read API で baseline 化しない。partial end は absolute metadata row range を検証し、全 batch / tile 完了後だけ dirty rect publish を行う。snapshot 候補は publish 前に作り、publish 成功後だけ committed snapshot と active frame removal を確定する。
...
F5ck の run writer cursor boundary は、`GuiRgba8888RowTileRleStorageOwner` を `GuiRgba8888RowTileRleWriteCursorOwner` へ変換し、1 step ごとに固定 12 byte record を書く。record は `pixel_offset i32 little-endian`、`pixel_count i32 little-endian`、`Rgba8888 r,g,b,a` であり、writer は checked base pointer projection 後に `pixel_offset`、`pixel_count`、packed RGBA word の 3 個の i32 word として一括 store する。writer は consuming `gui_rgba8888_row_tile_rle_cursor_next_run` で run metadata と next cursor を一度だけ作り、12 byte write がすべて成功した場合だけ next cursor を committed cursor として返す。store / projection failure では `gui_rgba8888_row_tile_rle_step_recover_start_cursor` により run 開始時の cursor を復元し、owner-bearing error で unchanged `written_run_count` / `written_byte_count` を返す。partial bytes が target slot に存在しても、その slot は uncommitted であり public reader はない。completion は `written_run_count == total_run_count` と lower cursor `Complete` の両方を要求し、ready cursor なら invariant error とする。この writer boundary は payload byte reader、encoded byte reader、host present、video memory host call、Canvas / DOM / minifb、platform surface、fallback、silent no-op には進まない。
...
`alloc/gui/font/sfnt/glyf_span_index`はcomplete classified collectionを消費し、contour spanを一度だけ順次構築するindexed collection ownerを返す。point stepは1 itemをexactly once読み、point index、glyph、point由来kindを検証し、endpoint itemでだけ1 spanをcommitして同じbuilder owner型を返す。別のcompletion APIはitem readとspan pushを行わずindexed ownerを直接返す。resourceを二重に持つownerをterminal enumへ包まない。owner-bearing start / step errorはcollectionとpartial span storageを回収可能にする。completed indexed ownerはcollectionとspan indexを分離せず、borrowed collectionと`&indexed_owner + contour_index`だけを受けるO(1) checked span lookupを公開する。lookupは`vec::get`をexactly once呼び、collection readとold full-scan helperを呼ばない。全buildはO(p + c)、storage O(c)であり、old O(p) per-lookup contour span helper、path action / command、outline storage、metric、layout、raster、render、platform、fallbackへ進まない。registered adapterとindexed path traversalは後続phaseで接続する。

`alloc/gui/font/registered_face_simple_glyph_provenance`はentry、mapping record、canonical `glyf` table、source glyphを検証するCopy success / typed error authorityであり、readerとindex adapterが共有する。`alloc/gui/font/registered_face_simple_glyph_collection`はpublic collection take APIを削除し、F5nu completed ownerをprivate field transferでF5nv builder / indexed ownerへ移す。start / step / complete failureは元ownerまたはpartial ownerを回収可能にし、collectionだけを取り出してprovenanceを捨てるpublic経路を提供しない。completed ownerはregistered evidenceとindexed collection ownerを同じnon-Copy ownerに保持する。

`alloc/gui/font/sfnt/glyf`はexisting point / edge / curve / event logicをchecked-span coreとlegacy full-scan adapterへ分け、`alloc/gui/font/sfnt/glyf_indexed_path`はshared coreだけを使う。checked-span contour stepはcursor contourとspan contourの不一致を`CursorContourMismatch`としてevent lookup前に拒否する。`alloc/gui/font/registered_face_simple_glyph_indexed_path`はregistered indexed owner、policy、`PendingContour` / `ActiveContour` / `Completed` stateを同じnon-Copy traversal ownerに束ねる。public step / budget drainはownerだけを消費して返すためraw collection、raw span、別index、foreign cursorを受けない。pending contourごとにO(1) index lookupを一度だけ行い、active contourではspanを再利用する。budget結果は常に唯一のpath ownerとCopy statusを持ち、`Emitted`、`Completed`、`StepBudgetExhausted`のどの状態でもowner回収経路を変えない。budget 0はlookup / readを行わず、eventはexisting pure sink-step projectionを通してexplicit off-curve / closure policyを適用する。F5nwはsink-step traversalまでを所有し、action / storage / command stream / stroke provenanceのindexed移行はF5nxが所有する。
...
### Registered stroke compositor F5nzf BeginFrame schedule boundary

`alloc/gui/font/registered_face/simple_glyph/indexed/stroke_compositor_tile_rle_begin_frame_schedule`はF5nzeのvalidated F5mv authorityをF5mw deterministic scheduleへrecord replayなしで接続する。F5mw Begin adoptionは`InFrame`とseen counters 0/0を要求し、同じdrain、slice counters 1/0、exact command-budget phaseを返す。F5nzf owner/errorはF5nze owner全体を保持する。F5mx host continuation、dispatch、host execution、platform presenterはこのmoduleの責務外である。
...
### Registered stroke compositor F5oaf Web terminal completion

F5oae successのactual Completed stateとexact EndFrame stepを同じownerから回収し、F5nzt共通transitionとF5nzv completionへ各一度渡す。successはactual/terminal completionとprovenance、spent budgetを保持する。phase、terminal command、cleanup errorはowner-bearing recoveryを提供し、消費済みschedule/session authorityを再構築しない。
...
### Web registered stroke compositor F5oac resumed Run execution

`GuiRgba8888CompositorTileRlePresentDispatchLoopScheduleOnlyStep` はprevious/next state、phase、exact recordを保持し、consuming adopterだけが同じrecordをhost requestへ変換する。失敗型は元stepと`GuiError`を保持するためcallerはrollback authorityを再構築しない。

`platforms/gui/web/font_registered_run_executor` はF5oab Run continuationを既存schedule、request、driver、session、Web executorへ各一度移し、actual outcomeをF5nh completionへ戻す。CompletionFailedはcategoryとcursor、owner-bearing F5nh lower recoveryをconsuming partsで返す。公開契約はraw importを再定義せず、EndFrameや複数command loopを実行しない。
