...
```text
web/src/gui-preview
    Web Playground の floating GUI window layer、typed preview command DTO、runtime bridge、host frame decoder、Canvas2D adapter
...
Preview の描画 data は `web/src/gui-preview/commands.ts` の `fill-rect` / `text-run` command union、`rgba8888` 相当の color struct、command frame で表す。`renderer.ts` は command stream と `mandelbrot` / `life` / `counter` metrics union だけを生成し、`CanvasRenderingContext2D`、DOM element、CSS color string、`null | undefined`、optional metric field を持たない。`host-bridge.ts` は unknown input を `GuiWebHostResult` の `ok` / `err` union で decode し、invalid frame、invalid command、invalid rect、invalid color、invalid text、unsupported command を typed error として返す。`runtime-bridge.ts` は presenter missing、invalid install target、host decode error を `GuiWebRuntimeResult` で返し、global `neplGuiHost.presentCommands` を floating window presenter へ接続する。Canvas 固有の色変換、font、text align、scale / viewport 計算は `canvas-renderer.ts` に閉じ込める。

ただし、この checkpoint はまだ NEPLg2 program から `DrawCommand` stream を JS / native host へ直接 export する ABI ではない。`CanvasRenderingContext2D`、DOM element、minifb は backend implementation detail であり、`core/gui`、`alloc/gui`、`std/gui` の public type には入れない。次の段階で NEPL/Wasm が生成した command stream を `neplGuiHost.presentCommands` へ渡し、native 側も正式 `GuiHost.present` へ寄せる。
...
| `web/src/gui-preview` | Web Playground display smoke backend | editor panel layout の上に floating GUI window layer を描画し、Mandelbrot / Life / Counter preview と host-decoded frame を typed command DTO 経由で Canvas2D adapter へ表示する。window manager は source / move / window mode / dock 状態を union で表し、`commands.ts` / `renderer.ts` は Canvas / DOM 型、`null | undefined`、optional metric field を持たない。`host-bridge.ts` は unknown input を `GuiWebHostResult` で decode し、Canvas / DOM 型や throw に依存しない。`runtime-bridge.ts` は global `neplGuiHost.presentCommands` と typed presenter registration を持ち、DOM / Canvas 型に依存しない。NEPL/Wasm 生成 command stream からの direct export / import 接続は未実装 |
