...
`platforms/gui/web/compositor_host_executor` の F5ni Web compositor host action executor backend bridge は、F5ni std bridge から得た expected action を `nepl_gui_web.compositor_tile_present_begin/run/end` host import ABI の begin / run / end いずれか 1 回に渡す。raw status `0` は success、負 status は `GuiError::Unsupported` / `InvalidCommand` / `ResourceExhausted` / `BackendFailure` などに写し、最後に F5ni bridge / F5nh outcome-only complete へ戻す。`nodesrc/run_test.js` と Web worker の default import は actual compositor 実装が接続されるまで fail-closed に `GuiError::Unsupported` 相当を返す。これは no video memory fallback の境界であり、legacy video memory path、Canvas / DOM 直接描画、queue、timer、scheduler、present loop、silent no-op は行わない。
