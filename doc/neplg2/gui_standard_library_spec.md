...
F5nj の Web compositor host import actual implementation は、Web worker 内で full-frame video memory surface backing を使い、`begin` が `GuiVideoMemorySurface` から write slot を取得して caller supplied frame id に紐付け、`run` が tile-local contiguous RLE pixel offset を RGBA8888 slot bytes へ書き、全 batch / tile の `end` が完了したときだけ full dirty として publish して既存 `gui_video_memory_present` ack path へ渡す。target は Window target only であり、Offscreen / Device は `GuiError::Unsupported` 相当へ fail-closed に写す。現行 slot は新しい write slot に前フレーム pixels を保持しないため、partial dirty unsupported until slot preservation/copy を明示し、metadata row range が full frame でない descriptor は `Unsupported` にする。compositor ABI に title が無いため、Web worker は deterministic title policy として surface handle 由来の固定 title を生成し、空文字 fallback や DOM / Canvas 直接描画には進まない。
