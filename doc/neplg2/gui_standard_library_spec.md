...
`examples/gui_video_memory_rows.nepl` は focused NEPL example として、`ByteBuilder` / `ByteBuf` owner で row bytes を構築し、borrowed `MemPtr u8` を `gui_web_video_memory_write_rgba8888_row` へ渡す。これは stdout `rgba-row` を使わない formal row host import の source contract を示すための例である。現行 CI の `run_test.js` は `nepl_gui_web` video memory host import を unsupported stub として持つため、この example の happy path は Web Playground host で実行する。fake positive `nepl_gui_web` host import harness と Mandelbrot の全面移行は後続作業である。
