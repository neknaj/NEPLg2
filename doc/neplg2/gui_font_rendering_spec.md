...
F5nq 以降、`alloc/gui/font/registered_face_glyph_lookup` は `GuiFontRegisteredFaceTableEntry` を借用し、table layer の entry validator が entry record と face owner から再導出した record を全 field で照合してから `gui_sfnt_lookup_glyph_id` を 1 回だけ呼ぶ。成功時は `GuiFontRegisteredFaceGlyphMapping` として table record、code point、`GuiGlyphId` の Copy evidence を返す。失敗時は owner を消費せず、同じ record、code point、`GuiSfntParseError` または `GuiFontRegisteredFaceTableEntryValidationError` を `GuiFontRegisteredFaceGlyphLookupError` として返す。glyph lookup error は shared validation payload を直接保持し、`InvalidFaceProjection` と `RecordMismatch` を失わない。F5nq は selected face index を table membership 由来の authority とし、resource request、display name、path suffix、browser / OS font API を glyph lookup authority にしない。
...
F5nr 以降、`alloc/gui/font/registered_face_horizontal_metric_lookup` は借用した `GuiFontRegisteredFaceTableEntry` と F5nq の `GuiFontRegisteredFaceGlyphMapping` を同時に受け取る。table layer の entry validator が返す canonical record と mapping record を照合し、同じ entry owner の bytes、canonical selected face index、mapping 内の `GuiGlyphId` だけを使って `gui_sfnt_lookup_horizontal_metric` を 1 回だけ呼ぶ。別 entry 由来の mapping は hmtx parse 前に `MappingRecordMismatch` として拒否する。
...
F5ns 以降、`alloc/gui/font/registered_face_simple_glyph_lookup` は借用した `GuiFontRegisteredFaceTableEntry` と F5nq mapping を受け、shared entry validation と mapping record 照合の後に、registered face が登録時から保持する `GuiSfntMetadata` を使って point-stream source を 1 回だけ解決する。success の `GuiFontRegisteredFaceSimpleGlyphPointStream` は mapping と、canonical `glyf` table record / `GuiSfntSimpleGlyphPointStream` を束ねた `GuiSfntSimpleGlyphPointStreamSource` の Copy evidence を保持する。別 entry 由来 mapping は `glyf` read 前に拒否し、SFNT metadata、cmap、hmtx lookup を再実行しない。

point stream 内の offset / length は registered face bytes 内 `glyf` table に対する範囲であり、source evidenceもbytes ownerではない。後続 point decode / path construction は同じ entry と F5ns evidence を借用し、entry validation と mapping record を再照合してからbytesを読む。entry close後のCopy sourceだけをoutline authorityとして使ってはならない。`MissingTable`、`UnsupportedLocaFormat`、`MalformedGlyfRecord`、`MissingGlyphOutline`、`UnsupportedGlyphOutlineFormat` はtyped lower errorとして保持し、fallback glyphやhost font APIへ切り替えない。F5nsはpoint coordinate decode、path construction、metricとの結合、scaling、shaping / layout、rasterization、render2d、platform presentationを完成させない。

F5nt 以降、`alloc/gui/font/registered_face_simple_glyph_reader` は同じ entry と F5ns evidence を借用し、entry / mapping provenance、face-owned metadataのcanonical `glyf` record、source topology glyphを再検証した後、source evidenceからlogical pointを順次復元する。登録時 metadata、topology、flag / coordinate rangeを再解析しない。sequential cursorはsource identity、logical index、raw flag cursor、active repeat run、x / y byte cursor、累積座標、contour endpoint cursorを保持し、raw cursor constructorを公開しない。registered cursorはF5ns evidenceとlower cursorを一体化し、stepは別source/evidenceを受けず`entry + cursor`だけで進む。各stepは償却O(1)、全point / contourはO(p + c)である。既存random-access point APIと、それをpointごとに呼ぶ旧outline drainはO(p^2)となるため、新しいfull outline / path pipelineの基盤にしてはならない。

F5nu 以降、`alloc/gui/font/registered_face_simple_glyph_collection` はF5nt reader cursorと既存`GuiSfntSimpleGlyphOutlinePointStreamItemCollection` ownerを一体化し、1 stepにつき1 logical pointだけをclassified itemへ変換してcollectionへ追加する。startはregistered evidence validationをallocationより先に行い、foreign evidenceではstorageを確保しない。capacityはsource topologyのexact point countから導出し、caller-supplied item limitを超える場合はtyped allocation errorを返す。stepはownerを消費し、p回の`Collecting owner`の後、次のEnd stepで`Completed collected_owner`を返す。push失敗ではcollectionを回収してpre-step cursorへ戻すため、retryで同じpointを再decodeし未commit pointを飛ばさない。owner-bearing terminal / errorはnon-Copyであり、errorはborrowed metadata accessor、consuming owner recovery、freeを公開する。reader / collection push / completion invariant failureはcollection ownerを失わず、全point収集はO(p + c)、追加storageはO(p)とする。completed ownerはF5ns evidenceとclassified collectionを保持し、既存contour/path consumerが借用できるが、このphaseではpath command、metric join、scale、shaping / layout、rasterization、render2d、platform presentationへ進まない。

F5nv 以降、`alloc/gui/font/sfnt/glyf_span_index`はcompleted classified point collectionを消費し、contour spanを一度だけ順次導出するindexed collection ownerを作る。既存`gui_sfnt_simple_glyph_outline_point_stream_item_collection_contour_span`は1 lookupごとにcollection全体を走査するため、path actionごとに使用するとO(a * p)になる。新しいfull path pipelineはこのfull-scan lookupを使わず、F5nv indexのO(1) span lookupをauthorityとする。

builder ownerはcollection、exact contour-capacityのspan storage、next point index、next contour index、current contour start indexを所有する。startはcomplete collection invariantとcaller contour limitを検査してからspan storageを確保する。point stepはcollection itemをindex順にexactly once読み、point index、glyph、point由来kindを検証し、endpoint itemのときだけ1 spanを追加して同じbuilder owner型を返す。p回のpoint step後、別のcompletion APIがitemを読まずcontour count、final endpoint、span len / capを検証し、non-Copy indexed collection ownerへ移す。pointが残るcompletion、または完了済みbuilderへのpoint stepはtyped progress errorとする。resourceを二重に持つownerをterminal enumへ包まず、各APIが一種類のownerを直接返す。read / payload validation / push / completion failureはcollectionとpartial indexを失わないowner-bearing typed errorとする。

indexed ownerはcollectionとspan storageを分離せず、borrowed collection accessorと`&indexed_owner + contour_index`だけを受けるO(1) checked span lookupを公開する。lookupは`vec::get`をexactly onceだけ使い、collection item readと旧full-scan helperを呼ばない。全index buildはpoint数pとcontour数cに対してO(p + c)、追加storage O(c)である。F5nvはregistered provenance、path action、outline storage、path command、metric join、layout、raster / render2d、platform、fallbackへ進まない。F5nwはF5nu completed ownerをF5nv indexed ownerへ移すregistered adapterを追加し、その後のpath scanはindexed span以外のspan discoveryを禁止する。

F5nw 以降、`alloc/gui/font/registered_face_simple_glyph_collection`はF5nu completed ownerのevidenceとcollectionをprivate transferで同時にF5nv builderへ移す。従来のpublic `gui_font_registered_face_simple_glyph_collected_owner_take_collection`は削除し、collectionだけをprovenanceから分離するpublic APIを残さない。index start、point step、completionの各失敗はevidenceとcollectionまたはpartial indexを含むownerを回収可能にし、登録済みfaceとの対応を失わない。成功した`GuiFontRegisteredFaceSimpleGlyphIndexedOwner`はregistered evidenceとF5nv indexed ownerを同じnon-Copy ownerに束ね、lower ownerやraw collectionを取り出すconsuming accessorを公開しない。

entry、mapping record、canonical `glyf` table、source glyphの検証は`alloc/gui/font/registered_face_simple_glyph_provenance`の独立したCopy validation result / typed errorをauthorityとする。readerはshared errorをreader errorへ明示的にmappingし、index startはshared errorを保持したまま元completed ownerを返す。reader固有cursor identityはshared provenanceとは分離する。

同phaseではexisting `sfnt/glyf`のpoint、edge、curve、event実装を、checked spanを明示的に受けるpure coreと旧full-scan adapterへ分割する。`alloc/gui/font/sfnt/glyf_indexed_path`はこのshared coreだけを呼び、検証ロジックを複製しない。checked-span contour stepはcursor glyphに加えてcursor contour indexとspan contour indexをevent lookup前に照合し、不一致を`CursorContourMismatch`として返す。`alloc/gui/font/registered_face_simple_glyph_indexed_path`はregistered indexed owner、policy、private traversal stateを同じnon-Copy ownerに束ねる。stateは`PendingContour contour_index`、`ActiveContour span cursor`、`Completed`のenumで表し、public step / drainはこのownerだけを消費して返す。`PendingContour`を最初に進めるときだけF5nv index lookupをexactly once行い、同じspanをcontour終了まで再利用する。budget drainは`Completed`判定、budget 0判定、stepの順であり、nonterminal budget 0はlookupとcollection readを行わない。budget結果は常に1つのpath ownerとCopy status `Emitted sink_step` / `Completed` / `StepBudgetExhausted`を返し、owner-bearing variantを複数持つterminal enumにはしない。これによりcallerはstatusを読んでから唯一のownerを回収できる。1 sink step成功時だけbudgetを1消費する。F5nwはeventを`gui_sfnt_simple_glyph_path_sink_step_from_contour_step`へ渡すところまで含み、off-curveと`KeepOpen` / `EmitCloseAfterFinalEvent` policyを既存pure projectionで適用する。座標一致からclosureを推測しない。
...
F5nxe PointX populationはF5nxd checked completed endpoint ownerだけをpublic start inputとする。外部collection、capacity、cursor、point index、PointX slotを受けない。start全体の失敗優先順位はcompleted endpoint invariant、canonical point countとowner-bound collection item count、checked PointX cursor construction、PointX phase invariantの順である。phase invariant内部のinvalid kind順位とは別の契約であり、先行start stageが失敗した場合は後続stageを評価しない。success ownerはF5nxd completed ownerとprivate `Active cursor | Completed cursor` stateをnested ownershipで保持し、raw collection/storage/indexed ownerを分離するpublic APIを持たない。

start error kindは`EndpointCompletionInvariantInvalid | CollectionItemCountMismatch | CursorStartFailed | PhaseInvariantInvalid`である。errorは元F5nxd completed owner、kind、endpoint invariant check、expected/actual item count、optional cursor error、PointX phase invariant checkを保持する。metadataはowner takeより先にborrowで読み、ownerはexactly once回収するかerror全体をfreeする。sealed collectionはregistered authorityとcollection ownership、collectionのcapacity/count/len/cap整合を保持するが、各item payloadのglyph/index/kindを事前保証しないためF5nxe stepが必ず再検証する。

PointXのlogical item indexは`cursor.next - cursor.start`だけから導出する。storage progressは`storage len == contour count + logical item index`であり、absolute scalar indexをcollection item indexとして使わない。stepはterminal確認、budget 0以下、owner-bound item read exactly once、item glyph/index/kind validation、typed PointX slot construction、owner-bound PointX push exactly onceの順で進む。1 successful operationはitem validationとpushを完了した1 point commitである。read、validation、push failureではcursor next、storage len、logical item indexを変更せずsame F5nxe ownerを回収できるtyped errorを返す。lower PointX push errorが返すstorageはfailure前と同じlenでなければならず、F5nxeは回収ownerを再構築した直後にphase invariantを再検査し、その結果をpush errorの`recovered_invariant` metadataへ保持する。read、identity validation、already completed errorではこのmetadataは`None`である。0以下のbudgetではread/pushを行わず、Completedはbudgetより優先する。
...
F5nxiはF5nxh checked completed PathCommandTag owner全体だけを入力authorityとし、registered command value lookup、bounded cursor、stream preparationを一つのowner chainへ統合する。command valueはlogical command indexに対応するstored tagとowner-bound checked-span event sourceを照合して得る。cursorはnext/endを保持し、summaryは`total_count`、各command kind count、`last_path_command_index`を保持する。初期summaryは全countが0、lastが-1であり、全drainの計算量はO(command + contour)、追加領域はO(1)である。

budgetはCompleted、remaining 0以下、最大1 lookup/summary commitの順に判定する。failureはsame completed PathCommandTag authority、失敗前cursor、失敗前summaryをtyped errorへ保持する。checked sealはcursor end、summary total、kind count合計、last index、phase invariantを検査し、completed prepare ownerだけをF5nxj command sink plan / allocation / writerへ渡す。raw collection/storage split、caller supplied tag/event/value、sink mutation、stroke、raster、render、platformへ進まない。

F5nxjはF5nxi checked completed prepare owner全体だけを入力authorityとし、`PlanReady -> Allocated -> Writing -> Completed`のregistered outer ownerでcommand sink plan、scalar storage allocation、writer completionを連結する。planはF5nxi summaryからchecked算術で導出し、旧prepare terminalやcaller supplied countをauthorityにしない。legacy F5ay/F5az/F5baのstorage-only capacity算術、2本のVec allocation、command scalar append algorithmは変更せず再利用するが、plan、storage、writerをregistered source ownerから分離してtake/pairingできない。

F5nxk authority adapterはF5nxj completed owner全体を保持する。command metadataとindexed contour spanを照合した後、sealed registered authorityから同じspanをowner-bound curve coreへexactly once渡し、sink eventはcurveからpure projectionする。command value、span、curve、eventを同じCopy commandへ束ね、raw collection、nested source、writerを公開しない。stroke metric provenance drainはsummary由来のexact draw capacityを持ち、terminal-before-budget、1 step 1 command、push-success-before-commitを守る。checked sealはcursor、kind counts、metric count、Vec len/capを再検査し、sealed ownerからCopy provenanceをO(1)で読む。actual numeric metric joinは後続とする。

F5nxlではF5ku actual metric ownerをopaque authorityとして扱い、count summary、owner-bound O(1) metric read、single freeだけを公開する。production checkpoint bridgeはcompleted legacy writerを消費してrequest / plan / cursor / metric drainを有限budgetで完走し、private段階の失敗ownerを公開errorから分離しない。registered transitionはsourceを成功checkpointまたは失敗errorと一体所有する。registered joinはF5nxk provenanceとactual metricを同じowner-bound metric indexで照合し、kind、segment index、line / quadratic座標、stroke widthが一致したCopy joined recordだけを返す。raw Vec / plan owner split、caller-supplied metric、checked commandからのstroke width推測、legacy full scanは禁止する。Line / Quadratic writer runtimeだけをregistered drawable fixtureやjoin完成と見なしてはならない。

F5nxmはF5nxl join ownerを唯一のmove-only authorityとして、一件ずつ既存F5kx offset geometryへ射影する。registered型をF5kxへ逆importせず、中立Copy provenanceとactual metric projectionだけを渡す。F5kxのprovenance guard、normal、finite guard、stroke width照合、quadratic partial degeneracyを再利用し、topology、source curve、normal、offset端点をCopy結果に保持する。geometry失敗では進行済みownerとrejected joined valueを同じtyped errorに残し、terminal-before-budgetとsingle freeを維持する。

join stepはterminalをbudgetより先に判定し、budget exhaustedではauthorityとcursorを変えない。進行時はprovenance、actual、provenance metric indexとactual segment index、variant、checked curve座標、canonical stroke widthの順に検査し、成功後だけcursorをcommitする。path command indexはMove / Skipを含むcommand列の独立metadataであり、draw/metric列のsegment indexとは比較しない。失敗errorは同じmove-only join ownerを回収可能に保持する。

join開始はF5nxk completed authorityを一度だけ消費してregistered source、sealed provenance、actual checkpointを一体化し、metric / Line / Quadratic countとactual storage shapeを走査前に検査する。canonical stroke widthは同じactual buildへ渡したpaintから保持し、actual値から期待値を作らない。開始失敗はbuild failureまたはpost-build mismatchの全authorityをowner-bearing errorに保持する。

Writingはcompleted prepare authorityをborrowしたowner-bound read-atとprivate cursorを使い、Completed、remaining 0以下、command read、legacy writer push、cursor commitの順に高々1commandを処理する。allocation/start/read failureはmetadataを読むまでregistered authorityを保持し、回収またはsingle freeできる。multi-scalar push途中のfailureはlegacy owner-bearing errorからmetadataを読むまでpartial writerを保持し、その場でsingle freeしてretryable通常ownerへ戻さない。outer cleanup-only errorはregistered authority、保存済み分類、failure cursorだけを保持する。checked sealはcursor end、written/kind counts、last index、plan/capacity、scalar len/capを再検査し、registered completed writer ownerだけをF5nxkへ渡す。raw collection/storage split、foreign pairing、旧full scan、stroke、rasterization、render、platformへ進まない。

Pendingからspan lookupに成功した後のtag/edge/event read failureでは、recovered ownerがchecked spanを保持するActiveへ進んでよい。これはcommand cursorとsummaryを進めないO(1) cache transitionでありsuccessful command commitではない。retryは同じActive spanを再利用し、contour lookupを繰り返さない。
...
Bevel join は F5kz join closure record の `from_end -> to_start` connector chord をそのまま保持する。Miter join は隣接する 2 本の line side edge の無限直線交点を miter point とし、`from_end -> miter` と `miter -> to_start` の 2 線分として後続 scan に渡す。miter limit は nested `GuiStroke` の stroke width と F5kz record の `miter_limit` を authority とし、交点が非有限、直線が平行、または miter limit を超える場合は typed bevel record として保持する。miter limit を超える場合は typed bevel record として保持し、通常 bevel と区別できるよう `miter_clipped` evidence を残す。miter / round join が quadratic side edge を含む場合は、line-only の miter intersection や round midpoint を捏造せず、F5kz の connector chord を `quadratic_bevel` 付き bevel record として保持する。

Round join は source vertex を中心にした 2 chord record として保持する。F5lc は referenced line side edge の directed source end / directed source start が同じ source vertex を指すことを検査し、offset endpoint の半径方向から `arc_mid` を計算して `from_end -> arc_mid` と `arc_mid -> to_start` を後続 scan に渡す。source center mismatch、stroke width evidence 不正、非有限 midpoint、180 度反転のように midpoint を一意に作れないケースは typed error として fail-closed にする。quadratic side edge を含む round join は round midpoint を捏造せず、`quadratic_bevel` 付き bevel connector として扱う。
...
F5la start は既存の shared raster coverage config / shape validation helper を再利用するが、fill coverage writer owner を直接呼び出さない。さらに completed F5lc owner について、nested F5kz closure invariant、join geometry Vec len/cap、bevel / miter / round count 合計を再検査する。
...
start は F5la writer の shape、cell storage、`written_cell_count == 0`、`quadratic_side_edge_segment_count > 0` を再検査し、さらに F5lc join geometry invariant を再検査する。quadratic side edge は endpoint chord として扱わず、明示 config による deterministic flattening だけで scan する。

coverage 計算は sample point に対して line side edge、quadratic side edge、F5lc join geometry record の交差数を数え、偶奇で inside を決める。F5lc join geometry record の交差数を数え、bevel は `from_end -> to_start` の 1 線分、miter は `from_end -> miter` と `miter -> to_start` の 2 線分、round は `from_end -> arc_mid` と `arc_mid -> to_start` の 2 線分として扱う。quadratic side edge は source quadratic point に start/end endpoint normal offset の線形補間を足した近似点列として扱う。left side は `+normal`、right side は `-normal` を用い、right side は F5ky の `SourceReverse` に従って reversed directed segment order で line crossing に渡す。f32 座標は scan 前に finite を確認し、不正なら typed error にする。
...
F5me は F5md の `GuiRgba8888CompositorTilePlanOwner` を、F5cb の `gui_rgba8888_row_tile_payload_prepare` へ 1 回だけ通す compositor tile payload bridge である。これは tile-relative byte payload view を作る phase であり、RLE encode、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。

F5me の success owner は `GuiRgba8888CompositorTilePayloadOwner` であり、lower `GuiRgba8888RowTilePayloadOwner` と `GuiRgba8888CompositorFrameEntryMetadata` を保持する。prepare error は `GuiRgba8888CompositorTilePayloadPrepareError` であり、`RowTilePayloadPrepareFailed lower_kind`、coarse `GuiError` category、回収済み `GuiRgba8888CompositorTilePlanOwner` を保持する。finish error は `GuiRgba8888CompositorTilePayloadFinishError` であり、`TilePlanFinishFailed lower_kind`、coarse `GuiError` category、回収済み `GuiRgba8888CompositorFrameEntryOwner` を保持する。success owner / prepare error / finish error は Clone / Copy を実装しない。error kind と metadata は Copy value である。

F5me は compositor tile plan owner を lower tile plan owner へ分解する前に metadata を Copy value として読む。prepare failure では lower row tile payload prepare error から kind / category を読んでから lower tile plan owner を `GuiRgba8888CompositorTilePlanOwner` へ正規化し、prepare error は `kind/category/plan` を持つ。descriptor / plan metadata は lower checked helper に委譲し、tile-relative byte read は lower `gui_rgba8888_row_tile_payload_byte_at` へ委譲する。finish では lower payload owner を消費する前に metadata を Copy value として読み、lower tile plan owner に戻してから F5md の finish entry recovery へ委譲する。`owner_free` は finish failure と finish 成功後の entry free failure を別の free error kind として返す。
...
F5mh は F5mf/F5mg の `GuiRgba8888CompositorTileRleCountOwner` を、lower `gui_rgba8888_row_tile_rle_count_completed_prepare` へ 1 回だけ渡す compositor tile RLE completed count bridge である。これは encoded transport の exact capacity evidence を作る phase であり、count step 再実行、encode、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5mh は no count step / encode / present の boundary で止まる。

F5mh の success owner は `GuiRgba8888CompositorTileRleCountCompletedOwner` であり、lower `GuiRgba8888RowTileRleCountCompletedOwner` と copied compositor metadata を保持する。completed owner / completed error は Clone / Copy を実装しない。error kind は `GuiRgba8888CompositorTileRleCountCompletedErrorKind::CompletedPrepareFailed lower_kind` として Copy value で保持する。

F5mh は compositor count owner を lower count owner へ分解する前に metadata を Copy value として読む。lower completed prepare success では lower completed owner を metadata 付き completed owner へ束ねる。lower completed prepare error では lower kind / category / total run count / cursor next pixel index を読んでから lower error の original count owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleCountOwner` へ戻す。pending cursor は `CountNotCompleted` を wrapped kind として返し、caller が count owner を free / finish / retry できる。

success completed owner と completed error の finish / free は、どちらも metadata 付き count owner へ戻してから F5mf の count owner finish / free へ委譲する。F5mh は count step、direct drain、encode cursor / writer / storage / encoded / packet、tile payload direct byte reader、row byte storage、raw `RegionToken` / `MemPtr`、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

### Render2d compositor tile RLE encode seed boundary

F5mi は F5mh の `GuiRgba8888CompositorTileRleCountCompletedOwner` を、lower `gui_rgba8888_row_tile_rle_encode_seed_prepare` へ 1 回だけ渡す compositor tile RLE encode seed bridge である。これは payload seed ownership を compositor metadata 付きで保持する phase であり、cursor restart、encode cursor、writer、storage、packet、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5mi は no cursor restart / encode / present の boundary で止まる。

F5mi の success owner は `GuiRgba8888CompositorTileRleEncodeSeedOwner` であり、lower `GuiRgba8888RowTileRleEncodeSeedOwner` と copied compositor metadata を保持する。seed owner / seed error は Clone / Copy を実装しない。error kind は `GuiRgba8888CompositorTileRleEncodeSeedErrorKind::SeedPrepareFailed lower_kind` として Copy value で保持する。

F5mi は compositor completed owner を lower completed owner へ分解する前に metadata を Copy value として読む。lower seed prepare success では lower seed owner を metadata 付き seed owner へ束ねる。lower seed prepare error では lower kind / category / total run count を読んでから lower error の completed owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleCountCompletedOwner` へ戻す。F5mi error は payload owner へ fallback せず、completed evidence recovery を保持する。
...
F5ml は F5mk の `GuiRgba8888CompositorTileRleWriterPlanOwner` を、lower `gui_rgba8888_row_tile_rle_storage_prepare` へ 1 回だけ渡す compositor tile RLE storage bridge である。これは exact encoded byte count に基づく encoded storage allocation phase であり、write cursor start、write step、encoded seal、packet、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5ml は no write / encoded / packet / present の boundary で止まる。

F5ml の success owner は `GuiRgba8888CompositorTileRleStorageOwner` であり、lower `GuiRgba8888RowTileRleStorageOwner` と copied compositor metadata を保持する。storage owner / prepare error / finish error は Clone / Copy を実装しない。prepare error kind は `GuiRgba8888CompositorTileRleStoragePrepareErrorKind::StoragePrepareFailed lower_kind`、finish error kind は `GuiRgba8888CompositorTileRleStorageFinishErrorKind::StorageFinishFailed lower_kind` として Copy value で保持する。

F5ml は compositor writer plan owner を lower writer plan owner へ分解する前に metadata を Copy value として読む。lower storage prepare success では lower storage owner を metadata 付き storage owner へ束ね、total run count、encoded byte count、cursor next pixel index、cursor pixel count を non-consuming accessor で返す。lower storage prepare error では lower kind / category / total run count / encoded byte count を読んでから lower writer plan owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleWriterPlanOwner` へ戻す。F5ml error は lower storage prepare error を公開 recovery payload にせず、retry / free 可能な F5mk writer plan owner recovery に正規化する。

success storage owner の `owner_finish_payload` は lower storage owner から storage deallocation 後の lower cursor を取り出し、lower payload を metadata 付き `GuiRgba8888CompositorTilePayloadOwner` へ戻す。storage deallocation が失敗した場合も lower finish error の cursor を metadata 付き payload owner へ戻し、`GuiRgba8888CompositorTileRleStorageFinishError` がその payload owner を保持する。`owner_free` は lower storage owner free に委譲し、lower storage finish kind を compositor storage finish kind に包む。storage finish と payload finish は error domain が異なるため、F5ml は `owner_finish_entry` を作らない。prepare error の finish / free は F5mk writer plan owner recovery から F5me payload finish / free へ委譲する。F5ml は direct write cursor start、write step、encoded seal、packet、packet record reader、tile payload direct byte reader、row byte storage accessor、raw `RegionToken` / `MemPtr`、raw load/store、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

### Render2d compositor tile RLE write cursor boundary

F5mm は F5ml の `GuiRgba8888CompositorTileRleStorageOwner` を、lower `gui_rgba8888_row_tile_rle_write_cursor_start` へ 1 回だけ渡す compositor tile RLE write cursor bridge である。これは encoded storage owner を lower write cursor owner に移す write cursor start phase であり、write step、encoded seal、packet、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5mm は no step / encoded / packet / present の boundary で止まる。

F5mm の success owner は `GuiRgba8888CompositorTileRleWriteCursorOwner` であり、lower `GuiRgba8888RowTileRleWriteCursorOwner` と copied compositor metadata を保持する。write cursor owner / start error / finish error は Clone / Copy を実装しない。start error kind は `GuiRgba8888CompositorTileRleWriteCursorStartErrorKind::WriteCursorStartFailed lower_kind`、finish error kind は `GuiRgba8888CompositorTileRleWriteCursorFinishErrorKind::WriteCursorFinishFailed lower_kind` として Copy value で保持する。

F5mm は compositor storage owner を lower storage owner へ分解する前に metadata を Copy value として読む。lower write cursor start success では lower write cursor owner を metadata 付き write cursor owner へ束ね、total run count、encoded byte count、written run count、written byte count、cursor next pixel index、cursor pixel count を non-consuming accessor で返す。start 直後の written run count と written byte count は 0 であり、後続 write step boundary だけが進める。lower write cursor start error では lower kind / category を読んでから lower storage owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleStorageOwner` へ戻す。F5mm error は lower write cursor start error を公開 recovery payload にせず、retry / free 可能な storage owner recovery に正規化する。
...
F5mo は F5mn の `GuiRgba8888CompositorTileRleWriteCursorOwner` を、lower `gui_rgba8888_row_tile_rle_encoded_seal` へ 1 回だけ渡す compositor tile RLE encoded seal bridge である。これは fully written lower writer を sealed encoded owner へ移す phase であり、packet、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5mo は no packet / present / raw byte boundary で止まる。

F5mo の success owner は `GuiRgba8888CompositorTileRleEncodedOwner` であり、lower `GuiRgba8888RowTileRleEncodedOwner` と copied compositor metadata を保持する。encoded owner / seal error / finish error は Clone / Copy を実装しない。seal error kind は `GuiRgba8888CompositorTileRleEncodedSealErrorKind::EncodedSealFailed lower_kind`、finish error kind は `GuiRgba8888CompositorTileRleEncodedFinishErrorKind::EncodedFinishFailed lower_kind` として Copy value で保持する。

F5mo は compositor write cursor owner を lower write cursor owner へ分解する前に metadata を Copy value として読む。lower encoded seal success では lower encoded owner を metadata 付き encoded owner へ束ね、total run count、encoded byte count、cursor next pixel index、cursor pixel count を non-consuming accessor で返す。lower encoded seal error では lower kind / category を読んでから lower write cursor owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleWriteCursorOwner` へ戻す。F5mo error は lower encoded seal error を公開 recovery payload にせず、retry / finish payload / free 可能な write cursor owner recovery に正規化する。

success encoded owner の `owner_finish_payload` は lower encoded owner から storage deallocation 後の lower cursor を取り出し、lower payload を metadata 付き `GuiRgba8888CompositorTilePayloadOwner` へ戻す。storage deallocation が失敗した場合も lower finish error の cursor を metadata 付き payload owner へ戻し、`GuiRgba8888CompositorTileRleEncodedFinishError` がその payload owner を保持する。seal error の `finish_payload` と `free` は F5mm write cursor owner finish payload / free に委譲する。`owner_free` は lower encoded owner free に委譲し、lower finish kind を compositor encoded finish kind に包む。encoded finish payload は packet completion ではなく recovery / abort helper であり、F5mo は `finish_entry` を作らない。F5mo は `gui_rgba8888_row_tile_rle_encoded_tile_descriptor_checked` / `gui_rgba8888_row_tile_rle_encoded_tile_plan_metadata_checked`、packet、packet record reader、tile payload direct byte reader、row byte storage accessor、raw `RegionToken` / `MemPtr`、raw load/store、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

F5mp は F5mo の `GuiRgba8888CompositorTileRleEncodedOwner` を、lower `gui_rgba8888_row_tile_rle_packet_prepare` へ 1 回だけ渡す compositor tile RLE packet bridge である。これは lower F5cm の descriptor authority validation を compositor metadata 付きで接続する phase であり、packet record reader、std present、host present、platform backend、fallback / silent no-op を作る phase ではない。F5mp は no packet record / present / raw byte boundary で止まる。

F5mp の success owner は `GuiRgba8888CompositorTileRlePacketOwner` であり、lower `GuiRgba8888RowTileRlePacketOwner` と copied compositor metadata を保持する。packet owner / prepare error は Clone / Copy を実装しない。`GuiRgba8888CompositorTileRlePacketDescriptor` は lower `GuiRgba8888RowTileRlePacketDescriptor` と copied metadata を束ねる metadata付き descriptor wrapper であり、Copy value として公開する。prepare error kind は `GuiRgba8888CompositorTileRlePacketPrepareErrorKind::PacketPrepareFailed lower_kind` として保持する。

F5mp は compositor encoded owner を lower encoded owner へ分解する前に metadata を Copy value として読む。lower packet prepare success では lower packet owner を metadata 付き packet owner へ束ねる。descriptor は lower descriptor を裸で返さず、metadata 付き descriptor wrapper として返す。descriptor accessor は frame id、batch index、tile index、plan row range、tile row range、surface shape、tile count、pixel count、run count、encoded byte count を lower packet descriptor accessor に委譲する。

lower packet prepare error では lower kind / category を読んでから lower encoded owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRleEncodedOwner` へ戻す。F5mp error は lower packet prepare error を公開 recovery payload にせず、retry / finish encoded / free 可能な encoded owner recovery に正規化する。`owner_finish_encoded` は lower packet owner から lower encoded owner を取り出し、metadata 付き encoded owner へ戻す recovery / abort helper である。`prepare_error_free` は F5mo encoded owner free に委譲し、`owner_free` は lower packet owner free に委譲して lower encoded finish kind を `GuiRgba8888CompositorTileRleEncodedFinishErrorKind::EncodedFinishFailed` に包む。finish_payload / finish_entry は F5mp に置かず、payload recovery が必要な caller は明示的に F5mo encoded owner helper へ戻す。F5mp は `gui_rgba8888_row_tile_rle_encoded_tile_descriptor_checked` / `gui_rgba8888_row_tile_rle_encoded_tile_plan_metadata_checked` を直接呼ばず、F5cm lower packet prepare に委譲する。F5mp は packet record reader、row byte storage accessor、raw `RegionToken` / `MemPtr`、raw load/store、std present、host import、host present、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

F5mq は F5mp の `GuiRgba8888CompositorTileRlePacketOwner` と caller supplied `SurfaceId` を、lower `gui_rgba8888_row_tile_rle_present_frame_prepare` へ 1 回だけ渡す compositor tile RLE present-frame bridge である。これは compositor metadata 付き packet owner を std present-frame owner へ昇格する phase であり、packet record reader、run cursor、host import、platform backend、fallback / silent no-op を作る phase ではない。F5mq は no cursor / record / host / platform boundary で止まる。

F5mq の success owner は `GuiRgba8888CompositorTileRlePresentFrameOwner` であり、lower `GuiRgba8888RowTileRlePresentFrameOwner` と copied compositor metadata を保持する。success owner / prepare error は Clone / Copy を実装しない。`GuiRgba8888CompositorTileRlePresentFrameDescriptor` は lower `GuiRgba8888RowTileRlePresentDescriptor` と copied metadata を束ねる Copy descriptor wrapper である。prepare error kind は `FrameIdInvalid`、`FrameIdMismatch`、`PresentFramePrepareFailed lower_kind` を持つ。

F5mq は lower packet owner を取り出す前に metadata と packet descriptor を Copy value として読み、metadata frame id と packet descriptor frame id を照合する。不一致は `FrameIdMismatch` として owner-bearing error にし、frame id が `frame_id_result` で `FrameId` を作れない場合は `FrameIdInvalid` として original compositor packet owner を保持する。surface id は compositor metadata にないため、caller supplied `SurfaceId` をそのまま F5cn へ渡し、surface raw validation と geometry validation は lower F5cn に委譲する。

lower F5cn present prepare error では lower kind / category を読んでから lower packet owner を取り出し、metadata 付き `GuiRgba8888CompositorTileRlePacketOwner` へ戻す。F5mq error は lower present prepare error を公開 recovery payload にせず、retry / finish packet / free 可能な packet owner recovery に正規化する。success `owner_finish_packet` は lower present frame owner から lower packet owner を取り出し、metadata 付き compositor packet owner へ戻す recovery / abort helper である。`owner_free` は lower present frame owner free に委譲し、lower encoded finish kind を `GuiRgba8888CompositorTileRleEncodedFinishErrorKind::EncodedFinishFailed` に包む。F5mq は `tile_present_run_cursor`、`tile_present_command_cursor`、packet record reader、`gui_rgba8888_row_tile_rle_packet_record_at`、surface id 再構築、raw `RegionToken` / `MemPtr`、raw load/store、finish_encoded / finish_payload / finish_entry、host import、host present、scheduler、video memory、Canvas / DOM / minifb、platform API、fallback / silent no-op へ進まない。

F5mq は後続 bridge が lower present owner を一度取り出した後に metadata 付き owner へ戻すため、狭い public rewrap helper `gui_rgba8888_compositor_tile_rle_present_frame_rewrap` を持つ。rewrap は lower present descriptor frame id と compositor metadata frame id を照合し、不一致は `GuiRgba8888CompositorTileRlePresentFrameRewrapErrorKind::FrameIdMismatch` として owner-bearing error にする。rewrap error も `GuiRgba8888CompositorTileRlePresentFrameOwner` を保持し、caller は finish packet / free を選べる。

F5mr は compositor tile RLE present run cursor bridge であり、F5mq の `GuiRgba8888CompositorTileRlePresentFrameOwner` を lower F5co `gui_rgba8888_row_tile_rle_present_run_cursor_start` へ 1 回だけ渡す。`GuiRgba8888CompositorTileRlePresentRunCursorOwner` は lower `GuiRgba8888RowTileRlePresentRunCursorOwner` と copied `GuiRgba8888CompositorFrameEntryMetadata` を保持し、success owner / start error は Clone / Copy を実装しない。start は metadata-before-owner-move を守り、lower start failure では lower kind / category を読んでから lower present owner を F5mq rewrap helper で metadata 付き present-frame owner へ戻す。rewrap failure は `PresentFrameRewrapFailed` として start error に保持し、direct `GuiRgba8888CompositorTileRlePresentFrameOwner` constructor は使わない。

F5mr の owner は metadata、next record index、total run count を読めるが、run step は実行しない。`owner_finish_present_frame` は lower run cursor owner から lower present owner を取り出し、F5mq rewrap helper の `Result` で metadata 付き present-frame owner へ戻す。`owner_free` は lower run cursor owner free に委譲し、lower encoded finish kind を compositor encoded finish kind に包む。F5mr は `tile_present_command_cursor`、`tile_present_host_command`、`tile_present_run_span`、run cursor step、packet record reader、host import、dispatch、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

F5ms は compositor tile RLE present run step bridge であり、F5mr の `GuiRgba8888CompositorTileRlePresentRunCursorOwner` を lower F5co `gui_rgba8888_row_tile_rle_present_run_cursor_step` へ 1 回だけ渡す。`GuiRgba8888CompositorTileRlePresentRunStep` は lower `GuiRgba8888RowTileRlePresentRunCursorStepResult` と metadata 付き next `GuiRgba8888CompositorTileRlePresentRunCursorOwner` を保持し、success step / step error は Clone / Copy を実装しない。step は metadata-before-owner-move を守り、success では lower result を Copy value として読んでから lower next owner を回収し、metadata 付き compositor run cursor owner へ戻す。

F5ms の lower step error は lower kind / category を読んでから lower run cursor owner を回収し、metadata 付き `GuiRgba8888CompositorTileRlePresentRunCursorOwner` を保持する owner-bearing error に正規化する。lower step error を公開 recovery payload にせず、caller は retry / finish present-frame / free を F5ms accessor から選ぶ。`step_finish_present_frame` と `step_error_finish_present_frame` は F5mr `owner_finish_present_frame` の `Result` を潰さず返し、`step_free` と `step_error_free` は F5mr `owner_free` に委譲する。F5ms は `tile_present_command_cursor`、`tile_present_host_command`、`tile_present_run_span`、packet record reader、host import、dispatch、scheduler、platform API、video memory、Canvas / DOM / minifb、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない。

F5mt は compositor tile RLE present command cursor であり、F5mq の present-frame owner から metadata 付き command stream へ昇格する。start は `GuiRgba8888CompositorTileRlePresentFrameDescriptor` を Copy value として読んでから F5mr `gui_rgba8888_compositor_tile_rle_present_run_cursor_start` を 1 回だけ呼び、`GuiRgba8888CompositorTileRlePresentCommandCursorOwner` に F5mr run cursor owner、descriptor、phase を保持する。command は `GuiRgba8888CompositorTileRlePresentCommand::BeginFrame`、`Run`、`GuiRgba8888CompositorTileRlePresentCommand::EndFrame` である。owner / step / error は owner-bearing なので Clone / Copy を実装しない。

F5mt step は one typed output per public step を守る。BeginPending は F5ms を呼ばず BeginFrame を返し、RunPending だけが F5ms `gui_rgba8888_compositor_tile_rle_present_run_step_one` を 1 回だけ呼ぶ。F5ms success の `RunReady` は Run command と RunPending continuation を返し、F5ms success の `Completed` は同じ public step で EndFrame を返して next owner を Completed phase にする。Completed phase は terminal Completed を返す。F5mr start error と F5ms step error は kind / category を読んでから owner を回収し、present-frame owner または command cursor owner を失わない。finish present-frame / free は F5mr owner helper に委譲し、F5mq rewrap `Result` を潰さない。F5mt は lower `tile_present_command_cursor`、`tile_present_host_command`、host import、packet record reader、raw storage、platform API、video memory、Canvas / DOM / minifb、finish_encoded / finish_payload / finish_entry、fallback / silent no-op へ進まない。

F5mu は compositor tile RLE present host-command record であり、F5mt の command cursor step を metadata 付き host-command record へ写す pure projection である。`GuiRgba8888CompositorTileRlePresentHostCommandRecord` は `BeginFrame`、`RunRecord`、`EndFrame` を持ち、`GuiRgba8888CompositorTileRlePresentHostCommandStepResult` は `Record` と terminal `Completed` を分ける。`GuiRgba8888CompositorTileRlePresentHostCommandRunRecord` は `GuiRgba8888CompositorTileRlePresentFrameDescriptor` と `GuiRgba8888RowTileRleRun` の両方を保持する。record shape は does not flatten to kind plus optional run であり、Run-without-run や run payload 付き EndFrame を表現できない形にする。

F5mu mapping は `&GuiRgba8888CompositorTileRlePresentCommandCursorStep` を受け、F5mt の public `gui_rgba8888_compositor_tile_rle_present_command_cursor_step_descriptor` と `gui_rgba8888_compositor_tile_rle_present_command_cursor_step_result` accessor だけを authority とする。step continuation owner を消費せず、advance / finish / free を行わない。BeginFrame / EndFrame では step descriptor accessor の `GuiRgba8888CompositorTileRlePresentFrameDescriptor` を record に保持し、Run では同じ descriptor と run を RunRecord に束ねる。F5mu は does not bypass F5mt。lower `tile_present_host_command` / `tile_present_command_cursor` / F5co run cursor、packet record reader、raw storage、host import、dispatch、scheduler、virtual drain、run-span、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

F5mv は compositor tile RLE present virtual drain であり、F5mu compositor host-command records only を入力にする target-free / headless test boundary である。`GuiRgba8888CompositorTileRlePresentVirtualDrain` は Begin / Run / End phase、optional surface / frame、optional `GuiRgba8888CompositorFrameEntryMetadata`、expected / seen run count、expected / seen pixel count を保持する。metadata is part of the drain authority であり、BeginFrame で固定した compositor metadata と RunRecord / EndFrame descriptor metadata の full equality を検査する。descriptor frame と metadata frame id の内部不一致も typed error として扱う。
...
F5nb は std layer compositor tile RLE present host executor validation boundary であり、actual Web / native / bare executor の手前で F5mz action と F5na report の対応を検査する。`GuiRgba8888CompositorTileRlePresentHostExecutorSupport` は Window / Offscreen / Device と非空の組み合わせだけを表す。`validate_report_for_action` は support check、report action read、full action equality の順序で進み、unsupported target を `UnsupportedAction`、wrong report action を `ReportActionMismatch` として返す。

F5nb の full action identity は variant だけでなく、Window action の `window_id_raw`、descriptor の `surface_id_raw` / `frame_id_raw` / expected run count / expected pixel count、packet descriptor の frame / batch / tile / row / width / height / stride / tile rows / pixel count / total run count / encoded byte count、compositor metadata の frame id / width / height / row start / row count / batch count / max rows per batch、Run の pixel offset / pixel count / RGBA channel を比較する。matching action の failed report は valid であり、executor outcome の解釈は F5na `report_outcome` と後続 completion boundary に残す。F5nb is not actual execution and does not execute host imports。F5my / F5mw / F5mv、F5mx request construction、lower row-tile host / dispatch / schedule / virtual paths、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

F5nc は std layer compositor tile RLE present dispatch loop outcome boundary であり、F5my の `RequestReady request plus post phase` を platform executor の host outcome と接続する one-shot pending value に包む。`GuiRgba8888CompositorTileRlePresentDispatchLoopPendingRequest` は previous loop state、next loop state、`GuiRgba8888CompositorTileRlePresentHostImportRequest`、post phase を保持し、Clone / Copy を持たない。`complete_request consumes pending` ため、同じ executor outcome を二重に state へ反映できない。host outcome が Err の場合は previous state を持つ `HostImportExecutionFailed` error を返し、Ok の場合だけ post phase に従って Continue / Yield / Completed の completion に next state を入れる。F5nc は F5my dispatch step only を authority とし、F5mw schedule、F5mx request construction、F5mv virtual drain を直接呼ばない。lower row-tile path、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

F5nd は std layer compositor tile RLE present host report loop bridge boundary であり、F5nc pending request、F5mz action decoding、F5na report_outcome、F5nb validate_report_for_action を validation before completion の順序で接続する。`GuiRgba8888CompositorTileRlePresentHostReportLoopBridgeError` は `ExecutorValidationFailed` または `LoopCompletionFailed` の lower typed error、category、loop state を保持する。validation failure では F5nc `complete_request` を呼ばず、pending previous state を返す。validation success の場合だけ F5na `report_outcome` を F5nc `complete_request` に渡して pending を消費する。matching action の failed report は F5nc の `HostImportExecutionFailed` へ進み、unsupported support や wrong action / metadata report は completion 前に `ExecutorValidationFailed` で止まる。F5nd は F5my / F5mw / F5mx / F5mv direct call、lower row-tile host / dispatch / schedule / virtual path、raw storage、Vec、queue、timer、scheduler、platform API、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

F5ne は std layer compositor tile RLE present host execution driver boundary であり、F5nc の one-shot pending request を actual Web / native / bare / headless executor が読む action と completion 用 pending value の組に束ねる。`GuiRgba8888CompositorTileRlePresentHostExecutionDriverPending` は `GuiRgba8888CompositorTileRlePresentDispatchLoopPendingRequest` と `GuiRgba8888CompositorTileRlePresentHostExecutionAction` を保持し、pending を所有するため Clone / Copy を持たない。`prepare` は pending request accessor から request を読み、F5mz action を 1 回だけ導出して original pending value と一緒に保持する。executor は `pending_action` で action を読み、実行結果は `Result unit GuiError` として `complete_outcome` に返す。`complete_outcome` は stored action と outcome から F5na report を作り、F5nd bridge に validation before completion と loop completion を委譲する。F5ne は F5nc `complete_request`、F5nb `validate_report_for_action`、F5mx request construction を直接呼ばず、actual platform API、DOM / Canvas / minifb、video memory、queue、timer、scheduler、F5my / F5mw / F5mv、lower row-tile path、raw storage、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_sink` は std layer compositor tile RLE present host action sink boundary であり、actual Web / native / bare / headless executor が返した executor-supplied outcome を `GuiRgba8888CompositorTileRlePresentHostActionSinkStep` に包む。F5nf sink は F5nb `require_supported` を step construction より前に呼び、unsupported target は `UnsupportedAction` として返す。F5nf sink does not manufacture success。`Result::Ok unit` や fallback success は作らず、caller から渡された `Result unit GuiError` を保持するだけである。この boundary は actual platform execution、F5ne driver pending ownership、F5ne completion、F5na report construction、F5nd bridge、F5mx request construction、queue、timer、scheduler、raw storage、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_sink_driver` は std layer compositor tile RLE present host action sink driver boundary であり、F5nf sink の support preflight / executor-supplied outcome packaging と F5ne の one-shot driver completion を接続する。`GuiRgba8888CompositorTileRlePresentHostActionSinkDriverStep` は F5nf sink step と dispatch loop completion を保持する。F5nf driver はまず F5ne driver pending から action を借用で読み、F5nf sink `gui_rgba8888_compositor_tile_rle_present_host_action_sink_step` に caller-supplied outcome を渡す。F5nf sink が `SinkRejected` になる場合は F5ne completion を呼ばず、original driver pending を owner-bearing error に入れて返す。F5nf sink が成功した場合だけ、同じ outcome で F5ne `complete_outcome` を呼ぶ。F5nf driver does not manufacture executor outcome。`Result::Ok unit` や synthetic `Result::Err` を作らず、actual executor が返した `Result unit GuiError` だけを渡す。この boundary は F5nc direct completion、F5nd bridge direct call、F5nb validation direct call、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、queue、timer、scheduler、raw storage、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_attempt_driver` は std layer compositor tile RLE present host action attempt driver boundary であり、actual executor の action attempt と F5ne driver pending の action identity を F5nf sink driver の前で固定する。`GuiRgba8888CompositorTileRlePresentHostActionAttempt` は attempted action と executor-supplied outcome を保持する。F5ng は driver pending から expected action を借用で読み、attempted action と F5nb full action equality で比較する。mismatch では F5nf sink driver を呼ばず、`AttemptActionMismatch` owner-bearing error に expected action、attempted action、`GuiError::InvalidCommand` category、original driver pending を保持する。match した場合だけ、attempt outcome を F5nf action sink driver に委譲する。F5ng does not manufacture executor outcome。`Result::Ok unit` や synthetic `Result::Err` を作らず、attempt が持つ `Result unit GuiError` だけを渡す。この boundary は F5nf sink direct call、F5ne direct completion、F5nd bridge direct call、F5nb validation direct call、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、queue、timer、scheduler、raw storage、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_executor_session` は std layer compositor tile RLE present host action executor session boundary であり、F5ne one-shot driver pending を actual Web / native / bare / headless executor が扱う request / complete contract に包む。`GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionState` は `Ready driver` または `Completed` である。`Completed` への request は terminal `Completed` を返し lower call を行わない。`Ready` への request は F5ne pending action を借用で読み、`GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionPending` に driver owner と borrowed-derived expected action value を保持する。F5nh の complete は outcome-only complete であり、executor から action identity を受け取らず、session pending の expected action と caller supplied outcome から F5ng attempt を作る。F5ng `attempt_driver_step` を 1 回だけ呼び、success では F5ng completion accessor から `GuiRgba8888CompositorTileRlePresentDispatchLoopCompletion` を返す。failure では F5ng error category accessor と lower F5ng error を `GuiRgba8888CompositorTileRlePresentHostActionExecutorSessionCompleteError` に保持する。F5nh does not manufacture executor outcome。`Result::Ok unit` や synthetic `Result::Err` を作らず、caller supplied `Result unit GuiError` だけを attempt に包む。この boundary は F5nf sink / sink driver direct call、F5ne direct completion、F5nd bridge direct call、F5nb validation direct call、F5na report construction、F5mx request construction、F5my / F5mw / F5mv、lower row-tile path、queue、timer、scheduler、raw storage、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`std/gui/compositor_tile_present_host_action_platform_bridge` は F5ni の std layer compositor tile RLE present host action platform bridge boundary であり、F5nh session pending から borrowed pending accessor で expected action を読み、target / record kind / descriptor を platform executor 用に投影する。completion は outcome-only complete を保ち、platform executor が返した `Result unit GuiError` を F5nh complete へ戻すだけである。この helper は action identity、report、attempt、raw host status、platform host import を作らず、does not manufacture platform outcome。F5ng / F5nf / F5ne lower completion、F5nd bridge、F5nb validation、F5na report construction、F5mx request construction、scheduler、queue、timer、DOM / Canvas / minifb、video memory、fallback、silent no-op を提供しない。

`platforms/gui/web/compositor_host_executor` は F5ni の Web compositor host action executor backend bridge であり、F5ni std bridge から得た expected action を `nepl_gui_web.compositor_tile_present_begin/run/end` host import ABI の begin / run / end いずれか 1 回に渡す。raw status `0` は `Result::Ok unit`、負 status は `GuiError::Unsupported` などの typed error に写し、最後に F5ni bridge / F5nh outcome-only complete へ戻す。`nodesrc/run_test.js` の default Web import は stdlib doctest 用の fail-closed boundary として `GuiError::Unsupported` 相当を返す。これは no video memory fallback の境界であり、legacy stdout transport、Canvas / DOM 直接描画、queue、timer、scheduler、present loop、silent no-op は行わない。

F5nj の Web compositor host import actual implementation は、Web worker 内で full-frame video memory surface backing を使い、`begin` が `GuiVideoMemorySurface` から write slot を取得して caller supplied frame id に紐付け、`run` が tile-local contiguous RLE pixel offset を RGBA8888 slot bytes へ書き、全 batch / tile の `end` が完了したときだけ publish して既存 `gui_video_memory_present` ack path へ渡す。target は Window target only であり、Offscreen / Device は `GuiError::Unsupported` 相当へ fail-closed に写す。compositor ABI に title が無いため、Web worker は deterministic title policy として surface handle 由来の固定 title を生成し、空文字 fallback や DOM / Canvas 直接描画には進まない。

F5nk の partial dirty slot preservation/copy は、Web surface record が committed snapshot を正式な baseline authority として保持する。full publish は write slot 全体を committed snapshot へ copy し、partial begin は committed snapshot が存在する場合だけ新しい write slot へ copy してから dirty row range の RLE run を書く。baseline が無い partial begin と raw rect publish は `InvalidCommand` 相当へ fail-closed にし、published slot を presenter API で読み返して baseline にしない。partial end は metadata row range を absolute row range として検証し、全 batch / tile の完了後だけ dirty rect publish を行う。snapshot 候補は publish 前に作り、publish 成功後だけ committed snapshot と frame state を確定するため、allocation failure や publish failure は retry 可能な active frame を残す。
...
### F5nyk registered stroke compositor RLE encode-cursor bridge

F5nykは専用extension `stroke_compositor_tile_rle_encode_cursor.nepl` に置き、public F5nyjを1回呼び、そのseed success ownerだけを既存F5mj encode-cursor startへ1回渡す。五層nested `Result`はstart、step、completed、seed、cursor errorを加工せず保持する。有効seedから自然到達不能なcursor errorを偽造せず、successはmetadata、run count、ready cursor範囲を保つ。F5nykはF5mk writer plan、storage、packet、presentへ進まない。
...
### F5nza registered stroke compositor RLE EndFrame step bridge

F5nzaはpublic F5nyzを1回だけ呼び、Run step successからcontinuation ownerを既存F5mt finish-ownerで1回だけ回収し、そのownerを既存F5mt stepへ1回だけ渡す。既存二十層とthird command stepの第二十一層をlossless nested `Result`として保持し、productionはRun result、EndFrame result、descriptor、phaseを解釈しない。production-derived fixtureはEndFrame payloadとcanonical descriptor双方のmetadata 263/16/3/1、surface 7、frame 263、expected run count 1、pixel count 16をevidence 63、next `Completed` phaseを64として検証し、owner free後の合計を127とする。production graphはfixture-freeで、次のF5nzbによるterminal `Completed` step、F5mu record、host executionより前で停止する。
