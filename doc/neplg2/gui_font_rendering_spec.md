...
F5nxkの先行authority adapterはF5nxj completed owner全体を保持し、owner-bound command readとindexed contour span lookupだけを借用転送する。commandが保持するcontour / global edge / local edgeとspan identityを照合し、raw collection、nested source、writerを公開しない。この段階はchecked source accessの接続であり、独立したchecked-span curve再照合、metric join、stroke provenance drain completionは後続F5nxkで行う。
