...
F5fx では Bare display operation-to-driver-adapter bridge として、`platforms/gui/bare/display_operation_driver_bridge` を追加する。これは `GuiBareDisplayMemoryOwner` と `GuiRgba8888RowTileRlePresentHostSpanOperation` だけを public input authority とし、owner 内 canonical driver state から framebuffer、storage、memory、actual display driver adapter の順に 1 step 進める。public `GuiBareFramebufferStepApplied`、`GuiBareDisplayStorageStepApplied`、`GuiBareDisplayMemoryStepApplied`、driver outcome、raw storage、`RegionToken`、`MemPtr` は受け取らない。

F5fx の validation failure は host import へ進まず、original owner と operation と lower error を回収する。adapter failure は adapter error が保持する recovered owner に従い、operation と lower adapter error を保持する。adapter / host が external side effect を accepted した後の failure では NEPL 側 owner recovery を返すが、host side effect rollback は主張しない。F5fx は long-running scheduler backend、timer queue、present loop、DOM、Canvas、minifb、video memory transport、hardware flush、fallback、silent no-op には進まない。

F5fy では Bare display presenter input boundary として、`platforms/gui/bare/display_presenter_input` を追加する。これは F5fx bridge の actual display driver adapter result を scheduler executor input path へ戻す境界であり、bare platform module に置く。`GuiBareDisplayMemoryOwner` は platform authority なので、この境界を `std/gui` には置かない。

F5fy の public input は `GuiBareDisplayMemoryOwner` と typed `ExecuteHostAction` だけである。action を消費する前に borrowed accessor で pending operation を読み、その operation と owner を `gui_bare_display_operation_driver_bridge_step` へ 1 回だけ渡す。success では `Result::Ok unit` を `gui_bare_scheduler_executor_input` へ渡し、bridge completed value と scheduler ready を同じ ready value に保持する。

category がある bridge failure では `Result::Err GuiError` を `gui_bare_scheduler_executor_input` へ渡し、lower bridge error と scheduler ready を同じ error value に保持する。category が無い bridge failure では scheduler ready を作らず、original `ExecuteHostAction` と lower bridge error を保持する。これは action owner と recovered display memory owner を失わないためであり、fallback error への丸めや silent no-op ではない。F5fy は direct host import、long-running scheduler backend、timer queue、present loop、DOM、Canvas、minifb、video memory transport、hardware flush、host side effect rollback claim を扱わない。
