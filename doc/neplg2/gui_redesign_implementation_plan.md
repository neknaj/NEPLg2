...
- `request_timer` は Worker から main thread へ typed `gui_timer_request` message を送り、ack `SharedArrayBuffer` に書かれた actual scheduler status を待ってから戻る。未提示 window は invalid、`interval_ms == 0` は clear、`repeating == 0` はこの checkpoint では invalid とする。
