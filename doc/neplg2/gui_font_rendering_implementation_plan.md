...
## Phase F5ks: sfnt simple glyph render stroke source segment cursor boundary

目的:

- F5kr の stroke segment plan owner を authority として、completed path sink scalar stream から source segment を順に読む cursor boundary を追加する。
- MoveTo は current point 更新だけにし、LineTo / QuadraticTo だけを source geometry value として emit する。
- actual stroke offset geometry、join / cap / dash / miter、stroke edge owner、coverage mask、packed mask、render command、pixel write、platform API へ進まない。
...
- `GuiSfntSimpleGlyphRenderStrokeSourceSegmentLine` と `GuiSfntSimpleGlyphRenderStrokeSourceSegmentQuadratic` を追加する。segment index、doubled start/control/end coordinates、stroke width を保持する value-only 型にする。
- `GuiSfntSimpleGlyphRenderStrokeSourceSegmentCursor` を追加する。plan owner、scalar index、emitted segment count、MoveTo / Line / Quadratic read count、current point state を保持し、`Clone` / `Copy` は実装しない。
- `GuiSfntSimpleGlyphRenderStrokeSourceSegmentCursorErrorKind`、start error、step error、terminal を追加する。start error は plan owner、step error は cursor を recovery payload とする。
- plan invariant revalidation は F5kr と同等に、capacity derivation、stored capacity、path sink scalar len/cap、raster mask len 0、writer counts、stroke width、checked draw count、plan draw count / derived raster edge capacity、stored owner counts を検査する。
- scalar read helper は index range、storage len/cap、`vec::get` missing を typed error に分ける。
- step は MoveTo / LineTo / QuadraticTo の record width を payload read 前に検査し、unknown tag と SkipNoSegment tag を fail closed にする。
- focused doctest label と source policy を追加し、path sink scalar read 以外の geometry expansion / fill alpha mask / raster edge owner / coverage / RenderCommand / DrawTarget / RenderTarget / platform / fallback / compositor 禁止、owner 型の非 Clone / 非 Copy、括弧なし prefix style を検査する。

完了条件:

- source policy が docs、Franklin plan approval、MoveTo state update、Line/Quadratic source segment emission、SkipNoSegment no-scalar rule、path sink scalar stream authority、completion count validation、owner recovery/free、focused doctest coverage label を検査する。
- `tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_stroke_source_segment_cursor.n.md` に plan authority、move state、line/quadratic geometry、missing current point、skip scalar tag、truncated record、completion counts、owner recovery、no offset / edge / mask / command / platform policy の coverage label を追加する。
- implementation review で F5ks が offset geometry / edge / coverage / command / platform へ進んでいないこと、F5kr plan owner を authority として保っていることを確認する。
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は stroke source segment cursor boundary 接続済み、後続の stroke offset geometry / stroke edge / stroke coverage、shadow rasterization、2D compositor drain を残件として更新する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_stroke_source_segment_cursor.n.md --no-tree -o tmp_gui_font_render_stroke_source_segment_cursor_f5ks.json -j 1
git diff --check
```
...
- bevel join は F5kz の `from_end -> to_start` connector chord を保持し、miter join は隣接 line side edge の直線交点から `from_end -> miter -> to_start` を保持し、round join は directed source center を検査して `from_end -> arc_mid -> to_start` の 2 chord record を保持する。
...
- quadratic side edge を含む miter / round join は line-only geometry を捏造せず、`quadratic_bevel` 付き typed bevel record として F5kz connector chord を保持する。
- coverage mask、packed mask、render command、pixel write、platform API、font fallback、shadow/compositor へ進まない。
...
- miter は有限 line intersection と miter limit evidence を必須にし、round は source vertex を中心にした two-chord policy を必須にする。
- quadratic side edge scan policy は F5le で F5lb config-driven flattening として接続する。

変更:

- `GuiSfntSimpleGlyphRenderStrokeJoinGeometryRecord`、bevel / miter / round geometry record、drain owner、completed owner、start / drain error、bounded terminal を追加する。
- start は F5kz closure invariant と join Vec capacity を再検査する。quadratic side edge count だけでは拒否しない。
- join drain は F5kz join closure record を 1 件ずつ読み、bevel / miter / round の geometry record を Vec に push する。
- F5la writer owner は completed F5lc join geometry owner を direct authority として保持するように変更する。
- F5lb scan converter は F5lc join geometry Vec を読み、bevel は 1 線分、miter / round は 2 線分の交差数として扱う。
- docs / source policy / focused doctest label / todo / note を F5lc に合わせて更新する。

完了条件:

- source policy が docs、Lovelace review result、F5kz authority、line side edge revalidation、bevel chord preservation、miter intersection / miter-limit clipping、round two-chord policy、quadratic bevel evidence、push/budget/completion、owner-bearing type no Clone/Copy、no packed/render/platform/fallback 接続を検査する。
- focused doctest label が F5kz authority、line side edge revalidation、bevel chord、miter intersection / limit、round two-chord policy、quadratic bevel connector、push/budget/completion、no packed/render/platform policy を固定する。
...
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は F5le 後の現在、quadratic side edge scan policy 接続済み、後続の packed stroke mask owner、glyph paint composition order、shadow rasterization、2D compositor drain を残件として更新する。
...
- start で F5lc completed owner の nested F5kz invariant、join geometry Vec len/cap、bevel / miter / round count 合計を再検査する。
...
- start で shape、writer progress/storage、`quadratic_side_edge_segment_count > 0`、F5lc join geometry invariant を再検査する。
- line side edge、明示 config で分割する quadratic side edge、F5lc bevel / miter / round join geometry の coverage scan に限定する。
- quadratic side edge は endpoint chord として扱わず、source quadratic point と endpoint normal offset の線形補間による deterministic flattening として扱う。
...
- F5lb/F5le は stroke coverage scan converter として進める。ただし F5la writer を消費する boundary に限定する。
- side edge + F5lc join geometry は explicit geometry authority であり、miter を bevel chord として代用してはいけない。
- quadratic side edge を endpoint chord として scan すると quadratic stroke support を偽るため、F5le では分割数 config、source/control/end 使用、endpoint normal interpolation、right-side reverse order を source policy で固定する。

変更:

- `GuiSfntSimpleGlyphRenderStrokeCoverageScanConfig`、`GuiSfntSimpleGlyphRenderStrokeCoverageScanOwner`、start / step error、bounded drain terminal を追加する。
- start は F5la writer shape/storage/progress、quadratic side edge segment count、F5lc join geometry invariant を再検査する。
- read helper は nested completed owner の side edge Vec と join Vec を bounds / len / cap / slot 付きで読む。
- line side edge、config-driven quadratic side edge、F5lc bevel / miter / round join geometry の crossing を f32 scaled coordinate で計算し、finite でない座標は typed error にする。
...
- source policy が docs、Archimedes/Hubble review result、F5la writer authority、F5lc join geometry revalidation、cell bounds、line side edge scan、quadratic side edge explicit policy、bevel/miter/round join geometry scan、push recovery、budget/completion、owner-bearing type no Clone/Copy、no F5be direct reuse / no earlier drain restart / no packed/render/platform/fallback 接続を検査する。
- focused doctest label が F5la authority、join geometry revalidation、cell bounds、line side edge scan、bevel/miter/round join geometry、quadratic explicit policy、push/budget/completion、no fill-scan/packed/render/platform policy を固定する。
- implementation review で F5lb が stroke coverage scan converter に留まり、quadratic support を偽装せず、packed/render/platform へ進んでいないことを確認する。
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は F5le quadratic side-edge scan policy 接続済み、後続の packed stroke mask owner、glyph paint composition order、shadow rasterization、2D compositor drain を残件として更新する。
...
## Phase F5lg: sfnt simple glyph render glyph paint composition order

目的:

- F5bh/F5bg の completed fill alpha mask owner と F5lf の completed stroke packed mask owner を direct authority とし、fill-before-stroke の glyph paint composition order owner を作る。
- stroke packed mask の nested chain から F5kq/F5kr の origin / fill / stroke / blend metadata を読み、fill owner の origin / fill paint / blend と一致することを検査する。
- stroke-only paint はこの boundary では扱わず、`MissingStrokeFillMetadata` として typed error にする。stroke-only composition は後続 sibling boundary に残す。
...
- Euler plan review 1 は `CHANGES_REQUESTED`。
- shape equality だけでは壊れた completed owner を隠すため、fill / stroke の shape と alpha storage を個別に再検査してから tuple 比較する必要があると指摘された。
- stroke-only をこの boundary に含めると fill owner がない状態を偽装するため、F5lg は fill+stroke 専用とし、stroke-only は後続 sibling boundary に残す。
- nested stroke metadata extraction chain、lower join geometry invariant error evidence、module-private non-Clone/non-Copy owner/error、split raw owner accessor 禁止、fill-before-stroke free order を明文化する必要があると指摘された。
- Euler revised plan review は `PLAN_APPROVED`。

変更:

- `GuiSfntSimpleGlyphRenderGlyphPaintCompositionOrderOwner` を追加する。fill owner、stroke owner、origin、fill paint、stroke、blend、`fill_order = 0`、`stroke_order = 1` を保持する。
- start error と recovery payload を owner-bearing にし、error kind と optional lower join error だけを参照 accessor として出す。owner recovery は両 owner をまとめて返す consuming path にする。
- fill owner invariant は shape positive / coverage max / cell count / alpha max / cell count / alpha len / alpha cap を検査する。
- stroke owner invariant は同じ alpha mask invariant を検査し、加えて `gui_sfnt_simple_glyph_render_stroke_join_geometry_owner_invariants_for_stroke_coverage` の lower error を保持する。
- shape tuple は `origin_x2`、`origin_y2`、`width_px`、`height_px`、`sample_scale`、`coverage_max`、`cell_count` の field-specific mismatch として検査する。
- stroke metadata は `stroke_packed.join_geometry_owner.edge_closure_owner.side_edge_owner.geometry_owner.source_owner.metric_owner.plan_owner` から読み、origin / fill / stroke / blend を比較する。
- stroke metadata の fill は origin / blend mismatch より先に読み、`None` なら `MissingStrokeFillMetadata` として返す。
- fill paint comparison は typed `GuiPaint` / `Rgba8888` accessor で行う。
- owner / error / recovery free path は fill owner を先に解放し、その後 stroke owner を解放する。
- docs / source policy / focused doctest label / todo / note を F5lg に合わせて更新する。

完了条件:

- source policy が docs、Euler revised plan review result、fill+stroke only scope、stroke-only sibling 残件、nested chain、per-owner invariant before tuple comparison、lower join geometry evidence、private non-Clone/non-Copy owner/error/recovery、single combined recovery、no split raw owner accessor、SourceOver-only、typed paint comparison、fill_order/stroke_order、free order、forbidden command/resource/pixel/platform/fallback/shadow/compositor、F5bf internals direct reuse 禁止、focused doctest coverage label を検査する。
- `tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_glyph_paint_composition_order.n.md` に fill+stroke scope、owner invariant、nested stroke metadata、shape tuple match、SourceOver/fill paint match、combined recovery/free order、no render/resource/platform/shadow/compositor policy の coverage label を追加する。
- implementation review で owner recovery が split raw accessor になっていないこと、stroke-only を受理していないこと、shape validation が tuple comparison より前にあること、nested metadata が plan owner 由来であることを確認する。
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は F5lg 後の stroke-only composition order、shadow rasterization、2D compositor drain を残件として更新する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_glyph_paint_composition_order.n.md --no-tree -o tmp_gui_font_render_glyph_paint_composition_order_f5lg.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i stdlib/alloc/gui/font/sfnt/glyf.nepl --no-tree -o tmp_gui_font_glyf_f5lg.json -j 1
git diff --check
trunk build
...
## Phase F5ln: sfnt simple glyph render shadow source blur mask owner

目的:

- F5lm completed shadow source raw coverage mask owner を direct authority とし、raw source coverage に spread / blur を適用して shadow coverage mask owner を作る。
- F5be generic packed mask、stroke coverage / packed path、render command、resource、pixel write、platform API、font fallback、2D compositor へ進まない。
- shadow paint / blend は completed blur owner にコピーせず、後続の packing / composition が nested F5lk context から読む。
- completion 時に raw source coverage cells を解放し、completed blur owner には blurred shadow cells と F5lk edge authority だけを残す。

plan review:

- Harvey plan review は `PLAN_APPROVED`。
- Blur は packing 前に raw coverage 上で行うのが正しいと確認された。これにより spread / average が coverage semantics を保ったまま、後続 alpha normalization へ渡せる。
- direct authority は F5lm completed coverage mask owner、cached source shape と derived shadow shape の再検査、shadow paint / blend をコピーしない設計、push failure で source owner / output Vec / unchanged progress を回収する設計が承認された。
- source policy は F5lm region end を F5ln marker へ切り替え、F5ln region end を F5kr marker にし、F5ln 自身の blur helper を禁止せず packed/render/platform/compositor/fallback を禁止する。

変更:

- `GuiSfntSimpleGlyphRenderShadowSourceBlurMaskBuildOwner` を追加する。F5lm completed coverage owner、derived shadow shape、output cells、cell index を保持し、`Clone` / `Copy` は実装しない。
- `GuiSfntSimpleGlyphRenderShadowSourceBlurMaskOwner` を追加する。F5lk edge owner、source shape、shadow shape、blurred cells、shadow cell count を保持し、`Clone` / `Copy` は実装しない。
- error kind、start error、build error、bounded terminal、free/recovery helper を追加する。
- completed F5lm owner invariant として、nested F5lk edge owner invariant、cached source shape equality、source shape arithmetic、raw source cell count / Vec len/cap、shadow extent / spread / blur metadata を再検査する。
- shadow shape を `shadow_extent = spread + blur_radius` から checked arithmetic で導出する。origin は `source_origin_x2/y2 - shadow_extent * 2`、width / height は `source_width/height + shadow_extent * 2`、cell count は width * height とする。
...
- kernel axis / cell count と `blur_kernel_count * coverage_max` を checked arithmetic で検査する。
- step は 1 shadow cell を計算して output Vec へ push する。push failure では returned Vec と pre-push `cell_index` を持つ owner-bearing error を返す。
- completion は exact full のときだけ raw source cells を free し、completed blur mask owner を返す。
- docs / source policy / focused doctest label / todo / note を F5ln に合わせて更新する。

完了条件:

- source policy が docs、Harvey approval、F5lm completed owner authority、private build/completed owner / error / terminal、error kind `Clone` / `Copy`、owner-bearing no `Clone` / `Copy`、source owner invariant、shadow shape derivation、kernel bounds、outside-source zero read、spread max-filter、box blur average、push recovery、exact completion / raw source free、budget / progress invariant、generic scan / packed / stroke / fill / render / platform / compositor / fallback 禁止、focused doctest coverage label を検査する。
- `tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_blur_mask_owner.n.md` に F5lm authority、source owner invariant、shadow shape derivation、kernel bounds、spread max filter、box blur、outside-source zero、push recovery、raw source free、budget progress、no packed/render/platform policy の coverage label を追加する。
- implementation review で F5ln が F5lm raw coverage owner 以外を direct authority にしていないこと、source cells を completion 後に保持していないこと、packing / composition / platform path へ進んでいないことを確認する。
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は F5ln 後の shadow source packing / composition、2D compositor drain を残件として更新する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_blur_mask_owner.n.md --no-tree -o tmp_gui_font_render_shadow_source_blur_mask_f5ln.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_coverage_scan_converter.n.md --no-tree -o tmp_gui_font_render_shadow_source_coverage_scan_f5ln_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i stdlib/alloc/gui/font/sfnt/glyf.nepl --no-tree -o tmp_gui_font_glyf_f5ln.json -j 1
git diff --check
trunk build
...
## Phase F5ls: sfnt simple glyph render shadow source resource reservation boundary

目的:

- F5lp completed shadow source composition order owner を、`AlphaMaskId` と同じ owner-bearing reservation value に束ねる。
- F5ls は resource table 登録ではなく、`RenderCommand::AlphaMaskRect` emission でもない。未登録の `AlphaMaskId` command が owner と切り離されることを防ぐ内部 alloc/font 境界にする。
- F5lr の per-sample `FillRect` bridge とは別の high-throughput resource path とし、F5lq cursor / F5lr command cursor を authority にしない。

plan review:

- Maxwell plan review は `PLAN_APPROVED`。
- F5lp が direct authority であり、F5lr は per-sample command bridge なので resource path の authority にしない。
- success owner が F5lp owner を保持するため `AlphaMaskId` lifetime は許容できる。ただし docs/tests では id が nonzero/reserved であるだけで、registered / unique / uploaded / renderable ではないことを明記する。
- rect は `source_placement_origin - shadow_extent` の x/y overflow を `gui_rect_new` 前に検査する。width/height は shadow storage shape validation で検査し、right/bottom overflow は後続 drain/command validation の責務にする。
- F5lq storage helper を使う場合は storage-only error を F5ls error kind に exhaustively map し、F5lq start/read/step/cursor creation を source policy で禁止する。実装では resource reservation 専用 helper を追加し、sample cursor error vocabulary を露出しない。

変更:

- `GuiSfntSimpleGlyphRenderShadowSourceResourceReservationConfig` を追加する。`mask_id %AlphaMaskId` だけを持つ value-only config とし、`Clone` / `Copy` を実装する。
- private `GuiSfntSimpleGlyphRenderShadowSourceResourceReservationOwner` を追加する。F5lp composition order owner、mask id、rect、paint、shadow/source order metadata を保持し、`Clone` / `Copy` は実装しない。
- `GuiSfntSimpleGlyphRenderShadowSourceResourceReservationStartErrorKind` を追加する。invalid mask id、F5lp invariant failure、shadow shape/storage invariant、unsupported blend、rect x/y overflow を enum variant として持つ。
- start error は original F5lp owner、config、`Option order_error` を保持し、owner recovery helper と free helper を持つ。
- start は `AlphaMaskId.raw > 0`、F5lp composition invariant、shadow shape / alpha storage invariant、`GuiBlendMode::SourceOver`、rect top-left checked subtraction を fail-closed に検査してから reservation owner を作る。
- success path は F5lp owner の `source_placement_origin - shadow_extent` と shadow shape width/height から rect を作り、shadow paint と shadow/source order をそのまま保持する。alpha Vec は copy しない。
- `render_command_alpha_mask_rect`、`render_command_fill_rect`、F5lq start/read/step、F5lr sample command bridge、resource table、DrawTarget / RenderTarget、platform / host / backend API、font fallback、zero-fill fallback、shadow rasterizer、software surface、2D compositor は呼ばない。
- `tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_resource_reservation.n.md` を追加し、source policy coverage label を固定する。

完了条件:

- source policy が docs、Maxwell approval、F5lp authority、config/owner/error types、owner-bearing no `Clone` / `Copy`、config/error kind `Clone` / `Copy`、mask id validation、lower `order_error` evidence、shadow storage invariant、SourceOver-only validation、checked rect derivation、paint/order metadata、owner recovery/free、no command emission、no table/platform/target/fallback/compositor、no F5lq start/read/step、no F5lr bridge、括弧なし prefix style、focused doctest coverage label を検査する。
- focused doctest と F5lr / F5lq 回帰、`stdlib/alloc/gui/font/sfnt/glyf.nepl` doctest が通る。
- implementation review で dangling `AlphaMaskId` command を作っていないこと、F5ls docs が registered / unique / uploaded / renderable を主張していないこと、note/todo 更新が staged set に含まれていることを確認する。
- `note.n.md` に plan review、実装、検証、subagent 実装レビュー、残件を記録する。
- `todo.md` は F5ls 後の shadow source resource table / prepared command / 2D compositor drain を残件として更新する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_resource_reservation.n.md --no-tree -o tmp_gui_font_render_shadow_source_resource_reservation_f5ls.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_sample_command_bridge.n.md --no-tree -o tmp_gui_font_render_shadow_source_sample_command_bridge_f5ls_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_font_sfnt_glyf_outline_point_stream_item_collection_render_shadow_source_sample_cursor.n.md --no-tree -o tmp_gui_font_render_shadow_source_sample_cursor_f5ls_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i stdlib/alloc/gui/font/sfnt/glyf.nepl --no-tree -o tmp_gui_font_glyf_f5ls.json -j 1
git diff --check
trunk build
...
## Phase F5md: render2d compositor tile plan bridge boundary

目的:

- F5mc の `GuiRgba8888CompositorByteStorageOwner` を authority とし、既存 F5ca `gui_rgba8888_row_tile_plan_prepare` を compositor metadata 付きで接続する。
- lower `gui_rgba8888_row_tile_plan_prepare` を 1 回だけ呼び、1 batch 分の row tile plan metadata を `GuiRgba8888CompositorTilePlanOwner` として返す。
- success / prepare error / finish error recovery のどれでも compositor metadata を保持し、byte storage owner または frame entry owner へ戻せるようにする。
- この phase では tile payload、RLE、std present、host / platform API、video memory、Canvas / DOM / minifb、transport、fallback / silent no-op へ進まない。no RLE / host present の tile plan metadata bridge で止める。

plan review:

- Bohr plan review は `PLAN_APPROVED`。
- F5md は F5mc の compositor byte storage owner と F5ca の row tile plan owner をつなぐ bridge として承認された。
- prepare error は `kind/category/storage` を持ち、lower kind / category を読んだ後で lower byte storage owner を `GuiRgba8888CompositorByteStorageOwner` へ戻す。
- finish error は `kind/category/entry` を持ち、F5mc の byte storage finish error から lower finish kind / category を読んだ後で returned entry owner を保持する。
- owner_free は finish failure と entry free failure を分ける。tile plan finish が成功した後に entry free が失敗した場合を失わず、finish failure では original lower finish kind を保持する。
- source policy で lower `gui_rgba8888_row_tile_plan_prepare` exact once、direct `gui_rgba8888_row_byte_storage_validate_authority` 禁止、`gui_rgba8888_row_tile_plan_storage_ref` / byte read / raw `RegionToken` / `MemPtr` / source storage 公開禁止、tile payload / RLE / std present / host / platform / fallback 禁止を検査する。
- focused doctest は success path に加え、invalid `tile_rows` prepare failure で metadata rewrap と `storage` recovery を実行する。

変更:

- `stdlib/alloc/gui/render2d/compositor_tile_plan.nepl` を追加する。
- `GuiRgba8888CompositorTilePlanPrepareErrorKind`、`GuiRgba8888CompositorTilePlanFinishErrorKind`、`GuiRgba8888CompositorTilePlanFreeErrorKind` を追加する。
- `GuiRgba8888CompositorTilePlanOwner`、`GuiRgba8888CompositorTilePlanPrepareError`、`GuiRgba8888CompositorTilePlanFinishError` を追加する。owner-bearing success / error は Clone / Copy を実装しない。
- `gui_rgba8888_compositor_tile_plan_prepare` は metadata を byte storage owner から Copy してから lower storage owner を取り出し、lower row tile plan prepare を 1 回だけ呼ぶ。
- prepare failure は lower kind / category を読んでから `GuiRgba8888CompositorByteStorageOwner` へ正規化し、prepare error は `kind/category/storage` を保持する。
- descriptor helper は lower `gui_rgba8888_row_tile_plan_descriptor_at` に委譲し、storage-relative descriptor metadata だけを返す。
- `gui_rgba8888_compositor_tile_plan_owner_finish_byte_storage` は metadata を Copy してから lower tile plan owner を byte storage owner へ戻す。
- `gui_rgba8888_compositor_tile_plan_owner_finish_entry` は F5mc byte storage finish に委譲し、finish error は `kind/category/entry` を保持する。
- prepare error / finish error recovery accessor、free helper を追加する。
- `stdlib/alloc/gui/render2d.nepl` facade から compositor tile plan を再公開する。
- `tests/stdlib/gui_render2d_compositor_tile_plan.n.md` を追加し、facade、prepare、descriptor metadata、invalid config recovery、finish entry recovery、no payload / platform / fallback label を固定する。
- `nodesrc/test_web_gui_font_rendering_contract.js` に F5md source policy を追加する。
- `doc/neplg2/gui_font_rendering_spec.md`、`doc/neplg2/gui_font_rendering_detailed_design.md`、`doc/neplg2/gui_standard_library_spec.md`、`note.n.md`、`todo.md` を更新する。

完了条件:

- source policy が docs、Bohr approval、facade export、typed lower error variants、success owner / prepare error / finish error no Clone / Copy、exact one lower row tile plan prepare call、metadata-before-storage-owner-consume、lower kind/category before storage normalization、descriptor_at delegation、metadata-before-tile-plan-finish、finish error kind/category before entry recovery、owner_free の finish failure / entry free failure split、direct lower authority validation / raw storage / storage_ref / byte read / payload / RLE / std present / host / platform / fallback 禁止、focused doctest label を検査する。
- focused doctest、module doctest、F5mc compositor byte storage regression、F5ca row tile plan regression、source policy、`git diff --check`、`trunk build`、playground editor JSON が通る。
- implementation review で metadata-after-move、owner loss、lower finish kind/category loss、entry free failure collapse、payload / host leakage がないことを確認する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_tile_plan.n.md --no-tree -o tmp_gui_render2d_compositor_tile_plan_f5md.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='180000'; node nodesrc/tests.js -i stdlib/alloc/gui/render2d/compositor_tile_plan.nepl --no-tree -o tmp_gui_render2d_compositor_tile_plan_module_f5md.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_byte_storage.n.md --no-tree -o tmp_gui_render2d_compositor_byte_storage_f5md_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_row_tile_plan.n.md --no-tree -o tmp_gui_render2d_row_tile_plan_f5md_regression.json -j 1
git diff --check
trunk build
...
## Phase F5mg: render2d compositor tile RLE count step bridge boundary

目的:

- F5mf の `GuiRgba8888CompositorTileRleCountOwner` を authority とし、既存 lower `gui_rgba8888_row_tile_rle_count_step_budget` を compositor metadata 付きで接続する。
- lower count step を 1 回だけ呼び、success では lower step status と metadata 付き next count owner を `GuiRgba8888CompositorTileRleCountStep` として返す。
- lower count step error では error 側で F5mf count owner を再生成しない。kind / category / accumulated run count / cursor next pixel index を読んだ後で lower cursor を payload owner へ戻し、metadata 付き payload owner へ正規化する。
- この phase では completed count、encode、storage、packet、std present、host / platform API、host present、video memory、Canvas / DOM / minifb、transport、fallback / silent no-op へ進まない。no completed / encode / present の compositor tile RLE count step bridge で止める。

plan review:

- Meitner plan review は `PLAN_APPROVED`。
- F5mg は F5mf count owner の count continuation boundary として承認された。
- lower count error は continuation count owner を返さない契約なので、F5mg error は fake continuation count owner を作らず、payload owner へ戻して F5me の finish/free に委譲する。
- success step は F5mf count owner finish / free に委譲する。
- error は kind / category / accumulated run count / cursor next pixel index を Copy してから lower cursor owner を payload owner へ戻す。
- source policy で lower count step exact once、metadata-before-lower-count-consume、status + next owner wrapping、lower error kind/category/progress before cursor-to-payload recovery、fake count owner 禁止、completed / encode / storage / packet / direct byte read / std present / host / platform / fallback 禁止を検査する。

変更:

- `stdlib/alloc/gui/render2d/compositor_tile_rle_count_step.nepl` を追加する。
- `GuiRgba8888CompositorTileRleCountStepErrorKind` を追加する。
- `GuiRgba8888CompositorTileRleCountStep` と `GuiRgba8888CompositorTileRleCountStepError` を追加する。owner-bearing success / error は Clone / Copy を実装しない。
- `gui_rgba8888_compositor_tile_rle_count_step_budget` は metadata を count owner から Copy してから lower count owner を取り出し、lower count step を 1 回だけ呼ぶ。
- success は lower status を読んでから lower next count owner を metadata 付き `GuiRgba8888CompositorTileRleCountOwner` へ戻す。
- lower error は lower kind / category / accumulated run count / cursor next pixel index を読んでから lower cursor owner を payload owner へ戻し、metadata 付き `GuiRgba8888CompositorTilePayloadOwner` を持つ step error に正規化する。
- step status / finish owner / error progress / error payload accessors、success finish/free helper、error finish/free helper を追加する。
- `stdlib/alloc/gui/render2d.nepl` facade から compositor tile RLE count step を再公開する。
- `tests/stdlib/gui_render2d_compositor_tile_rle_count_step.n.md` を追加し、facade、zero-budget pending、completion status / accumulated count、metadata、negative budget payload recovery、no fake owner / completed / encode / present / fallback label を固定する。
- `nodesrc/test_web_gui_font_rendering_contract.js` に F5mg source policy を追加する。
- `doc/neplg2/gui_font_rendering_spec.md`、`doc/neplg2/gui_font_rendering_detailed_design.md`、`doc/neplg2/gui_standard_library_spec.md`、`note.n.md`、`todo.md` を更新する。

完了条件:

- source policy が docs、Meitner approval、facade export、typed lower error variant、status + count-owner success step、progress + payload-bearing error、owner-bearing success / error no Clone / Copy、lower count step exact once、metadata-before-lower-count-consume、success lower status before next owner wrapping、lower error kind/category/progress before payload normalization、fake count owner 禁止、success finish/free の F5mf 委譲、error finish/free の F5me 委譲、direct drain / completed count / encode / storage / packet / std present / host / platform / fallback 禁止、focused doctest label を検査する。
- focused doctest、module doctest、F5mf compositor tile RLE count regression、F5ce row tile RLE count regression、source policy、documentation contract、`git diff --check`、`trunk build`、playground editor JSON が通る。
- implementation review で metadata-after-move、owner loss、fake continuation owner、lower error progress loss、completed/encode/present leakage がないことを確認する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_tile_rle_count_step.n.md --no-tree -o tmp_gui_render2d_compositor_tile_rle_count_step_f5mg.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='180000'; node nodesrc/tests.js -i stdlib/alloc/gui/render2d/compositor_tile_rle_count_step.nepl --no-tree -o tmp_gui_render2d_compositor_tile_rle_count_step_module_f5mg.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_tile_rle_count.n.md --no-tree -o tmp_gui_render2d_compositor_tile_rle_count_f5mg_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_row_tile_rle_count.n.md --no-tree -o tmp_gui_render2d_row_tile_rle_count_f5mg_regression.json -j 1
node nodesrc/test_stdlib_documentation_contract.js
git diff --check
trunk build
...
## Phase F5mm: render2d compositor tile RLE write cursor start bridge boundary

目的:

- F5ml storage owner を authority とし、既存 lower `gui_rgba8888_row_tile_rle_write_cursor_start` を compositor metadata 付きで接続する。
- lower write cursor start を 1 回だけ呼び、success では lower write cursor owner と copied metadata を `GuiRgba8888CompositorTileRleWriteCursorOwner` として束ねる。
- lower write cursor start error は original storage owner を返す契約なので、kind / category を読んだ後で lower storage owner を metadata 付き F5ml storage owner へ戻す。
- この phase では write cursor start に留め、write step、encoded seal、packet、packet record、std present、host / platform API、host present、video memory、Canvas / DOM / minifb、transport、fallback / silent no-op へ進まない。no step / encoded / packet / present の compositor tile RLE write cursor bridge で止める。

plan review:

- Locke plan review は `PLAN_APPROVED`。
- lower write cursor start は失敗時に original storage owner を保持し、成功時だけ storage owner を write cursor owner へ移す契約なので、success は lower `GuiRgba8888RowTileRleWriteCursorOwner` + metadata でよいと確認された。
- error recovery は payload owner ではなく `GuiRgba8888CompositorTileRleStorageOwner` が適切と確認された。start validation failure は storage owner を消費する前の failure なので、retry / free 可能な storage owner recovery を保持する。
- `start_error_free` は recovered owner が storage owner なので `GuiRgba8888CompositorTileRleStorageFinishErrorKind` を返し、F5ml storage owner free に委譲する。
- write cursor finish は deallocation failure を返しうるため、`owner_finish_payload` は fallible にし、write cursor finish failure でも lower cursor を metadata 付き payload owner へ戻す。
- storage finish failure と payload finish failure は error domain が異なるため、owner_finish_entry は作らない。caller は `owner_finish_payload` の後で F5me payload finish を呼ぶ。
- source policy で lower write cursor start exact once、metadata-before-lower-storage-consume、owner-bearing success / start error / finish error no Clone / Copy、lower error read-before-storage-recovery、lower write cursor start error を公開 recovery payload にしないこと、start error free の F5ml storage owner free 委譲、finish error payload recovery、owner free の lower write cursor free delegation、write step / encoded seal / packet / std present / host / platform / raw byte access / fallback 禁止を検査する。

変更:

- `stdlib/alloc/gui/render2d/compositor_tile_rle_write_cursor.nepl` を追加する。
- `GuiRgba8888CompositorTileRleWriteCursorStartErrorKind` と `GuiRgba8888CompositorTileRleWriteCursorFinishErrorKind` を追加する。
- `GuiRgba8888CompositorTileRleWriteCursorOwner`、`GuiRgba8888CompositorTileRleWriteCursorStartError`、`GuiRgba8888CompositorTileRleWriteCursorFinishError` を追加する。owner-bearing success / error は Clone / Copy を実装しない。
...
- success は lower write cursor owner を metadata 付き write cursor owner へ戻し、metadata / total run count / encoded byte count / written run count / written byte count / cursor next pixel index / cursor pixel count accessor を提供する。
- lower start error は lower kind / category を読んでから lower storage owner を取り出し、metadata 付き F5ml storage owner を持つ start error に正規化する。
- write cursor owner の finish payload、free helper を追加する。finish payload は lower write cursor finish cursor 経由で F5me payload owner へ戻し、deallocation failure では finish error が payload owner を保持する。free helper は lower write cursor owner free に委譲する。
- start error の finish storage owner、free helper を追加し、free は F5ml storage owner free に委譲する。start error から payload へ短絡する helper は作らない。
- write cursor finish error の finish payload、free helper を追加し、F5me payload free に委譲する。
- `owner_finish_entry` は作らない。
- `stdlib/alloc/gui/render2d.nepl` facade から compositor tile RLE write cursor を再公開する。
- `tests/stdlib/gui_render2d_compositor_tile_rle_write_cursor.n.md` を追加し、facade、storage-to-start、count/progress、written counts zero、metadata、finish payload recovery、start error storage recovery source policy label、lower write cursor free delegation、no step / encoded / packet / present / fallback label、malformed storage owner 直 constructor 禁止 compile_fail を固定する。
- `nodesrc/test_web_gui_font_rendering_contract.js` に F5mm source policy を追加する。
- `doc/neplg2/gui_font_rendering_spec.md`、`doc/neplg2/gui_font_rendering_detailed_design.md`、`doc/neplg2/gui_standard_library_spec.md`、`note.n.md`、`todo.md` を更新する。

完了条件:

- source policy が docs、Locke approval、facade export、typed lower write-cursor start/finish error variant、write-cursor success、storage-owner recovery start error、payload recovery finish error、owner-bearing success / start error / finish error no Clone / Copy、lower write cursor start exact once、metadata-before-lower-storage-consume、lower error kind/category before storage-owner recovery、lower write cursor start error 公開 recovery 禁止、success finish payload の metadata-preserving payload recovery、finish error payload recovery、start error free の F5ml storage owner free 委譲、owner free の lower write cursor free 委譲、`owner_finish_entry` 禁止、write step / encoded / packet / raw byte access / std present / host / platform / fallback 禁止、focused doctest label を検査する。malformed storage owner は通常 application code から直 constructor で作れないため、start error recovery の forged branch は source policy と compile_fail で固定する。
- focused doctest、module doctest、F5ml compositor storage regression、F5cj/F5ck row tile RLE storage regression、source policy、documentation contract、`git diff --check`、`trunk build`、playground editor JSON が通る。
- implementation review で metadata-after-move、owner loss、lower write cursor start error exposure、storage owner recovery loss、write step/encoded/packet/present leakage、mixed-domain `owner_finish_entry` がないことを確認する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_tile_rle_write_cursor.n.md --no-tree -o tmp_gui_render2d_compositor_tile_rle_write_cursor_f5mm.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='180000'; node nodesrc/tests.js -i stdlib/alloc/gui/render2d/compositor_tile_rle_write_cursor.nepl --no-tree -o tmp_gui_render2d_compositor_tile_rle_write_cursor_module_f5mm.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='600000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_compositor_tile_rle_storage.n.md --no-tree -o tmp_gui_render2d_compositor_tile_rle_storage_f5mm_regression.json -j 1
$env:NEPL_TEST_CASE_TIMEOUT_MS='60000'; node nodesrc/tests.js -i tests/stdlib/gui_render2d_row_tile_rle_storage.n.md --no-tree -o tmp_gui_render2d_row_tile_rle_storage_f5mm_regression.json -j 1
node nodesrc/test_stdlib_documentation_contract.js
git diff --check
trunk build
...
## Phase F5ms: std compositor tile RLE present run step bridge boundary

目的:

- F5mr の `GuiRgba8888CompositorTileRlePresentRunCursorOwner` を authority とし、既存 lower F5co step `gui_rgba8888_row_tile_rle_present_run_cursor_step` を compositor metadata 付きで 1 回だけ進める。
- lower step result と copied metadata 付き next run cursor owner を `GuiRgba8888CompositorTileRlePresentRunStep` として束ねる。
- lower F5co step error は original run cursor owner を返す契約なので、kind / category を読んだ後で lower run cursor owner を metadata 付き F5mr run cursor owner へ戻す。
- この phase では run cursor step bridge に留め、`tile_present_command_cursor`、packet record reader、host / platform API、host present、dispatch、scheduler、video memory、Canvas / DOM / minifb、transport execution、fallback / silent no-op へ進まない。no command / record / host / platform の compositor tile RLE present run step bridge で止める。

plan review:

- Chandrasekhar plan review は `PLAN_APPROVED`。
- F5mr は run cursor start bridge、F5co は run cursor step と packet record read、F5cp は command cursor 化を所有するため、F5ms は F5mr owner から lower F5co step を 1 回だけ進める metadata wrapper bridge とする。
- metadata-before-owner-move を守り、metadata は F5mr owner から Copy してから lower run cursor owner を取り出す。
- success では lower step result を Copy してから lower next owner を回収し、metadata 付き `GuiRgba8888CompositorTileRlePresentRunCursorOwner` へ戻す。
- error では lower kind / category を先に読み、lower step error から run cursor owner を回収して metadata 付き F5mr owner へ戻す。lower step error を公開 recovery payload にしない。
- F5mr run cursor wrapper 復元は direct struct constructor でよい。F5mq present-frame owner rewrap と違い、run cursor wrapper には frame id 再検査 invariant がない。
- success / error の finish present-frame は F5mr `owner_finish_present_frame` に委譲して F5mq rewrap `Result` を潰さない。success / error free は F5mr `owner_free` に委譲する。
- source policy で facade export、typed lower step error variant、lower result + run cursor owner success、run cursor owner recovery error、owner-bearing success / error no Clone / Copy、lower F5co step exact once、metadata-before-owner-move、lower result before lower next owner consume、lower error kind/category before run cursor owner recovery、success / error finish present-frame の F5mr delegation、success / error free の F5mr delegation、`finish_encoded` / `finish_payload` / `finish_entry` 禁止、command cursor / packet record / raw byte access / host / platform / fallback 禁止を検査する。

変更:

- `stdlib/std/gui/compositor_tile_present_run_step.nepl` を追加する。
- `GuiRgba8888CompositorTileRlePresentRunStepErrorKind`、`GuiRgba8888CompositorTileRlePresentRunStep`、`GuiRgba8888CompositorTileRlePresentRunStepError` を追加する。owner-bearing success / error は Clone / Copy を実装しない。
- `gui_rgba8888_compositor_tile_rle_present_run_step_one` は metadata を Copy してから lower run cursor owner を取り出し、lower F5co step を 1 回だけ呼ぶ。success では lower result と next owner を metadata 付き owner へ戻す。
- lower F5co step error は lower kind / category を読んでから lower run cursor owner を取り出し、metadata 付き run cursor owner を持つ step error に正規化する。
- `step_finish_owner`、`step_error_finish_owner`、metadata / progress accessors、success / error finish present-frame、success / error free を追加する。
- `finish_encoded` / `finish_payload` / `finish_entry` は作らない。
- `stdlib/std/gui.nepl` facade から compositor tile RLE present run step bridge を再公開する。
- `tests/stdlib/gui_std_compositor_tile_present_run_step.n.md` を追加し、default runtime smoke で facade / lower result / wrapped lower step error kind を固定し、source policy fixture で lower F5co step、metadata-before-owner-move、result and owner recovery、free delegation、no command / record / host / platform / fallback label を固定する。
- `nodesrc/test_web_gui_font_rendering_contract.js` に F5ms source policy を追加する。
- `doc/neplg2/gui_font_rendering_spec.md`、`doc/neplg2/gui_font_rendering_detailed_design.md`、`doc/neplg2/gui_standard_library_spec.md`、`note.n.md`、`todo.md` を更新する。

完了条件:

- source policy が docs、Chandrasekhar plan review、facade export、typed lower step error variant、lower result + run cursor owner success、run cursor owner recovery error、owner-bearing success / error no Clone / Copy、lower F5co step exact once、metadata-before-owner-move、lower result before lower next owner consume、lower error kind/category before run cursor owner recovery、success / error finish present-frame の F5mr delegation、success / error free の F5mr delegation、`finish_encoded` / `finish_payload` / `finish_entry` 禁止、command cursor / packet record / raw byte access / host / platform / fallback 禁止、runtime smoke label と source policy fixture label を検査する。
- focused doctest、module doctest、F5mr compositor present run cursor regression、F5co tile present run cursor regression、source policy、documentation contract、`git diff --check`、`trunk build`、playground editor JSON が通る。
- implementation review で metadata-after-move、owner loss、lower step error exposure、run cursor owner recovery loss、command cursor / record / host / platform leakage、`finish_encoded` / `finish_payload` / `finish_entry` がないことを確認する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/tests.js -i tests/stdlib/gui_std_compositor_tile_present_run_step.n.md --no-tree -o tmp_gui_std_compositor_tile_present_run_step_f5ms.json -j 1
node nodesrc/tests.js -i stdlib/std/gui/compositor_tile_present_run_step.nepl --no-tree -o tmp_gui_std_compositor_tile_present_run_step_module_f5ms.json -j 1
node nodesrc/tests.js -i tests/stdlib/gui_std_compositor_tile_present_run_cursor.n.md --no-tree -o tmp_gui_std_compositor_tile_present_run_cursor_f5ms_regression.json -j 1
node nodesrc/tests.js -i tests/stdlib/gui_std_tile_present_run_cursor.n.md --no-tree -o tmp_gui_std_tile_present_run_cursor_f5ms_regression.json -j 1
node nodesrc/test_stdlib_documentation_contract.js
git diff --check
trunk build
...
## Phase F5mz: std compositor tile RLE present host execution action boundary

目的:

- F5mx request accessor だけで compositor host continuation request を読み、std layer compositor tile RLE present host execution action boundary を追加する。
- `GuiRgba8888CompositorTileRlePresentHostExecutionAction` は Window / Offscreen / Device と BeginFrame / RunRecord / EndFrame の flat target x record action とする。
- Window payload は `WindowId` と metadata-preserving compositor descriptor / run record を保持する。
- Begin / End action は `GuiRgba8888CompositorTileRlePresentFrameDescriptor`、Run action は `GuiRgba8888CompositorTileRlePresentHostCommandRunRecord` を保持し、lower row-tile descriptor へ投影しない。
- F5mz consumes F5mx requests, not F5my ready requests。F5mz does not depend on F5my and does not depend on F5mw/F5mv schedule or drain state。
- F5mz does not execute host imports。capability validation と request construction は F5mx の責務なので、F5mz は新しい `Result` path を作らない。
- lower F5cw / F5cr / F5cq、F5my dispatch、F5mw schedule、F5mv virtual drain、queue、timer、scheduler、platform API、raw storage、Vec、video memory、Canvas / DOM / minifb、fallback / silent no-op へ進まない。

plan review:

- Boyle design/source-policy review は F5mz policy/doc/test/facade wiring が未追加であることを required changes として指摘した。
- F5mz の invariant は lower F5cw の compositor counterpart として、入力を F5mx request、出力を `GuiRgba8888CompositorTileRlePresentHostExecutionAction`、action shape を flat target x record action に固定する。
- source policy では F5mu record type / variants と accessors は許可し、F5mu constructors、F5my/F5mw/F5mv imports / calls、lower row-tile host execution / import / command、host/platform/raw/fallback leakage を禁止する。

変更:

- `stdlib/std/gui/compositor_tile_present_host_execution.nepl` を追加する。
- `GuiRgba8888CompositorTileRlePresentHostExecutionWindowBegin`、`WindowRun`、`WindowEnd`、`GuiRgba8888CompositorTileRlePresentHostExecutionAction` を追加する。
- Window payload accessors と `gui_rgba8888_compositor_tile_rle_present_host_execution_action` を追加する。
- `action` は F5mx request target / record accessor の順に読み、Window / Offscreen / Device の target と F5mu BeginFrame / RunRecord / EndFrame を action enum に写す。
- `stdlib/std/gui.nepl` facade から compositor tile RLE present host execution action boundary を再公開する。
- `tests/stdlib/gui_std_compositor_tile_present_host_execution.n.md` を追加し、runtime smoke と source policy labels を固定する。
- `nodesrc/test_web_gui_font_rendering_contract.js` に F5mz source policy を追加する。
- `doc/neplg2/gui_font_rendering_spec.md`、`doc/neplg2/gui_font_rendering_detailed_design.md`、`doc/neplg2/gui_standard_library_spec.md`、`note.n.md`、`todo.md` を更新する。

完了条件:

- source policy が docs、Boyle design/source-policy review、facade export、action enum shape、F5mx request-only input、metadata-preserving descriptor/run-record payload、F5mx request accessor order、F5mu Begin/Run/End mapping、no F5my/F5mw/F5mv、lower row-tile / host / raw / platform / Vec / fallback 禁止、runtime smoke label を検査する。
- focused doctest、module doctest、F5mx compositor host-import regression、F5my compositor dispatch regression、F5cw lower host execution regression、source policy、documentation contract、`git diff --check`、`trunk build`、playground editor JSON が通る。
- implementation review で F5mx request-only、F5my/F5mw/F5mv 非依存、metadata preservation、no host execution / platform leakage がないことを確認する。
...
node --check nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/test_web_gui_font_rendering_contract.js
node nodesrc/tests.js -i tests/stdlib/gui_std_compositor_tile_present_host_execution.n.md --no-tree -o tmp_gui_std_compositor_tile_present_host_execution_f5mz.json -j 1
node nodesrc/tests.js -i stdlib/std/gui/compositor_tile_present_host_execution.nepl --no-tree -o tmp_gui_std_compositor_tile_present_host_execution_module_f5mz.json -j 1
node nodesrc/tests.js -i tests/stdlib/gui_std_compositor_tile_present_host_import.n.md --no-tree -o tmp_gui_std_compositor_tile_present_host_import_f5mz_regression.json -j 1
node nodesrc/tests.js -i tests/stdlib/gui_std_compositor_tile_present_dispatch.n.md --no-tree -o tmp_gui_std_compositor_tile_present_dispatch_f5mz_regression.json -j 1
node nodesrc/tests.js -i tests/stdlib/gui_std_tile_present_host_execution.n.md --no-tree -o tmp_gui_std_tile_present_host_execution_f5mz_regression.json -j 1
node nodesrc/test_stdlib_documentation_contract.js
git diff --check
trunk build
