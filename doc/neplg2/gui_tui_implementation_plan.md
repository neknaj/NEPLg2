...
- F5fr では bare raw display memory ownership boundary として、`platforms/gui/bare/display_memory_owner` を追加済みである。owner は canonical `GuiBareDisplayDriverState`、private `RegionToken u8`、surface byte count、verified byte count を保持し、public write は supplied `GuiBareDisplayDriverByteEchoVerified` を authority にせず owner state から F5fq を再実行する。single-byte store / readback success の後だけ owner state を進め、owner-bearing error は Clone / Copy にしない。
- F5fs では bare display memory span write/readback boundary として、`platforms/gui/bare/display_memory_span_readback` を追加済みである。owner 内 canonical driver state から F5fo apply を再実行し、canonical `SpanWrite` / `SpanWriteAccepted` の byte range 全体を owner raw memory に store し、full readback が成功した後だけ owner state を進める。F5fs evidence は span readback であり frame ready / present ready evidence ではない。
- F5ft では bare actual display driver adapter boundary として、`platforms/gui/bare/display_driver_adapter` を追加済みである。adapter は owner 内 driver state から F5fp host import step を呼び、returned host accepted ledger step だけを authority にする。SpanWrite は F5fs full span write/readback success まで owner を進めず、Begin / Present は host accepted ledger step completed evidence に限定して owner state を進める。span readback failure after host accepted は owner-bearing error で owner を回収するが、external host side effect rollback や frame readiness は主張しない。long-running scheduler backend、timer queue、present loop、DOM、Canvas、minifb、video memory transport、fallback、silent no-op は後続 slice へ残す。
