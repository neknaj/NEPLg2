...
- 現在の実装は bounded data contract と flat arena tree の初期 checkpoint を優先した段階であり、arena を使う focus traversal、pointer routing、diff / invalidation、arena order の linear layout connector、parent-local stack layout policy、stack cross-axis alignment、overflow rejection は実装済みである。Web Playground には表示 smoke として floating GUI window layer、backend-neutral preview command DTO、Canvas adapter、TypeScript 側 host frame decode / present 境界、runtime bridge、native には minifb optional window runner を追加済みである。flex / grid / scroll layout、stateful pointer routing、NEPL/Wasm 生成 command stream から Web runtime bridge への実接続、mobile backend はまだ未実装である。
...
- Web Playground は browser 上の editor / terminal と、その panel layout の上に重ねる floating GUI window layer を持つ。preview renderer は typed command DTO を経由して Canvas adapter に渡し、`host-bridge.ts` は unknown input の `present-commands` 風 frame を typed frame へ decode できる。`runtime-bridge.ts` は floating GUI manager を presenter として登録し、global `neplGuiHost.presentCommands` から typed Result 境界で frame を渡せる。ただし、NEPL/Wasm が生成した `DrawCommand` stream をその境界へ直接渡す compiler/runtime 接続はまだ未実装である。
...
2026-06-02 checkpoint では、Web Playground workspace に `gui-preview` pane を追加した後、editor の panel layout の上に重なる independent floating GUI window layer へ表示経路を移した。header の `GUI` button と editor header の `G` button は `web/src/gui-preview/window-manager.ts` を通して floating window を開く。window は minimize、maximize / restore、drag move、edge / corner resize、dock restore を持つ。`window-manager.ts` の transient state は `idle` / `drag` / `resize`、source は `source-path` / `preview-kind`、window mode は `normal` / `minimized previousMode` / `maximized restoreRect`、dock button は `none` / `mounted` 形式の union で表し、maximize 中に minimize しても original restore rect を失わない。`window-manager.ts` と `panel.ts` には `null` / `undefined` / non-null assertion に依存しない source policy を追加した。

`web/src/gui-preview/commands.ts` は `fill-rect` / `text-run` の preview command DTO、`rgba8888` 相当の color struct、text align enum、command frame を持つ。`web/src/gui-preview/renderer.ts` は `examples/gui_mandelbrot.nepl`、`examples/gui_life.nepl`、`examples/gui_counter.nepl` と同じ metric contract を持つ scene を作り、rect/text split ではなく typed command stream を返す。metrics も optional field ではなく `mandelbrot` / `life` / `counter` の discriminated union で表す。`web/src/gui-preview/host-bridge.ts` は unknown input を `GuiWebHostResult` の `ok` / `err` union で decode し、invalid color byte、unsupported command、invalid frame shape を typed error として返す。`web/src/gui-preview/runtime-bridge.ts` は presenter の未登録、global install 先の不正、host decode error を `GuiWebRuntimeResult` の `ok` / `err` union で扱い、Playground 初期化時に floating GUI manager を global `neplGuiHost.presentCommands` へ登録する。`web/src/gui-preview/canvas-renderer.ts` だけが `CanvasRenderingContext2D` と CSS color 変換を所有し、`web/src/gui-preview/panel.ts` は floating window または compatibility pane 内 Canvas へ adapter を呼び出す。これは user が GUI example を視覚確認し、TypeScript host boundary の shape を先に固定するための表示 smoke であり、まだ NEPL/Wasm が生成した `DrawCommand` stream を直接 export / import する compiler/runtime 接続ではない。DOM / Canvas は Web frontend の backend detail であり、`core/gui`、`alloc/gui`、`std/gui` の public type には入れない。

次の Web checkpoint では NEPL/Wasm runtime が実際に生成した command stream を `neplGuiHost.presentCommands` へ渡し、必要なら `present-commands` convenience path と `begin-frame` / `push-command` / `end-frame` streaming path を同じ runtime bridge 上に追加する。現在の preview renderer は、その接続後に smoke fixture として残す。
