...
- `DropInitialized` / `ReplaceDropOld` は droppable payload であれば、slot から raw load された value origin が actual `ResourceOp::Drop` / assignment overwrite auto-drop / scope auto-drop に到達した proof を消費する。raw load fact は materialize proof であって destructor execution proof ではないため、`MoveOutLoadedCell` だけで slot を dropped に進めることは禁止する。関連 issue は [ISS-20260521T031943441Z-COLLECTION-SLOT-DROP-LIFECYCLE-NEEDS-DCBDB1EC](../../issues/items/ISS-20260521T031943441Z-COLLECTION-SLOT-DROP-LIFECYCLE-NEEDS-DCBDB1EC.md)。
