...
- `alloc/collections/vec/sort` root facade は raw `MemPtr` helper と raw slice adapter を再公開しない。さらに direct-importable な `sort/raw/*` helper module も残さず、raw traversal は `sort_quick` / `sort_heap` / simple sort / merge range など、len・storage view・scratch owner を扱う検査済み implementation file 内の private boundary に閉じる。ordinary caller は `Vec` の sort API と `sort_is_sorted` observer だけを使う。
