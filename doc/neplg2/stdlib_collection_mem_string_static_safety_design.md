...
| `Vec<T>` | `len/cap/storage/data` を持ち、型/storage/access/raw helper の責務は submodule に分離済み。空 owner state は `VecStorageState::Empty`。 | 最重要残件。module 責務は改善したが、`MemPtr` が owner と view を兼ねる。 | `OwnedBuffer<T>` + `StorageState<T>` + initialized prefix に再実装する。 |
...
- `Vec<T>` は `len/cap/storage/data` を持ち、空 Vec は `VecStorageState::Empty` で表す。型/storage/access/raw helper の責務は submodule に分離済みで、`data = mem_ptr_wrap 0` は Empty storage の raw pointer view としてだけ残り、owner state ではない。ただし基礎型 `Vec` がまだ `MemPtr<T>` を storage owner field として持つため owner model の根は残っている。
