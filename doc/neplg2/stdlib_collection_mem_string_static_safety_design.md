...
- 残件として、`InitializeEmpty` / `MoveOut` / `ReplaceReturnOld` は payload value-flow proof と結び付いていなければならない。slot state transition だけを annotation として許すと、実際の payload consume / materialize が欠落しても Resource IR が所有権移動を信じてしまうため、次段階で lifecycle event と payload place proof を統合する。関連 issue は [ISS-20260521T010410090Z-COLLECTION-SLOT-OWNER-TRANSFER-LIFEC-3C1056B2](../../issues/items/ISS-20260521T010410090Z-COLLECTION-SLOT-OWNER-TRANSFER-LIFEC-3C1056B2.md)。
