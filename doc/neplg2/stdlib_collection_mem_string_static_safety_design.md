...
- `ByteBuf` / `ByteBuilder` は `Option<MemPtr<u8>>` owner field をやめ、storage owner を `RegionToken<u8>` field に集約した。さらに `ByteBuf` は `ByteBufStorage::Empty | Owned(RegionToken<u8>)` に移し、空 storage の zero-size `RegionToken` sentinel を削除した。`region_ptr` / `io_bytebuf_data_ptr_ref` / `byte_builder_data_ptr_ref` は参照から non-owning `MemPtr` view だけを返す。残件 baseline は 4 field ではなく 2 field であり、`ByteBuf.ptr` と `ByteBuilder.ptr` を transitional allowlist から外した。
...
- `ByteBuf` / `ByteBuilder` の `Option<MemPtr<u8>>` owner field は削除済みである。`ByteBuf` は `ByteBufStorage::Empty | Owned(RegionToken<u8>)` に移し、empty sentinel も削除済みである。次は `ByteBuilder` の empty sentinel を storage enum へ移し、その後 forgeable `RegionToken<u8>` を `StorageState<u8>` または compiler-issued `OwnedBytes` に移す。
