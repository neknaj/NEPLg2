...
| `alloc/collections/Vec` | `VecStorageState::Empty/Owned` と `data: MemPtr<T>` を分け、空 storage は enum state で表す。型定義は `vec/types`、storage allocation/free は `vec/storage`、borrowed observer は `vec/access`、raw load/store と scan/fold helper は `vec/raw`、owner-consuming transform は `vec/transform`、borrowed query は `vec/query` が所有する。 | 過渡。root の巨大責務は大きく減ったが、`MemPtr` が storage owner field である点は `OwnedBuffer<T>` 化まで残る。 | `OwnedBuffer<T>` + initialized prefix へ移行し、read/copy/move/drop/free を分離する。 |
...
| `Vec<T>` | `len/cap/storage/data` を持ち、型/storage/access/raw helper/transform/query の責務は submodule に分離済み。空 owner state は `VecStorageState::Empty`。 | 最重要残件。module 責務は改善したが、`MemPtr` が owner と view を兼ねる。 | `OwnedBuffer<T>` + `StorageState<T>` + initialized prefix に再実装する。 |
...
- `Vec<T>` は `len/cap/storage/data` を持ち、空 Vec は `VecStorageState::Empty` で表す。型/storage/access/raw helper/transform/query の責務は submodule に分離済みで、`data = mem_ptr_wrap 0` は Empty storage の raw pointer view としてだけ残り、owner state ではない。ただし基礎型 `Vec` がまだ `MemPtr<T>` を storage owner field として持つため owner model の根は残っている。
