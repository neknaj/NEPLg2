...
- [ISS-20260517T124911215Z-WASM-CODEGEN-RESPONSIBILITY-FREEZE-R-49B83608](../../issues/items/ISS-20260517T124911215Z-WASM-CODEGEN-RESPONSIBILITY-FREEZE-R-49B83608.md): WASM backend root file の responsibility freeze 再回帰。
...
| `nepl-core/src/codegen_wasm.rs` | 2481 | WASM module assembly、instruction emission、runtime helper lowering が残る。string data layout、aggregate field selector、local slot / scope stack は分離済み。 |
...
- `codegen_wasm/local_map.rs`: WASM local slot、block scope stack、temporary local allocation、alloc helper index を root から分離した。
- `nodesrc/test_parser_backend_responsibility_policy.js`: root file の line freeze を 2525 行に下げ、上記 3 module の存在と責務上限を監視する。
