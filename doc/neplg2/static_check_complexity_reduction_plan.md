...
| `STD-ENV-CLIARG-GET-CHECKED-ACCEPTS-N` | Stage 6 の std/env cliarg memory-safety issue。`cliarg_get_checked` が負 index を拒否せず raw slot 計算へ進む。 | 2026-05-16 に発見。`cliarg_get` public API の下限検査を追加する次 issue として分離する。 |
| `STD-ENV-CLIARG-CSTR-DOCTESTS-USE-MEM` | Stage 6 の std/env cliarg doctest issue。`cstr.nepl` doctest が ordinary source から `mem_ptr_add` / `store_u8` を使う。 | 2026-05-16 に発見。静的検査を緩めず fixture を現在の raw boundary 方針へ更新する。 |
...
同じ確認で `cliarg_get_checked` が負 index を拒否せず raw slot 計算へ進む memory-safety issue と、`std/env/cliarg/cstr.nepl` doctest が ordinary source から `mem_ptr_add` / `store_u8` を使う stale fixture issue を発見した。これらは静的検査を緩めず別 issue として扱い、`cliarg_get` の下限検査と doctest fixture の設計を次段で直す。
