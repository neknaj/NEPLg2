...
`ISS-20260516T073151046Z-SOURCE-CAPABILITY-RAW-HELPER-BODY-EV-43BC8055` は 2026-05-16 に修正した。unified proof 導入後、raw helper definition evidence は function walk lifecycle に乗ったが、body evidence を全 active function frame へ伝播していたため、`alloc_raw` という外側関数の中に `load_i32` を呼ぶ nested helper があるだけで `Alloc` capability まで付与され得た。修正後は `function_has_raw_memory_evidence.last_mut()` だけを更新し、関数名と body evidence の結び付きはその関数自身の body に限定する。nested body の `load_i32` は module-level `RawMemoryOp::Load` evidence として残るため、source AST からの証明力は落とさず、raw helper 名への self-operation 付与だけを lexical function scope に閉じる。
