...
| `OWNER-AGGREGATE-BOUNDARY-TREATS-QUAL` | Stage 6 の compiler source capability issue。qualified enum variant を owner aggregate constructor evidence と誤分類していた。 | 2026-05-15 に fixed。constructor evidence は unqualified symbol に限定し、`Result::Ok` / `Option::Some` だけでは `OwnerAggregateBoundary` を付与しない。 |
