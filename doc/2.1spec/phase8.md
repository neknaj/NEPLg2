...
**証明オブジェクトの resource usage**: 証明オブジェクトは `Copy` 型である。証明は「性質が成立するという事実」であり、使用後に消費する必要はない。同じ証明を複数の呼び出しに渡してよい。

**決定不可能命題について**: アルゴリズム的に決定できない性質（外部の数学的定理など）は NEPLg2.1 の型システムに統合しない。言語が扱う命題型は、有限時間で判定可能（decidable）な性質に限定する。外部の証明支援系との連携は将来の研究課題とするが、Phase 8 設計の必須要件ではない。
