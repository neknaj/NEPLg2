...
> **将来展望**: NEPLg2.1 は「括弧なし前置記法」を言語の基盤として確立するフェーズである。セルフホスト達成後の将来バージョンでは、括弧付き記法・中置記法の段階的な再導入を検討する可能性がある。これはコア意味論を変えず、同じ AST に対して複数の記法を提供する「多記法サポート」として設計される予定。NEPLg2.1 の間は括弧なし前置記法のみを正式仕様とする。
