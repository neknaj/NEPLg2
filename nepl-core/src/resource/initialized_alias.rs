...
use super::initialized_alias_order::{
...
impl RawCellAddressAliases {
    ...
    pub(super) fn clear_exact(&mut self, place: &Place) {
        for group in &mut self.groups {
            group.retain(|existing| existing != place);
        }
        self.groups.retain(|group| !group.is_empty());
        self.marked.retain(|existing| existing != place);
        self.i32_values.retain(|fact| fact.place != *place);
        self.i32_conditions.retain(|fact| fact.place != *place);
    }
