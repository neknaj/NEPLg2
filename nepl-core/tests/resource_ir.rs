...
use nepl_core::resource::{
    ...
    ResourceFunctionCheck, ResourceI32RelationOp, ResourceId, ResourceLocal, ResourceMatchArm,
...
#[test]
fn resource_ir_collection_slot_source_non_copy_store_and_move_out_use_raw_value_flow_proof() {
    let source = r#"
#indent 4
#target wasm
#no_prelude
...
fn store_then_move <(&RegionToken<LocalOwner>,LocalOwner)->LocalOwner> (storage, payload):
...
"#;
    ...
    let (module, types) = typecheck_resource_stdlib_source(
        source,
        "alloc/collections/vec/non_copy_slot_source.nepl",
        CompileTarget::Wasm,
    );
    ...
    assert!(
        report.diagnostics.is_empty(),
        "source-level raw store/load must prove non-Copy collection slot initialize and move-out without stdlib allowlists: {:#?}\nresource:\n{}",
        report.diagnostics,
        resource.dump_text()
    );
    assert!(
        resource.functions.iter().any(|function| {
            function.origin_name == "store_then_move"
        ...
        "source lowering must emit typed collection slot lifecycle events for the non-Copy owner payload:\n{}",
        resource.dump_text()
    );
}
...
fn resource_ir_collection_slot_move_transfers_slot_state_to_output() {
    ...
    let unit_ty = types.unit();
    let span = Span::dummy();
    let storage = Place::local("buffer".to_string(), storage_ty);
    let moved_storage = Place::temporary(ResourceId(604), storage_ty);
    ...
    assert_eq!(
        ...
        "move should transfer collection slot state to the moved storage; final slots: {:#?}",
...
fn resource_ir_collection_slot_construct_transfers_slot_state_to_field() {
    ...
    assert_eq!(
        ...
        "construct should transfer collection slot state to the aggregate field; final slots: {:#?}",
        report.functions[0].final_collection_slots
    );
}
...
#[test]
fn resource_ir_collection_slot_call_summary_transfers_caller_slot_through_returned_parameter() {
    let (mut types, owned_ty) = types_with_copy_owned();
    let storage_ty = register_non_copy_collection_storage(&mut types);
    let unit_ty = types.unit();
    let span = Span::dummy();
    ...
    let returned_storage = Place::temporary(ResourceId(710), storage_ty);
    let returned_slot = returned_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
...
#[test]
fn resource_ir_collection_slot_call_summary_transfers_caller_slot_through_returned_raw_owner_alias()
{
    let (mut types, owned_ty) = types_with_copy_owned();
    let storage_ty = register_non_copy_collection_storage(&mut types);
    let unit_ty = types.unit();
    let span = Span::dummy();
    let param_storage = Place::local("storage".to_string(), storage_ty);
    let alias_storage = Place::temporary(ResourceId(725), storage_ty);
    let caller_storage = Place::local("caller_storage".to_string(), storage_ty);
    let caller_slot = caller_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let returned_storage = Place::temporary(ResourceId(726), storage_ty);
    let returned_slot = returned_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let resource = ResourceModule {
        functions: vec![
            ResourceFunction {
                name: "return_storage_alias".to_string(),
                origin_name: "return_storage_alias".to_string(),
                type_params: Vec::new(),
                params: vec![ResourceLocal {
                    name: "storage".to_string(),
                    ty: storage_ty,
...
#[test]
fn resource_ir_collection_slot_call_summary_transfers_match_bound_returned_payload() {
    let (mut types, owned_ty) = types_with_copy_owned();
    let storage_ty = register_non_copy_collection_storage(&mut types);
    let unit_ty = types.unit();
    let i32_ty = types.i32();
    let result_ty = register_storage_result(&mut types, i32_ty, storage_ty);
    let span = Span::dummy();
    let callee_result_param = Place::local("result".to_string(), result_ty);
    let callee_match_output = Place::temporary(ResourceId(735), storage_ty);
    let callee_err_storage = Place::local("storage".to_string(), storage_ty);
    let callee_ok_payload = Place::local("ok_value".to_string(), i32_ty);
    let callee_fallback_storage = Place::temporary(ResourceId(736), storage_ty);
    let caller_storage = Place::local("caller_storage".to_string(), storage_ty);
    let caller_slot = caller_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let caller_result = Place::temporary(ResourceId(737), result_ty);
    let returned_storage = Place::temporary(ResourceId(738), storage_ty);
    let returned_slot = returned_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let resource = ResourceModule {
        functions: vec![
            ResourceFunction {
                name: "extract_err_storage".to_string(),
                origin_name: "extract_err_storage".to_string(),
                type_params: Vec::new(),
                params: vec![ResourceLocal {
                    name: "result".to_string(),
                    ty: result_ty,
                    mutable: false,
                    place: callee_result_param.clone(),
                }],
                result: storage_ty,
                effect: Effect::Pure,
                entry_block: ResourceBlockId(0),
                blocks: vec![ResourceBlock {
                    id: ResourceBlockId(0),
                    ops: vec![ResourceOp::Match {
                        output: callee_match_output.clone(),
                        scrutinee: callee_result_param,
                        scrutinee_is_borrow_target: false,
                        arms: vec![
                            ResourceMatchArm {
                                pattern: ResourceMatchPattern::Variant("Err".to_string()),
                                bind_local: Some(callee_err_storage.clone()),
                                bind_source_name: Some("storage".to_string()),
                                bind_mode: Some(ResourceMatchBindMode::Owned),
                                ops: vec![],
                                value: callee_err_storage,
                                span,
                            },
                            ResourceMatchArm {
                                pattern: ResourceMatchPattern::Variant("Ok".to_string()),
                                bind_local: Some(callee_ok_payload),
                                bind_source_name: Some("ok_value".to_string()),
                                bind_mode: Some(ResourceMatchBindMode::Owned),
                                ops: vec![ResourceOp::Expr {
                                    kind: ResourceExprKind::Literal,
                                    output: callee_fallback_storage.clone(),
                                    ty: storage_ty,
                                    span,
                                }],
                                value: callee_fallback_storage,
                                span,
                            },
                        ],
                        span,
                    ...
                    terminator: ResourceTerminator::Return {
                        value: Some(callee_match_output),
                        span,
                    },
                    span,
                }],
            ...
            ResourceFunction {
                ...
                blocks: vec![ResourceBlock {
                    ...
                    ops: vec![
                        ...
                        ResourceOp::Construct {
                            output: caller_result.clone(),
                            kind: AggregateKind::Enum {
                                name: "StorageResult".to_string(),
                                variant: "Err".to_string(),
                            },
                            inputs: vec![caller_storage],
                            span,
                        },
                        ResourceOp::Call {
                            output: returned_storage.clone(),
...
#[test]
fn resource_ir_collection_slot_call_summary_collects_match_bound_lifecycle_event() {
    let (mut types, owned_ty) = types_with_copy_owned();
    let storage_ty = register_non_copy_collection_storage(&mut types);
    let unit_ty = types.unit();
    let i32_ty = types.i32();
    ...
    let span = Span::dummy();
    let callee_result_param = Place::local("result".to_string(), result_ty);
    let callee_match_output = Place::temporary(ResourceId(751), unit_ty);
    let callee_err_storage = Place::local("storage".to_string(), storage_ty);
    let callee_ok_payload = Place::local("ok_value".to_string(), i32_ty);
    let err_arm_value = Place::temporary(ResourceId(752), unit_ty);
    let ok_arm_value = Place::temporary(ResourceId(753), unit_ty);
    let caller_storage = Place::local("caller_storage".to_string(), storage_ty);
    let caller_slot = caller_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let caller_result = Place::temporary(ResourceId(754), result_ty);
    let expected_dealloc_slot = caller_result
        .clone()
        .with_projection(
    ...
    let call_output = Place::temporary(ResourceId(755), unit_ty);
    let resource = ResourceModule {
        functions: vec![
            ResourceFunction {
                name: "free_err_storage".to_string(),
                origin_name: "free_err_storage".to_string(),
                type_params: Vec::new(),
                params: vec![ResourceLocal {
                    name: "result".to_string(),
                    ty: result_ty,
    ...
    assert_eq!(
        ...
        vec![ResourceCheckDiagnostic::CollectionSlotRefuted {
            function: "main".to_string(),
            target: expected_dealloc_slot,
            reason: CollectionSlotLifecycleRefutation::LiveSlotDuringStorageDealloc {
                slot_ty: owned_ty,
            },
            span,
        }],
        "callee summary must collect lifecycle events emitted after an owned enum payload match bind; final slots: {:#?}",
        report.functions[0].final_collection_slots
    );
}
...
fn resource_ir_collection_slot_call_summary_transfers_caller_slot_through_returned_enum_payload() {
    ...
    let span = Span::dummy();
    let param_storage = Place::local("storage".to_string(), storage_ty);
    let helper_result = Place::temporary(ResourceId(712), result_ty);
    let caller_storage = Place::local("caller_storage".to_string(), storage_ty);
    let caller_slot = caller_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let returned_result = Place::temporary(ResourceId(713), result_ty);
    let recovered_storage = Place::local("recovered".to_string(), storage_ty);
    let recovered_slot = recovered_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let match_output = Place::temporary(ResourceId(714), unit_ty);
    let err_arm_value = Place::temporary(ResourceId(715), unit_ty);
    let ok_payload = Place::local("ok_value".to_string(), i32_ty);
    let ok_arm_value = Place::temporary(ResourceId(716), unit_ty);
    let resource = ResourceModule {
        functions: vec![
            ResourceFunction {
                name: "return_err_storage".to_string(),
    ...
    assert_eq!(
        ...
        "callee return summary must transfer caller slot state through Result::Err payload; final slots: {:#?}",
        report.functions[0].final_collection_slots
    );
}
...
fn assert_collection_slot_nested_returned_enum_payload_transfer(
    ...
    let returned_result = Place::temporary(ResourceId(719), result_ty);
    let recovered_storage = Place::local("recovered".to_string(), storage_ty);
    let recovered_slot = recovered_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let match_output = Place::temporary(ResourceId(720), unit_ty);
    ...
    let resource = ResourceModule {
        functions: vec![
            ...
            ResourceFunction {
                name: "return_nested_err_storage".to_string(),
                origin_name: "return_nested_err_storage".to_string(),
                type_params: Vec::new(),
                params: vec![ResourceLocal {
                    name: "storage".to_string(),
                    ty: storage_ty,
...
#[test]
fn resource_ir_collection_slot_indirect_return_summary_preserves_path_correlation() {
    let (mut types, owned_ty) = types_with_copy_owned();
    let storage_ty = register_non_copy_collection_storage(&mut types);
    let function_ty = types.function(vec![], vec![storage_ty], storage_ty, Effect::Pure);
    let unit_ty = types.unit();
    ...
    let fresh_return_storage = Place::temporary(ResourceId(740), storage_ty);
    let caller_storage = Place::local("caller_storage".to_string(), storage_ty);
    let caller_slot = caller_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    ...
    let condition = Place::temporary(ResourceId(741), i32_ty);
    let branch_output = Place::temporary(ResourceId(742), unit_ty);
    let then_value = Place::temporary(ResourceId(743), unit_ty);
    let else_value = Place::temporary(ResourceId(744), unit_ty);
    let returned_storage = Place::temporary(ResourceId(745), storage_ty);
    let returned_slot = returned_storage.clone().with_projection(
        PlaceProjection::StorageOffset(ResourceOffset::Known(0)),
        owned_ty,
    );
    let call_output = Place::temporary(ResourceId(746), unit_ty);
    let resource = ResourceModule {
        functions: vec![
            ResourceFunction {
                name: "identity_storage".to_string(),
    ...
    assert!(
        report.diagnostics.is_empty(),
        "indirect return summary must preserve callee/slot path correlation and avoid impossible live-slot transfer to {:?}: {:#?}",
        returned_slot,
        report.diagnostics
    );
}
...
#[test]
fn resource_ir_branch_path_alternatives_do_not_keep_invalid_output_initialized() {
    let types = TypeCtx::new();
    let unit_ty = types.unit();
    let i32_ty = types.i32();
    let span = Span::dummy();
    let condition = Place::local("cond".to_string(), i32_ty);
    let branch_output = Place::temporary(ResourceId(747), unit_ty);
    let then_value = Place::temporary(ResourceId(748), unit_ty);
    let else_value = Place::temporary(ResourceId(749), unit_ty);
    let moved_output = Place::temporary(ResourceId(750), unit_ty);
    let resource = ResourceModule {
        functions: vec![ResourceFunction {
            name: "main".to_string(),
            origin_name: "main".to_string(),
    ...
    assert!(
        report.diagnostics.iter().any(|diagnostic| matches!(
            diagnostic,
            ResourceCheckDiagnostic::CellUnavailable {
                operation: ResourceCheckOperation::BranchCondition,
                place,
                state: CellState::Uninit,
                ..
            } if *place == condition
        )),
        "branch condition should be reported as unavailable: {:#?}",
        report.diagnostics
    );
    assert!(
        report.diagnostics.iter().any(|diagnostic| matches!(
            ...
            ResourceCheckDiagnostic::CellUnavailable {
                ...
                state: CellState::Uninit,
                ..
            } if *place == branch_output
        )),
        "path alternatives must not keep an invalid branch output initialized: {:#?}",
        report.diagnostics
    );
}
...
fn register_storage_result(types: &mut TypeCtx, ok_ty: TypeId, err_ty: TypeId) -> TypeId {
    types.register_named(
        "StorageResult".to_string(),
        TypeKind::Enum {
            name: "StorageResult".to_string(),
            type_params: vec![],
            variants: vec![
