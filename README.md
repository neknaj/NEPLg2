...
現在の主作業は、現行 NEPLg2 を NEPLg2.1 表層構文へ切り替える移行です。NEPLg2.1 では `%` 型注釈、prefix 型式、`unit` unit 記法、0 引数関数 marker `void`、`\` 関数リテラル、明示 generic postfix 撤廃を導入します。NEPLg3 はまだ未着手・未確定の将来設計であり、現在の実装や移行作業の正仕様ではありません。
...
- 0 引数関数 marker `void`: [`doc/neplg2/zero_arg_void_marker_spec.md`](doc/neplg2/zero_arg_void_marker_spec.md)
