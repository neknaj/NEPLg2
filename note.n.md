...
- [状況]:
  - `examples/counter2.nepl` に旧式の関数型表記 `fn main <()*> ()>` と、alias なしの `#import` が残っていた。
- [修正]:
  - import を `as *` 付きに揃え、`main` の型表記を `fn main <()*>()>` に更新した。
  - carriage return と ANSI クリアを使った出力仕様は変更していない。
- [確認済み]:
  - `trunk build`: 通過
  - `node nodesrc/tests.js -i examples/counter2.nepl --no-tree -o tmp/counter2-example-tests.json -j 2`: 1/1 passed
- [plan.mdとの差異]:
