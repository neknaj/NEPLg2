# 2026-03-27 作業メモ (doc: compare / 2.1impl / examples の再点検)

- [目的]:
  - `2.1spec` 以外に、`doc/compare`、`doc/2.1impl`、`doc/examples` に Zenn #1 / #2 と衝突する記法が残っていないかを再点検する。
- [確認結果]:
  - `doc/examples/` は主要サンプル 01〜07 を再確認し、現在残っている構文は Zenn #1 / #2 を正とする説明と両立することを確認した。
  - `doc/compare/` の旧記法は、旧 2.0 / 旧 2.1 案との比較対象として意図的に残っているものであり、現行仕様として案内している箇所は見当たらなかった。
  - `doc/2.1impl/` では表層構文の呼称にわずかに旧案が混ざっていたため補正した。
- [変更]:
...
- [目的]:
  - ルート `README.md` に残っていた旧記法サンプルが、Zenn #1 / #2 を正とする現在の仕様文書と食い違っていたため、入口文書の表記を揃える。
- [変更]:
  - `README.md`
    - クイックサンプルを旧 `#import` / `fn main <...>` / `unit` 前提の例から、`let main \()` / `if cond a b` / `block:` を使う Zenn 基準のコア構文例へ更新。
    - 現行実装と 2.1 設計文書がまだ完全一致していないことを注記し、正の仕様として `doc/2.1spec/` を参照するよう明記。
    - NEPLg2.1 の説明文を、`%fn` や juxtaposition だけでなく、`let <name> <expr>`、`%` の式レベル型注釈、`if` / `match` / `block:` まで含めた表現へ更新。
...
- [目的/もくてき]:
  - 現行 `nepl-core/src/` の構造上の問題を整理し、NEPLg2.1 ブートストラップコンパイラの目標ファイル/フォルダ構成を設計する。
- [変更/へんこう]:
  - `doc/2.1impl/compiler_structure.md` 新規作成
    - 現行の問題点一覧（typecheck.rs 8871行巨大・Resource IR 不在・フラット構造等）
    - NEPLg2.1 Rust ブートストラップコンパイラ（`nepl-core-2.1`）のフォルダ構成
    - パイプラインステージ = ディレクトリ階層という設計原則
    - セルフホスト（`stdlib/neplg2/`）との命名パリティ設計
    - 現行ファイルと新規ファイルの対応表（25件）
    - Stage 1–6 の移行戦略
- [設計決定/せっけいけってい]:
  - `typecheck.rs` を `check/` 7 ファイルに分割（最大の変更）
  - `resource/` モジュールを新設（Resource IR の第一級配置）
  - `nm/` をコアコンパイラから独立（ツールチェーン補助）
  - `nepl-core-2.1` として現行と並行開発し、Stage 6 で切り替え
  - セルフホストはブートストラップが Stage 4 以降になってから着手
...
- [目的/もくてき]:
  - 外部レビュー（NEPLg2.1 仕様の一貫性監査）で指摘された5つの不整合を修正する。
- [変更/へんこう]:
  - `doc/2.1spec/syntax.md`:
    - §4 の `<expr>` BNF から `let`/`set` を除去し、`<stmt>` として独立させた。「文として扱う」とコメントしながら `<expr>` の選択肢に含めていた矛盾を解消。
    - `<suite>` 定義を追加（インライン式 または インデントブロック）。`if`/`match`/`while`/クロージャ本体の文法を `<block>` から `<suite>` に変更し、`if ge score 90: "A"` のようなインライン式と仕様の乖離を解消。
    - §4.1 juxtaposition の「左結合」説明を修正: 「flat chain として受理し、型/arity で境界決定」と明記した。
    - §14.2 のナンバリングミスを修正（§11.2 になっていた）。
  - `doc/2.1spec/traits.md`:
    - §3 に「クロスモジュール Coherence（Orphan Rule）」を追加。同一モジュール内のみの禁止では別モジュールからの impl 衝突を防げないため。
    - §7 のオーバーロード解決例に `[Phase 8 example]` 注記を追加。`where %IsLess idx len` は依存型導入後に有効な例であり Phase 0-7 仕様と混同しないよう境界を明示。
  - `doc/2.1spec/patterns.md`:
    - §4.1 match 構文の BNF を `<suite>` に合わせて更新。
- [差異/さい]:
  - これらは仕様の追加・変更ではなく、すでに「そうである」はずの事実をBNF/定義に正確に反映した修正。
- [残課題/のこかだい]:
  - `while` 式の 0 回実行時の値（§8 の仕様保留）は未解決のまま（型安全性との整合確認後に別途決定）。
...
- [目的/もくてき]:
  - NEPLg2 基本理念（前置記法括弧なし・強力な静的検査・型安全メモリ安全・依存型準備・マルチプラットフォーム）が doc/ 全体に徹底されているか確認し、不整合を修正する。
- [調査結果]:
  - `doc/2.1spec/` は 5 原則すべてについて完全に整合が取れている。
  - 問題は主に周辺ドキュメントに存在した。
- [変更/へんこう]:
  - `doc/stdlib_doc_comment_policy.md`:
    - 存在しない `doc/purity_ownership_memory_spec.md` への参照を `doc/2.1spec/memory.md §2` に修正（破損リンク修正）。
  - `doc/2.1spec/types.md`:
    - `fn TypeExpr*` を `fn TypeExpr+` に修正（引数は 1 つ以上。入力不要な場合は `fn unit -> T` を使う）。
    - `fn -> T`（引数ゼロ）という形式を廃止し、`fn unit -> T` を正規形に統一。
  - `doc/compare/syntax.md`:
    - `() -> i32 → fn -> i32` を `fn unit -> i32` に修正（旧 `()` = unit 型なので `unit` に対応させる）。
    - `() *> i32 → fn* -> i32` を `fn* unit -> i32` に修正。
  - `doc/lsp_api.md`:
    - 冒頭に「現行 Bootstrap 実装（NEPLg2.0）の API を記述。`fn` definition kind は NEPLg2.1 の `let` 統一仕様と異なる」という注意書きを追加。
  - `doc/cli.md`:
    - `--target` セクションを追加し、`wasm`・`wasi`・`llvm` 3 ターゲットを記載（マルチターゲット原則の反映）。
  - `doc/self_host.md`:
    - 指示文から設計仕様文書へ全面改訂。二層構造・ディレクトリ構成・ブートストラップ手順・テスト方針を記述。
- [根拠/こんきょ]:
  - `declarations.md §2.1` は `%fn unit -> T` を「入力不要な関数」の標準形として使っているため、`fn -> T`（引数ゼロ）は不要・混乱を招く。
...
- [目的/もくてき]:
  - `doc/` 以下のすべての仕様や計画 (`plan.md`, `todo.md` を含む) と最新の実装状況を精査し、NEPLg2 の目標と新たに策定した安全施策 (`purity_ownership_memory_spec.md`, `memory_safety_migration_plan.md`) との間で齟齬がないように統一を図る。
- [変更と監査結果/へんこうとかんさけっか]:
  - `plan.md`:
    - 文字列 (`str`, `ByteBuf`, `StringBuilder`) の記述を更新し、旧式の借用ビューや `String` 型への言及を削除。(済)
  - `doc/runtime.md`:
    - GCなしのメモリ管理について、手動 `alloc/dealloc` ベースの古い説明を削除し、**Region Inference (純粋永続値)** と **Drop Elaboration (一意所有リソース)** の二段構えモデルに書き換えた。
    - Wasm/LLVM のランタイム差分は `#if[target=...]` で吸収され、コンパイラの安全意味論は共通である旨を明記した。
  - `doc/error.md`:
    - 旧式のヒープ確保前提である `Error` レコードの説明を削除し、最新の `Diag`、`Outcome<T, E>`、`Result<T, StdErrorKind>` を核とするエラーモデルに更新した。メモリの確保と解放はGCや手動 `alloc` ではなく、新しい所有権モデルに委ねられる旨を記載した。
  - `doc/move_effect_spec.md` & `doc/memory_safety_compiler_design.md` & `doc/stdlib_breaking_reboot.md`:
    - すでに統合仕様を反映済みであり、内容に矛盾がないことを確認した。
  - `todo.md`:
    - 「メモリ安全型モデルを統合仕様に基づいて実装する」のタスク（Phase 1: Effect拡張とVarState追加、Phase 2: 型分離とRegion推論）が詳細に記載されており、実装状況および計画と完全に一致していることを確認した。
- [結論/けつろん]:
  - これにより、NEPLg2 における純粋性・所有権・メモリ管理の根幹となるドキュメントと実装計画が完全に一点に統合・整理され、すべての古い GC/手動解放ベースの記述が払拭された。以後はこのドキュメント群および `todo.md` の Phase 1 / 2 に則ってコンパイラと標準ライブラリの実装を安全に進めることができる。

---

# 2026-03-15 作業メモ (doc: NEPLg2目標と新仕様の整合性検討)

- [目的/もくてき]:
  - Zenn記事（NEPLg2の歴史と設計思想）の目標に対し、作成したPurity・Ownership・Memory仕様と実装計画で到達可能かを深く検討し、不足があれば修正する。
- [検討事項と結果/けんとうじこうとけっか]:
  1. **マルチプラットフォームと同等の結果（Wasm/LLVMの抽象化）**:
     - **結果**: 達成可能。統合仕様§12 および 移行計画§10.8 により、Resource IR パスで「安全意味論」を完全に保証し、codegen フェーズでは物理レイアウト（Linear memory vs Native pointer）の違いのみを吸収する設計になっている。Zenn記事の目標と完全に一致する。
  2. **自作言語プラットフォームとセルフホスト（コンパイラを書ける言語か）**:
     - **結果**: 達成可能だが一部仕様に明記が必要だったため修正した。コンパイラ（ASTや環境）を実装するには、複雑なデータ構造を通じた能力の伝播が必要。
     - **修正**: `purity_ownership_memory_spec.md` に **「5.3 型の能力の合成則」** を追加。AST（immutable tree）は pure persistent、環境やBuilderは UniqueMutable、開いたファイルを含む構造体は LinearCapability となるよう、複合型の能力伝播ルールを明文化し、コンパイラ記述に耐えうる堅牢な型システム設計を確立した。
  3. **協力な静的検査と括弧の根絶**:
     - **結果**: 達成可能。Resource IR を用いた Dataflow 解析 (use-after-move, borrow conflict, linear 漏れ等の検査) は Zenn記事の「強力な検査装置」に直接貢献する。構文的特徴（前置記法・オフサイドルール）とは独立した IR 層での検査であるため、括弧の根絶目標とも衝突しない。
  4. **既存ドキュメントの矛盾解消**:
     - **結果**: `plan.md` の文字列に関する記述が旧思想のままだったため修正した。
     - **修正**: `plan.md` 上の「`str`: 借用, `String`: 所有」という記述を削除し、新仕様の「`str` (純粋永続値) / `ByteBuf` (一意所有バイト列) / `StringBuilder` (構築用状態)」に更新した。
  5. **コンパイラパス順序の不整合**:
     - **結果**: `memory_safety_migration_plan.md` のパス順序が暫定のままだったため修正した。
     - **修正**: 統合仕様に合わせて `effect attribution`, `resource_ir_gen`, `region_inference` を正しい順序でパイプライン（§10.5.1, §10.7）に組み込んだ。
- [結論/けつろん]:
  - 今回の純粋性・所有権の拡張仕様は、Zenn記事で掲げられた「プラットフォーム非依存の抽象化」「自作言語プラットフォームに耐えうる堅牢な型システム」の中核を成すものであり、提示された移行計画で段階的に実装を進めることで目標達成は十分に可能であると判断した。
...
# 2026-03-15 作業メモ (doc: 開発計画と仕様の再確認)

- [目的/もくてき]:
  - `nepl-core` と `doc/` に書かれている仕様、および `todo.md` の開発計画について実現可能か、適切かを確認する。
- [確認結果/かくにんけっか]:
  - `doc/memory_safety_migration_plan.md` の「15. 実装優先順位」セクションで定義されている Phase 1（基盤修正）および Phase 2（型・API 分離）の区分けと、`todo.md` の「4. メモリ安全型モデルを統合仕様に基づいて実装する」のサブ項目が完全に一致していることを確認した。
  - 実装方針（`InternalAlloc` の利用、`MemPtr` の隔離、`List` の persistent 化、`VarState` の導入、領域推論など）は、GCを使用せずメモリ安全性を確保するNEPLg2の目標達成のために非常に論理的かつ実現可能に組み立てられている。
  - したがって、現在の `todo.md` および `plan.md` は適切であり、修正は必要ないと判断した。

# 2026-03-15 作業メモ (doc: todo.md の順序と優先順位の適正化)

- [目的/もくてき]:
  - `todo.md` の「stdlib 再構築 本流」セクションの作業順序が、マイグレーション計画 (`memory_safety_migration_plan.md`) の Phase 分割と矛盾し、破綻している（全てのメモリ安全化コンパイラ実装が `alloc` や `std` 層のポインタ隔離再構築の前に置かれている）問題を解決し、アーキテクチャの依存関係に沿った適切な順序に並び替える。
- [作業内容/さぎょうないよう]:
  - `todo.md` 内のタスク網羅性を完全に維持しつつ、依存度の強い基盤から進める原則 (`diag/trait` -> `compiler 前提` -> `core/mem` -> `alloc` -> `コンパイラ後段パス` -> `runtimes` -> `std` -> `features`) に沿って再構成。
  - メモリ安全マイグレーションの各段階を、レイヤーごとの整備フェーズに適切に分散・配置した:
    - コンパイラ基盤と診断整備 (Phase 0) を最序盤に配置。
    - `core/mem` および `alloc` の生ポインタ隔離 (Phase 1, 2) をライブラリ層再構築の前半に配置。
    - Purity追跡の変更、Resource IRによるDrop Elaboration、およびRegion 推論 (Phase 4, 5, 6) を、`alloc` が安全化された後の「コンパイラ解析パスの強化」タスクとして配置。
    - `std/io` 等への `ExternalIO` 効果宣言付与 (Phase 3) を `std` 層構築フェーズに配置。
- [確認結果/かくにんけっか]:
  - ポインタを隔離するというライブラリ前提をクリアしてからコンパイラの所有権等による自動管理機能 (Resource IR) を導入するよう軌道修正され、現実的で論理破綻のないタスクリストに修正されたことを確認した。

# 2026-03-15 作業メモ (doc: 全体仕様の依存型・形式証明パラダイムへの到達実現性評価)

- [目的/もくてき]:
  - `doc/` 以下の全ドキュメントを精査し、最終目標である「強力な静的・型・所有権検査」「完全な検証」、そして「依存型（Dependent Types）による形式証明」が完全に実現可能か慎重に検討する。
- [確認ファイル/かくにんファイル]:
  - `trait_system_design.md`, `move_effect_spec.md`, `error.md`, `shadowing.md`, `stdlib_breaking_reboot.md`, `testing.md`, `rewrite_plan.md`, `runtime.md`, `new_tutorial_plan.md` およびメモリ安全系仕様
- [検討結果・分析/けんとうけっか・ぶんせき]:
  - **完璧な実現可能性を確認**: 現在の各種仕様は、依存型や形式証明の導入を妨げる「暗黙の副作用・状態・非決定性」をコンパイラアーキテクチャの根底から徹底的に排除するよう設計されている。
  - **CTFE（Compile-Time Function Evaluation）の強力な土台**: `move_effect_spec.md` と `purity_ownership_memory_spec.md` にある「内部的な `InternalAlloc`（生メモリ操作）を Surface の `Pure` に畳み込む」仕様と「Escape Analysis」は、依存型実現における最強の武器となる。「内部的にはミュータブルで高速に実行できるが、外部（型システム側）からは完全に純粋な数学的関数に見える」という性質が保証されているため、型チェッカがコンパイル時に安全にコードを評価（メタ計算）できる。
  - **決定論的な名前・型解決**: `rewrite_plan.md` の「DefId ベースのモジュール解決」「`noshadow`」や、`trait_system_design.md` の「構造的型同値による trait 制約」は、証明・検証時に必須となる「名前や型の決定論的同一性」を担保している。
  - **GCレスと純粋不変性の両立**: `runtime.md` 等に特記される「Region Inference による Persistent な値のスコープ管理」は、値が勝手に書き換わらないこと（不変性）の静的証明そのものと言える。依存型の型パラメータに安全に値を持ち込める。
- [結論/けつろん]:
  - NEPLg2 が現計画で目指している「純粋性」「局所可変状態の隔離」「型付きメモリ所有権」の3本柱は、まさに定理証明支援系（Coq, Agda等）相当の型システムを汎用プログラミング言語上に構築するための **Must-Have（必須要件）** をすべて満たしている。
  - アーキテクチャの変更を全く必要とせず、現在のマイルストーンを完遂した延長線上に「CTFEの拡充」「Totality（停止性）チェックの導入」「命題型の追加」という形で自然に依存型・形式証明を接続可能であると結論づけた。

# 2026-03-15 作業メモ (doc: 依存型・形式証明に向けた下書き仕様書の作成)

- [目的/もくてき]:
  - 依存型の導入と形式証明を見据えた `doc/dependent_type_proof_plan.md` を作成し、現在進行中のメモリ安全・Purity化のプロジェクトがどのように将来の強力な型検査（CTFE, Totality Checker, 命題型）へと接続されるかの青写真を残す。
- [作業内容/さぎょうないよう]:
...
# 2026-03-27 作業メモ (root doc / compare の全文読み継続)

- [目的/もくてき]:
  - `doc/README.md` と `doc/compare/` の表現を、`doc/2.1spec/index.md` の現在のステータス整理と揃える。
- [作業内容/さぎょうないよう]:
  - `doc/README.md` の `2.1spec/` 説明を、「完全な仕様」と断定する表現から、各章で draft / 将来仕様を明示する現在の整理に合わせた説明へ更新した。
  - `doc/compare/syntax.md` の 0 引数関数例を、`let main \\(): ...` という旧 block 省略の見え方から、現在の宣言説明と齟齬の少ない `let main \() 0` へ差し替えた。
- [plan.mdとの差異/さ]:
  - `plan.md` の目標自体に変更はない。
  - 正の仕様を案内する入口文書と比較文書の表現を、現在の `2.1spec` のステータス表示に合わせた。

# 2026-03-27 作業メモ (root tool doc の全文読み継続)

- [目的/もくてき]:
  - root の補助文書群でも、現行 Bootstrap 実装の説明と NEPLg2.1 の正の仕様が混ざって見えないように境界を揃える。
- [作業内容/さぎょうないよう]:
  - `doc/debug.md` に対象実装注記を追加し、この文書が現行 `nepl-core` / `nepl-cli` の debug build 挙動を説明するもので、正の仕様は `doc/2.1spec/` だと明記した。
  - `doc/llvm_ir_setup.md` に対象実装注記を追加し、この文書が LLVM target の開発環境メモであり、target 設計そのものは `doc/2.1spec/platform.md` を参照すべきだと明記した。
- [plan.mdとの差異/さ]:
  - `plan.md` の目標自体に変更はない。
