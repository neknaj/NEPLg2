...
function buildPacket(parsed) {
    ...
    return [
        ...
        "  個別 review response では、実際に作業した subagent の id だけを `subagent_review_ids` に記録し、`subagent_review_count: 1` としてください。",
        "  個別 review response は `node nodesrc/selfhost_zenn_review_response_check.js --review-kind individual --input <review-response.md>` で検査します。",
        "  agent は 2 件以上の個別 review response を集約し、最終受理時に `--review-kind final --record <note-or-issue.md>` で durable evidence を検査します。",
        "  `subagent_review_ids` と `subagent_review_count` は、個別 review でも最終集約 review でも、実際に作業した subagent の id と件数に一致させてください。",
        ...
        "  `source_policy: required`、`source_policy: follow-up`、検証未実行、未説明の residual risk、今回差分由来 warning が残る場合は `MERGE_APPROVED` にしないでください。",
        "  durable record 側にも `policy/spec` と `implementation/test` の両方の `source_policy`、`residual_risk`、`unexecuted_verification`、`existing warnings`、`new warnings` または同等の機械可読 field が必要です。",
        "  `zenn_check` は `yes` や `確認済み` だけで済ませず、対象 file、関数、test、source policy、authority boundary などの具体的な根拠を書いてください。`Result`、`enum`、`match` などの抽象語だけを code span にしても具体証跡にはなりません。",
