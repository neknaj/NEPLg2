...
const code = [rootCode, typesCode, storageCode, queryCode, apiCode].join('\n');
...
assert.match(storageCode, /fn\s+dsu_load_owned\s+<\(&Vec<i32>,i32\)->Option<i32>>[\s\S]*vec::get<i32>[\s\S]*Option::None:[\s\S]*none<i32>/, 'DisjointSet must read parent and size cells through Vec.get and expose missing cells as Option');
assert.match(storageCode, /fn\s+dsu_store_owned\s+<\(&Vec<i32>,i32,i32\)\*>\(\)>[\s\S]*vec::replace<i32>/, 'DisjointSet must update parent and size cells through Vec.replace');
...
assert.match(apiCode, /fn\s+new\s+<\(i32\)\*>Result<DisjointSet,\s*Diag>>[\s\S]*lt\s+n\s+0[\s\S]*dsu_diag_len[\s\S]*vec::filled<i32>\s+n\s+0[\s\S]*vec::filled<i32>\s+n\s+1/, 'DisjointSet.new must reject negative lengths and allocate initialized typed storage through Vec.filled');
assert.match(apiCode, /fn\s+len\s+<\(&DisjointSet\)->i32>\s+\(dsu\):/, 'DisjointSet.len must borrow the owner');
assert.match(apiCode, /fn\s+find\s+<\(&DisjointSet,i32\)\*>Result<i32,\s*Diag>>\s+\(dsu,\s*idx\):/, 'DisjointSet.find must borrow the owner');
assert.match(apiCode, /fn\s+same\s+<\(&DisjointSet,i32,i32\)\*>Result<bool,\s*Diag>>\s+\(dsu,\s*a,\s*b\):/, 'DisjointSet.same must borrow the owner');
assert.match(apiCode, /fn\s+size\s+<\(&DisjointSet,i32\)\*>Result<i32,\s*Diag>>\s+\(dsu,\s*idx\):/, 'DisjointSet.size must borrow the owner');
assert.match(apiCode, /fn\s+union\s+<\(DisjointSet,i32,i32\)\*>Result<DisjointSet,\s*DisjointSetUpdateError>>\s+\(dsu,\s*a,\s*b\):/, 'DisjointSet.union must return an owner-carrying error type');
assert.doesNotMatch(apiCode, /fn\s+union\s+<\(DisjointSet,i32,i32\)\*>Result<DisjointSet,\s*Diag>>/, 'DisjointSet.union must not lose the owner through Err(Diag)');
assert.match(apiCode, /let\s+e\s+<DisjointSetUpdateError>\s+DisjointSetUpdateError\s+dsu\s+d[\s\S]*err<DisjointSet,\s*DisjointSetUpdateError>\s+e/, 'DisjointSet.union Err path must return the input owner in DisjointSetUpdateError');
assert.match(apiCode, /fn\s+free\s+<\(DisjointSet\)->\(\)>\s+\(dsu\):[\s\S]*field::get\s+dsu\s+"parent"[\s\S]*field::get\s+dsu\s+"sizes"[\s\S]*vec::free<i32>\s+parent[\s\S]*vec::free<i32>\s+sizes/, 'DisjointSet.free must consume and close typed Vec<i32> storage');
assert.doesNotMatch(apiCode, /fn\s+free\s+<\(DisjointSet\)->\(\)>\s+\(dsu\):[\s\S]*field::get_ref\s+&dsu\s+"(?:parent|sizes)"/, 'DisjointSet.free must not borrow-read owned array fields');
