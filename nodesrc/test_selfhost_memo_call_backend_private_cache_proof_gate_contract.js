...
const conditionFactPartRelPath = "stdlib/neplg2/core/codegen/memo_call_backend_scope_condition_fact_part.nepl";
const branchPayloadPartRelPath = "stdlib/neplg2/core/codegen/memo_call_backend_scope_branch_payload_part.nepl";
...
const matchVariantMembershipPartRelPath = "stdlib/neplg2/core/codegen/memo_call_backend_scope_match_variant_membership_part.nepl";
...
const conditionFactPartSource = fs.readFileSync(path.join(repoRoot, conditionFactPartRelPath), "utf8").replace(/\r\n/g, "\n");
const branchPayloadPartSource = fs.readFileSync(path.join(repoRoot, branchPayloadPartRelPath), "utf8").replace(/\r\n/g, "\n");
...
const matchVariantMembershipPartSource = fs.readFileSync(path.join(repoRoot, matchVariantMembershipPartRelPath), "utf8").replace(/\r\n/g, "\n");
...
const conditionFactPartMergeDirective = '#import "./memo_call_backend_scope_condition_fact_part" as @merge';
const branchPayloadPartMergeDirective = '#import "./memo_call_backend_scope_branch_payload_part" as @merge';
...
const matchVariantMembershipPartMergeDirective = '#import "./memo_call_backend_scope_match_variant_membership_part" as @merge';
...
const sourceWithConstruct = sourceWithIndirectCall.replace(
    constructStage0PartMergeDirective,
    `${constructStage0PartMergeDirective}\n${constructStage0PartSource}`,
);
const sourceWithConditionFact = sourceWithConstruct.replace(
    conditionFactPartMergeDirective,
    `${conditionFactPartMergeDirective}\n${conditionFactPartSource}`,
);
const sourceWithBranchPayload = sourceWithConditionFact.replace(
    branchPayloadPartMergeDirective,
    `${branchPayloadPartMergeDirective}\n${branchPayloadPartSource}`,
);
const sourceWithBranchStage0 = sourceWithBranchPayload.replace(
    branchStage0PartMergeDirective,
    `${branchStage0PartMergeDirective}\n${branchStage0PartSource}`,
);
const sourceWithLoopPayload = sourceWithBranchStage0.replace(
...
const sourceWithMatchVariantMembership = sourceWithMatchPayload.replace(
    matchVariantMembershipPartMergeDirective,
    `${matchVariantMembershipPartMergeDirective}\n${matchVariantMembershipPartSource}`,
);
const source = sourceWithMatchVariantMembership.replace(
...
const selfhostTypeRecord = fs.readFileSync(path.join(repoRoot, "stdlib/neplg2/core/ty/ty/record.nepl"), "utf8").replace(/\r\n/g, "\n");
const selfhostTypeConstructor = fs.readFileSync(path.join(repoRoot, "stdlib/neplg2/core/resolve/type_resolver/constructor.nepl"), "utf8").replace(/\r\n/g, "\n");
...
assert.match(rustResourceOp, /Loop \{\s*condition_ops: Vec<ResourceOp>,\s*condition: Place,\s*condition_fact: Option<ResourceConditionFact>,\s*body_ops: Vec<ResourceOp>,\s*span: Span,\s*\}/, "Rust Loop must retain condition ops, condition Place, optional fact, body ops, and span in order");
assert.match(rustResourceLower, /HirExprKind::While \{ cond, body \} => \{[\s\S]*let condition_fact = resource_condition_fact\(cond, ctx\);[\s\S]*let mut condition_ops = Vec::new\(\);[\s\S]*let condition = lower_expr_skeleton\(cond, &mut condition_ops, ctx, env\);[\s\S]*let mut body_ops = Vec::new\(\);[\s\S]*lower_expr_skeleton\(body, &mut body_ops, ctx, env\);[\s\S]*ops\.push\(ResourceOp::Loop \{\s*condition_ops,\s*condition,\s*condition_fact,\s*body_ops,\s*span: expr\.span,\s*\}\);[\s\S]*push_expr\(ops, ResourceExprKind::Loop, expr, ctx\)/, "Rust While lowering must connect every Loop field and keep the separate expression output");
...
for (const [partName, mergeDirective] of [
    ["memo_call_backend_scope_condition_fact_part", conditionFactPartMergeDirective],
    ["memo_call_backend_scope_branch_payload_part", branchPayloadPartMergeDirective],
    ["memo_call_backend_scope_branch_stage0_part", branchStage0PartMergeDirective],
    ["memo_call_backend_scope_loop_payload_part", loopPayloadPartMergeDirective],
    ["memo_call_backend_scope_loop_stage0_part", loopStage0PartMergeDirective],
]) {
    const references = listNeplFiles(path.join(repoRoot, "stdlib/neplg2"))
        .flatMap((filePath) => fs.readFileSync(filePath, "utf8").replace(/\r\n/g, "\n").split("\n")
            .filter((line) => line.includes(partName))
            .map((line) => ({ filePath: path.relative(repoRoot, filePath), line })));
    assert.deepEqual(references, [{ filePath: relPath, line: mergeDirective }], `${partName} must be referenced exactly once by the proof-gate anchor`);
}
...
assert.match(anchorSource, /memo_call_backend_scope_condition_fact_part" as @merge/, "proof-gate anchor must merge condition facts privately");
assert.match(anchorSource, /memo_call_backend_scope_branch_payload_part" as @merge/, "proof-gate anchor must merge Branch payload privately");
assert.match(anchorSource, /memo_call_backend_scope_branch_stage0_part" as @merge/, "proof-gate anchor must merge Branch stage0 privately");
assert.doesNotMatch(conditionFactPartSource, /^pub\s/m, "condition fact part must not expose public declarations");
assert.doesNotMatch(branchPayloadPartSource, /^pub\s/m, "Branch payload part must not expose public declarations");
assert.doesNotMatch(branchStage0PartSource, /^pub\s/m, "Branch stage0 part must not expose public declarations");
assert.match(conditionFactPartSource, /enum SelfhostMemoCallBackendPrivateCacheResourceIrConditionFactKind:[\s\S]*EqZero[\s\S]*NeZero[\s\S]*Positive[\s\S]*NonPositive[\s\S]*Negative[\s\S]*NonNegative[\s\S]*I32Relation[\s\S]*Any[\s\S]*All/, "condition fact owner must preserve every Rust fact variant without fallback");
assert.match(conditionFactPartSource, /preorder_validate_result[\s\S]*child_node_ordinal next_node[\s\S]*cursor\.next_node[\s\S]*cursor\.next_edge/, "condition fact validator must consume direct child subtrees in canonical preorder");
assert.match(conditionFactPartSource, /not eq root start[^\n]*ConditionFactPreorderInvalid/, "condition fact root must begin its exclusive owner range");
const branchPayloadSignature = branchPayloadPartSource.match(/^fn selfhost_memo_call_backend_private_cache_resource_ir_branch_payload_inventory_validate_loop[^\n]+/m)?.[0] ?? "";
assert.match(branchPayloadSignature, /OperationKindInventory.*PlaceInventory/, "Branch merge validator must require global kind and Place owners");
assert.doesNotMatch(branchPayloadSignature, /ConditionFact/, "Branch sparse validator must not own generic fact exhaustion");
assert.doesNotMatch(branchPayloadPartSource.match(/^struct SelfhostMemoCallBackendPrivateCacheResourceIrBranchPayloadRecord:[\s\S]*?(?=^\S)/m)?.[0] ?? "", /condition_fact_root/, "Branch header must not duplicate the generic root binding authority");
assert.match(conditionFactPartSource, /struct SelfhostMemoCallBackendPrivateCacheResourceIrConditionFactRootBindingRecord:[\s\S]*operation_ordinal %i32[\s\S]*root %Option i32/, "generic fact owner must preserve an explicit Branch/Loop root binding");
assert.match(conditionFactPartSource, /condition_fact_inventory_validate_loop[\s\S]*OperationKind::Branch: true[\s\S]*OperationKind::Loop: true[\s\S]*ConditionFactRootBindingUnexpected/, "generic fact validator must allow exactly Branch/Loop bindings and reject non-control bindings");
assert.match(inventoryPartSource, /branch_payload_inventory_validate_loop branch_payloads operation_kinds places key graph_id/, "central authority must validate Branch headers independently");
assert.match(inventoryPartSource, /condition_fact_inventory_validate_loop condition_fact_nodes condition_fact_edges condition_fact_bindings operation_kinds places key graph_id/, "central authority must validate the generic fact owner without a bypass");
assertOrdered(topLevelBlock(inventoryPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_inventory_scope_authority_result"), ["inventory_operation_topology_validate_result", "branch_payload_inventory_validate_loop", "condition_fact_inventory_validate_loop"], "central authority must validate topology, Branch header, then generic facts");
assert.match(anchorSource, /memo_call_backend_scope_loop_payload_part" as @merge/, "proof-gate anchor must merge Loop payload privately");
assert.match(anchorSource, /memo_call_backend_scope_loop_stage0_part" as @merge/, "proof-gate anchor must merge Loop stage0 privately");
assert.doesNotMatch(loopPayloadPartSource, /^pub\s/m, "Loop payload part must not expose public declarations");
assert.doesNotMatch(loopStage0PartSource, /^pub\s/m, "Loop stage0 part must not expose public declarations");
assert.match(loopPayloadPartSource, /struct SelfhostMemoCallBackendPrivateCacheResourceIrLoopPayloadRecord:[\s\S]*key %[\s\S]*graph_id %[\s\S]*operation_ordinal %i32[\s\S]*condition %SelfhostMemoCallBackendPrivateCacheResourceIrReturnPlace/, "Loop sparse header must retain only identity, ordinal, and condition Place");
assert.doesNotMatch(topLevelBlock(loopPayloadPartSource, "struct", "SelfhostMemoCallBackendPrivateCacheResourceIrLoopPayloadRecord"), /condition_ops|body_ops|condition_fact|root|range|start|count/, "Loop header must not duplicate topology ranges or generic fact roots");
assertOrdered(topLevelBlock(loopPayloadPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_loop_payload_inventory_validate_loop"), ["LoopPayloadUnexpected payload_idx", "OperationKind::Loop", "LoopPayloadMissing operation_idx", "LoopPayloadIdentityMismatch operation_idx", "LoopPayloadOrdinalMismatch payload.operation_ordinal", "loop_record_validate_result"], "Loop sparse owner must reject excess, gaps, identity mismatch, ordinal mismatch, and malformed Place");
assertOrdered(topLevelBlock(inventoryPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_inventory_scope_authority_result"), ["inventory_operation_topology_validate_result", "branch_payload_inventory_validate_loop", "loop_payload_inventory_validate_loop", "condition_fact_inventory_validate_loop"], "central authority must validate topology, Branch header, Loop header, then generic facts");
assert.match(topLevelBlock(inventoryPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_inventory_scope_with_empty_call_owners_result"), /loop_payload_inventory_new[\s\S]*inventory_scope_authority_result[\s\S]*loop_payload_inventory_free/, "compatibility wrapper must own and close the empty Loop payload table");
assert.match(loopStage0PartSource, /operation_kind_inventory_stage0_result key graph_id 23/, "Loop stage0 must build the exact Loop global ordinal fixture");
assert.match(loopStage0PartSource, /condition_fact_inventory_validate_loop &nodes &edges &bindings kinds places key graph_id 0 23/, "Loop stage0 must pass a real Loop binding through the generic fact validator");
assertOrdered(topLevelBlock(loopStage0PartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_loop_stage0_payload_push_result"), ["if eq mode 6 23", "if eq mode 3 0 22"], "Loop ordinal mismatch fixture must remain after the Loop operation instead of colliding with the preceding Branch");
...
assert.match(anchorSource, /memo_call_backend_scope_match_variant_membership_part" as @merge/, "proof-gate anchor must merge Match enum membership privately");
...
assert.doesNotMatch(matchVariantMembershipPartSource, /^pub\s/m, "Match enum membership part must not expose public declarations");
...
assert.doesNotMatch(topLevelBlock(selfhostTypeRecord, "enum", "SelfhostTypeRecord"), /Enum|Variant/, "TypeArena records do not yet carry enum declaration or variant membership authority");
assert.doesNotMatch(topLevelBlock(selfhostTypeConstructor, "enum", "SelfhostTypeConstructorKind"), /Enum|Struct|Variant/, "type constructor arity must not be mistaken for enum declaration membership");
const matchPatternValidator = topLevelBlock(matchPayloadPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_pattern_validate_result");
assert.match(matchPatternValidator, /MatchPattern::Variant symbol:[\s\S]*call_symbol_member symbols symbol\.canonical/, "current Match Variant transport must validate canonical spelling membership");
assert.doesNotMatch(matchPatternValidator, /bind_source_name|MatchBindSourceName|type_arena|TypeArena|PlaceInventory/, "canonical transport validation must stay separate from typed enum membership");
assertOrdered(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_scrutinee_nominal_result"), ["place_inventory_get places scrutinee.place_id.index", "selfhost_type_arena_get_record types place.ty", "SelfhostTypeRecord::Named", "SelfhostTypeRecord::Applied", "MatchScrutineeTypeUnsupported"], "Variant membership must resolve the scrutinee Place type through Named and Applied nominal identities");
assertOrdered(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_validate_result"), ["match_enum_definition_validate_loop", "match_variant_membership_payload_loop"], "Match semantic validation must first close the borrowed enum owner before checking patterns");
assert.match(matchVariantMembershipPartSource, /match_enum_definition_find_loop definitions nominal_id[\s\S]*match_enum_variant_member_loop variants symbol\.canonical definition\.variant_start/, "Variant symbols must be members of the scrutinee nominal definition range");
assertOrdered(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_enum_definition_validate_loop"), ["MatchEnumDefinitionIdentityMismatch", "match_enum_definition_duplicate_loop", "MatchEnumDefinitionDuplicate", "MatchEnumDefinitionRangeInvalid"], "enum definition validation must reject duplicate nominal authority before consuming its range");
assertOrdered(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_stage0_owner_error_matches"), ["MatchEnumDefinitionIdentityMismatch", "MatchEnumDefinitionDuplicate", "MatchEnumDefinitionRangeInvalid", "MatchEnumVariantIdentityMismatch", "MatchEnumVariantOrdinalMismatch", "MatchEnumVariantSymbolMissing", "MatchEnumVariantDuplicate", "MatchEnumVariantUnexpected"], "runtime owner matrix must exact-match every enum owner error class");
assert.match(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_stage0_types_result"), /eq mode 11[\s\S]*selfhost_type_arena_add_applied_named[\s\S]*eq mode 13 eq mode 14[\s\S]*selfhost_type_arena_add_primitive/, "runtime type matrix must cover Applied and nonnominal scrutinee records");
assert.match(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_stage0_semantic_case"), /eq mode 14[\s\S]*MatchPattern::IntLiteral[\s\S]*MatchVariantNotMember[\s\S]*MatchScrutineeTypeUnsupported/, "runtime semantic matrix must cover non-Variant nonnominal acceptance, wrong member, and nonnominal Variant rejection");
assert.match(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_stage0_matrix"), /stage0_owner_case 0 10[\s\S]*stage0_semantic_case 10 15/, "runtime enum membership matrix must execute every owner and semantic mode");
assert.doesNotMatch(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_validate_result"), /bind_source_name|MatchBindSourceName/, "bind-source names must not participate in enum member identity");
assert.doesNotMatch(topLevelBlock(matchVariantMembershipPartSource, "struct", "SelfhostMemoCallBackendPrivateCacheResourceIrMatchEnumDefinitionRecord"), /SelfhostTypeId|canonical_type|spelling|name/, "enum membership authority must stay keyed only by nominal identity and its dense resolved member range");
assertOrdered(topLevelBlock(matchVariantMembershipPartSource, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_compat_validate_result"), ["match_enum_definition_inventory_new", "match_enum_variant_inventory_new", "match_variant_membership_validate_result", "match_enum_variant_inventory_free", "match_enum_definition_inventory_free"], "compatibility path must own, validate, and close both enum membership tables");
...
assert.match(inventoryPartSource, /MatchPayloadRecord key graph_id 23 place0 place0 false 0 1[\s\S]*MatchArmRecord key graph_id 23 0[\s\S]*MatchPattern::Variant/, "central fixture must pass a typed Variant Match arm through the shared authority");
assert.match(inventoryPartSource, /inventory_scope_authority_result key graph_id[\s\S]*Result::Ok scope:[\s\S]*match_variant_membership_compat_validate_result &match_payloads &match_arms places types symbols key control_fixture[\s\S]*Result::Ok _match_variant_membership: Result::Ok scope/, "central compatibility path must validate structural authority before borrowing validated Match ranges for enum membership");
assert.match(inventoryPartSource, /branch_central_stage0_case 0[\s\S]*branch_central_stage0_case 1[\s\S]*branch_central_stage0_case 2[\s\S]*branch_central_stage0_case 3/, "central matrix must execute positive, topology-negative, wrong-enum-owner, and duplicate-definition cases");
assert.match(inventoryPartSource, /MatchEnumDefinitionMissing ordinal: eq ordinal 23/, "wrong-owner central case must match the exact Match operation ordinal");
assert.match(inventoryPartSource, /MatchEnumDefinitionDuplicate ordinal: eq ordinal 1/, "duplicate enum definition central case must match the second definition ordinal");
assert.match(inventoryPartSource, /if or eq mode 1 eq mode 2 2[\s\S]*OperationTopologyRejected topology_error/, "topology-negative central case must outrank a simultaneous wrong enum owner");
...
for (const errorName of ["MatchEnumDefinitionIdentityMismatch", "MatchEnumDefinitionDuplicate", "MatchEnumDefinitionRangeInvalid", "MatchEnumVariantUnexpected", "MatchEnumVariantIdentityMismatch", "MatchEnumVariantOrdinalMismatch", "MatchEnumVariantSymbolMissing", "MatchEnumVariantDuplicate", "MatchScrutineePlaceReadFailed", "MatchScrutineeTypeMissing", "MatchScrutineeTypeUnsupported", "MatchEnumDefinitionMissing", "MatchVariantNotMember"]) {
    assert.match(inventoryPartSource, new RegExp(`${errorName} %i32`), `Match enum membership typed error ${errorName} must remain declared`);
}
...
assert.match(anchorSource, /match_stage0_matrix \(\) selfhost_memo_call_backend_private_cache_resource_ir_match_variant_membership_stage0_matrix \(\)/, "proof-gate lifecycle must execute Match enum membership after structural Match matrix");
...
for (const errorName of ["BranchPayloadMissing", "BranchPayloadUnexpected", "BranchPayloadIdentityMismatch", "BranchPayloadOrdinalMismatch", "BranchOutputMissing", "BranchConditionMissing", "BranchThenValueMissing", "BranchElseValueMissing", "ConditionFactRootInvalid", "ConditionFactNodeIdentityMismatch", "ConditionFactNodeOrdinalMismatch", "ConditionFactNodeOwnerMismatch", "ConditionFactEdgeRangeInvalid", "ConditionFactEdgeIdentityMismatch", "ConditionFactEdgeOrdinalMismatch", "ConditionFactEdgeParentMismatch", "ConditionFactEdgeChildMissing", "ConditionFactEdgeChildOwnerMismatch", "ConditionFactLeafHasChildren", "ConditionFactPreorderInvalid", "ConditionFactEdgeUnexpected", "ConditionFactPlaceMissing", "ConditionFactPlaceGraphMismatch", "ConditionFactRootBindingMissing", "ConditionFactRootBindingUnexpected", "ConditionFactRootBindingIdentityMismatch", "ConditionFactRootBindingOrdinalMismatch", "ConditionFactRootMissing", "ConditionFactUnexpected"]) {
    assert.ok(branchStage0PartSource.includes(errorName), `Branch stage0 matrix must typed-match ${errorName}`);
}
assert.match(anchorSource, /operation_topology_stage0_matrix \(\) selfhost_memo_call_backend_private_cache_resource_ir_branch_stage0_matrix \(\)/, "proof-gate lifecycle must execute Branch matrix with recursive topology matrix");
...
assert.match(inventoryPartSource, /branch_central_stage0_case 0[\s\S]*branch_central_stage0_case 1[\s\S]*branch_central_stage0_case 2[\s\S]*branch_central_stage0_case 3/, "central fixture must execute positive, topology-negative, enum-owner-negative, and duplicate-definition cases");
assert.match(inventoryPartSource, /if eq mode 1 22 23[\s\S]*OperationTopologyRejected topology_error[\s\S]*ScopeParentInvalid scope_id: eq scope_id 6/, "central mismatch negative must remove exactly the Match operation and match the MatchArm scope taxonomy");
...
assert.match(inventoryPartSource, /gt control_fixture 0[\s\S]*branch_stage0_payload_push_result[\s\S]*branch_stage0_bindings_fill_result[\s\S]*branch_stage0_nodes_fill_result/, "central fixture must pass nonempty Branch and fact owners to the shared authority path");
...
for (const partSource of [conditionFactPartSource, branchPayloadPartSource, branchStage0PartSource, loopPayloadPartSource, loopStage0PartSource, matchPayloadPartSource, matchVariantMembershipPartSource, matchStage0PartSource]) {
...
assert.match(conditionFactPartSource, /impl Copy for SelfhostMemoCallBackendPrivateCacheResourceIrConditionFactRootBindingRecord/);
assert.doesNotMatch(
    inventoryPartSource,
    /^pub\s/m,
...
assertOrdered(
    ...
    [
        ...
        "FunctionParameterTableAllocFailed e",
    ],
...
assertOrdered(
    ...
    [
        ...
        "selfhost_type_parameter_record_binding parameter",
        "selfhost_type_parameter_binding_is_valid binding",
        "FunctionTypeParameterBindingInvalid SelfhostMemoCallBackendPrivateCacheResourceFunctionTypeParameterBindingError ordinal type_index binder_depth parameter_index",
        "binder_depth 0",
        "parameter_index ordinal",
        "FunctionTypeParameterBindingMismatch SelfhostMemoCallBackendPrivateCacheResourceFunctionTypeParameterBindingError ordinal type_index binder_depth parameter_index",
        "resource_ir_function_inventory_validate_type_params_loop inventory types add ordinal 1 count",
        "FunctionTypeParameterKindMismatch SelfhostMemoCallBackendPrivateCacheResourceFunctionTypeParameterError ordinal type_index",
...
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_function_inventory_push_parameter"),
    [
        'field::get inventory "type_params"',
        'field::get inventory "params"',
        'field::get inventory "span"',
        ...
        "v::free type_params",
        "v::free blocks",
        "FunctionParameterPushFailed error",
    ],
    "parameter push must preserve every other function owner and close consumed owners on failure",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_function_inventory_parameter_duplicate_before_loop"),
    [
        "previous.local_identity parameter.local_identity",
        "FunctionParameterIdentityDuplicate end",
        "resource_place_id_eq previous.place_id parameter.place_id",
        "FunctionParameterPlaceDuplicate end",
    ],
    "parameter identity and Place ownership must be unique under distinct typed errors",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_function_inventory_validate_parameters_loop"),
    [
        "resource_ir_function_inventory_parameter_get inventory ordinal",
        "resource_ir_function_inventory_parameter_duplicate_before_loop inventory parameter 0 ordinal",
        "parameter.local_identity 0",
        "FunctionParameterIdentityInvalid ordinal",
        "selfhost_type_id_index parameter.ty 0",
        "FunctionParameterTypeInvalid ordinal",
        "selfhost_type_arena_get_record types parameter.ty",
        "FunctionParameterTypeMissing ordinal",
        "resource_ir_place_inventory_get places parameter.place_id.index",
        "FunctionParameterPlaceMissing ordinal",
        "SelfhostMemoCallBackendPrivateCacheResourceIrPlaceRoot::Local stable_symbol_identity",
        "FunctionParameterPlaceIdentityMismatch SelfhostMemoCallBackendPrivateCacheResourceFunctionParameterIdentityMismatch ordinal stable_symbol_identity parameter.local_identity",
        "selfhost_type_id_eq place.ty parameter.ty",
        "FunctionParameterPlaceTypeMismatch ordinal",
        "place.projection_count 0",
        "FunctionParameterPlaceProjected ordinal",
        "FunctionParameterPlaceRootKindMismatch ordinal",
    ],
...
assertOrdered(
    ...
    "scope authority must validate Expr, DeclareLocal, Read, then Assign sparse payloads before retaining the non-production origin",
);
assert.doesNotMatch(
...
for (const token of [
    "SelfhostMemoCallBackendPrivateCacheResourceIrAssignPayloadRecord",
    "target %SelfhostMemoCallBackendPrivateCacheResourceIrReturnPlace",
    "value %SelfhostMemoCallBackendPrivateCacheResourceIrReturnPlace",
    "SelfhostMemoCallBackendPrivateCacheResourceIrAssignPayloadInventory",
    "AssignPayloadMissing",
    "AssignPayloadUnexpected",
    "AssignPayloadIdentityMismatch",
    "AssignPayloadOrdinalMismatch",
    "AssignTargetMissing",
    "AssignTargetGraphMismatch",
    "AssignValueMissing",
    "AssignValueGraphMismatch",
]) assert.match(inventoryPartSource, new RegExp(token), `Assign payload contract must retain ${token}`);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_assign_payload_record_validate_result"),
    ["AssignPayloadIdentityMismatch", "AssignPayloadOrdinalMismatch", "assign_place_validate_result record.target", "assign_place_validate_result record.value"],
    "Assign must validate payload identity and ordinal before both graph-bound Place handles",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_assign_payload_inventory_validate_loop"),
    ["operation_kind_inventory_get kinds operation_idx", "resource_ir_operation_kind_is_assign operation.kind", "AssignPayloadMissing operation_idx", "assign_payload_record_validate_result", "AssignPayloadUnexpected operation_idx"],
    "Assign sparse payloads must be joined to operation kinds with a fail-closed merge cursor",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_assign_payload_stage0"),
    ["stage0_case 0", "stage0_case 1", "stage0_case 2", "stage0_case 3", "stage0_case 4", "stage0_case 5", "stage0_case 6", "stage0_case 7", "stage0_case 8", "stage0_case 9"],
    "Assign runtime matrix must cover positive, sparse join, identity, ordinal, and both Place boundaries",
);
assert.match(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_inventory_scope_stage0"),
    /resource_ir_assign_payload_stage0/,
    "the public inventory runtime facade must execute the private Assign payload matrix",
);
for (const token of [
    "SelfhostMemoCallBackendPrivateCacheResourceIrBorrowPayloadRecord", "kind %SelfhostMemoCallBackendPrivateCacheResourceIrBorrowKind", "synthetic %bool",
    "SelfhostMemoCallBackendPrivateCacheResourceIrBorrowPayloadInventory", "BorrowPayloadMissing", "BorrowPayloadUnexpected",
    "BorrowPayloadIdentityMismatch", "BorrowPayloadOrdinalMismatch", "BorrowSourceMissing", "BorrowSourceGraphMismatch", "BorrowOutputMissing", "BorrowOutputGraphMismatch",
]) assert.match(inventoryPartSource, new RegExp(token), `Borrow payload contract must retain ${token}`);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_borrow_payload_record_validate_result"),
    ["BorrowPayloadIdentityMismatch", "BorrowPayloadOrdinalMismatch", "borrow_place_validate_result record.source", "borrow_place_validate_result record.output"],
    "Borrow must validate identity and ordinal before both graph-bound Place handles",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_borrow_payload_inventory_validate_loop"),
    ["operation_kind_inventory_get kinds operation_idx", "resource_ir_operation_kind_is_borrow operation.kind", "BorrowPayloadMissing operation_idx", "borrow_payload_record_validate_result", "BorrowPayloadUnexpected operation_idx"],
    "Borrow sparse payloads must use a fail-closed merge cursor",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_borrow_payload_stage0"),
    ["stage0_case 0", "stage0_case 1", "stage0_case 2", "stage0_case 3", "stage0_case 4", "stage0_case 5", "stage0_case 6", "stage0_case 7", "stage0_case 8", "stage0_case 9", "stage0_case 10", "stage0_case 11", "stage0_case 12", "stage0_case 13"],
    "Borrow runtime matrix must cover failures and all kind/synthetic combinations",
);
assert.match(topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_inventory_scope_stage0"), /resource_ir_borrow_payload_stage0/, "public runtime must execute Borrow matrix");
for (const token of [
    "SelfhostMemoCallBackendPrivateCacheResourceIrMovePayloadRecord", "SelfhostMemoCallBackendPrivateCacheResourceIrMovePayloadInventory",
    "MovePayloadMissing", "MovePayloadUnexpected", "MovePayloadIdentityMismatch", "MovePayloadOrdinalMismatch",
    "MoveSourceMissing", "MoveSourceGraphMismatch", "MoveOutputMissing", "MoveOutputGraphMismatch",
]) assert.match(inventoryPartSource, new RegExp(token), `Move payload contract must retain ${token}`);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_move_payload_record_validate_result"),
    ["MovePayloadIdentityMismatch", "MovePayloadOrdinalMismatch", "move_place_validate_result record.source", "move_place_validate_result record.output"],
    "Move must validate identity and ordinal before both graph-bound Place handles",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_move_payload_inventory_validate_loop"),
    ["operation_kind_inventory_get kinds operation_idx", "resource_ir_operation_kind_is_move operation.kind", "MovePayloadMissing operation_idx", "move_payload_record_validate_result", "MovePayloadUnexpected operation_idx"],
    "Move sparse payloads must use a fail-closed merge cursor",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_move_payload_stage0"),
    ["stage0_case 0", "stage0_case 1", "stage0_case 2", "stage0_case 3", "stage0_case 4", "stage0_case 5", "stage0_case 6", "stage0_case 7", "stage0_case 8", "stage0_case 9"],
...
for (const symbol of [
    "SelfhostMemoCallBackendPrivateCacheResourceIrEndScopePayloadRecord", "SelfhostMemoCallBackendPrivateCacheResourceIrEndScopePayloadInventory",
    "SelfhostMemoCallBackendPrivateCacheResourceIrEndScopeLocalRecord", "SelfhostMemoCallBackendPrivateCacheResourceIrEndScopeLocalInventory",
    "EndScopePayloadMissing", "EndScopePayloadUnexpected", "EndScopePayloadIdentityMismatch", "EndScopePayloadOrdinalMismatch",
    "EndScopeLocalRangeInvalid", "EndScopeLocalMissing", "EndScopeLocalUnexpected", "EndScopeLocalOrdinalMismatch",
    "EndScopeLocalPlaceMissing", "EndScopeLocalPlaceGraphMismatch", "EndScopeResultPlaceMissing", "EndScopeResultPlaceGraphMismatch",
]) assert.match(source, new RegExp(`\\b${symbol}\\b`), `EndScope structural payload contract must contain ${symbol}`);
assertOrdered(topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_end_scope_payload_record_validate_result"), ["EndScopePayloadIdentityMismatch", "EndScopePayloadOrdinalMismatch", "EndScopeLocalRangeInvalid", "EndScopeResultPlaceMissing", "EndScopeResultPlaceGraphMismatch"], "EndScope header must reject identity/range/result failures in deterministic order");
assertOrdered(topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_end_scope_payload_inventory_validate_loop"), ["resource_ir_operation_kind_is_end_scope", "EndScopePayloadMissing", "end_scope_local_range_validate_loop", "EndScopePayloadUnexpected"], "EndScope sparse header and dense local endpoint owners must merge against operation kinds");
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_end_scope_payload_stage0_error_matches"),
    ["EndScopePayloadMissing", "EndScopePayloadUnexpected", "EndScopePayloadIdentityMismatch", "EndScopePayloadOrdinalMismatch", "EndScopeLocalRangeInvalid", "EndScopeLocalMissing", "EndScopeLocalUnexpected", "EndScopeLocalOrdinalMismatch", "EndScopeLocalPlaceMissing", "EndScopeLocalPlaceGraphMismatch", "EndScopeResultPlaceMissing", "EndScopeResultPlaceGraphMismatch"],
    "EndScope runtime errors must retain exact typed variants and ordinals",
);
assertOrdered(
    topLevelBlock(source, "fn", "selfhost_memo_call_backend_private_cache_resource_ir_end_scope_payload_stage0"),
    Array.from({ length: 20 }, (_, mode) => `stage0_case ${mode}`),
    "EndScope runtime matrix must cover three positives and every sparse header, dense endpoint, range, identity, ordinal, and Place rejection",
);
...
assertOrdered(
    ...
    [
        ...
        "FunctionTypeParameterKindMismatch detail",
        "FunctionTypeParameterBindingMismatch detail",
        "FunctionParameterIdentityInvalid ordinal",
        "FunctionParameterTypeInvalid ordinal",
        "FunctionParameterTypeMissing ordinal",
        "FunctionParameterPlaceMissing ordinal",
        "FunctionParameterPlaceIdentityMismatch detail",
        "FunctionParameterPlaceTypeMismatch ordinal",
        "FunctionParameterPlaceRootKindMismatch ordinal",
        "FunctionParameterPlaceProjected ordinal",
