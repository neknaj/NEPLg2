...
assert.match(apiCode, /fn\s+insert\s+<\(SparseSet,i32\)\*>Result<SparseSet,\s*SparseSetUpdateError>>\s+\(s,\s*value\):/, "SparseSet.insert must return an owner-carrying error type");
assert.match(apiCode, /fn\s+remove\s+<\(SparseSet,i32\)\*>Result<SparseSet,\s*SparseSetUpdateError>>\s+\(s,\s*value\):/, "SparseSet.remove must return an owner-carrying error type");
