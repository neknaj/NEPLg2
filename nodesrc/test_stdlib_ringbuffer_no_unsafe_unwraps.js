...
assert.match(code, /fn\s+free\s+<\.T:\s*Copy>\s+<\(RingBuffer<\.T>\)->\(\)>[\s\S]*vec::free<Option<\.T>>\s+field::get\s+rb\s+"items"/, 'RingBuffer.free must close the Vec<Option<T>> owner through the Copy storage fast path');
