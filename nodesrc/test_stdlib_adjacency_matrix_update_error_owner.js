...
assert.match(apiCode, /fn\s+insert\s+<\(AdjacencyMatrix,i32,i32\)\*>Result<AdjacencyMatrix,\s*AdjacencyMatrixUpdateError>>\s+\(g,\s*from,\s*to\):/, "AdjacencyMatrix.insert must return an owner-carrying error type");
assert.match(apiCode, /fn\s+remove\s+<\(AdjacencyMatrix,i32,i32\)\*>Result<AdjacencyMatrix,\s*AdjacencyMatrixUpdateError>>\s+\(g,\s*from,\s*to\):/, "AdjacencyMatrix.remove must return an owner-carrying error type");
