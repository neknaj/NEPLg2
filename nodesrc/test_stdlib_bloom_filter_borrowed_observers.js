...
assert.match(
    code,
    /fn\s+len\s+<\.T,\.H>\s+<\(&BloomFilter<\.T,\.H>\)->i32>\s+\(bf\):/,
    "BloomFilter.len must borrow the owner",
);
assert.doesNotMatch(code, /fn\s+len\s+<\.T,\.H>\s+<\(BloomFilter<\.T,\.H>\)->i32>/, "BloomFilter.len must not consume the owner");
