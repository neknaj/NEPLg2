...
const file = path.join(repoRoot, "tests", "stdlib", "neplg2_module_loader.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [5, 2];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "selfhost module loader doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `selfhost module loader doctest#${index + 1}`;
...
console.log("selfhost module loader report contract passed");
