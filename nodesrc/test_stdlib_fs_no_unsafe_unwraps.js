...
assert.match(pathNormalizeCode, /fn\s+fs_normalize_relative_builder_raw_errno\s+<\(str\)\*>Result<StringBuilder,i32>>\s+\(path\):[\s\S]*str_split_next\s+path\s+"\/"\s+cursor[\s\S]*match\s+get\s+step\s+"kind":[\s\S]*StrSplitStepKind::Part:/, 'fs_normalize_relative_builder raw boundary must scan path components with allocation-free split steps and an explicit impure owner effect');
...
assert.match(pathNormalizeRangeStackCode, /\bfn\s+fs_normalize_range_push_raw_errno\s+<\(Vec<i32>,i32,i32\)\*>Result<Vec<i32>,i32>>[\s\S]*\bv::push\s+stack\s+start[\s\S]*\bv::push\s+with_start\s+end/, 'range-stack push must stay in normalize/range_stack with an explicit impure owner effect');
assert.match(pathNormalizeRangeStackCode, /\bfn\s+fs_normalize_range_push\s+<\(Vec<i32>,i32,i32\)\*>Result<Vec<i32>,FsError>>[\s\S]*fs_normalize_range_push_raw_errno\s+stack\s+start\s+end[\s\S]*fs_error_from_errno\s+FsOperation::NormalizePathBuilder\s+e/, 'range-stack normal push must be a typed wrapper over the raw errno boundary');
assert.match(pathNormalizeRangeStackCode, /\bfn\s+fs_normalize_range_pop\s+<\(Vec<i32>\)\*>Vec<i32>>[\s\S]*\bv::pop\s+stack[\s\S]*\bfield::get\s+popped_end\s+"vec"[\s\S]*\bv::pop\s+without_end[\s\S]*\bfield::get\s+popped_start\s+"vec"/, 'range-stack pop must stay in normalize/range_stack with an explicit impure owner effect');
...
assert.match(pathNormalizeCode, /fn\s+fs_normalize_relative_raw_errno\s+<\(str\)\*>Result<str,i32>>\s+\(path\):[\s\S]*fs_normalize_relative_builder_raw_errno\s+path[\s\S]*sb_build_result\s+sb/, 'fs_normalize_relative raw boundary must delegate through the builder boundary with an explicit impure owner effect');
