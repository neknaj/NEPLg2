...
assert.match(writeFdCode, /\bfn\s+fs_write_fd_mem_result\b[\s\S]*\balloc_region<u8>\s+8[\s\S]*\blet\s+iov_buf\s+<MemPtr<u8>>\s+region_ptr\s+&iov_region[\s\S]*\balloc_region<u8>\s+4[\s\S]*\blet\s+nwritten_buf\s+<MemPtr<u8>>\s+region_ptr\s+&nwritten_region[\s\S]*\bfs_fd_write_from_result\b[\s\S]*\bdealloc_region<u8>\s+nwritten_region[\s\S]*\bdealloc_region<u8>\s+iov_region/, 'fd write loop must own iovec/nwritten scratch through RegionToken and keep raw ABI layout inside std/fs/write/fd');
...
assert.match(pathEntryCode, /\bfn\s+fs_sort_strings\s+<\(&Vec<str>\)\*>Result<unit,i32>>\s+\(entries\):[\s\S]*\bv::get<str>\s+entries\s+i[\s\S]*\bv::replace<str>\s+entries\s+j\s+key/, 'directory entry sort must use Vec public get/replace boundary with an explicit mutation effect');
...
assert.match(dirReadFdCode, /\bmatch\s+fs_sort_strings\s+&entries:[\s\S]*Result::Err\s+e:[\s\S]*v::free<str>\s+entries[\s\S]*Result::Err\s+e/, 'fs_read_dir_fd must sort through the Vec boundary and free entries if sorting reports an invariant error');
