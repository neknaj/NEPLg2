...
assert.match(dirReadFdCode, /fn\s+fs_read_dir_fd\s+<\(i32\)\*>Result<Vec<str>,i32>>\s+\(fd\):[\s\S]*\bwasi_fd_readdir\b[\s\S]*match\s+v::push\s+entries\s+name:[\s\S]*Result::Err\s+e:[\s\S]*set\s+entries\s+v::vec_push_error_vec\s+e[\s\S]*set\s+err\s+12/, 'fs_read_dir_fd must preserve the entry Vec owner while mapping accumulation push failure to errno 12');
