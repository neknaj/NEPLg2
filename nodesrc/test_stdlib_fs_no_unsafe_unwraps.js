...
assert.match(pathEntryCode, /\bfn\s+fs_sort_strings\s+<\(&Vec<str>\)\*>Result<unit,i32>>\s+\(entries\):[\s\S]*\bv::get\s+entries\s+i[\s\S]*\bv::replace<str>\s+entries\s+j\s+key/, 'directory entry sort must use Vec public get/replace boundary with an explicit mutation effect');
...
assert.match(dirReadFdCode, /fn\s+fs_read_dir_fd\s+<\(i32\)\*>Result<Vec<str>,i32>>\s+\(fd\):[\s\S]*\bwasi_fd_readdir\b[\s\S]*match\s+v::push<str>\s+entries\s+name:[\s\S]*Result::Err\s+e:[\s\S]*set\s+entries\s+v::vec_push_error_vec\s+e[\s\S]*set\s+err\s+12/, 'fs_read_dir_fd must preserve the entry Vec owner while mapping accumulation push failure to errno 12');
