...
assert.match(apiCleanupCode, /fn\s+free\s+<\(BitSet\)->unit>[\s\S]*field::get\s+bs\s+"bits"[\s\S]*vec::free<u8>\s+bits/, 'BitSet.free must consume and close typed Vec<u8> storage');
