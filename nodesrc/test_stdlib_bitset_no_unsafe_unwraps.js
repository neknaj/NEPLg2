...
assert.match(storageCode, /fn\s+bitset_alloc_bits\s+<\(i32,i32\)\*>Result<Vec<u8>,\s*Diag>>[\s\S]*vec::filled\s+nbytes\s+byte/, 'BitSet must allocate initialized typed byte storage through Vec.filled');
assert.match(storageCode, /fn\s+bitset_byte_at\s+<\(&Vec<u8>,i32\)->Option<i32>>[\s\S]*vec::get\s+bits\s+byte_idx[\s\S]*Option::None:[\s\S]*none/, 'BitSet must read bit bytes through Vec.get and expose missing bytes as Option');
assert.match(storageCode, /fn\s+bitset_store_byte\s+<\(&Vec<u8>,i32,i32\)\*>unit>[\s\S]*vec::replace\s+bits\s+byte_idx\s+byte/, 'BitSet must update bit bytes through Vec.replace');
...
assert.match(apiUpdateCode, /let\s+e\s+<BitSetUpdateError>\s+BitSetUpdateError\s+bs\s+d[\s\S]*err\s+e/, 'BitSet mutating Err paths must return the input owner in BitSetUpdateError');
