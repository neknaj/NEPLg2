...
async function run() {
    ...
    const result = await runSingle({
        id: "gui-font-web-registered-two-run-supplied-owner/begin",
        source: `#entry main
#indent 4
#target std
#import "platforms/gui/web/font_registered_two_run_supplied_owner_test" as * with tests

fn main %impure fn void i32 \\void:
    gui_font_web_registered_two_run_supplied_owner_test_contract unit
`,
        file: path.resolve(__dirname, "..", "tests", "gui_font_web_registered_two_run_supplied_owner.nepl"),
    ...
    assert.equal(result.return_value, 2);
    assert.deepEqual(calls, { begin: 1, run: 0, end: 0 });
    return { ok: true, evidence: 2, calls };
}
