...
assert.match(code.diags, /fn\s+diags_one\s+<\(Diag\)\*>Diags>\s+\(d\):[\s\S]*match\s+v::push<Diag>\s+items0\s+d:[\s\S]*Result::Err\s+_e:[\s\S]*Diags\s+diag_empty_diag_vec/, 'diags_one must convert push failure to an empty Diags sentinel');
assert.match(code.diags, /fn\s+diags_push\s+<\(Diags,Diag\)\*>Diags>\s+\(ds,\s*d\):[\s\S]*match\s+v::push<Diag>\s+field::get\s+ds\s+"items"\s+d:[\s\S]*Result::Err\s+_e:[\s\S]*Diags\s+diag_empty_diag_vec/, 'diags_push must convert grow failure to an empty Diags sentinel');
