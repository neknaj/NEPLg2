...
function runWebGuiMandelbrotTransportContractRegression() {
    ...
    assert.match(mandelbrotSource, /--video-memory-progressive-once/);
    assert.match(mandelbrotSource, /--video-memory-progressive-test/);
    assert.match(mandelbrotSource, /--video-memory-progressive-loop-test/);
    ...
    assert.match(mandelbrotSource, /gui_web_video_memory_publish_rect/);
    ...
    assert.match(mandelbrotSource, /fn mandelbrot_video_memory_publish_rect_present[\s\S]*gui_web_video_memory_publish_rect/);
    assert.match(mandelbrotSource, /fn mandelbrot_video_memory_render_batch_present[\s\S]*mandelbrot_video_memory_publish_rect_present/);
    assert.match(mandelbrotSource, /fn mandelbrot_run_video_memory_progressive_test[\s\S]*mandelbrot_run_video_memory_progressive_once/);
    assert.match(mandelbrotSource, /fn mandelbrot_video_memory_progressive_event_is_tick[\s\S]*gui_web_event_timer[\s\S]*timer_event_timer_id/);
    assert.match(mandelbrotSource, /fn mandelbrot_video_memory_progressive_loop_with_limit[\s\S]*gui_web_wait_event_result[\s\S]*mandelbrot_video_memory_render_batch_present/);
    ...
    assert.match(specSource, /`--video-memory-progressive-once`/);
    assert.match(specSource, /`--video-memory-progressive-loop-test`/);
    ...
    assert.match(planSource, /`--video-memory-progressive-once`/);
    assert.match(planSource, /`--video-memory-progressive-loop-test`/);
    ...
    assert.match(planSource, /`--video-memory-progressive-once` は row batch ごとの dirty rect publish を検査する finite checkpoint/);
    assert.match(planSource, /matching timer id の event 1 件につき 1 row batch だけ進める finite checkpoint/);
    assert.match(planSource, /FHD 60 fps 実測、formal tiled transport、formal timer registration ABI、real scheduler policy は後続 slice/);
    ...
    assert.match(specSource, /Mandelbrot の progressive video memory path は row batch ごとに dirty rect を publish する/);
    assert.match(specSource, /Timer event driven progressive loop checkpoint は matching timer id の event だけを batch 進行条件にし/);
    assert.match(specSource, /formal timer registration ABI、formal tiled rendering、real scheduler policy/);
    ...
    return {
        ...
        checks: [
            ...
            "Mandelbrot progressive video memory path publishes dirty rectangles per row batch",
            "Mandelbrot progressive video memory loop advances batches from typed timer events",
