...
function assertReportMetadataPolicySelfTest() {
    const good = {
...
function scanReportMetadataPolicy(options) {
    ...
    for (const root of roots) {
        for (const file of walkFiles(path.join(repoRoot, root), extension)) {
            const rel = path.relative(repoRoot, file);
            const parsed = parseFile(file);
            parsed.doctests.forEach((doctest, index) => {
                const hits = reportMetadataViolations(rel, doctest, index);
                if (hits.length > 0) {
    ...
    return { violations, reportDoctestCount };
}

module.exports = {
    assertReportMetadataPolicySelfTest,
    reportMetadataViolations,
