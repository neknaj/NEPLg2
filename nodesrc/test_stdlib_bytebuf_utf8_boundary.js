...
assert.match(fsCode, /fn\s+fs_bytes_to_string_result\s+<\(ByteBuf\)\*>Result<str,i32>>\s+\(buf\):[\s\S]*io_bytebuf_to_str_result\s+buf/, 'fs_bytes_to_string_result must use the checked ByteBuf-to-str boundary');
...
assert.match(fsCode, /fn\s+fs_string_from_bytes\s+<\(i32,i32\)->Result<str,i32>>\s+\(src,\s*byte_len\):[\s\S]*string_utf8_validate_mem\s+src_ptr\s+byte_len[\s\S]*string_from_mem_unchecked_result\s+src_ptr\s+byte_len/, 'directory entry byte ranges must be UTF-8 validated before str construction');
