...
assert.match(connector, /selfhost_type_arena_get_record arena scrutinee_type[\s\S]*if:[\s\S]*selfhost_enum_member_qualifier_matches[\s\S]*then:[\s\S]*selfhost_resolved_enum_session_member_lookup_result[\s\S]*else:[\s\S]*OwnerMismatch/, "owner mismatch must guard member lookup");
