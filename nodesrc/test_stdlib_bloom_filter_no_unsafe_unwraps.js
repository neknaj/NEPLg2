...
assert.match(apiCode, /fn\s+len\s+<\.T,\.H>\s+<\(&BloomFilter<\.T,\.H>\)->i32>\s+\(bf\):/, 'BloomFilter.len must borrow the owner and not require Copy or HashKey for metadata-only observation');
...
assert.doesNotMatch(code, /fn\s+free\s+<\.T,\.H>\s+<\(BloomFilter<\.T,\.H>\)->unit>[\s\S]*dealloc_ptr/, 'BloomFilter.free must not unwrap checked deallocation for owned storage');
...
assert.doesNotMatch(apiCode, /fn\s+free\s+<\.T,\.H>\s+<\(BloomFilter<\.T,\.H>\)->unit>/, 'BloomFilter.free must not accept unconstrained key/hasher payloads');
