...
const file = path.join(repoRoot, "tests", "stdlib", "neplg2_stdlib_map.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [8, 9, 1];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "selfhost stdlib map doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `selfhost stdlib map doctest#${index + 1}`;
...
console.log("selfhost stdlib map report contract passed");
