...
const jsonSectionSrc = fs.readFileSync(path.join(repoRoot, jsonSectionPath), 'utf8');
...
const jsonSection = implementationSource(jsonSectionSrc);
...
assert.match(document, /pub\s+struct\s+Document:[\s\S]*source\s+<str>/, 'Document must be a Copy source view');
assert.match(document, /pub\s+fn\s+parse_markdown\s+<\(str\)->Document>\s+\(input\):\s+Document\s+input/, 'parse_markdown must not allocate a non-Copy block AST');
assert.match(document, /pub\s+fn\s+document_to_json\s+<\(Document\)->str>\s+\(doc\):[\s\S]*let\s+src\s+<str>\s+get\s+doc\s+"source"/, 'document_to_json must direct-serialize from source');
