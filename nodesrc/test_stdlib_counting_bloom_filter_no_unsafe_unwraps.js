...
assert.match(code, /fn\s+free\s+<\.T,\.H>\s+<\(CountingBloomFilter<\.T,\.H>\)->\(\)>[\s\S]*dealloc_raw\s+mem_ptr_addr\s+counters\s+nbytes/, 'CountingBloomFilter.free must use raw owner cleanup for counter storage');
