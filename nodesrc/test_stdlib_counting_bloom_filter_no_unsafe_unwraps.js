...
assert.match(storageCode, /fn\s+counting_bloom_alloc_counters\s+<\(i32,i32\)\*>Result<Vec<u8>,\s*Diag>>[\s\S]*vec::filled\s+nslots\s+counter/, 'CountingBloomFilter must allocate initialized counter storage through Vec.filled');
assert.match(storageCode, /fn\s+counting_bloom_counter_at\s+<\(&Vec<u8>,i32\)->Option<i32>>[\s\S]*vec::get\s+counters\s+idx[\s\S]*Option::None:[\s\S]*none/, 'CountingBloomFilter must read counters through Vec.get and expose missing counters as Option');
assert.match(storageCode, /fn\s+counting_bloom_store_counter\s+<\(&Vec<u8>,i32,i32\)\*>unit>[\s\S]*vec::replace\s+counters\s+idx\s+counter/, 'CountingBloomFilter must update counters through Vec.replace');
assert.match(storageCode, /fn\s+counting_bloom_free_counters\s+<\(Vec<u8>\)\*>unit>[\s\S]*vec::free\s+counters/, 'CountingBloomFilter storage module must centralize Vec cleanup through an impure owner-consuming boundary');
assert.match(mutationCode, /fn\s+counting_bloom_counter_inc\s+<\(&Vec<u8>,i32\)\*>unit>[\s\S]*lt\s+cur\s+255[\s\S]*counting_bloom_store_counter\s+counters\s+idx\s+add\s+cur\s+1/, 'CountingBloomFilter mutation module must own saturating increment');
assert.match(mutationCode, /fn\s+counting_bloom_counter_dec\s+<\(&Vec<u8>,i32\)\*>unit>[\s\S]*lt\s+0\s+cur[\s\S]*counting_bloom_store_counter\s+counters\s+idx\s+sub\s+cur\s+1/, 'CountingBloomFilter mutation module must own lower-bounded decrement');
...
assert.match(apiCode, /fn\s+insert\s+<\.T:\s*HashKey&Copy,\.H:\s*Hasher<\.T>&Copy>[\s\S]*counting_bloom_hash0[\s\S]*counting_bloom_counter_inc\s+&counters\s+i0[\s\S]*counting_bloom_counter_inc\s+&counters\s+i2/, 'CountingBloomFilter.insert must use hash and mutation helpers through a borrowed counter owner');
assert.match(apiCode, /fn\s+remove\s+<\.T:\s*HashKey&Copy,\.H:\s*Hasher<\.T>&Copy>[\s\S]*counting_bloom_hash0[\s\S]*counting_bloom_counter_dec\s+&counters\s+i0[\s\S]*counting_bloom_counter_dec\s+&counters\s+i2/, 'CountingBloomFilter.remove must use hash and mutation helpers through a borrowed counter owner');
...
assert.match(apiCode, /fn\s+free\s+<\.T:\s*HashKey&Copy,\.H:\s*Hasher<\.T>&Copy>\s+<\(CountingBloomFilter<\.T,\.H>\)\*>unit>[\s\S]*counting_bloom_free_counters\s+counters/, 'CountingBloomFilter.free must close typed Vec<u8> storage through an impure cleanup helper with Copy-only key/hasher bounds');
...
assert.doesNotMatch(code, /fn\s+free\s+<\.T,\.H>\s+<\(CountingBloomFilter<\.T,\.H>\)->unit>[\s\S]*dealloc_ptr/, 'CountingBloomFilter.free must not unwrap checked deallocation for owned storage');
...
assert.doesNotMatch(apiCode, /fn\s+free\s+<\.T,\.H>\s+<\(CountingBloomFilter<\.T,\.H>\)->unit>/, 'CountingBloomFilter.free must not accept unconstrained key/hasher payloads');
