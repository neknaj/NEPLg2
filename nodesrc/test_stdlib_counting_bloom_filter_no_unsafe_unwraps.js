...
assert.match(apiCode, /fn\s+len\s+<\.T:\s*HashKey&Copy,\.H:\s*Hasher<\.T>&Copy>\s+<\(&CountingBloomFilter<\.T,\.H>\)->i32>\s+\(bf\):/, 'CountingBloomFilter.len must borrow the owner and remain Copy-only while drop traversal is incomplete');
