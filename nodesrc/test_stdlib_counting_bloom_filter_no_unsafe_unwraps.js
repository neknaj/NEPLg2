...
assert.match(storageCode, /fn\s+counting_bloom_free_counters\s+<\(Vec<u8>\)->unit>[\s\S]*vec::free\s+counters/, 'CountingBloomFilter storage module must centralize Vec cleanup');
...
assert.match(apiCode, /fn\s+free\s+<\.T:\s*HashKey&Copy,\.H:\s*Hasher<\.T>&Copy>\s+<\(CountingBloomFilter<\.T,\.H>\)->unit>[\s\S]*counting_bloom_free_counters\s+counters/, 'CountingBloomFilter.free must close typed Vec<u8> storage through cleanup helper with Copy-only key/hasher bounds');
