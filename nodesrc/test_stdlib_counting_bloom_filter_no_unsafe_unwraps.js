...
assert.match(storageCode, /fn\s+counting_bloom_counter_at\s+<\(&Vec<u8>,i32\)->Option<i32>>[\s\S]*vec::get<u8>[\s\S]*Option::None:[\s\S]*none/, 'CountingBloomFilter must read counters through Vec.get and expose missing counters as Option');
assert.match(storageCode, /fn\s+counting_bloom_store_counter\s+<\(&Vec<u8>,i32,i32\)\*>unit>[\s\S]*vec::replace<u8>/, 'CountingBloomFilter must update counters through Vec.replace');
assert.match(storageCode, /fn\s+counting_bloom_free_counters\s+<\(Vec<u8>\)->unit>[\s\S]*vec::free<u8>\s+counters/, 'CountingBloomFilter storage module must centralize Vec cleanup');
