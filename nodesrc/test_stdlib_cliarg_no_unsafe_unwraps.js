...
assert.match(rawCode, /fn\s+cli_load_u8_result\s+<\(MemPtr<u8>,i32\)->Result<i32,i32>>\s+\(base,\s*off\):[\s\S]*Option::None:[\s\S]*Result::Err\s+1/, 'cliarg must map invalid byte loads to shim errno 1');
...
assert.match(cstrCode, new RegExp(String.raw`fn\s+cstr_len_bounded_result\s+<\(MemPtr<u8>,i32\)\*>Result<i32,str>>\s+\(p,\s*max_len\):[\s\S]*le\s+max_len\s+0[\s\S]*let\s+mut\s+([A-Za-z_][A-Za-z0-9_]*)\s+<i32>\s+1[\s\S]*while\s+and\s+and\s+and\s+eq\s+done\s+0\s+eq\s+\1\s+1\s+ge\s+i\s+0\s+lt\s+i\s+max_len[\s\S]*Option::None:[\s\S]*set\s+\1\s+0[\s\S]*eq\s+\1\s+0[\s\S]*Result::Err\s+"cliarg\.cstr_len invalid pointer"[\s\S]*Result::Err\s+"cliarg\.cstr_len missing nul terminator"`), 'bounded cstr length must prove 0 <= i < max_len and reject invalid pointers or missing NUL before reading beyond the caller extent');
assert.match(cstrCode, new RegExp(String.raw`fn\s+cstr_to_str_bounded_result\s+<\(MemPtr<u8>,i32\)\*>Result<str,str>>\s+\(p,\s*max_len\):[\s\S]*cstr_len_bounded_result\s+p\s+max_len[\s\S]*Result::Ok\s+([A-Za-z_][A-Za-z0-9_]*):[\s\S]*string_from_utf8_mem_result\s+p\s+\1`), 'bounded cstr conversion must preserve the caller extent and validate UTF-8 before str construction');
...
assert.match(rawCode, /fn\s+__cli_copy_to_cstr\s+<\(str\)\*>Result<RegionToken<u8>,i32>>[\s\S]*\balloc_region<u8>\s+size[\s\S]*\blet\s+dst\s+<MemPtr<u8>>\s+region_ptr\s+&dst_region[\s\S]*Result::Ok\s+dst_region[\s\S]*\blet\s+cpath\s+<MemPtr<u8>>\s+region_ptr\s+&cpath_region[\s\S]*\bdealloc_region<u8>\s+cpath_region/, 'LLVM cliarg cmdline C string scratch must be owned through RegionToken');
