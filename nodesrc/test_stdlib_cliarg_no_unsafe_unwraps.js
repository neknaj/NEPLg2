...
assert.match(cstrCode, /fn\s+cstr_len_bounded_result\s+<\(MemPtr<u8>,i32\)\*>Result<i32,str>>\s+\(p,\s*max_len\):[\s\S]*le\s+max_len\s+0[\s\S]*while\s+and\s+and\s+and\s+eq\s+done\s+0\s+eq\s+ok\s+1\s+ge\s+i\s+0\s+lt\s+i\s+max_len[\s\S]*Option::None:[\s\S]*set\s+ok\s+0[\s\S]*Result<i32,str>::Err\s+"cliarg\.cstr_len invalid pointer"[\s\S]*Result<i32,str>::Err\s+"cliarg\.cstr_len missing nul terminator"/, 'bounded cstr length must prove 0 <= i < max_len and reject invalid pointers or missing NUL before reading beyond the caller extent');
