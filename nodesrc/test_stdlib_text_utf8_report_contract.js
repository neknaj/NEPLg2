...
const file = path.join(repoRoot, "tests", "stdlib", "text_utf8.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [1, 3, 1, 1, 1, 1, 1, 1, 1];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "text_utf8 doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `text_utf8 doctest#${index + 1}`;
...
console.log("stdlib text_utf8 report contract passed");
