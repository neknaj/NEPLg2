...
const updateCode = sourceWithoutComments("stdlib/alloc/collections/segment_tree/api/update.nepl");
...
assert.match(apiCode, /pub\s+#import\s+"\.\/api\/update"\s+as\s+@merge/, "SegmentTree api facade must re-export owner-preserving update APIs through api/update");
assert.doesNotMatch(apiCode, /\bfn\s+/, "SegmentTree api facade must not keep duplicate update wrappers");
assert.match(updateCode, /fn\s+replace\s+<\(SegmentTree,i32,i32\)\*>Result<SegmentTree,\s*SegmentTreeUpdateError>>\s+\(st,\s*idx,\s*value\):/, "SegmentTree.replace must return an owner-carrying error type");
