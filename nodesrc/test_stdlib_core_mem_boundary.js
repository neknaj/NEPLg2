...
assertMatch(pointerRegion, /^\s*fn\s+realloc_region_bytes_keep_relocating\s+<\.T>\s+<\(RegionToken<\.T>,i32\)->Result<RegionToken<\.T>,\s*RegionReallocError<\.T>>>/m, "RegionToken realloc relocation boundary must be private");
assertNoMatch(pointerRegion, /^\s*pub\s+fn\s+realloc_region_bytes_keep_relocating\b/m, "RegionToken realloc relocation boundary must not be public");
assertMatch(pointerRegion, /\brealloc_region_bytes_keep_relocating[\s\S]*not\s+alloc_payload_fits\s+new_size[\s\S]*let\s+old_size\s+<i32>\s+get\s+region\s+"size"[\s\S]*let\s+old_raw\s+<i32>\s+get\s+region\s+"raw"[\s\S]*allocator::realloc_raw\s+old_raw\s+old_size\s+new_size[\s\S]*let\s+grown\s+<RegionToken<\.T>>\s+region_new<\.T>\s+next_raw\s+new_size[\s\S]*#intrinsic\s+"collection_slot_storage_relocate"\s+<>\s+\(&region,\s*&grown\)[\s\S]*RegionReallocError<\.T>\s+region/, "private RegionToken realloc boundary must prove raw realloc success and storage relocation before returning the owner");
