...
const relPath = "stdlib/neplg2/core/check/module/memo_trait_operation_drop_resource_no_escape_materializer.nepl";
...
assertOrdered(
    source,
    [
        "# check/module/memo_trait_operation_drop_resource_no_escape_materializer",
        "[目的/もくてき]:",
        "[契約/けいやく]:",
        "[現状/げんじょう]:",
        "[計算量/けいさんりょう]:",
    ...
    "Resource no-escape materializer must document purpose, contract, current limits, complexity, and doctest",
);
assert.ok(
    source.includes("Resource graph traversal") &&
        source.includes("typed summary table") &&
        source.includes("producer が読む `SelfhostMemoTraitOperationDropResourceNoEscapeTable`"),
    "docs must keep the materializer between Resource traversal summary and the existing observation table",
);
assert.ok(
...
assert.ok(
    source.includes("full Resource IR graph traversal 本体はまだ別 stage") &&
        source.includes("proof table 化は既存 producer が担当"),
    "docs must separate this summary materializer from full graph traversal and proof production",
);
assert.ok(
...
for (const [docSnippet, declarationSnippet] of [
    [
        "SelfhostMemoTraitOperationDropResourceNoEscapeTraversalRecord",
        "pub struct SelfhostMemoTraitOperationDropResourceNoEscapeTraversalRecord:",
    ],
    [
        "SelfhostMemoTraitOperationDropResourceNoEscapeTraversalTable",
        "pub struct SelfhostMemoTraitOperationDropResourceNoEscapeTraversalTable:",
    ],
    [
        "SelfhostMemoTraitOperationDropResourceNoEscapeMaterializerErrorKind",
        "pub enum SelfhostMemoTraitOperationDropResourceNoEscapeMaterializerErrorKind:",
    ],
    [
        "selfhost_memo_trait_operation_drop_resource_no_escape_materializer_table_result",
        "pub fn selfhost_memo_trait_operation_drop_resource_no_escape_materializer_table_result",
    ],
]) {
...
assert.doesNotMatch(
    facade,
    /memo_trait_operation_drop_resource_no_escape_materializer/,
    "Resource no-escape materializer must remain facade-private until full Resource proof orchestration is ready",
);
assert.doesNotMatch(
    tySourceList,
    /memo_trait_operation_drop_resource_no_escape_materializer/,
    "checker-layer Resource no-escape materializer must not be registered in the ty source list",
);
assert.ok(
    runner.includes("nodesrc/test_selfhost_memo_trait_operation_drop_resource_no_escape_materializer_contract.js"),
    "source policy runner must execute the Resource no-escape materializer contract",
);
assertOrdered(
    source,
    [
        ...
        "#import \"./memo_trait_operation_drop_resource_no_escape_producer\" as *",
    ],
    "materializer must depend only on typed HIR id, effect/type ids, and the existing Resource observation producer boundary",
);
assert.doesNotMatch(
    code,
    ...
    "materializer must not import backend, Resource graph/proof internals, proof store/artifact, canonical-key, public-surface, evidence producer, impl table, scanner/materializer, purity gate, PrivateCache, or PrivateState layers",
);
assertOrdered(
    source,
    [
...
assert.doesNotMatch(
    ...
    "traversal summary record must not use source/display/hash authority instead of typed body identity",
);
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_record_key_eq"),
    [
        ...
        "selfhost_memo_trait_operation_drop_resource_no_escape_materializer_escape_state_eq a.escape b.escape",
    ],
    "traversal record key equality must compare type, module origin, body root, effect, and escape",
);
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_materializer_validate_record_result"),
    [
        ...
        "TraversalEffectNotInternalAlloc record.effect",
    ],
    "validation must accept only non-placeholder InternalAlloc + NotApplicable traversal summaries",
);
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_table_push"),
    [
        "selfhost_memo_trait_operation_drop_resource_no_escape_materializer_validate_record_result record",
        "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_table_contains_key &table record",
        "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_table_free table",
        "SelfhostMemoTraitOperationDropResourceNoEscapeMaterializerErrorKind::TraversalRecordDuplicate",
        "Result::Err e:",
...
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_status_to_observation_status"),
    [
...
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_record_to_observation_record"),
    [
        "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_status_to_observation_status record.status",
        "selfhost_memo_trait_operation_drop_resource_no_escape_record_new record.type_id record.body_module_fingerprint record.body_root record.effect record.escape status",
    ],
    "materializer must preserve typed body identity, effect, and escape when building observation records",
);
assertOrdered(
    functionBlock(source, "selfhost_memo_trait_operation_drop_resource_no_escape_materialize_loop"),
    [
        "selfhost_memo_trait_operation_drop_resource_no_escape_materializer_validate_record_result record",
        "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_duplicate_before_result source record idx",
        "SelfhostMemoTraitOperationDropResourceNoEscapeMaterializerErrorKind::TraversalRecordDuplicate",
...
assertOrdered(
    ...
    [
        "selfhost_memo_trait_operation_drop_resource_no_escape_traversal_record_to_observation_record record",
        "selfhost_memo_trait_operation_drop_resource_no_escape_table_push output observation",
        "SelfhostMemoTraitOperationDropResourceNoEscapeMaterializerErrorKind::ObservationTableRejected producer_error",
    ],
    "output push must use the existing Resource observation table boundary and preserve nested typed producer errors",
);
assert.doesNotMatch(
    code,
    /\b(selfhost_memo_trait_operation_drop_resource_no_escape_producer_table_result|selfhost_memo_trait_operation_drop_resource_no_escape_status_to_proof_status|selfhost_memo_trait_operation_drop_no_escape_proof_table_|SelfhostMemoTraitOperationDropNoEscapeProof(?:Table|Record|Key|Status))\b/,
    "materializer may import the producer module for observation table types, but must not call proof conversion or no-escape proof table APIs",
);
assert.doesNotMatch(
    code,
