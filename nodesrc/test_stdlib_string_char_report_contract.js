...
const file = path.join(repoRoot, "tests", "stdlib", "string_char.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [8, 7, 2];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "string_char doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `string_char doctest#${index + 1}`;
...
console.log("stdlib string_char report contract passed");
