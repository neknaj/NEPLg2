...
assert.match(
    doctest.stdout,
    /^test_report name="string_trim_doc" count=8 failed=0\n/,
...
assert.match(doctest.code, /str_trim\s+""/, `${name} must cover empty input`);
assert.match(doctest.code, /str_trim\s+" \\t\\r\\n"/, `${name} must cover all-whitespace input`);
