...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(standardSpec, /`NotConfigured`、`RequestPending`、`Ready`、`Failed`/);
    ...
    assert.match(platformDoc, /reader setup state からの top-level request 自動生成/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11TopLevelWindowRequestWriteState\s*\{[\s\S]*NotConfigured,[\s\S]*RequestPending,[\s\S]*Ready,[\s\S]*Failed/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11EventSourceObservationError\s*\{[\s\S]*TopLevelWindowRequestPreviouslyFailed\s*\{[\s\S]*descriptor: NativeWindowLinuxWindowEventSourceDescriptor[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteFailed\s*\{[\s\S]*code: u32[\s\S]*TopLevelWindowRequestWriteReturnedZero[\s\S]*TopLevelWindowRequestWriteOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /new_with_setup_request\([\s\S]*top_level_window_request: None,[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured,[\s\S]*top_level_window_request_written_len: 0/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn ensure_top_level_window_request_ready[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured =>[\s\S]*return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending =>[\s\S]*self\.write_top_level_window_request\(raw_fd\)\?[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Ready => return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Failed =>[\s\S]*TopLevelWindowRequestPreviouslyFailed/);
