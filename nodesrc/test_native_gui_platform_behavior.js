...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(libSource, /pub enum NativeWindowRunLoopWaitBackend\s*\{[\s\S]*MinifbInternalTargetFps,[\s\S]*HostOwnedDeadlineTimer,[\s\S]*PlatformWait\(NativeWindowHostLoopPlatformWaitBackendSelection\)/);
    ...
    assert.match(libSource, /pub enum NativeWindowRunLoopPlatformWaitBackendConfigError\s*\{[\s\S]*NotPlatformWaitBackend\s*\{[\s\S]*requested: NativeWindowRunLoopWaitBackend/);
    ...
    assert.match(libSource, /pub type NativeWindowWindowsPlatformWaitHostLoopError[\s\S]*NativeWindowHostLoopSingleOwnerInterruptibleDeadlineWaitAdapterError[\s\S]*NativeWindowHostLoopPlatformWaitBackendError<NativeWindowHostLoopWindowsWaitBackendError>/);
    ...
    assert.match(nativeWindowPlatformWaitBackendKind, /native_window_run_loop_platform_wait_backend_selection[\s\S]*match config\.wait_backend[\s\S]*NativeWindowRunLoopWaitBackend::PlatformWait\(selection\) => Ok\(selection\)[\s\S]*NotPlatformWaitBackend\s*\{\s*requested\s*\}/);
    ...
    assert.doesNotMatch(nativeWindowPlatformWaitBackendKind, /LinuxSelectorTimerFd\(/);
    ...
    assert.match(nativeWindowPlatformWaitBackendKind, /pub type NativeWindowHostLoopPlatformWaitRunLoopHost<Host,\s*Api>\s*=[\s\S]*NativeWindowHostLoopSingleOwnerInterruptibleDeadlineWaitRunLoopHost<[\s\S]*NativeWindowHostLoopPlatformWaitBackend<Api>/);
    assert.match(nativeWindowPlatformWaitBackendKind, /pub fn native_window_host_loop_platform_wait_run_loop_host_from_backend<Host,\s*Api>\([\s\S]*host: Host,[\s\S]*backend: NativeWindowHostLoopPlatformWaitBackend<Api>,[\s\S]*\) -> NativeWindowHostLoopPlatformWaitRunLoopHost<Host,\s*Api>[\s\S]*NativeWindowHostLoopSingleOwnerInterruptibleDeadlineWaitAdapter::new\(backend\)[\s\S]*NativeWindowHostLoopSingleOwnerInterruptibleDeadlineWaitRunLoopHost::new\(host,\s*wait_adapter\)/);
    ...
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /pub trait NativeWindowHostLoopLinuxSelectorTimerFdRawApi\s*\{[\s\S]*create_selector_raw[\s\S]*create_timer_fd_raw[\s\S]*register_timer_fd_raw[\s\S]*arm_timer_fd_relative_timespec[\s\S]*selector_wait_for_timer_or_event_raw[\s\S]*selector_wait_for_event_raw[\s\S]*close_selector_raw[\s\S]*close_timer_fd_raw[\s\S]*last_error_code/);
    ...
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /native_window_host_loop_linux_selector_timer_fd_wake_from_status[\s\S]*STATUS_TIMER_FIRED[\s\S]*TimerFired[\s\S]*STATUS_HOST_EVENT_READY[\s\S]*HostEventReady[\s\S]*STATUS_FAILED[\s\S]*SelectorWaitFailed[\s\S]*UnexpectedSelectorStatus/);
    ...
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /pub struct NativeWindowHostLoopLinuxSelectorTimerFdBackend<[\s\S]*origin: std::time::Instant,[\s\S]*api: Api,[\s\S]*selector: Option<NativeWindowHostLoopLinuxSelectorFd>,[\s\S]*timer: Option<NativeWindowHostLoopLinuxTimerFd>/);
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /pub fn new\(mut api: Api\)[\s\S]*create_selector_raw\(\)[\s\S]*create_timer_fd_raw\(\)[\s\S]*register_timer_fd_raw\(&selector,\s*&timer\)/);
    ...
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /pub fn close_handles_if_open\(&mut self\) -> bool[\s\S]*self\.timer\.take\(\)[\s\S]*self\.selector\.take\(\)[\s\S]*close_timer_fd_raw[\s\S]*close_selector_raw/);
    assert.match(nativeWindowLinuxSelectorTimerFdBackend, /pub fn wait_for_host_event[\s\S]*selector_wait_for_event_raw\(selector\)[\s\S]*native_window_host_loop_linux_selector_timer_fd_host_event_from_status/);
    ...
    assert.match(nativeWindowWindowsPlatformWaitRunner, /native_window_run_loop_platform_wait_backend_from_config\(config\)[\s\S]*PlatformWaitBackendFromConfigFailed[\s\S]*NativeWindowBackendLoop::new_for_scale[\s\S]*Window::new/);
