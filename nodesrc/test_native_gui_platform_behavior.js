...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    const guiRedesignImplementationPlan = readRepoFile("doc", "neplg2", "gui_redesign_implementation_plan.md");
    ...
    const nativeWindowKeyboardEventImpl = textSliceBetween(
        libSource,
        "impl NativeWindowKeyboardEvent",
        "impl NativeWindowKeyboardModifierState",
    );
    const nativeWindowLinuxX11GetKeyboardMappingConstants = textSliceBetween(
        libSource,
        "pub const NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_OPCODE",
        "const NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_ERROR",
    );
    ...
    const nativeWindowLinuxWindowEventSourceFdAcquisitionTypes = textSliceBetween(
        libSource,
        "pub enum NativeWindowLinuxWindowEventSourceFdAcquisitionKind",
        "pub struct NativeWindowLinuxWindowEventSourceDescriptor",
    );
    const nativeWindowLinuxWindowEventSourceFdAcquisitionImpl = textSliceBetween(
        libSource,
    ...
    const nativeWindowLinuxWindowEventSourcePreparePlatformWaitConfigError = textSliceBetween(
        libSource,
        "pub enum NativeWindowLinuxWindowEventSourcePreparePlatformWaitConfigError",
        "#[derive(Debug)]\npub struct NativeWindowLinuxWindowEventSourcePreparedRunLoopConfig",
    );
    const nativeWindowLinuxX11EventSourceObservationSurface = textSliceBetween(
        libSource,
        "pub const NATIVE_WINDOW_LINUX_X11_SETUP_REQUEST_BYTE_LEN",
        "pub struct NativeWindowLinuxX11EventSourceSysApi",
    );
    const nativeWindowLinuxX11ObservationError = textSliceBetween(
        libSource,
        "pub enum NativeWindowLinuxX11EventSourceObservationError",
        "pub trait NativeWindowLinuxX11EventSourceRawApi",
    );
    const nativeWindowLinuxX11ObservationReader = textSliceBetween(
        libSource,
        "pub struct NativeWindowLinuxX11EventSourceObservationReader",
        "pub struct NativeWindowLinuxX11WindowEventSourceObservationProvider",
    );
    const nativeWindowLinuxX11EventPacketToObservation = textSliceBetween(
        libSource,
        "fn native_window_linux_x11_event_packet_to_observation",
        "impl<Api> NativeWindowLinuxX11EventSourceObservationReader<Api>",
    );
    const nativeWindowLinuxX11ObservationReaderImpl = textSliceBetween(
        libSource,
        "impl<Api> NativeWindowLinuxX11EventSourceObservationReader<Api>",
        "impl<Provider, Api> NativeWindowLinuxX11WindowEventSourceObservationProvider",
    );
    const nativeWindowLinuxX11XauthorityCredentialSetupRequest = textSliceBetween(
        libSource,
    ...
    const nativeWindowLinuxX11InternAtomReplyTypes = textSliceBetween(
        libSource,
        "pub struct NativeWindowLinuxX11AtomId",
        "pub struct NativeWindowLinuxX11WmProtocolAtoms",
    );
    const nativeWindowLinuxX11GetKeyboardMappingTypes = textSliceBetween(
        libSource,
        "pub struct NativeWindowLinuxX11GetKeyboardMappingRequest",
        "pub struct NativeWindowLinuxX11WmProtocolAtoms",
    );
    const nativeWindowLinuxX11SetupKeyboardMappingTypes = textSliceBetween(
        libSource,
        "pub struct NativeWindowLinuxX11SetupKeyboardMappingRequest",
        "pub struct NativeWindowLinuxX11KeyboardMappingReplyHeader",
    );
    const nativeWindowLinuxX11KeyboardMappingSelectionTypes = textSliceBetween(
        libSource,
        "pub struct NativeWindowLinuxX11KeyboardMappingRange",
    ...
    const nativeWindowLinuxX11InternAtomReplyImpl = textSliceBetween(
        libSource,
        "impl NativeWindowLinuxX11AtomId",
        "impl NativeWindowLinuxX11WmProtocolAtoms",
    );
    const nativeWindowLinuxX11GetKeyboardMappingImpl = textSliceBetween(
        libSource,
        "impl NativeWindowLinuxX11GetKeyboardMappingRequest",
        "impl NativeWindowLinuxX11WmProtocolAtoms",
    );
    const nativeWindowLinuxX11SetupKeyboardMappingImpl = textSliceBetween(
        libSource,
    ...
    const nativeWindowLinuxX11KeyboardMappingSelectionImpl = textSliceBetween(
        libSource,
        "impl NativeWindowLinuxX11KeyboardMappingRange",
    ...
    const nativeWindowLinuxX11LocalAuthorityAddressHelpers = textSliceBetween(
        libSource,
        "pub fn native_window_linux_x11_local_authority_address_with_limit",
        "fn native_window_linux_x11_xauthority_display_number_bytes",
    );
    const nativeWindowLinuxX11LocalAuthorityAddressBridge = textSliceBetween(
        libSource,
        "pub fn native_window_linux_x11_xauthority_local_selector_criteria_from_authority_address",
        "fn native_window_linux_x11_xauthority_remaining_byte_count",
    );
    const nativeWindowLinuxX11LocalAuthorityAddressSurface = [
        nativeWindowLinuxX11LocalAuthorityAddressTypes,
    ...
    const nativeWindowLinuxX11InternAtomReplySurface = [
        nativeWindowLinuxX11InternAtomReplyTypes,
        nativeWindowLinuxX11InternAtomReplyImpl,
    ].join("\n");
    const nativeWindowLinuxX11GetKeyboardMappingSurface = [
        nativeWindowLinuxX11GetKeyboardMappingConstants,
        nativeWindowLinuxX11GetKeyboardMappingTypes,
        nativeWindowLinuxX11GetKeyboardMappingImpl,
    ].join("\n");
    const nativeWindowLinuxX11SetupKeyboardMappingSurface = [
        nativeWindowLinuxX11SetupKeyboardMappingTypes,
        nativeWindowLinuxX11SetupKeyboardMappingImpl,
    ].join("\n");
    const nativeWindowLinuxX11KeyboardMappingSelectionSurface = [
        nativeWindowLinuxX11KeyboardMappingSelectionTypes,
        nativeWindowLinuxX11KeyboardMappingSelectionImpl,
    ].join("\n");
    ...
    const nativeWindowLinuxX11XauthorityFileBytesTypes = textSliceBetween(
        libSource,
        "pub trait NativeWindowLinuxX11XauthorityFileBytesReader",
        "#[derive(Clone, Copy, Debug, Eq, PartialEq)]\npub enum NativeWindowLinuxX11XauthoritySelection",
    );
    const nativeWindowLinuxX11XauthorityFileBytesImpl = textSliceBetween(
        libSource,
        "impl NativeWindowLinuxX11XauthorityFileBytes",
        "fn native_window_linux_x11_xauthority_display_number_bytes",
    );
    const nativeWindowLinuxX11XauthorityFileBytesSurface = [
        nativeWindowLinuxX11XauthorityFileBytesTypes,
        nativeWindowLinuxX11XauthorityFileBytesImpl,
    ].join("\n");
    ...
    const nativeWindowLinuxX11XauthorityVfsFileBytesAdapter = textSliceBetween(
        libSource,
        "pub trait NativeWindowLinuxX11XauthorityVfsFileBytesSource",
    ...
    const nativeWindowPlatformWaitBackendKindPolicySurface =
        nativeWindowPlatformWaitBackendKind
            ...
            .replace(nativeWindowLinuxX11XauthorityVfsFileBytesAdapter, "");
    ...
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub enum NativeWindowLinuxWindowEventSourceFdAcquisitionKind\s*\{[\s\S]*X11Display,[\s\S]*WaylandDisplay/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub enum NativeWindowLinuxWindowEventSourceFdAcquisitionError\s*\{[\s\S]*MissingEnvironment\s*\{[\s\S]*variable: NativeWindowLinuxWindowEventSourceEnvironmentVariable[\s\S]*UnsupportedDisplayForm\s*\{[\s\S]*kind: NativeWindowLinuxWindowEventSourceFdAcquisitionKind[\s\S]*SocketPathTooLong\s*\{[\s\S]*byte_len: usize,[\s\S]*max_byte_len: usize[\s\S]*SocketCreateFailed\s*\{[\s\S]*code: u32[\s\S]*ConnectFailed\s*\{[\s\S]*code: u32,[\s\S]*close_code: Option<u32>[\s\S]*CloseFailed\s*\{[\s\S]*raw_fd: i32,[\s\S]*code: u32[\s\S]*Closed\s*\{[\s\S]*kind: NativeWindowLinuxWindowEventSourceFdAcquisitionKind/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub trait NativeWindowLinuxWindowEventSourceFdAcquisitionRawApi\s*\{[\s\S]*environment_variable\([\s\S]*NativeWindowLinuxWindowEventSourceEnvironmentVariable[\s\S]*Option<String>[\s\S]*create_unix_stream_socket_raw[\s\S]*connect_unix_stream_socket_raw\([\s\S]*raw_fd: i32,[\s\S]*path: &str[\s\S]*close_unix_stream_socket_raw\([\s\S]*raw_fd: i32[\s\S]*last_error_code/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /enum NativeWindowLinuxWindowEventSourceOwnedFdState\s*\{[\s\S]*Open\s*\{\s*raw_fd: i32\s*\}[\s\S]*Closed,[\s\S]*CloseFailed\s*\{\s*raw_fd: i32,\s*code: u32\s*\}/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub struct NativeWindowLinuxWindowEventSourceOwnedFd<Api>[\s\S]*where[\s\S]*Api: NativeWindowLinuxWindowEventSourceFdAcquisitionRawApi,[\s\S]*kind: NativeWindowLinuxWindowEventSourceFdAcquisitionKind,[\s\S]*state: NativeWindowLinuxWindowEventSourceOwnedFdState,[\s\S]*api: Api/);
    assert.doesNotMatch(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /#\[derive\([^\]]*(Clone|Copy)[^\]]*\)\]\s*pub struct NativeWindowLinuxWindowEventSourceOwnedFd|#\[derive\([^\]]*(Clone|Copy)[^\]]*\)\]\s*pub struct NativeWindowLinuxWindowEventSourceOwnedFdProvider/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /native_window_linux_wayland_socket_path[\s\S]*display_name\.is_empty\(\)[\s\S]*display_name\.starts_with\('\/'\)[\s\S]*display_name\.bytes\(\)\.any\([\s\S]*byte == b'\/'[\s\S]*UnsupportedDisplayForm[\s\S]*WaylandDisplay/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub enum NativeWindowLinuxX11DisplayNameError\s*\{[\s\S]*UnsupportedDisplayForm,[\s\S]*InvalidDisplayNumber/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /native_window_linux_x11_display_number[\s\S]*strip_prefix\("unix\/"\)[\s\S]*strip_prefix\(':'\)[\s\S]*NativeWindowLinuxX11DisplayNameError::UnsupportedDisplayForm[\s\S]*checked_mul\(10\)[\s\S]*NativeWindowLinuxX11DisplayNameError::InvalidDisplayNumber/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /native_window_linux_x11_fd_acquisition_error_from_display_name[\s\S]*NativeWindowLinuxX11DisplayNameError::UnsupportedDisplayForm[\s\S]*NativeWindowLinuxWindowEventSourceFdAcquisitionKind::X11Display[\s\S]*NativeWindowLinuxX11DisplayNameError::InvalidDisplayNumber[\s\S]*NativeWindowLinuxWindowEventSourceFdAcquisitionError::InvalidDisplayNumber/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /native_window_linux_window_event_source_acquire_fd_with_api[\s\S]*native_window_linux_window_event_source_socket_path_from_env\(&mut api, kind\)[\s\S]*api\.create_unix_stream_socket_raw\(\)[\s\S]*api\.connect_unix_stream_socket_raw\(raw_fd, &path\)[\s\S]*api\.close_unix_stream_socket_raw\(raw_fd\)[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFd\s*\{[\s\S]*state: NativeWindowLinuxWindowEventSourceOwnedFdState::Open\s*\{\s*raw_fd\s*\},[\s\S]*api/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub fn raw_fd\(&self\)[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::Open\s*\{\s*raw_fd\s*\}\s*=> Ok\(\*raw_fd\)[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::CloseFailed\s*\{\s*raw_fd,\s*code\s*\}[\s\S]*CloseFailed\s*\{[\s\S]*raw_fd: \*raw_fd,[\s\S]*code: \*code/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /pub fn close\(&mut self\)[\s\S]*std::mem::replace\([\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::Closed[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::Open\s*\{\s*raw_fd\s*\}[\s\S]*self\.state = NativeWindowLinuxWindowEventSourceOwnedFdState::CloseFailed\s*\{[\s\S]*raw_fd,[\s\S]*code,[\s\S]*CloseFailed\s*\{[\s\S]*raw_fd,[\s\S]*code/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /impl<Api> Drop for NativeWindowLinuxWindowEventSourceOwnedFd<Api>[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::Open\s*\{\s*raw_fd\s*\}[\s\S]*std::mem::replace\([\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdState::Closed[\s\S]*close_unix_stream_socket_raw\(raw_fd\)/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /impl<Api> NativeWindowLinuxWindowEventSourceProvider[\s\S]*NativeWindowLinuxWindowEventSourceOwnedFdProvider<Api>[\s\S]*type Error = NativeWindowLinuxWindowEventSourceFdAcquisitionError[\s\S]*self\.owned_fd\.raw_fd\(\)\?[\s\S]*descriptor_source_kind\(\)/);
    assert.doesNotMatch(nativeWindowLinuxWindowEventSourceFdAcquisitionSurface, /run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|run_minifb|WindowOptions|ScaleMode|window\.update\(|update_with_buffer|set_target_fps|epoll|timerfd_create|eventfd\(|read\(|drain|selector_wait|register_window_event_source_fd_raw|validate_native_window_run_loop_platform_wait_runner_support_for_platform|PlatformRunnerIntegrationMissing|DOM|Canvas|video_memory|stdout_protocol|fallback|silent no-op|synthetic/i);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSysApi, /std::env::var\(name\)\.ok\(\)/);
    assert.match(nativeWindowLinuxWindowEventSourceFdAcquisitionSysApi, /libc::socket\(libc::AF_UNIX,[\s\S]*libc::SOCK_STREAM \| libc::SOCK_CLOEXEC/);
    ...
    assert.match(nativeWindowLinuxWindowEventSourceObservationTypes, /pub struct NativeWindowLinuxWindowEventSourceObservation\s*\{[\s\S]*event_kind: NativeWindowEventPumpEventKind,[\s\S]*os_close_requested: bool,[\s\S]*exit_shortcut_requested: bool,[\s\S]*current_size: NativeWindowSize,[\s\S]*mouse_down: bool,[\s\S]*pointer_raw: Option<\(f32, f32\)>/);
    ...
    assert.match(nativeWindowLinuxWindowEventSourceObservationImpl, /pub fn native_window_linux_window_event_source_snapshot_from_observation\([\s\S]*descriptor: NativeWindowLinuxWindowEventSourceDescriptor,[\s\S]*input: NativeWindowEventPumpInput,[\s\S]*observation: NativeWindowLinuxWindowEventSourceObservation[\s\S]*match observation\.event_kind\(\)[\s\S]*NativeWindowEventPumpEventKind::Poll => build_native_window_event_pump_snapshot_from_raw\([\s\S]*observation\.pointer_raw\(\),[\s\S]*event_kind =>[\s\S]*build_native_window_event_pump_snapshot_from_raw_with_event_kind_and_keyboard_event\([\s\S]*observation\.pointer_raw\(\),[\s\S]*event_kind,[\s\S]*observation\.keyboard_event\(\),[\s\S]*SnapshotConstructionFailed\s*\{[\s\S]*descriptor,[\s\S]*error/);
    ...
    assert.match(guiRedesignImplementationPlan, /Phase F5jp: Native Linux X11 top-level window request partial-write boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5jq: Native Linux X11 setup resource info and resource id allocation boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5jr: Native Linux X11 setup-backed top-level window request owner boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5js: Native Linux X11 reader setup-backed top-level request generation boundary/);
    ...
    assert.match(guiRedesignImplementationPlan, /Phase F5jy: Native Linux X11 top-level CreateWindow\/MapWindow split request owner boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5jz: Native Linux X11 InternAtom reply packet AtomId owner boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5ka: Native Linux X11 InternAtom batch partial-write scheduling boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5kb: Native Linux X11 InternAtom reply sequence correlation boundary/);
    ...
    assert.match(guiRedesignImplementationPlan, /Phase F5kl: Native Linux X11 GetKeyboardMapping request\/reply owner boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5km: Native Linux X11 setup-owned keyboard mapping request boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5kn: Native Linux X11 setup-owned keyboard mapping reader scheduling boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5ko: Native Linux X11 raw keyboard mapping selection boundary/);
    assert.match(guiRedesignImplementationPlan, /Phase F5kp: Native Linux X11 keymap projection evidence boundary/);
    ...
    assert.match(guiRedesignImplementationPlan, /fixed 32 byte reply packet から nonzero Atom ID/);
    assert.match(guiRedesignImplementationPlan, /current reader の generic reply handling を変更せず/);
    ...
    assert.match(guiRedesignImplementationPlan, /GetKeyboardMapping` request bytes と reply raw keysym table/);
    assert.match(guiRedesignImplementationPlan, /request length `2` words \/ 8 byte/);
    assert.match(guiRedesignImplementationPlan, /raw keysyms are not projected to `NativeWindowPortableKey` in F5kl/);
    assert.match(guiRedesignImplementationPlan, /event packet decode、reader state、fd IO、runner、queue、IME \/ text input、shortcut policy、fallback、support gate `Ok` 化には接続しない/);
    assert.match(guiRedesignImplementationPlan, /setup success body parser は offset 26 \/ 27 を読み/);
    assert.match(guiRedesignImplementationPlan, /`min_keycode < 8` と `max_keycode < min_keycode` を typed parse error/);
    assert.match(guiRedesignImplementationPlan, /`keycode_count = max - min \+ 1` は `u16` へ拡張して `checked_sub` \/ `checked_add` \/ `u8::try_from`/);
    assert.match(guiRedesignImplementationPlan, /8 byte request 全体が accepted された時だけ/);
    assert.match(guiRedesignImplementationPlan, /generic server reply drain で body を捨てず/);
    assert.match(guiRedesignImplementationPlan, /keycode -> keysym layout selection、modifier group \/ level selection、portable key projection/);
    assert.match(guiRedesignImplementationPlan, /`NativeWindowLinuxX11KeyboardMappingRange` owner/);
    assert.match(guiRedesignImplementationPlan, /computed `last_keycode`/);
    assert.match(guiRedesignImplementationPlan, /keycode index と keysym table offset は checked arithmetic/);
    assert.match(guiRedesignImplementationPlan, /shift column[\s\S]*`NoSymbol`/);
    assert.match(guiRedesignImplementationPlan, /Lock \/ Caps、AltGr \/ Mod bit、group selection は unsupported evidence/);
    assert.match(guiRedesignImplementationPlan, /selection -> final_raw_keysym\(\) -> project_portable_key\(\)/);
    assert.match(guiRedesignImplementationPlan, /event decoder は F5kp では projection evidence を作らず `None`/);
    ...
    assert.match(guiRedesignImplementationPlan, /NativeWindowLinuxX11TopLevelWindowRequestWriteState/);
    assert.match(guiRedesignImplementationPlan, /hard failure、zero write、overflow は typed error にし、state を `Failed`/);
    assert.match(guiRedesignImplementationPlan, /TopLevelWindowRequestPreviouslyFailed/);
    assert.match(guiRedesignImplementationPlan, /support gate `Ok` 化はまだ行わない/);
    assert.match(guiRedesignImplementationPlan, /caller supplied typed plan/);
    assert.match(guiRedesignImplementationPlan, /SetupBackedBuildPending/);
    assert.match(guiRedesignImplementationPlan, /setup-backed plan missing fail-closed/);
    assert.match(guiRedesignImplementationPlan, /TopLevelWindowRequestSetupResourceInfoMissing/);
    assert.match(guiRedesignImplementationPlan, /reader は geometry、event mask、serial を生成しない/);
    assert.match(guiRedesignImplementationPlan, /32 byte fixed body、vendor string \+ 4 byte padding、pixmap format list、first screen header/);
    assert.match(guiRedesignImplementationPlan, /first root window id は server-owned id/);
    assert.match(guiRedesignImplementationPlan, /sparse mask に対応し、serial bit を mask set bit へ low-to-high/);
    assert.match(guiRedesignImplementationPlan, /empty setup body は historical scripted observation compatibility/);
    ...
    assert.match(guiRedesignImplementationPlan, /`AuthorityFilePath` は `XAUTHORITY`/);
    assert.match(guiRedesignImplementationPlan, /`?NoMatchingRecord`? は no-auth fallback にせず/);
    ...
    assert.match(standardSpec, /F5jp Native Linux X11 top-level window request partial-write boundary/);
    assert.match(standardSpec, /F5jq Native Linux X11 setup resource info and resource id allocation boundary/);
    assert.match(standardSpec, /F5jr Native Linux X11 setup-backed top-level window request owner boundary/);
    assert.match(standardSpec, /F5js Native Linux X11 reader setup-backed top-level request generation boundary/);
    ...
    assert.match(standardSpec, /F5jy Native Linux X11 top-level CreateWindow\/MapWindow split request owner boundary/);
    assert.match(standardSpec, /F5jz Native Linux X11 InternAtom reply packet AtomId owner boundary/);
    assert.match(standardSpec, /F5kb Native Linux X11 InternAtom reply sequence correlation boundary/);
    assert.match(standardSpec, /F5ka Native Linux X11 InternAtom batch partial-write scheduling boundary/);
    ...
    assert.match(standardSpec, /F5kl Native Linux X11 GetKeyboardMapping request\/reply owner boundary/);
    assert.match(standardSpec, /F5km Native Linux X11 setup-owned keyboard mapping request boundary/);
    assert.match(standardSpec, /F5kn Native Linux X11 setup-owned keyboard mapping reader scheduling boundary/);
    assert.match(standardSpec, /F5ko Native Linux X11 raw keyboard mapping selection boundary/);
    assert.match(standardSpec, /F5kp Native Linux X11 keymap projection evidence boundary/);
    ...
    assert.match(standardSpec, /opcode `101`、request length `2` words \/ 8 byte/);
    assert.match(standardSpec, /raw keysyms are not projected to `NativeWindowPortableKey` in F5kl/);
    assert.match(standardSpec, /`GetKeyboardMapping` request\/reply owner と raw keysym table validation だけを担当する/);
    assert.match(standardSpec, /offset 26 の `min-keycode` と offset 27 の `max-keycode`/);
    assert.match(standardSpec, /`min-keycode < 8` と `max-keycode < min-keycode` を typed error/);
    assert.match(standardSpec, /`wrapping_\*`、`saturating_\*`、unchecked `as u8` を使わない/);
    assert.match(standardSpec, /sequence は 8 byte の `GetKeyboardMapping` request 全体が raw fd write に受理された後だけ/);
    assert.match(standardSpec, /`NotConfigured`、`SetupBackedBuildPending`、`RequestPending`、`ReplyPending`、`Ready`、`Failed`/);
    assert.match(standardSpec, /header packet と body bytes を dedicated pending owner に保持/);
    assert.match(standardSpec, /X11 keycode からどの keysym を選ぶか、modifier state と group \/ level の解釈、portable key projection/);
    assert.match(standardSpec, /selector input は loose な first keycode ではなく `NativeWindowLinuxX11KeyboardMappingRange` owner/);
    assert.match(standardSpec, /raw keysyms len が `keycode_count \* keysyms_per_keycode` と一致しない場合/);
    assert.match(standardSpec, /Shift column が `NoSymbol` で base column が存在する場合/);
    assert.match(standardSpec, /Lock \/ Caps、AltGr \/ Mod bit、group selection は F5ko では解釈せず/);
    assert.match(standardSpec, /`NativeWindowLinuxX11KeyboardMappingProjectionEvidence` は selection と keysym projection を別々の public input として受け取らない/);
    assert.match(standardSpec, /raw keycode が一致しない projection evidence は typed error/);
    ...
    assert.match(standardSpec, /後続 integration phase は 32 byte reply packet を generic header へ縮約する前に retain \/ parse/);
    ...
    assert.match(standardSpec, /X11 setup が `Ready` になった後、event packet read より前に CreateWindow \/ MapWindow request bytes/);
    assert.match(standardSpec, /`NotConfigured`、`SetupBackedBuildPending`、`RequestPending`、`Ready`、`Failed`/);
    assert.match(standardSpec, /would-block は `TopLevelWindowRequestWriteWouldBlock` として retryable error/);
    assert.match(standardSpec, /failed state で再 poll された場合は `TopLevelWindowRequestPreviouslyFailed`/);
    assert.match(standardSpec, /native_window_linux_x11_setup_resource_info_from_little_endian_success_body/);
    assert.match(standardSpec, /first root window id は server-owned id/);
    assert.match(standardSpec, /mask が連続していないことを fallback や unsupported success にしない/);
    assert.match(standardSpec, /SetupResourceInfoParseFailed/);
    assert.match(standardSpec, /TopLevelWindowRequestSetupBackedPlanMissing/);
    assert.match(standardSpec, /TopLevelWindowRequestSetupResourceInfoMissing/);
    assert.match(standardSpec, /TopLevelWindowRequestBuildFailed/);
    assert.match(standardSpec, /reader 内 hidden default ではない/);
    ...
    assert.match(platformDoc, /F5jp/);
    assert.match(platformDoc, /F5jq/);
    assert.match(platformDoc, /F5jr/);
    assert.match(platformDoc, /F5js/);
    ...
    assert.match(platformDoc, /F5km では、X11 setup success response body/);
    assert.match(platformDoc, /`keycode_count = max - min \+ 1` は checked arithmetic/);
    ...
    assert.match(platformDoc, /CreateWindow \/ MapWindow request owner を X11 observation reader の optional state/);
    assert.match(platformDoc, /setup success 後、event read 前に partial write/);
    assert.match(platformDoc, /setup success body から resource-id-base、resource-id-mask、first screen root window id/);
    assert.match(platformDoc, /setup resource info と caller supplied serial から generated client window id/);
    assert.match(platformDoc, /typed plan を reader setup state と合成して request owner を自動生成/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11XauthoritySelectorCriteria<'a>\s*\{[\s\S]*family: NativeWindowLinuxX11XauthorityFamily,[\s\S]*address: &'a \[u8\],[\s\S]*display_number_bytes: \[u8; NATIVE_WINDOW_LINUX_X11_XAUTHORITY_DISPLAY_NUMBER_MAX_BYTE_LEN\],[\s\S]*display_number_byte_len: usize,[\s\S]*preferred_protocol_name: Option<&'a \[u8\]>/);
    assert.match(nativeWindowLinuxX11LocalAuthorityAddressSurface, /pub struct NativeWindowLinuxX11LocalAuthorityAddress\s*\{[\s\S]*bytes: Vec<u8>[\s\S]*pub trait NativeWindowLinuxX11LocalAuthorityAddressReader\s*\{[\s\S]*type Error;[\s\S]*read_x11_local_authority_address\(&mut self\) -> Result<Vec<u8>, Self::Error>[\s\S]*pub enum NativeWindowLinuxX11LocalAuthorityAddressReadError<ReaderError>\s*\{[\s\S]*ReadFailed\s*\{[\s\S]*error: ReaderError[\s\S]*EmptyAddress[\s\S]*AddressTooLong\s*\{[\s\S]*byte_len: usize,[\s\S]*max_byte_len: usize[\s\S]*AddressContainsNul\s*\{[\s\S]*byte_index: usize/);
    assert.match(nativeWindowLinuxX11LocalAuthorityAddressSurface, /impl NativeWindowLinuxX11LocalAuthorityAddress[\s\S]*fn new\(bytes: Vec<u8>\) -> Self[\s\S]*pub fn as_bytes\(&self\) -> &\[u8\][\s\S]*pub fn len\(&self\) -> usize/);
    assert.match(nativeWindowLinuxX11LocalAuthorityAddressSurface, /pub fn native_window_linux_x11_local_authority_address_with_limit<Reader>[\s\S]*reader\.read_x11_local_authority_address\(\)[\s\S]*ReadFailed[\s\S]*bytes\.is_empty\(\)[\s\S]*EmptyAddress[\s\S]*bytes\.len\(\) > max_byte_len[\s\S]*AddressTooLong[\s\S]*bytes\.iter\(\)\.enumerate\(\)[\s\S]*AddressContainsNul[\s\S]*NativeWindowLinuxX11LocalAuthorityAddress::new\(bytes\)/);
    ...
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_EVENT_MASK_KEY_PRESS: u32 = 0x0000_0001/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_EVENT_MASK_KEY_RELEASE: u32 = 0x0000_0002/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_EVENT_MASK_EXPOSURE: u32 = 0x0000_8000/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_EVENT_MASK_STRUCTURE_NOTIFY: u32 = 0x0002_0000/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_RESOURCE_ID_UNUSED_HIGH_BITS: u32 = 0xe000_0000/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_FIXED_BODY_BYTE_LEN: usize = 32/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_RESOURCE_ID_BASE_OFFSET: usize = 4/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_RESOURCE_ID_MASK_OFFSET: usize = 8/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_SETUP_SCREEN_FIXED_BYTE_LEN: usize = 40/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub struct NativeWindowLinuxX11SetupBackedTopLevelWindowCreateInput\s*\{[\s\S]*setup_resource_info: NativeWindowLinuxX11SetupResourceInfo,[\s\S]*window_resource_serial: u32,[\s\S]*x: i16,[\s\S]*y: i16,[\s\S]*width: u16,[\s\S]*height: u16,[\s\S]*border_width: u16,[\s\S]*background_pixel: u32,[\s\S]*event_mask: u32/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub struct NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan\s*\{[\s\S]*window_resource_serial: u32,[\s\S]*x: i16,[\s\S]*y: i16,[\s\S]*width: u16,[\s\S]*height: u16,[\s\S]*border_width: u16,[\s\S]*background_pixel: u32,[\s\S]*event_mask: u32/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub struct NativeWindowLinuxX11CreateWindowRequest\s*\{[\s\S]*window_id: u32,[\s\S]*bytes: Vec<u8>/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub struct NativeWindowLinuxX11MapWindowRequest\s*\{[\s\S]*window_id: u32,[\s\S]*bytes: Vec<u8>/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub enum NativeWindowLinuxX11SetupBackedTopLevelWindowCreateRequestError\s*\{[\s\S]*ResourceIdAllocationFailed\s*\{[\s\S]*error: NativeWindowLinuxX11ResourceIdAllocationError[\s\S]*TopLevelWindowCreateRequestBuildFailed\s*\{[\s\S]*error: NativeWindowLinuxX11TopLevelWindowCreateRequestBuildError/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /impl NativeWindowLinuxX11SetupBackedTopLevelWindowCreateInput[\s\S]*pub fn new\([\s\S]*setup_resource_info: NativeWindowLinuxX11SetupResourceInfo,[\s\S]*window_resource_serial: u32[\s\S]*native_window_linux_x11_top_level_window_default_event_mask\(\)[\s\S]*pub fn setup_resource_info\(&self\) -> NativeWindowLinuxX11SetupResourceInfo[\s\S]*pub fn window_resource_serial\(&self\) -> u32/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /impl NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan[\s\S]*pub fn new\([\s\S]*window_resource_serial: u32[\s\S]*native_window_linux_x11_top_level_window_default_event_mask\(\)[\s\S]*pub fn create_input\([\s\S]*setup_resource_info: NativeWindowLinuxX11SetupResourceInfo[\s\S]*NativeWindowLinuxX11SetupBackedTopLevelWindowCreateInput::new_with_event_mask/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn as_bytes\(&self\) -> &\[u8\][\s\S]*&self\.bytes/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn create_window_request_byte_len\(&self\) -> usize[\s\S]*NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_REQUEST_BYTE_LEN/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn map_window_request_byte_len\(&self\) -> usize[\s\S]*NATIVE_WINDOW_LINUX_X11_MAP_WINDOW_REQUEST_BYTE_LEN/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn native_window_linux_x11_top_level_window_default_event_mask\(\) -> u32[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_KEY_PRESS[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_KEY_RELEASE[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_BUTTON_PRESS[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_BUTTON_RELEASE[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_POINTER_MOTION[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_EXPOSURE[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_MASK_STRUCTURE_NOTIFY/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /native_window_linux_x11_top_level_window_validate_root_parent_resource_id[\s\S]*value == 0[\s\S]*ResourceIdZero[\s\S]*ParentWindow[\s\S]*Ok\(\(\)\)/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /native_window_linux_x11_create_window_request_length_units[\s\S]*checked_add\(NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_VALUE_COUNT\)[\s\S]*RequestLengthOverflow/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn native_window_linux_x11_create_window_request\([\s\S]*NativeWindowLinuxX11TopLevelWindowResourceIdKind::Window[\s\S]*input\.window_id[\s\S]*NativeWindowLinuxX11TopLevelWindowResourceIdKind::ParentWindow[\s\S]*input\.parent_window_id[\s\S]*native_window_linux_x11_create_window_request_after_resource_id_validation\(input\)/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn native_window_linux_x11_map_window_request\([\s\S]*NativeWindowLinuxX11TopLevelWindowResourceIdKind::Window[\s\S]*window_id[\s\S]*native_window_linux_x11_map_window_request_after_resource_id_validation\(window_id\)/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /native_window_linux_x11_top_level_window_create_request_after_resource_id_validation[\s\S]*native_window_linux_x11_create_window_request_after_resource_id_validation\(input\)\?[\s\S]*native_window_linux_x11_map_window_request_after_resource_id_validation\(input\.window_id\)[\s\S]*bytes\.extend_from_slice\(create_window_request\.as_bytes\(\)\)[\s\S]*bytes\.extend_from_slice\(map_window_request\.as_bytes\(\)\)/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn native_window_linux_x11_top_level_window_create_request\([\s\S]*NativeWindowLinuxX11TopLevelWindowResourceIdKind::Window[\s\S]*input\.window_id[\s\S]*NativeWindowLinuxX11TopLevelWindowResourceIdKind::ParentWindow[\s\S]*input\.parent_window_id[\s\S]*native_window_linux_x11_top_level_window_create_request_after_resource_id_validation\(input\)/);
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub fn native_window_linux_x11_top_level_window_create_request_from_setup_resource_info[\s\S]*native_window_linux_x11_resource_id_from_serial[\s\S]*input\.setup_resource_info[\s\S]*input\.window_resource_serial[\s\S]*ResourceIdAllocationFailed[\s\S]*let parent_window_id = input\.setup_resource_info\.first_root_window_id\(\)[\s\S]*native_window_linux_x11_top_level_window_validate_root_parent_resource_id\(parent_window_id\)[\s\S]*native_window_linux_x11_top_level_window_create_request_after_resource_id_validation/);
    ...
    assert.match(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /bytes\.push\(NATIVE_WINDOW_LINUX_X11_MAP_WINDOW_REQUEST_OPCODE\)[\s\S]*NATIVE_WINDOW_LINUX_X11_MAP_WINDOW_REQUEST_LENGTH_UNITS\.to_le_bytes\(\)[\s\S]*window_id\.to_le_bytes\(\)/);
    assert.doesNotMatch(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /pub window_id:|pub parent_window_id:|pub bytes:/);
    assert.doesNotMatch(nativeWindowLinuxX11TopLevelWindowCreateRequestSurface, /NativeWindowLinuxX11EventSourceObservationReader|NativeWindowLinuxX11SetupRequest|NativeWindowLinuxX11XauthoritySelector|NativeWindowLinuxX11XauthoritySelection|NoMatchingRecord|std::env|std::fs|vfs|\bDISPLAY\b|\bXAUTHORITY\b|\bHOME\b|read_xauthority_environment_value|read_xauthority_file_bytes|native_window_linux_x11_xauthority_select_credential|native_window_linux_x11_setup_request_from_xauthority|native_window_linux_x11_setup_request_from_authorization|write_x11_bytes_raw|read_x11_bytes_raw|sequence|AtomId|reply|validate_native_window_run_loop_platform_wait_runner_support_for_platform|run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|run_minifb|WindowOptions|ScaleMode|window\.update\(|update_with_buffer|set_target_fps|WM_DELETE|InternAtom|ChangeProperty|ClientMessage|IME|fallback|silent no-op|synthetic/i);
    assert.match(libSource, /native_window_linux_x11_create_window_request_encodes_standalone_bytes/);
    assert.match(libSource, /native_window_linux_x11_map_window_request_encodes_standalone_bytes/);
    assert.match(libSource, /native_window_linux_x11_map_window_request_rejects_invalid_window_ids/);
    ...
    assert.match(libSource, /native_window_linux_x11_setup_backed_top_level_window_create_request_uses_allocated_window_and_root_parent/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_top_level_window_create_request_preserves_allocation_failure/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_top_level_window_create_request_preserves_build_failure/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_builds_top_level_request_from_setup_resource_info/);
    ...
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_INTERN_ATOM_REPLY_EXPECTED_LENGTH_UNITS: u32 = 0/);
    assert.match(libSource, /NATIVE_WINDOW_LINUX_X11_INTERN_ATOM_REPLY_ATOM_ID_OFFSET: usize = 8/);
    ...
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /pub struct NativeWindowLinuxX11AtomId\s*\{[\s\S]*raw: u32/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /pub enum NativeWindowLinuxX11AtomIdError\s*\{[\s\S]*Zero/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /pub struct NativeWindowLinuxX11InternAtomReply\s*\{[\s\S]*sequence: u16,[\s\S]*atom_id: NativeWindowLinuxX11AtomId/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /pub enum NativeWindowLinuxX11InternAtomReplyParseError\s*\{[\s\S]*InvalidResponseType\s*\{[\s\S]*response_type: u8[\s\S]*UnexpectedLengthUnits\s*\{[\s\S]*length_units: u32,[\s\S]*expected_length_units: u32[\s\S]*AtomIdInvalid\s*\{[\s\S]*raw: u32,[\s\S]*error: NativeWindowLinuxX11AtomIdError/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /impl NativeWindowLinuxX11AtomId[\s\S]*pub fn new\(raw: u32\) -> Result<Self, NativeWindowLinuxX11AtomIdError>[\s\S]*raw == 0[\s\S]*NativeWindowLinuxX11AtomIdError::Zero[\s\S]*pub fn raw\(self\) -> u32/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /impl NativeWindowLinuxX11InternAtomReply[\s\S]*pub fn sequence\(&self\) -> u16[\s\S]*pub fn atom_id\(&self\) -> NativeWindowLinuxX11AtomId/);
    assert.match(nativeWindowLinuxX11InternAtomReplySurface, /pub fn native_window_linux_x11_intern_atom_reply_from_packet\([\s\S]*&\[u8; NATIVE_WINDOW_LINUX_X11_EVENT_PACKET_BYTE_LEN\][\s\S]*response_type != NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_REPLY[\s\S]*InvalidResponseType[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_REPLY_LENGTH_UNITS_OFFSET[\s\S]*UnexpectedLengthUnits[\s\S]*NATIVE_WINDOW_LINUX_X11_INTERN_ATOM_REPLY_ATOM_ID_OFFSET[\s\S]*AtomIdInvalid[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_REPLY_SEQUENCE_OFFSET/);
    assert.doesNotMatch(nativeWindowLinuxX11InternAtomReplySurface, /NativeWindowLinuxX11EventSourceObservationReader|pending_server_reply|ServerReplyReceived|ServerErrorReceived|write_x11_bytes_raw|read_x11_bytes_raw|accepted|correlation|request_start_byte_offset|request_end_byte_offset|WmProtocols|WmDeleteWindow|ChangeProperty|ClientMessage|MapWindow|run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|WindowOptions|window\.update\(|update_with_buffer|support gate|fallback|silent no-op|synthetic/i);
    assert.match(libSource, /native_window_linux_x11_intern_atom_reply_packet_preserves_sequence_and_atom_id/);
    assert.match(libSource, /native_window_linux_x11_intern_atom_reply_packet_rejects_wrong_response_type/);
    assert.match(libSource, /native_window_linux_x11_intern_atom_reply_packet_rejects_nonzero_length_units/);
    assert.match(libSource, /native_window_linux_x11_intern_atom_reply_packet_rejects_zero_atom_id/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl<'a> NativeWindowLinuxX11XauthoritySelectorCriteria<'a>[\s\S]*pub fn display_number\(&self\) -> &\[u8\][\s\S]*pub fn selector\(&self\) -> NativeWindowLinuxX11XauthoritySelector<'_>/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl<'a> NativeWindowLinuxX11XauthorityLookupInput<'a>[\s\S]*pub fn new\([\s\S]*authority_file_path: Option<&'a str>,[\s\S]*home_directory_path: Option<&'a str>[\s\S]*pub fn authority_file_path\(&self\) -> Option<&'a str>[\s\S]*pub fn home_directory_path\(&self\) -> Option<&'a str>/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11XauthorityPathPlan[\s\S]*pub fn source\(&self\) -> NativeWindowLinuxX11XauthorityPathSource[\s\S]*pub fn path\(&self\) -> &str/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_XAUTHORITY_HOME_DEFAULT_FILE_NAME: &str = "\.Xauthority"/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_xauthority_path_contains_nul[\s\S]*as_bytes\(\)\.iter\(\)\.any/);
    ...
    assert.match(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /pub enum NativeWindowLinuxX11XauthorityCredentialSetupRequestError\s*\{[\s\S]*ParseFailed\s*\{[\s\S]*error: NativeWindowLinuxX11XauthorityParseError[\s\S]*NoMatchingCredential[\s\S]*SetupRequestBuildFailed\s*\{[\s\S]*error: NativeWindowLinuxX11SetupRequestBuildError/);
    assert.match(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /pub fn native_window_linux_x11_setup_request_from_xauthority\([\s\S]*file_bytes: &NativeWindowLinuxX11XauthorityFileBytes,[\s\S]*criteria: &NativeWindowLinuxX11XauthoritySelectorCriteria<'_>[\s\S]*Result<[\s\S]*NativeWindowLinuxX11SetupRequest,[\s\S]*NativeWindowLinuxX11XauthorityCredentialSetupRequestError/);
    assert.match(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /native_window_linux_x11_xauthority_select_credential\([\s\S]*file_bytes\.as_bytes\(\),[\s\S]*criteria\.selector\(\)[\s\S]*\)[\s\S]*NativeWindowLinuxX11XauthorityCredentialSetupRequestError::ParseFailed/);
    assert.match(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /match selection[\s\S]*NativeWindowLinuxX11XauthoritySelection::Selected\s*\{ credential \}[\s\S]*native_window_linux_x11_setup_request_from_authorization\(credential\)[\s\S]*SetupRequestBuildFailed/);
    assert.match(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /NativeWindowLinuxX11XauthoritySelection::NoMatchingRecord[\s\S]*NativeWindowLinuxX11XauthorityCredentialSetupRequestError::NoMatchingCredential/);
    assert.doesNotMatch(nativeWindowLinuxX11XauthorityCredentialSetupRequest, /AuthorizationCredential::none|no_authorization|std::env|std::fs|vfs|read_xauthority_file_bytes|read_xauthority_environment_value|write_x11_bytes_raw|read_x11_bytes_raw|validate_native_window_run_loop_platform_wait_runner_support_for_platform|run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|run_minifb|WindowOptions|ScaleMode|window\.update\(|update_with_buffer|set_target_fps|fallback|silent no-op|synthetic/i);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11TopLevelWindowRequestWriteState\s*\{[\s\S]*NotConfigured,[\s\S]*SetupBackedBuildPending,[\s\S]*RequestPending,[\s\S]*Ready,[\s\S]*Failed/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState\s*\{[\s\S]*NotConfigured,[\s\S]*BatchPending,[\s\S]*Ready,[\s\S]*Failed/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11ServerErrorCorrelation\s*\{[\s\S]*Unmatched,[\s\S]*TopLevelWindowCreate\s*\{\s*window_id: u32\s*\},[\s\S]*TopLevelWindowMap\s*\{\s*window_id: u32\s*\}/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11TopLevelWindowRequestSequencePlan\s*\{[\s\S]*window_id: u32,[\s\S]*create_window_sequence: Option<u16>,[\s\S]*map_window_sequence: Option<u16>/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11TopLevelWindowRequestSequencePlan[\s\S]*pub fn new\(window_id: u32\) -> Self[\s\S]*pub fn create_window_sequence\(&self\) -> Option<u16>[\s\S]*pub fn map_window_sequence\(&self\) -> Option<u16>[\s\S]*server_error_correlation/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /server_error_correlation[\s\S]*self\.create_window_sequence == Some\(error\.sequence\(\)\)[\s\S]*TopLevelWindowCreate[\s\S]*self\.map_window_sequence == Some\(error\.sequence\(\)\)[\s\S]*TopLevelWindowMap/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11WmProtocolAtomInternReplyCorrelation\s*\{[\s\S]*WmProtocols,[\s\S]*WmDeleteWindow/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11WmProtocolAtomMeaning\s*\{[\s\S]*WmProtocols,[\s\S]*WmDeleteWindow/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11WmProtocolAtomInternRequestSequencePlan\s*\{[\s\S]*wm_protocols_sequence: Option<u16>,[\s\S]*wm_delete_window_sequence: Option<u16>/);
    assert.match(nativeWindowLinuxX11WmProtocolAtomAssignmentSurface, /pub struct NativeWindowLinuxX11WmProtocolAtoms\s*\{[\s\S]*wm_protocols_atom: NativeWindowLinuxX11AtomId,[\s\S]*wm_delete_window_atom: NativeWindowLinuxX11AtomId/);
    assert.match(nativeWindowLinuxX11WmProtocolAtomAssignmentSurface, /pub struct NativeWindowLinuxX11WmProtocolAtomAssignmentState\s*\{[\s\S]*wm_protocols_atom: Option<NativeWindowLinuxX11AtomId>,[\s\S]*wm_delete_window_atom: Option<NativeWindowLinuxX11AtomId>/);
    assert.match(nativeWindowLinuxX11WmProtocolAtomAssignmentSurface, /pub enum NativeWindowLinuxX11WmProtocolAtomAssignmentError\s*\{[\s\S]*DuplicateAssignment\s*\{[\s\S]*meaning: NativeWindowLinuxX11WmProtocolAtomMeaning,[\s\S]*existing: NativeWindowLinuxX11AtomId,[\s\S]*incoming: NativeWindowLinuxX11AtomId/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11WmProtocolAtomInternRequestSequencePlan[\s\S]*pub fn new\(\) -> Self[\s\S]*wm_protocols_sequence\(&self\) -> Option<u16>[\s\S]*wm_delete_window_sequence\(&self\) -> Option<u16>[\s\S]*record_request_sequence[\s\S]*take_reply_correlation/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11WmProtocolAtomMeaning[\s\S]*from_reply_correlation[\s\S]*WmProtocolAtomInternReplyCorrelation::WmProtocols[\s\S]*Self::WmProtocols[\s\S]*WmProtocolAtomInternReplyCorrelation::WmDeleteWindow[\s\S]*Self::WmDeleteWindow/);
    assert.match(nativeWindowLinuxX11WmProtocolAtomAssignmentSurface, /impl NativeWindowLinuxX11WmProtocolAtomAssignmentState[\s\S]*pub fn new\(\) -> Self[\s\S]*wm_protocols_atom: None[\s\S]*wm_delete_window_atom: None[\s\S]*pub fn completed_atoms\(&self\) -> Option<NativeWindowLinuxX11WmProtocolAtoms>[\s\S]*self\.wm_protocols_atom\?[\s\S]*self\.wm_delete_window_atom\?[\s\S]*pub fn assign_reply[\s\S]*from_reply_correlation\(correlation\)[\s\S]*reply\.atom_id\(\)[\s\S]*pub fn assign_atom[\s\S]*DuplicateAssignment[\s\S]*existing[\s\S]*incoming/);
    assert.doesNotMatch(nativeWindowLinuxX11WmProtocolAtomAssignmentSurface, /ChangeProperty|ClientMessage|register|registration|write_x11_bytes_raw|read_x11_bytes_raw|run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|WindowOptions|window\.update\(|update_with_buffer|support gate|fallback|silent no-op|synthetic/i);
    assert.match(nativeWindowLinuxX11ObservationError, /TopLevelWindowRequestPreviouslyFailed\s*\{[\s\S]*descriptor: NativeWindowLinuxWindowEventSourceDescriptor[\s\S]*TopLevelWindowRequestSetupBackedPlanMissing[\s\S]*TopLevelWindowRequestSetupResourceInfoMissing[\s\S]*TopLevelWindowRequestBuildFailed\s*\{[\s\S]*error: NativeWindowLinuxX11SetupBackedTopLevelWindowCreateRequestError[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteFailed\s*\{[\s\S]*code: u32[\s\S]*TopLevelWindowRequestWriteReturnedZero[\s\S]*TopLevelWindowRequestWriteOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize[\s\S]*WmProtocolAtomInternBatchPreviouslyFailed\s*\{[\s\S]*descriptor: NativeWindowLinuxWindowEventSourceDescriptor[\s\S]*WmProtocolAtomInternBatchWriteWouldBlock[\s\S]*WmProtocolAtomInternBatchWriteFailed\s*\{[\s\S]*code: u32[\s\S]*WmProtocolAtomInternBatchWriteReturnedZero[\s\S]*WmProtocolAtomInternBatchWriteOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize/);
    assert.match(nativeWindowLinuxX11ObservationError, /EventReadOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerErrorReceived\s*\{[\s\S]*error: NativeWindowLinuxX11ServerErrorPacket,[\s\S]*correlation: NativeWindowLinuxX11ServerErrorCorrelation,[\s\S]*ServerReplyBodyLengthOverflow\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*units: u32,[\s\S]*ServerReplyBodyReadWouldBlock\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerReplyBodyReadFailed\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*code: u32,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerReplyBodyReadEof\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerReplyBodyReadOverflow\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize,[\s\S]*WmProtocolAtomInternReplyParseFailed\s*\{[\s\S]*correlation: NativeWindowLinuxX11WmProtocolAtomInternReplyCorrelation,[\s\S]*error: NativeWindowLinuxX11InternAtomReplyParseError,[\s\S]*WmProtocolAtomAssignmentFailed\s*\{[\s\S]*correlation: NativeWindowLinuxX11WmProtocolAtomInternReplyCorrelation,[\s\S]*reply: NativeWindowLinuxX11InternAtomReply,[\s\S]*error: NativeWindowLinuxX11WmProtocolAtomAssignmentError,[\s\S]*WmProtocolAtomInternReplyReceived\s*\{[\s\S]*correlation: NativeWindowLinuxX11WmProtocolAtomInternReplyCorrelation,[\s\S]*reply: NativeWindowLinuxX11InternAtomReply,[\s\S]*completed_atoms: Option<NativeWindowLinuxX11WmProtocolAtoms>,[\s\S]*ServerReplyReceived\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*ClientMessageWmProtocolNotRegistered[\s\S]*ClientMessageFormatMismatch\s*\{[\s\S]*format: u8,[\s\S]*expected_format: u8,[\s\S]*ClientMessageWindowMismatch\s*\{[\s\S]*window_id: u32,[\s\S]*registered_window_id: u32,[\s\S]*ClientMessageTypeAtomMismatch\s*\{[\s\S]*type_atom_raw: u32,[\s\S]*expected_wm_protocols_atom: NativeWindowLinuxX11AtomId,[\s\S]*ClientMessageProtocolAtomMismatch\s*\{[\s\S]*protocol_atom_raw: u32,[\s\S]*expected_wm_delete_window_atom: NativeWindowLinuxX11AtomId,[\s\S]*EventTypeUnsupported/);
    assert.match(nativeWindowLinuxX11ObservationError, /SetupResourceInfoParseFailed\s*\{[\s\S]*error: NativeWindowLinuxX11SetupResourceInfoParseError/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11ServerErrorPacket\s*\{[\s\S]*error_code: u8,[\s\S]*sequence: u16,[\s\S]*bad_value: u32,[\s\S]*minor_opcode: u16,[\s\S]*major_opcode: u8/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11ServerErrorPacket[\s\S]*pub fn error_code\(&self\) -> u8[\s\S]*pub fn sequence\(&self\) -> u16[\s\S]*pub fn bad_value\(&self\) -> u32[\s\S]*pub fn minor_opcode\(&self\) -> u16[\s\S]*pub fn major_opcode\(&self\) -> u8/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11ServerReplyHeader\s*\{[\s\S]*reply_data: u8,[\s\S]*sequence: u16,[\s\S]*length_units: u32/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11ServerReplyHeader[\s\S]*pub fn reply_data\(&self\) -> u8[\s\S]*pub fn sequence\(&self\) -> u16[\s\S]*pub fn length_units\(&self\) -> u32/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_MIN_KEYCODE_OFFSET:\s*usize\s*=\s*26/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_MAX_KEYCODE_OFFSET:\s*usize\s*=\s*27/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_MIN_KEYCODE_PROTOCOL_MIN:\s*u8\s*=\s*8/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11SetupResourceInfo\s*\{[\s\S]*resource_id_base: u32,[\s\S]*resource_id_mask: u32,[\s\S]*first_root_window_id: u32,[\s\S]*min_keycode: u8,[\s\S]*max_keycode: u8/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /impl NativeWindowLinuxX11SetupResourceInfo[\s\S]*pub fn min_keycode\(&self\) -> u8[\s\S]*pub fn max_keycode\(&self\) -> u8/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11SetupResourceInfoParseError\s*\{[\s\S]*BodyTooShort[\s\S]*ResourceIdMaskEmpty[\s\S]*ResourceIdBaseMaskOverlap[\s\S]*ClientResourceIdSpaceUsesHighBits[\s\S]*MinKeycodeBelowProtocolMinimum[\s\S]*MaxKeycodeBelowMinKeycode[\s\S]*FirstRootWindowIdZero/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11ResourceIdAllocationError\s*\{[\s\S]*SerialZero[\s\S]*SerialExhausted[\s\S]*GeneratedResourceIdZero[\s\S]*GeneratedResourceIdOutsideBaseMask/);
    assert.match(nativeWindowLinuxX11ObservationReader, /pub struct NativeWindowLinuxX11EventSourceObservationReader<Api>\s*\{[\s\S]*setup_request: NativeWindowLinuxX11SetupRequest,[\s\S]*setup_state: NativeWindowLinuxX11EventSourceSetupState,[\s\S]*setup_request_written_len: usize,[\s\S]*setup_backed_top_level_window_create_plan:\s*Option<NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan>,[\s\S]*top_level_window_request: Option<NativeWindowLinuxX11TopLevelWindowCreateRequest>,[\s\S]*top_level_window_request_state: NativeWindowLinuxX11TopLevelWindowRequestWriteState,[\s\S]*top_level_window_request_written_len: usize,[\s\S]*top_level_window_request_sequence_plan:\s*Option<NativeWindowLinuxX11TopLevelWindowRequestSequencePlan>,[\s\S]*wm_protocol_atom_intern_request_batch:\s*Option<NativeWindowLinuxX11WmProtocolAtomInternRequestBatch>,[\s\S]*wm_protocol_atom_intern_batch_write_state:\s*NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState,[\s\S]*wm_protocol_atom_intern_batch_written_len: usize,[\s\S]*wm_protocol_atom_intern_request_sequence_plan:\s*Option<NativeWindowLinuxX11WmProtocolAtomInternRequestSequencePlan>,[\s\S]*wm_protocol_atom_assignment_state:\s*NativeWindowLinuxX11WmProtocolAtomAssignmentState,[\s\S]*next_x11_request_sequence: u16,[\s\S]*setup_prefix_len: usize,[\s\S]*setup_body_remaining_len: usize,[\s\S]*setup_body_bytes: Vec<u8>,[\s\S]*setup_resource_info: Option<NativeWindowLinuxX11SetupResourceInfo>,[\s\S]*event_packet_len: usize,[\s\S]*pending_server_reply_header: Option<NativeWindowLinuxX11ServerReplyHeader>,[\s\S]*pending_server_reply_packet: Option<\[u8; NATIVE_WINDOW_LINUX_X11_EVENT_PACKET_BYTE_LEN\]>,[\s\S]*server_reply_body_remaining_len: usize/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_setup_resource_info_validate_client_id_space[\s\S]*resource_id_mask == 0[\s\S]*ResourceIdMaskEmpty[\s\S]*resource_id_base & resource_id_mask != 0[\s\S]*ResourceIdBaseMaskOverlap[\s\S]*ClientResourceIdSpaceUsesHighBits/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_setup_resource_info_validate_keycode_range[\s\S]*min_keycode < NATIVE_WINDOW_LINUX_X11_MIN_KEYCODE_PROTOCOL_MIN[\s\S]*MinKeycodeBelowProtocolMinimum[\s\S]*max_keycode < min_keycode[\s\S]*MaxKeycodeBelowMinKeycode/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn native_window_linux_x11_setup_resource_info_from_little_endian_success_body[\s\S]*Result<[\s\S]*Option<NativeWindowLinuxX11SetupResourceInfo>[\s\S]*NativeWindowLinuxX11SetupResourceInfoParseError[\s\S]*body\.is_empty\(\)[\s\S]*Ok\(None\)[\s\S]*NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_FIXED_BODY_BYTE_LEN[\s\S]*native_window_linux_x11_u32_le[\s\S]*NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_RESOURCE_ID_BASE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_RESOURCE_ID_MASK_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_MIN_KEYCODE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SETUP_SUCCESS_MAX_KEYCODE_OFFSET[\s\S]*native_window_linux_x11_setup_resource_info_validate_keycode_range[\s\S]*NoRootScreen[\s\S]*FirstRootWindowIdZero/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn native_window_linux_x11_resource_id_from_serial[\s\S]*if serial == 0[\s\S]*SerialZero[\s\S]*for bit_index in 0\.\.u32::BITS[\s\S]*resource_id_mask & bit != 0[\s\S]*serial_bits >>= 1[\s\S]*SerialExhausted[\s\S]*generated_id & !resource_id_mask/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn new_with_setup_request\([\s\S]*api: Api,[\s\S]*setup_request: NativeWindowLinuxX11SetupRequest[\s\S]*Self[\s\S]*setup_request,/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /new_with_setup_request\([\s\S]*setup_backed_top_level_window_create_plan: None,[\s\S]*top_level_window_request: None,[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured,[\s\S]*top_level_window_request_written_len: 0,[\s\S]*top_level_window_request_sequence_plan: None,[\s\S]*wm_protocol_atom_intern_request_batch: None,[\s\S]*NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState::NotConfigured,[\s\S]*wm_protocol_atom_intern_batch_written_len: 0,[\s\S]*wm_protocol_atom_intern_request_sequence_plan: None,[\s\S]*NativeWindowLinuxX11WmProtocolAtomAssignmentState::new\(\)[\s\S]*next_x11_request_sequence: NATIVE_WINDOW_LINUX_X11_FIRST_NORMAL_REQUEST_SEQUENCE,[\s\S]*pending_server_reply_header: None,[\s\S]*pending_server_reply_packet: None,[\s\S]*server_reply_body_remaining_len: 0/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn new_with_setup_request_and_top_level_window_request\([\s\S]*setup_request: NativeWindowLinuxX11SetupRequest,[\s\S]*top_level_window_request: NativeWindowLinuxX11TopLevelWindowCreateRequest[\s\S]*Self::new_with_setup_request\(api, setup_request\)[\s\S]*reader\.install_top_level_window_request\(top_level_window_request\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn new_with_setup_request_top_level_window_request_and_wm_protocol_atom_intern_request_batch\([\s\S]*top_level_window_request: NativeWindowLinuxX11TopLevelWindowCreateRequest,[\s\S]*wm_protocol_atom_intern_request_batch: NativeWindowLinuxX11WmProtocolAtomInternRequestBatch[\s\S]*new_with_setup_request_and_top_level_window_request[\s\S]*reader\.install_wm_protocol_atom_intern_request_batch\(wm_protocol_atom_intern_request_batch\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn install_top_level_window_request[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestSequencePlan::new[\s\S]*top_level_window_request\.window_id\(\)[\s\S]*top_level_window_request_written_len = 0[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn install_wm_protocol_atom_intern_request_batch[\s\S]*wm_protocol_atom_intern_request_batch: NativeWindowLinuxX11WmProtocolAtomInternRequestBatch[\s\S]*wm_protocol_atom_intern_request_batch = Some\(wm_protocol_atom_intern_request_batch\)[\s\S]*wm_protocol_atom_intern_batch_written_len = 0[\s\S]*wm_protocol_atom_intern_request_sequence_plan =[\s\S]*Some\(NativeWindowLinuxX11WmProtocolAtomInternRequestSequencePlan::new\(\)\)[\s\S]*wm_protocol_atom_assignment_state =[\s\S]*NativeWindowLinuxX11WmProtocolAtomAssignmentState::new\(\)[\s\S]*NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState::BatchPending/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn new_with_setup_request_and_setup_backed_top_level_window_create_plan\([\s\S]*plan: NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan[\s\S]*Self::new_with_setup_request\(api, setup_request\)[\s\S]*reader\.setup_backed_top_level_window_create_plan = Some\(plan\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::SetupBackedBuildPending/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn top_level_window_request\([\s\S]*Option<&NativeWindowLinuxX11TopLevelWindowCreateRequest>[\s\S]*self\.top_level_window_request\.as_ref\(\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn top_level_window_request_state\([\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState[\s\S]*self\.top_level_window_request_state/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn top_level_window_request_written_len\(&self\) -> usize[\s\S]*self\.top_level_window_request_written_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn top_level_window_request_sequence_plan\([\s\S]*Option<NativeWindowLinuxX11TopLevelWindowRequestSequencePlan>[\s\S]*self\.top_level_window_request_sequence_plan/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn wm_protocol_atom_intern_request_batch\([\s\S]*Option<&NativeWindowLinuxX11WmProtocolAtomInternRequestBatch>[\s\S]*self\.wm_protocol_atom_intern_request_batch\.as_ref\(\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn wm_protocol_atom_intern_batch_write_state\([\s\S]*NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState[\s\S]*self\.wm_protocol_atom_intern_batch_write_state/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn wm_protocol_atom_intern_batch_written_len\(&self\) -> usize[\s\S]*self\.wm_protocol_atom_intern_batch_written_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn wm_protocol_atom_intern_request_sequence_plan\([\s\S]*Option<NativeWindowLinuxX11WmProtocolAtomInternRequestSequencePlan>[\s\S]*self\.wm_protocol_atom_intern_request_sequence_plan/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn wm_protocol_atom_assignment_state\([\s\S]*NativeWindowLinuxX11WmProtocolAtomAssignmentState[\s\S]*self\.wm_protocol_atom_assignment_state/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn next_x11_request_sequence\(&self\) -> u16[\s\S]*self\.next_x11_request_sequence/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn setup_backed_top_level_window_create_plan\([\s\S]*Option<NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan>[\s\S]*self\.setup_backed_top_level_window_create_plan/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn setup_resource_info\(&self\) -> Option<NativeWindowLinuxX11SetupResourceInfo>[\s\S]*self\.setup_resource_info/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn pending_server_reply_header\(&self\) -> Option<NativeWindowLinuxX11ServerReplyHeader>[\s\S]*self\.pending_server_reply_header/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn pending_server_reply_packet\([\s\S]*Option<\[u8; NATIVE_WINDOW_LINUX_X11_EVENT_PACKET_BYTE_LEN\]>[\s\S]*self\.pending_server_reply_packet/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn server_reply_body_remaining_len\(&self\) -> usize[\s\S]*self\.server_reply_body_remaining_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_setup_request[\s\S]*let request_len = self\.setup_request\.len\(\)[\s\S]*self\.setup_request\.as_bytes\(\)[\s\S]*write_x11_bytes_raw/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_top_level_window_request_until[\s\S]*target_len: usize[\s\S]*let Some\(request_len\)[\s\S]*map\(NativeWindowLinuxX11TopLevelWindowCreateRequest::len\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*let target_len = target_len\.min\(request_len\)[\s\S]*while self\.top_level_window_request_written_len < target_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_top_level_window_request_until[\s\S]*match self\.top_level_window_request\.as_ref\(\)[\s\S]*Some\(request\) => request\.as_bytes\(\),[\s\S]*None =>[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*write_x11_bytes_raw[\s\S]*&request\[self\.top_level_window_request_written_len\.\.target_len\]/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn take_next_x11_request_sequence[\s\S]*let sequence = self\.next_x11_request_sequence;[\s\S]*wrapping_add\(1\)[\s\S]*sequence/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn record_top_level_window_request_accepted_range[\s\S]*previous_written_len < NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_REQUEST_BYTE_LEN[\s\S]*accepted_end_len >= NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_REQUEST_BYTE_LEN[\s\S]*record_create_window_sequence[\s\S]*previous_written_len < NATIVE_WINDOW_LINUX_X11_TOP_LEVEL_WINDOW_REQUEST_BYTE_LEN[\s\S]*accepted_end_len >= NATIVE_WINDOW_LINUX_X11_TOP_LEVEL_WINDOW_REQUEST_BYTE_LEN[\s\S]*record_map_window_sequence/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn record_wm_protocol_atom_intern_batch_accepted_range[\s\S]*request_end_byte_offset\([\s\S]*WmProtocols[\s\S]*request_end_byte_offset\([\s\S]*WmDeleteWindow[\s\S]*previous_written_len < wm_protocols_end_byte_offset[\s\S]*accepted_end_len >= wm_protocols_end_byte_offset[\s\S]*take_next_x11_request_sequence[\s\S]*record_request_sequence[\s\S]*WmProtocols[\s\S]*previous_written_len < wm_delete_window_end_byte_offset[\s\S]*accepted_end_len >= wm_delete_window_end_byte_offset[\s\S]*take_next_x11_request_sequence[\s\S]*record_request_sequence[\s\S]*WmDeleteWindow/);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn drain_pending_server_reply_body[\s\S]*Option<\[u8; NATIVE_WINDOW_LINUX_X11_EVENT_PACKET_BYTE_LEN\]>[\s\S]*let Some\(packet\) = self\.pending_server_reply_packet[\s\S]*return Ok\(None\)[\s\S]*self\.drain_server_reply_body\(raw_fd, packet\)\.map\(Some\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn server_reply_error_from_packet[\s\S]*native_window_linux_x11_server_reply_header\(&packet\)[\s\S]*take_reply_correlation\(reply\.sequence\(\)\)[\s\S]*ServerReplyReceived\s*\{[\s\S]*reply[\s\S]*native_window_linux_x11_intern_atom_reply_from_packet\(&packet\)[\s\S]*assign_reply\(correlation, reply\)[\s\S]*WmProtocolAtomInternReplyReceived[\s\S]*completed_atoms[\s\S]*WmProtocolAtomAssignmentFailed[\s\S]*WmProtocolAtomInternReplyParseFailed/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /self\.ensure_setup_ready\(descriptor\)\?;[\s\S]*self\.ensure_top_level_window_request_ready\(descriptor\)\?;[\s\S]*if let Some\(packet\) = self\.drain_pending_server_reply_body\(descriptor\.raw_fd\(\)\)\?[\s\S]*server_reply_error_from_packet\(packet\)[\s\S]*self\.read_event_packet\(descriptor\.raw_fd\(\)\)\?/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11WindowEventSourceObservationProvider<Provider,\s*Api>\s*\{[\s\S]*provider: Provider,[\s\S]*reader: NativeWindowLinuxX11EventSourceObservationReader<Api>/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NativeWindowLinuxX11WindowEventSourceObservationProvider::new_with_setup_request_and_top_level_window_request[\s\S]*provider,[\s\S]*api,[\s\S]*setup_request,[\s\S]*top_level_window_request/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_window_event_source_observation_provider_with_setup_and_top_level_window_request/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /new_with_setup_request_top_level_window_request_and_wm_protocol_atom_intern_request_batch[\s\S]*wm_protocol_atom_intern_request_batch: NativeWindowLinuxX11WmProtocolAtomInternRequestBatch[\s\S]*NativeWindowLinuxX11EventSourceObservationReader::new_with_setup_request_top_level_window_request_and_wm_protocol_atom_intern_request_batch/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_window_event_source_observation_provider_with_setup_top_level_window_request_and_wm_protocol_atom_intern_request_batch/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /new_with_setup_request_and_setup_backed_top_level_window_create_plan[\s\S]*plan: NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan[\s\S]*NativeWindowLinuxX11EventSourceObservationReader::new_with_setup_request_and_setup_backed_top_level_window_create_plan/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /native_window_linux_x11_window_event_source_observation_provider_with_setup_and_setup_backed_top_level_window_create_plan/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupWriteWouldBlock[\s\S]*SetupPrefixReadWouldBlock[\s\S]*SetupBodyReadWouldBlock[\s\S]*EventReadWouldBlock/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*WmProtocolAtomInternBatchWriteWouldBlock[\s\S]*EventReadWouldBlock/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupPreviouslyFailed/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupPrefixReadEof[\s\S]*SetupBodyReadEof/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupRejected[\s\S]*SetupAuthRequiredUnsupported[\s\S]*SetupStatusInvalid/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_ERROR: u8 = 0[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_REPLY: u8 = 1/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn native_window_linux_x11_response_type_raw\([\s\S]*packet\[0\]/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn native_window_linux_x11_server_error_packet[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_ERROR_CODE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_ERROR_SEQUENCE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_ERROR_BAD_VALUE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_ERROR_MINOR_OPCODE_OFFSET[\s\S]*NATIVE_WINDOW_LINUX_X11_SERVER_ERROR_MAJOR_OPCODE_OFFSET/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /KeyboardKeycodeInvalid/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_KEY_PRESS/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_KEY_RELEASE/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_CONFIGURE_NOTIFY/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_MOTION_NOTIFY/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_BUTTON_PRESS/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_BUTTON_RELEASE/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_MAP_NOTIFY/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_EXPOSE/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_CLIENT_MESSAGE/);
    assert.match(libSource, /pub enum NativeWindowEventPumpEventKind\s*\{[\s\S]*Poll,[\s\S]*CloseRequested,[\s\S]*WindowResized,[\s\S]*PointerMotion,[\s\S]*PointerButton,[\s\S]*Keyboard,[\s\S]*WindowMapped,[\s\S]*RedrawRequested/);
    assert.match(libSource, /pub struct NativeWindowKeyboardModifierState\s*\{[\s\S]*raw_state: u16/);
    assert.match(libSource, /impl NativeWindowKeyboardModifierState[\s\S]*pub const fn empty\([\s\S]*raw_state: 0[\s\S]*pub const fn new\(raw_state: u16\)[\s\S]*pub fn raw_state\(self\) -> u16/);
    ...
    assert.match(nativeWindowLinuxX11KeysymProjectionSurface, /pub struct NativeWindowLinuxX11KeyboardMappingProjectionEvidence\s*\{[\s\S]*selection: NativeWindowLinuxX11KeyboardMappingSelection,[\s\S]*keysym_projection: Option<NativeWindowLinuxX11KeysymProjection>/);
    assert.match(nativeWindowLinuxX11KeysymProjectionSurface, /impl NativeWindowLinuxX11KeyboardMappingProjectionEvidence[\s\S]*pub fn from_selection\(selection: NativeWindowLinuxX11KeyboardMappingSelection\) -> Self[\s\S]*final_raw_keysym\(\)[\s\S]*project_portable_key[\s\S]*pub fn selection\(self\) -> NativeWindowLinuxX11KeyboardMappingSelection[\s\S]*pub fn keysym_projection\(self\) -> Option<NativeWindowLinuxX11KeysymProjection>[\s\S]*pub fn final_portable_key\(self\) -> Option<NativeWindowPortableKey>[\s\S]*pub fn raw_keycode\(self\) -> u8/);
    ...
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /pub struct NativeWindowLinuxX11GetKeyboardMappingRequest\s*\{[\s\S]*first_keycode: u8,[\s\S]*keycode_count: u8,[\s\S]*bytes: \[u8; NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_REQUEST_BYTE_LEN\]/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_OPCODE:\s*u8\s*=\s*101/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_REQUEST_WORD_LEN:\s*u16\s*=\s*2/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_REQUEST_BYTE_LEN[\s\S]*\* 4/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /first_keycode\s*==\s*0[\s\S]*FirstKeycodeInvalid/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /keycode_count\s*==\s*0[\s\S]*KeycodeCountInvalid/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /bytes\[0\]\s*=\s*NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_OPCODE[\s\S]*bytes\[2\.\.4\]\.copy_from_slice[\s\S]*NATIVE_WINDOW_LINUX_X11_GET_KEYBOARD_MAPPING_REQUEST_WORD_LEN[\s\S]*bytes\[4\]\s*=\s*first_keycode[\s\S]*bytes\[5\]\s*=\s*keycode_count/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /pub struct NativeWindowLinuxX11KeyboardMappingReplyHeader\s*\{[\s\S]*keysyms_per_keycode: u8,[\s\S]*sequence: u16,[\s\S]*length_units: u32/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /pub struct NativeWindowLinuxX11KeyboardMappingRawKeysyms\s*\{[\s\S]*keycode_count: u8,[\s\S]*keysyms_per_keycode: u8,[\s\S]*raw_keysyms: Vec<NativeWindowLinuxX11KeysymValue>/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /length_units[\s\S]*checked_mul\(4\)/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /keycode_count[\s\S]*keysyms_per_keycode[\s\S]*checked_mul/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /body\.len\(\)[\s\S]*BodyByteLenMismatch[\s\S]*expected_body_byte_len[\s\S]*ExpectedBodyByteLenMismatch/);
    assert.match(nativeWindowLinuxX11GetKeyboardMappingSurface, /NativeWindowLinuxX11KeysymValue::new\(u32::from_le_bytes/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /pub struct NativeWindowLinuxX11KeyboardMappingRange\s*\{[\s\S]*first_keycode: u8,[\s\S]*keycode_count: u8,[\s\S]*last_keycode: u8/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /pub enum NativeWindowLinuxX11KeyboardMappingSelection\s*\{[\s\S]*BaseColumn[\s\S]*ShiftColumn[\s\S]*ShiftNoSymbolBaseColumn[\s\S]*UnsupportedModifierState/);
    assert.doesNotMatch(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /ShiftColumnUnavailableBaseColumn/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /RawKeysymsKeycodeCountMismatch[\s\S]*RawKeysymsLenMismatch[\s\S]*ExpectedKeysymCountOverflow[\s\S]*KeycodeBelowRange[\s\S]*KeycodeAboveRange[\s\S]*KeycodeIndexOverflow[\s\S]*RowOffsetOverflow[\s\S]*ColumnOutOfRange[\s\S]*KeysymIndexOverflow[\s\S]*KeysymIndexOutOfRange/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /native_window_linux_x11_keyboard_mapping_range_from_setup_request[\s\S]*request\.min_keycode\(\)[\s\S]*request\.keycode_count\(\)/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /native_window_linux_x11_keyboard_mapping_selection_expected_len[\s\S]*checked_mul/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /checked_sub\(usize::from\(range\.first_keycode\(\)\)\)[\s\S]*checked_mul\(usize::from\(raw_keysyms\.keysyms_per_keycode\(\)\)\)[\s\S]*checked_add\(column\)/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /unsupported_unknown_mask[\s\S]*!\s*\([\s\S]*NATIVE_WINDOW_X11_CORE_STATE_KEY_SELECTION_SUPPORTED_MASK[\s\S]*NATIVE_WINDOW_X11_CORE_STATE_KEY_SELECTION_UNSUPPORTED_KNOWN_MASK[\s\S]*unsupported_mask[\s\S]*UnsupportedModifierState/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /NATIVE_WINDOW_LINUX_X11_KEYMAP_SHIFT_COLUMN[\s\S]*\?\s*;[\s\S]*NATIVE_WINDOW_X11_KEYSYM_NO_SYMBOL[\s\S]*ShiftNoSymbolBaseColumn/);
    assert.match(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /impl NativeWindowLinuxX11KeyboardMappingSelection[\s\S]*pub fn raw_keycode\(self\) -> u8[\s\S]*UnsupportedModifierState[\s\S]*pub fn final_raw_keysym\(self\) -> Option<NativeWindowLinuxX11KeysymValue>/);
    assert.doesNotMatch(nativeWindowLinuxX11KeyboardMappingSelectionSurface, /native_window_linux_x11_event_packet_to_observation|NativeWindowLinuxX11EventSourceObservationReader|read_x11_bytes_raw|write_x11_bytes_raw|libc::read|libc::recv|libc::send|NativeWindowPortableKey|NativeWindowLinuxX11KeysymProjection|NativeWindowLinuxX11KeyboardMappingProjectionEvidence|x11_keymap_projection|project_portable_key|from_x11_keysym|IME|TextInput|text_input|shortcut|run_linux_platform_wait_window_loop|validate_native_window_run_loop_platform_wait_runner_support_for_platform|PlatformRunnerIntegrationMissing|queue|fallback|silent no-op|synthetic/i);
    assert.match(nativeWindowLinuxX11EventPacketToObservation, /keyboard_mapping_request_sequence: Option<u16>/);
    assert.match(nativeWindowLinuxX11EventPacketToObservation, /keyboard_mapping_request_sequence == Some\(error\.sequence\(\)\)[\s\S]*NativeWindowLinuxX11ServerErrorCorrelation::KeyboardMapping/);
    assert.doesNotMatch(nativeWindowLinuxX11EventPacketToObservation, /GetKeyboardMapping|keysyms_per_keycode|NativeWindowLinuxX11KeyboardMappingReply|raw_keysyms|read_x11_bytes_raw|write_x11_bytes_raw|NativeWindowPortableKey|NativeWindowLinuxX11KeyboardMappingProjectionEvidence|x11_keymap_projection|from_x11_keysym|project_portable_key/i);
    assert.doesNotMatch(nativeWindowLinuxX11GetKeyboardMappingSurface, /native_window_linux_x11_event_packet_to_observation|NativeWindowLinuxX11EventSourceObservationReader|pending_|read_x11_bytes_raw|write_x11_bytes_raw|run_linux_platform_wait_window_loop|queue|IME|TextInput|text_input|shortcut|fallback|silent no-op|synthetic/i);
    assert.doesNotMatch(nativeWindowLinuxX11GetKeyboardMappingSurface, /NativeWindowPortableKey|NativeWindowLinuxX11KeysymProjection|project_portable_key|from_x11_keysym/i);
    assert.match(libSource, /native_window_linux_x11_get_keyboard_mapping_request_encodes_owner_bytes/);
    assert.match(nativeWindowLinuxX11SetupKeyboardMappingSurface, /pub struct NativeWindowLinuxX11SetupKeyboardMappingRequest\s*\{[\s\S]*min_keycode: u8,[\s\S]*max_keycode: u8,[\s\S]*request: NativeWindowLinuxX11GetKeyboardMappingRequest/);
    assert.match(nativeWindowLinuxX11SetupKeyboardMappingSurface, /pub enum NativeWindowLinuxX11SetupKeyboardMappingRequestBuildError\s*\{[\s\S]*MaxKeycodeBelowMinKeycode[\s\S]*KeycodeCountTooLarge[\s\S]*GetKeyboardMappingRequestBuildFailed/);
    assert.match(nativeWindowLinuxX11SetupKeyboardMappingSurface, /native_window_linux_x11_setup_keyboard_mapping_keycode_count[\s\S]*max_keycode < min_keycode[\s\S]*checked_sub\(u16::from\(min_keycode\)\)[\s\S]*checked_add\(1\)[\s\S]*u8::try_from\(keycode_count\)/);
    assert.match(nativeWindowLinuxX11SetupKeyboardMappingSurface, /native_window_linux_x11_setup_keyboard_mapping_request[\s\S]*let min_keycode = setup_resource_info\.min_keycode\(\)[\s\S]*let max_keycode = setup_resource_info\.max_keycode\(\)[\s\S]*native_window_linux_x11_setup_keyboard_mapping_keycode_count\(min_keycode, max_keycode\)[\s\S]*native_window_linux_x11_get_keyboard_mapping_request\(min_keycode, keycode_count\)/);
    assert.doesNotMatch(nativeWindowLinuxX11SetupKeyboardMappingSurface, /wrapping_|saturating_|as\s+u8/);
    assert.doesNotMatch(nativeWindowLinuxX11SetupKeyboardMappingSurface, /NativeWindowLinuxX11EventSourceObservationReader|native_window_linux_x11_event_packet_to_observation|read_x11_bytes_raw|write_x11_bytes_raw|pending|sequence|reply|raw_keysyms|keymap|layout|NativeWindowPortableKey|project_portable_key|from_x11_keysym|IME|TextInput|text_input|shortcut|queue|runner|support gate|fallback|silent no-op|synthetic/i);
    assert.match(libSource, /native_window_linux_x11_setup_keyboard_mapping_request_uses_setup_keycode_range/);
    assert.match(libSource, /native_window_linux_x11_setup_keyboard_mapping_request_rejects_invalid_internal_range/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_reply_preserves_raw_keysyms/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_reply_rejects_shape_mismatch/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_selection_uses_keycode_offset_and_column/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_selection_rejects_range_and_shape_errors/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_selection_rejects_missing_shift_column/);
    assert.match(libSource, /native_window_linux_x11_keyboard_mapping_selection_reports_unsupported_modifiers/);
    assert.match(nativeWindowLinuxX11ObservationReader, /keyboard_mapping_request: Option<NativeWindowLinuxX11SetupKeyboardMappingRequest>[\s\S]*keyboard_mapping_request_state: NativeWindowLinuxX11KeyboardMappingRequestWriteState[\s\S]*keyboard_mapping_request_sequence: Option<u16>[\s\S]*pending_keyboard_mapping_reply_body_bytes: Vec<u8>/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn build_setup_backed_keyboard_mapping_request[\s\S]*native_window_linux_x11_setup_keyboard_mapping_request\(setup_resource_info\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_keyboard_mapping_request[\s\S]*KeyboardMappingRequestWriteWouldBlock[\s\S]*keyboard_mapping_request_sequence[\s\S]*Some\(self\.take_next_x11_request_sequence\(\)\)[\s\S]*NativeWindowLinuxX11KeyboardMappingRequestWriteState::ReplyPending/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn complete_keyboard_mapping_reply_body_read[\s\S]*native_window_linux_x11_keyboard_mapping_raw_keysyms_from_reply/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn drain_pending_keyboard_mapping_reply_body[\s\S]*KeyboardMappingReplyBodyReadWouldBlock/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /self\.ensure_keyboard_mapping_request_ready\(descriptor\)\?[\s\S]*self\.drain_pending_keyboard_mapping_reply_body\(descriptor\.raw_fd\(\)\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /self\.keyboard_mapping_request_sequence == Some\(reply\.sequence\(\)\)[\s\S]*self\.start_keyboard_mapping_reply_body_read\(packet\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /NativeWindowLinuxX11ServerErrorCorrelation::KeyboardMapping/);
    assert.doesNotMatch(nativeWindowLinuxX11ObservationReaderImpl, /XLookupString|Xutf8LookupString|XmbLookupString|NativeWindowPortableKey|NativeWindowLinuxX11KeyboardMappingProjectionEvidence|x11_keymap_projection|from_x11_keysym|project_portable_key|IME|TextInput|text_input|shortcut|run_linux_platform_wait_window_loop|validate_native_window_run_loop_platform_wait_runner_support_for_platform|PlatformRunnerIntegrationMissing|queue|fallback|silent no-op|synthetic/i);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_missing_setup_resource_info_fails/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_partial_write_preserves_sequence/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_reply_preserves_raw_keysyms/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_reply_body_would_block_recovers/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_reply_parse_failure_fails_closed/);
    assert.match(libSource, /native_window_linux_x11_setup_backed_keyboard_mapping_server_error_is_correlated/);
    ...
    assert.match(libSource, /pub struct NativeWindowKeyboardEvent\s*\{[\s\S]*kind: NativeWindowKeyboardEventKind,[\s\S]*raw_keycode: u8,[\s\S]*modifier_state: NativeWindowKeyboardModifierState,[\s\S]*portable_modifiers: NativeWindowPortableKeyboardModifiers,[\s\S]*x11_keymap_projection: Option<NativeWindowLinuxX11KeyboardMappingProjectionEvidence>/);
    assert.match(nativeWindowKeyboardEventImpl, /pub fn new\([\s\S]*NativeWindowKeyboardModifierState::empty\(\)[\s\S]*pub fn new_with_modifier_state\([\s\S]*modifier_state: NativeWindowKeyboardModifierState[\s\S]*None[\s\S]*pub fn new_with_modifier_state_and_x11_keymap_projection\([\s\S]*x11_keymap_projection: NativeWindowLinuxX11KeyboardMappingProjectionEvidence[\s\S]*Some\(x11_keymap_projection\)[\s\S]*KeyboardMappingProjectionKeycodeMismatch[\s\S]*NativeWindowPortableKeyboardModifiers::from_x11_core_state\(modifier_state\)[\s\S]*x11_keymap_projection,[\s\S]*pub fn x11_keymap_projection\([\s\S]*Option<NativeWindowLinuxX11KeyboardMappingProjectionEvidence>/);
    assert.match(nativeWindowLinuxWindowEventSourceObservationSurface, /pub struct NativeWindowLinuxWindowEventSourceObservation\s*\{[\s\S]*event_kind: NativeWindowEventPumpEventKind,[\s\S]*keyboard_event: Option<NativeWindowKeyboardEvent>/);
    assert.match(nativeWindowLinuxWindowEventSourceObservationSurface, /pub fn new_with_event_kind\([\s\S]*event_kind: NativeWindowEventPumpEventKind[\s\S]*Self\s*\{[\s\S]*event_kind/);
    assert.match(nativeWindowLinuxWindowEventSourceObservationSurface, /pub fn new_with_event_kind_and_keyboard_event\([\s\S]*event_kind: NativeWindowEventPumpEventKind,[\s\S]*keyboard_event: Option<NativeWindowKeyboardEvent>[\s\S]*Self\s*\{[\s\S]*event_kind,[\s\S]*keyboard_event/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub struct NativeWindowLinuxX11RegisteredWmProtocolContext\s*\{[\s\S]*window_id: u32,[\s\S]*atoms: NativeWindowLinuxX11WmProtocolAtoms/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn native_window_linux_x11_client_message_close_observation\([\s\S]*registered_context: Option<NativeWindowLinuxX11RegisteredWmProtocolContext>[\s\S]*ClientMessageWmProtocolNotRegistered[\s\S]*NATIVE_WINDOW_LINUX_X11_CLIENT_MESSAGE_FORMAT_32[\s\S]*ClientMessageFormatMismatch[\s\S]*ClientMessageWindowMismatch[\s\S]*ClientMessageTypeAtomMismatch[\s\S]*ClientMessageProtocolAtomMismatch[\s\S]*NativeWindowLinuxWindowEventSourceObservation::new_with_event_kind\([\s\S]*NativeWindowEventPumpEventKind::CloseRequested[\s\S]*true,[\s\S]*false,[\s\S]*input\.previous_size,[\s\S]*input\.previous_mouse_down,[\s\S]*None/);
    ...
    assert.match(nativeWindowLinuxX11EventPacketToObservation, /registered_wm_protocol_context: Option<NativeWindowLinuxX11RegisteredWmProtocolContext>/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /NATIVE_WINDOW_LINUX_X11_EVENT_STATE_OFFSET: usize = 28/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn native_window_linux_x11_keyboard_event\([\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_DETAIL_OFFSET[\s\S]*NativeWindowKeyboardModifierState::new\([\s\S]*native_window_linux_x11_u16_le\([\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_STATE_OFFSET[\s\S]*NativeWindowKeyboardEvent::new_with_modifier_state\([\s\S]*KeyboardKeycodeInvalid/);
    assert.match(nativeWindowLinuxX11EventPacketToObservation, /match native_window_linux_x11_event_response_type\(packet\)[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_KEY_PRESS[\s\S]*native_window_linux_x11_keyboard_event\([\s\S]*NativeWindowKeyboardEventKind::Pressed[\s\S]*NativeWindowEventPumpEventKind::Keyboard[\s\S]*Some\(keyboard_event\)[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_KEY_RELEASE[\s\S]*native_window_linux_x11_keyboard_event\([\s\S]*NativeWindowKeyboardEventKind::Released[\s\S]*NativeWindowEventPumpEventKind::Keyboard[\s\S]*Some\(keyboard_event\)[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_CONFIGURE_NOTIFY[\s\S]*NativeWindowEventPumpEventKind::WindowResized[\s\S]*NativeWindowSize::new[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_MOTION_NOTIFY[\s\S]*NativeWindowEventPumpEventKind::PointerMotion[\s\S]*native_window_linux_x11_event_pointer_raw[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_BUTTON_PRESS[\s\S]*NativeWindowEventPumpEventKind::PointerButton[\s\S]*true[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_BUTTON_RELEASE[\s\S]*NativeWindowEventPumpEventKind::PointerButton[\s\S]*false[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_CLIENT_MESSAGE[\s\S]*native_window_linux_x11_client_message_close_observation\([\s\S]*registered_wm_protocol_context[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_MAP_NOTIFY[\s\S]*NativeWindowEventPumpEventKind::WindowMapped[\s\S]*NATIVE_WINDOW_LINUX_X11_EVENT_TYPE_EXPOSE[\s\S]*NativeWindowEventPumpEventKind::RedrawRequested/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /pub fn registered_wm_protocol_context\([\s\S]*Option<NativeWindowLinuxX11RegisteredWmProtocolContext>[\s\S]*wm_protocol_registration_write_state[\s\S]*NativeWindowLinuxX11WmProtocolRegistrationWriteState::Ready[\s\S]*return None[\s\S]*wm_protocol_registration_request[\s\S]*NativeWindowLinuxX11RegisteredWmProtocolContext::new\([\s\S]*request\.window_id\(\),[\s\S]*request\.atoms\(\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /native_window_linux_x11_event_packet_to_observation\([\s\S]*self\.top_level_window_request_sequence_plan,[\s\S]*self\.keyboard_mapping_request_sequence,[\s\S]*self\.wm_protocol_registration_sequence_plan,[\s\S]*self\.registered_wm_protocol_context\(\)/);
    assert.match(libSource, /native_window_linux_x11_client_message_decode_masks_send_event_bit/);
    assert.match(libSource, /native_window_linux_x11_registered_context_requires_accepted_registration/);
    assert.match(libSource, /native_window_linux_x11_structure_expose_decode_preserves_event_kind/);
    assert.match(libSource, /native_window_linux_x11_event_decode_marks_pointer_and_close_kinds/);
    assert.match(libSource, /native_window_linux_x11_keyboard_decode_preserves_raw_key_and_modifier_evidence/);
    ...
    assert.doesNotMatch(nativeWindowLinuxX11EventPacketToObservation, /XLookupString|Xutf8LookupString|XmbLookupString|Keysym|KeySym|NativeWindowPortableKey|NativeWindowLinuxX11KeyboardMappingProjectionEvidence|x11_keymap_projection|from_x11_keysym|project_portable_key|IME|TextInput|text_input|text input|shortcut|run_linux_platform_wait_window_loop|validate_native_window_run_loop_platform_wait_runner_support_for_platform|PlatformRunnerIntegrationMissing|fallback|silent no-op|synthetic/i);
    ...
    assert.match(nativeWindowLinuxX11XauthorityProcessEnvironmentAdapter, /HomeDirectoryPath => "HOME"/);
    assert.match(nativeWindowLinuxX11XauthorityProcessEnvironmentAdapter, /std::env::VarError::NotPresent\) => Ok\(None\)/);
    ...
    assert.match(libSource, /native_window_linux_x11_wm_protocol_atom_intern_reply_sequences_are_retained/);
    ...
    assert.match(libSource, /native_window_linux_x11_setup_resource_info_rejects_missing_or_truncated_screen/);
    assert.match(libSource, /native_window_linux_x11_resource_id_from_serial_maps_sparse_mask/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_resumes_partial_setup_resource_body/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_fails_closed_after_setup_resource_parse_failure/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_reports_server_reply_header/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_drains_server_reply_body_before_next_event/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_resumes_server_reply_body_after_would_block/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_keeps_server_reply_body_state_after_eof/);
    assert.match(libSource, /native_window_linux_x11_observation_provider_keeps_server_reply_body_state_after_read_failure/);
    assert.match(libSource, /top_level_window_request_state\(\),[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured/);
    assert.match(libSource, /native_window_linux_x11_xauthority_selects_exact_local_record_zero_copy/);
    assert.match(libSource, /native_window_linux_x11_xauthority_path_plan_preserves_explicit_path/);
    ...
    assert.match(libSource, /native_window_linux_x11_xauthority_vfs_file_bytes_reader_forwards_exact_path/);
    assert.match(libSource, /native_window_linux_x11_xauthority_vfs_file_bytes_reader_preserves_source_failure/);
    assert.match(libSource, /native_window_linux_x11_xauthority_vfs_helper_uses_f5jg_validation/);
    assert.match(libSource, /native_window_linux_x11_xauthority_setup_request_uses_selected_credential/);
    assert.match(libSource, /native_window_linux_x11_xauthority_setup_request_rejects_no_matching_record/);
    assert.match(libSource, /native_window_linux_x11_xauthority_setup_request_preserves_parse_failure/);
    ...
    assert.match(libSource, /native_window_linux_x11_xauthority_file_bytes_preserves_reader_failure_source_and_path/);
    assert.match(libSource, /native_window_linux_x11_xauthority_selector_criteria_owns_display_number_bytes/);
    assert.match(libSource, /native_window_linux_x11_xauthority_selector_criteria_uses_local_display_number/);
    assert.match(libSource, /native_window_linux_x11_xauthority_selector_criteria_rejects_host_and_malformed_display/);
    ...
    assert.doesNotMatch(nativeWindowPlatformWaitBackendKindPolicySurface, /std::env|env::var|env::consts|from_str|parse::<|stringify|to_string|format!|HeadlessScripted\s*\)|=>\s*Ok\(\s*NativeWindowHostLoopPlatformWaitBackendKind::HeadlessScripted|Minifb|WindowOptions|ScaleMode|window\.update\(|update_with_buffer|set_target_fps|execute_native_window_host_loop_interruptible_deadline_wait_with_adapter|execute_native_window_host_loop_wait_with_owner|std::thread::sleep|Duration|setTimeout|setInterval|DOM|Canvas|video_memory|stdout_protocol|fallback|silent no-op/i);
    ...
    return {
        ...
        checks: [
            ...
            "Native Linux window event source fd acquisition keeps socket ownership explicit",
            ...
            "Native Linux X11 GetKeyboardMapping request/reply owner keeps raw keysyms without event decode",
            "Native Linux X11 setup-owned keyboard mapping request derives checked range without event decode",
            "Native Linux X11 setup-owned keyboard mapping reader schedules raw keysyms without event decode",
            "Native Linux X11 raw keyboard mapping selection keeps event decode disconnected",
            "Native Linux X11 keymap projection evidence stays optional before event decode integration",
            "Native Linux X11 Xauthority selector parses exact zero-copy records",
            "Native Linux X11 Xauthority file bytes reader keeps exact path and typed byte owner",
