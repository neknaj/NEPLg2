...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /new_with_setup_request\([\s\S]*setup_backed_top_level_window_create_plan: None,[\s\S]*top_level_window_request: None,[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured,[\s\S]*top_level_window_request_written_len: 0,[\s\S]*top_level_window_request_sequence_plan: None,[\s\S]*wm_protocol_atom_intern_request_batch: None,[\s\S]*NativeWindowLinuxX11WmProtocolAtomInternBatchWriteState::NotConfigured,[\s\S]*wm_protocol_atom_intern_batch_written_len: 0,[\s\S]*next_x11_request_sequence: NATIVE_WINDOW_LINUX_X11_FIRST_NORMAL_REQUEST_SEQUENCE,[\s\S]*pending_server_reply_header: None,[\s\S]*server_reply_body_remaining_len: 0/);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn record_wm_protocol_atom_intern_batch_accepted_range[\s\S]*request_end_byte_offset\([\s\S]*WmProtocols[\s\S]*request_end_byte_offset\([\s\S]*WmDeleteWindow[\s\S]*previous_written_len < wm_protocols_end_byte_offset[\s\S]*accepted_end_len >= wm_protocols_end_byte_offset[\s\S]*take_next_x11_request_sequence[\s\S]*previous_written_len < wm_delete_window_end_byte_offset[\s\S]*accepted_end_len >= wm_delete_window_end_byte_offset[\s\S]*take_next_x11_request_sequence/);
    ...
    assert.doesNotMatch(nativeWindowLinuxX11ObservationReaderImpl, /native_window_linux_x11_intern_atom_reply_from_packet|NativeWindowLinuxX11InternAtomReply|NativeWindowLinuxX11AtomId|ChangeProperty|ClientMessage|validate_native_window_run_loop_platform_wait_runner_support_for_platform|run_linux_platform_wait_window_loop|run_windows_platform_wait_window_loop|WindowOptions|window\.update\(|update_with_buffer|support gate|fallback|silent no-op|synthetic/i);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn start_server_reply_body_drain[\s\S]*native_window_linux_x11_server_reply_body_byte_len\(reply\)\?[\s\S]*pending_server_reply_header = Some\(reply\)[\s\S]*server_reply_body_remaining_len = body_byte_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn drain_server_reply_body[\s\S]*while self\.server_reply_body_remaining_len > 0[\s\S]*read_x11_bytes_raw\(raw_fd,[\s\S]*ServerReplyBodyReadWouldBlock[\s\S]*ServerReplyBodyReadFailed[\s\S]*ServerReplyBodyReadEof[\s\S]*ServerReplyBodyReadOverflow[\s\S]*self\.server_reply_body_remaining_len -= read[\s\S]*self\.pending_server_reply_header = None/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn drain_pending_server_reply_body[\s\S]*let Some\(reply\) = self\.pending_server_reply_header[\s\S]*return Ok\(None\)[\s\S]*self\.drain_server_reply_body\(raw_fd, reply\)\.map\(Some\)/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /self\.ensure_setup_ready\(descriptor\)\?;[\s\S]*self\.ensure_top_level_window_request_ready\(descriptor\)\?;[\s\S]*if let Some\(reply\) = self\.drain_pending_server_reply_body\(descriptor\.raw_fd\(\)\)\?[\s\S]*ServerReplyReceived[\s\S]*self\.read_event_packet\(descriptor\.raw_fd\(\)\)\?/);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /if native_window_linux_x11_response_type_raw\(&packet\)[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_REPLY[\s\S]*let reply = native_window_linux_x11_server_reply_header\(&packet\)[\s\S]*self\.start_server_reply_body_drain\(reply\)\?[\s\S]*self\.drain_server_reply_body\(descriptor\.raw_fd\(\), reply\)\?[\s\S]*ServerReplyReceived/);
