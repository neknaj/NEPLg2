...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11EventSourceObservationError\s*\{[\s\S]*TopLevelWindowRequestPreviouslyFailed\s*\{[\s\S]*descriptor: NativeWindowLinuxWindowEventSourceDescriptor[\s\S]*TopLevelWindowRequestSetupBackedPlanMissing[\s\S]*TopLevelWindowRequestSetupResourceInfoMissing[\s\S]*TopLevelWindowRequestBuildFailed\s*\{[\s\S]*error: NativeWindowLinuxX11SetupBackedTopLevelWindowCreateRequestError[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteFailed\s*\{[\s\S]*code: u32[\s\S]*TopLevelWindowRequestWriteReturnedZero[\s\S]*TopLevelWindowRequestWriteOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11EventSourceObservationError\s*\{[\s\S]*EventReadOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerErrorReceived\s*\{[\s\S]*error: NativeWindowLinuxX11ServerErrorPacket,[\s\S]*correlation: NativeWindowLinuxX11ServerErrorCorrelation,[\s\S]*ServerReplyReceived\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*EventTypeUnsupported/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11EventSourceObservationError\s*\{[\s\S]*SetupResourceInfoParseFailed\s*\{[\s\S]*error: NativeWindowLinuxX11SetupResourceInfoParseError/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn new_with_setup_request\([\s\S]*api: Api,[\s\S]*setup_request: NativeWindowLinuxX11SetupRequest[\s\S]*Self[\s\S]*setup_request,/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /new_with_setup_request\([\s\S]*setup_backed_top_level_window_create_plan: None,[\s\S]*top_level_window_request: None,[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured,[\s\S]*top_level_window_request_written_len: 0,[\s\S]*top_level_window_request_sequence_plan: None,[\s\S]*next_x11_request_sequence: NATIVE_WINDOW_LINUX_X11_FIRST_NORMAL_REQUEST_SEQUENCE/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn new_with_setup_request_and_top_level_window_request\([\s\S]*setup_request: NativeWindowLinuxX11SetupRequest,[\s\S]*top_level_window_request: NativeWindowLinuxX11TopLevelWindowCreateRequest[\s\S]*Self::new_with_setup_request\(api, setup_request\)[\s\S]*reader\.install_top_level_window_request\(top_level_window_request\)/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn install_top_level_window_request[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestSequencePlan::new[\s\S]*top_level_window_request\.window_id\(\)[\s\S]*top_level_window_request_written_len = 0[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn new_with_setup_request_and_setup_backed_top_level_window_create_plan\([\s\S]*plan: NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan[\s\S]*Self::new_with_setup_request\(api, setup_request\)[\s\S]*reader\.setup_backed_top_level_window_create_plan = Some\(plan\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::SetupBackedBuildPending/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn top_level_window_request\([\s\S]*Option<&NativeWindowLinuxX11TopLevelWindowCreateRequest>[\s\S]*self\.top_level_window_request\.as_ref\(\)/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn top_level_window_request_state\([\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState[\s\S]*self\.top_level_window_request_state/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn top_level_window_request_written_len\(&self\) -> usize[\s\S]*self\.top_level_window_request_written_len/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn top_level_window_request_sequence_plan\([\s\S]*Option<NativeWindowLinuxX11TopLevelWindowRequestSequencePlan>[\s\S]*self\.top_level_window_request_sequence_plan/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn next_x11_request_sequence\(&self\) -> u16[\s\S]*self\.next_x11_request_sequence/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn setup_backed_top_level_window_create_plan\([\s\S]*Option<NativeWindowLinuxX11SetupBackedTopLevelWindowCreatePlan>[\s\S]*self\.setup_backed_top_level_window_create_plan/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn setup_resource_info\(&self\) -> Option<NativeWindowLinuxX11SetupResourceInfo>[\s\S]*self\.setup_resource_info/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn write_setup_request[\s\S]*let request_len = self\.setup_request\.len\(\)[\s\S]*self\.setup_request\.as_bytes\(\)[\s\S]*write_x11_bytes_raw/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn write_top_level_window_request[\s\S]*let Some\(request_len\)[\s\S]*top_level_window_request[\s\S]*map\(NativeWindowLinuxX11TopLevelWindowCreateRequest::len\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*while self\.top_level_window_request_written_len < request_len/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn write_top_level_window_request[\s\S]*match self\.top_level_window_request\.as_ref\(\)[\s\S]*Some\(request\) => request\.as_bytes\(\),[\s\S]*None =>[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*write_x11_bytes_raw/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn take_next_x11_request_sequence[\s\S]*let sequence = self\.next_x11_request_sequence;[\s\S]*wrapping_add\(1\)[\s\S]*sequence/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn record_top_level_window_request_accepted_range[\s\S]*previous_written_len < NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_REQUEST_BYTE_LEN[\s\S]*accepted_end_len >= NATIVE_WINDOW_LINUX_X11_CREATE_WINDOW_REQUEST_BYTE_LEN[\s\S]*record_create_window_sequence[\s\S]*previous_written_len < NATIVE_WINDOW_LINUX_X11_TOP_LEVEL_WINDOW_REQUEST_BYTE_LEN[\s\S]*accepted_end_len >= NATIVE_WINDOW_LINUX_X11_TOP_LEVEL_WINDOW_REQUEST_BYTE_LEN[\s\S]*record_map_window_sequence/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn write_top_level_window_request[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteFailed[\s\S]*TopLevelWindowRequestWriteReturnedZero[\s\S]*TopLevelWindowRequestWriteOverflow[\s\S]*checked_add\(written\)[\s\S]*record_top_level_window_request_accepted_range[\s\S]*top_level_window_request_written_len = accepted_end_len[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Ready/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn build_setup_backed_top_level_window_request[\s\S]*let Some\(plan\) = self\.setup_backed_top_level_window_create_plan[\s\S]*TopLevelWindowRequestSetupBackedPlanMissing[\s\S]*TopLevelWindowRequestSetupResourceInfoMissing[\s\S]*native_window_linux_x11_top_level_window_create_request_from_setup_resource_info[\s\S]*plan\.create_input\(setup_resource_info\)[\s\S]*TopLevelWindowRequestBuildFailed[\s\S]*self\.install_top_level_window_request\(request\)/);
    assert.doesNotMatch(nativeWindowLinuxX11EventSourceObservationSurface, /let Some\(plan\) = self\.setup_backed_top_level_window_create_plan else \{[\s\S]{0,500}NotConfigured[\s\S]{0,500}Ok\(\(\)\)/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn ensure_top_level_window_request_ready[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured =>[\s\S]*return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::SetupBackedBuildPending =>[\s\S]*self\.build_setup_backed_top_level_window_request\(\)\?[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending =>[\s\S]*self\.write_top_level_window_request\(raw_fd\)\?[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Ready => return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Failed =>[\s\S]*TopLevelWindowRequestPreviouslyFailed/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /fn drain_setup_body[\s\S]*self\.setup_body_bytes\.extend_from_slice\(&scratch\[..read\]\)[\s\S]*native_window_linux_x11_setup_resource_info_from_little_endian_success_body[\s\S]*SetupResourceInfoParseFailed[\s\S]*self\.setup_state = NativeWindowLinuxX11EventSourceSetupState::Ready/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /self\.ensure_setup_ready\(descriptor\)\?;[\s\S]*self\.ensure_top_level_window_request_ready\(descriptor\)\?;[\s\S]*self\.read_event_packet\(descriptor\.raw_fd\(\)\)\?/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /match native_window_linux_x11_response_type_raw\(packet\)[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_ERROR[\s\S]*let error = native_window_linux_x11_server_error_packet\(packet\)[\s\S]*let correlation = match top_level_window_request_sequence_plan[\s\S]*ServerErrorReceived[\s\S]*error,[\s\S]*correlation,[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_REPLY[\s\S]*ServerReplyReceived[\s\S]*native_window_linux_x11_server_reply_header\(packet\)[\s\S]*match native_window_linux_x11_event_response_type\(packet\)/);
