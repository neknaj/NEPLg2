...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(libSource, /pub enum NativeWindowEventPumpEventKind\s*\{[\s\S]*Poll,[\s\S]*CloseRequested,[\s\S]*WindowResized,[\s\S]*PointerMotion,[\s\S]*PointerButton,[\s\S]*WindowMapped,[\s\S]*RedrawRequested/);
    assert.match(nativeWindowLinuxWindowEventSourceObservationSurface, /pub struct NativeWindowLinuxWindowEventSourceObservation\s*\{[\s\S]*event_kind: NativeWindowEventPumpEventKind/);
