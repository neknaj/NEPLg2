...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_top_level_window_request[\s\S]*let Some\(request_len\)[\s\S]*top_level_window_request[\s\S]*map\(NativeWindowLinuxX11TopLevelWindowCreateRequest::len\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*while self\.top_level_window_request_written_len < request_len/);
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_top_level_window_request[\s\S]*match self\.top_level_window_request\.as_ref\(\)[\s\S]*Some\(request\) => request\.as_bytes\(\),[\s\S]*None =>[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured[\s\S]*write_x11_bytes_raw/);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn write_top_level_window_request[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteFailed[\s\S]*TopLevelWindowRequestWriteReturnedZero[\s\S]*TopLevelWindowRequestWriteOverflow[\s\S]*checked_add\(written\)[\s\S]*record_top_level_window_request_accepted_range[\s\S]*top_level_window_request_written_len = accepted_end_len[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Ready/);
    ...
    assert.match(nativeWindowLinuxX11ObservationReaderImpl, /fn ensure_top_level_window_request_ready[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured =>[\s\S]*return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::SetupBackedBuildPending =>[\s\S]*self\.build_setup_backed_top_level_window_request\(\)\?[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending =>[\s\S]*self\.write_top_level_window_request\(raw_fd\)\?[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Ready => return Ok\(\(\)\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::Failed =>[\s\S]*TopLevelWindowRequestPreviouslyFailed/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /SetupWriteWouldBlock[\s\S]*TopLevelWindowRequestWriteWouldBlock[\s\S]*EventReadWouldBlock/);
