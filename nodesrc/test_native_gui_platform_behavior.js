...
function runNativeGuiPlatformBehaviorRegression() {
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub enum NativeWindowLinuxX11EventSourceObservationError\s*\{[\s\S]*EventReadOverflow\s*\{[\s\S]*byte_count: usize,[\s\S]*remaining_byte_count: usize,[\s\S]*ServerErrorReceived\s*\{[\s\S]*error: NativeWindowLinuxX11ServerErrorPacket,[\s\S]*ServerReplyReceived\s*\{[\s\S]*reply: NativeWindowLinuxX11ServerReplyHeader,[\s\S]*EventTypeUnsupported/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /new_with_setup_request\([\s\S]*setup_backed_top_level_window_create_plan: None,[\s\S]*top_level_window_request: None,[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::NotConfigured,[\s\S]*top_level_window_request_written_len: 0/);
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /pub fn new_with_setup_request_and_top_level_window_request\([\s\S]*setup_request: NativeWindowLinuxX11SetupRequest,[\s\S]*top_level_window_request: NativeWindowLinuxX11TopLevelWindowCreateRequest[\s\S]*Self::new_with_setup_request\(api, setup_request\)[\s\S]*reader\.top_level_window_request = Some\(top_level_window_request\)[\s\S]*NativeWindowLinuxX11TopLevelWindowRequestWriteState::RequestPending/);
    ...
    assert.match(nativeWindowLinuxX11EventSourceObservationSurface, /match native_window_linux_x11_response_type_raw\(packet\)[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_ERROR[\s\S]*ServerErrorReceived[\s\S]*native_window_linux_x11_server_error_packet\(packet\)[\s\S]*NATIVE_WINDOW_LINUX_X11_RESPONSE_TYPE_REPLY[\s\S]*ServerReplyReceived[\s\S]*native_window_linux_x11_server_reply_header\(packet\)[\s\S]*match native_window_linux_x11_event_response_type\(packet\)/);
