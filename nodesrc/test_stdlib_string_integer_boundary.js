...
for (const name of [
    ...
    assert.match(integerCode, new RegExp(`struct\\s+${name}\\b`), `${integerRelPath} must own ${name}`);
}
...
assert.ok(integerSrc.split(/\r?\n/).length <= 1050, `${integerRelPath} should stay within the integer conversion boundary`);
