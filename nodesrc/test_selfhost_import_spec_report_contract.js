...
const file = path.join(repoRoot, "tests", "stdlib", "neplg2_import_spec.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [7, 2, 2];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "selfhost import spec doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `selfhost import spec doctest#${index + 1}`;
...
console.log("selfhost import spec report contract passed");
