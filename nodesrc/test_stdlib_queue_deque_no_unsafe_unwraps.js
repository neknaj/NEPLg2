...
assert.match(queue, /fn\s+free\s+<\.T:\s*Copy>\s+<\(Queue<\.T>\)->\(\)>[\s\S]*vec::free<Option<\.T>>\s+field::get\s+q\s+"items"/, 'Queue.free must close the Vec<Option<T>> owner through the Copy storage fast path');
...
assert.match(deque, /fn\s+free\s+<\.T:\s*Copy>\s+<\(Deque<\.T>\)->\(\)>[\s\S]*vec::free<Option<\.T>>\s+field::get\s+dq\s+"items"/, 'Deque.free must close the Vec<Option<T>> owner through the Copy storage fast path');
