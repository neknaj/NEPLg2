...
assert.ok(sliceSrc.split(/\r?\n/).length <= 35, `${sliceRelPath} should stay as a small facade`);
assert.ok(sliceByteSrc.split(/\r?\n/).length <= 90, `${sliceByteRelPath} should stay narrowly scoped`);
assert.ok(sliceCharSrc.split(/\r?\n/).length <= 290, `${sliceCharRelPath} should stay narrowly scoped`);
assert.ok(sliceTrimSrc.split(/\r?\n/).length <= 150, `${sliceTrimRelPath} should stay narrowly scoped`);
