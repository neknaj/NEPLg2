...
assert.match(
    source,
    /selfhost_check_expr_stage1_make_pipe_chain_trailing_block_tokens[\s\S]*TokenKind::Pipe source_span_new_unchecked 0 11 13[\s\S]*TokenKind::Ident source_span_new_unchecked 0 23 26[\s\S]*selfhost_check_expr_stage1_success_has_pipe_chain_trailing_block_argument_order[\s\S]*selfhost_check_expr_stage1_argument_is_checked_expr_range &first 0 4[\s\S]*selfhost_check_expr_stage1_argument_is_block_result_checked_expr_range &second 6 6[\s\S]*selfhost_check_expr_stage1_pipe_chain_trailing_block_body_call_ok[\s\S]*SelfhostCheckedExprNodeKind::DirectCall[\s\S]*selfhost_check_expr_stage1_argument_is_i32_literal_range &first 1 7 8[\s\S]*selfhost_check_expr_stage1_argument_is_i32_literal_range &second 1 8 9[\s\S]*selfhost_check_expr_stage1_pipe_chain_trailing_block_node_ok[\s\S]*SelfhostCheckedExprNodeKind::BlockResult[\s\S]*selfhost_check_expr_stage1_pipe_chain_root_links_previous_direct_call_and_block_result[\s\S]*selfhost_check_expr_stage1_pipe_chain_previous_call_arguments_ok tree previous_expr[\s\S]*selfhost_check_expr_stage1_pipe_chain_trailing_block_node_ok tree block_expr[\s\S]*selfhost_check_expr_stage1_pipe_chain_trailing_block_ok_with_i32[\s\S]*"1 \|> add 2 \|> add:\\n    add 1 1"[\s\S]*selfhost_check_expr_stage1_run_pipe_chain_trailing_block_with_tokens/,
    "stage1 must smoke-test that a pipe-chain trailing block is accepted only by the final pipe segment and connected as a BlockResult checked-tree node",
);
assert.match(
    source,
