...
const cli = read("nodesrc/cli.js");
const htmlPlay = read("nodesrc/html_gen_playground.js");
const playgroundRuntime = read("nodesrc/static/playground_runtime.js");
const buildJob = jobBlock("build");
const pagesContentJob = jobBlock("pages-content");
const pagesFastBundleJob = jobBlock("pages-fast-bundle");
const pagesFinalBundleJob = jobBlock("pages-final-bundle");
const pagesFinalDeployJob = jobBlock("pages-final-deploy");
...
assertIncludes(workflow, "Build repository metrics into optional Pages content", "CI must generate repo metrics for Pages");
assertIncludes(pagesContentJob, "fetch-depth: 0", "metrics history generation needs full first-parent commit history in the optional content job");
...
assertIncludes(buildJob, "name: playground-site", "CI must publish a playground-only Pages base artifact before optional content generation");
assertNotIncludes(buildJob, "Build repository metrics into optional Pages content", "metrics generation must not be part of the playground build gate");
assertNotIncludes(buildJob, "Build doc + stdlib HTML into optional Pages content", "documentation generation must not be part of the playground build gate");
assertIncludes(pagesContentJob, "continue-on-error: true", "optional Pages content generation must not fail the playground deployment path");
assertIncludes(pagesContentJob, "Build repository metrics into optional Pages content", "CI must generate repo metrics for Pages as optional content");
assertIncludes(workflow, "node nodesrc/run_repo_metrics.js --root . --json dist-content/metrics/repo_metrics.json --csv dist-content/metrics/repo_metrics.csv", "CI must run repo_metrics.ts through the wrapper");
assertIncludes(workflow, "node nodesrc/run_repo_metrics_history.js --root . --limit 100 --json dist-content/metrics/repo_metrics_history.json", "CI must publish approximately 100 sampled historical repo metrics for the Pages chart");
assertIncludes(pagesFastBundleJob, "name: playground-site", "pending Pages bundle must start from the playground artifact");
assertNotIncludes(pagesFastBundleJob, "name: bootstrap-build", "pending Pages bundle must not unpack compiler/test build artifacts");
assertIncludes(pagesFinalBundleJob, "name: playground-site", "final Pages bundle must start from the playground artifact");
assertIncludes(pagesFinalBundleJob, "name: pages-content", "final Pages bundle should merge optional content when it exists");
assertIncludes(pagesFinalDeployJob, "needs: pages-final-bundle", "final Pages deploy should be able to run without a successful fast deploy");
assertNotIncludes(pagesFinalDeployJob, "pages-fast-deploy", "final Pages deploy must not depend on the pending deploy result");
assertIncludes(cli, "writeScopeSearchIndex", "html_play generation must write the shared search index once per scope");
assertIncludes(cli, "search-index.json", "html_play generation must publish a stable external search index file");
assertIncludes(htmlPlay, "searchIndexPath", "html_play pages must reference the shared search index by path");
assertIncludes(playgroundRuntime, "fetch(searchIndexPath", "playground runtime must load external search indexes instead of requiring inline duplication");
