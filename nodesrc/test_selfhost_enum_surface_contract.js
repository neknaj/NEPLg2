...
assert.match(source, /pub struct SelfhostResolvedEnumDefinition:[\s\S]*parameter_first_token %Option i32[\s\S]*parameter_count %i32[\s\S]*variant_start %i32[\s\S]*variant_count %i32/, "resolved enum definitions must retain typed optional generic binder token authority separately from variants");
const parameterNameSpan = topLevelBlock("pub fn", "selfhost_resolved_enum_session_definition_parameter_name_span");
assert.match(parameterNameSpan, /parameter_ordinal[\s\S]*definition\.parameter_count[\s\S]*definition\.parameter_first_token[\s\S]*Option::None: none[\s\S]*Option::Some first_token[\s\S]*gt parameter_ordinal 715827882[\s\S]*mul parameter_ordinal 3[\s\S]*gt first_token sub sub 2147483647 offset 1[\s\S]*TokenKind::Dot[\s\S]*TokenKind::Ident[\s\S]*dot_inside[\s\S]*name_inside[\s\S]*dot_token\.span\.end name_token\.span\.start/, "generic binder lookup must guard index arithmetic and validate typed range presence, same-session Dot/Ident token shape, declaration containment, and token order");
assert.doesNotMatch(parameterNameSpan, /string_slice|str_eq|canonical|split/, "generic binder lookup must not reconstruct declaration order from source spelling");
