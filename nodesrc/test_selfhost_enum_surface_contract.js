...
assert.match(memberId, /selfhost_resolved_enum_session_definition_get[\s\S]*variant_ordinal[\s\S]*definition\.variant_count[\s\S]*definition\.variant_start[\s\S]*selfhost_resolved_enum_session_variant_get[\s\S]*SelfhostResolvedEnumMemberId definition\.nominal_id variant_ordinal/, "member identity must be issued only from same-session definition range membership");
