...
assert.match(mapCode, /fn\s+len\s+<\.K,\.V>\s+<\(&BTreeMap<\.K,\.V>\)->i32>\s+\(hm\):/, 'BTreeMap.len must borrow the owner and not require Copy for metadata-only observation');
...
assert.match(setCode, /fn\s+len\s+<\.T>\s+<\(&BTreeSet<\.T>\)->i32>\s+\(set0\):/, 'BTreeSet.len must borrow the owner and not require Copy for metadata-only observation');
