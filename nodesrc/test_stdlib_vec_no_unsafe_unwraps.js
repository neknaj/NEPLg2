...
assert.match(withCapacitySection, /fn\s+with_capacity\s+<\.T:\s*Copy>[\s\S]*if:\s+lt\s+cap\s+0\s+then:\s+(?:Result::Err<Vec<\.T>,\s*StdErrorKind>|Result::Err)\s+StdErrorKind::InvalidOperation[\s\S]*else:\s+alloc::vec_alloc_empty<\.T>\s+cap/, 'Vec.with_capacity Copy overload must reject negative capacity before allocating owned storage');
assert.match(withCapacitySection, /fn\s+with_capacity\s+<\.T:\s*Drop>[\s\S]*if:\s+lt\s+cap\s+0\s+then:\s+(?:Result::Err<Vec<\.T>,\s*StdErrorKind>|Result::Err)\s+StdErrorKind::InvalidOperation[\s\S]*else:\s+alloc::vec_alloc_empty<\.T>\s+cap/, 'Vec.with_capacity Drop overload must share the same checked allocation path instead of bypassing capacity validation');
...
assert.match(vecStorageCleanupCode, /fn\s+vec_free_storage\s+<\.T:\s*Copy>\s+<\(VecStorage<\.T>\)->unit>/, 'vec/storage/cleanup.nepl storage-only cleanup must take the owner-carrying storage enum and remain Copy-only until element drop traversal exists');
...
assert.match(dataMemPtrUsageExample, /\bmatch\s+data_mem_view\s+&v:[\s\S]*VecDataView::Empty:[\s\S]*VecDataView::Data\s+_p:[\s\S]*VecDataView::Invalid\s+_reason:[\s\S]*\bfree\s+v/, 'Vec.data_mem_view usage example must show exhaustive typed observer use while retaining the Vec owner for cleanup');
...
assert.match(pushSection, /fn\s+push\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\.T\)\*>Result<Vec<\.T>,\s*VecPushError<\.T>>>[\s\S]*vec_push_storage_checked<\.T>\s+v\s+item[\s\S]*fn\s+push\s+<\.T:\s*Drop>\s+<\(Vec<\.T>,\.T\)\*>Result<Vec<\.T>,\s*VecPushError<\.T>>>[\s\S]*vec_push_storage_checked<\.T>\s+v\s+item/, 'Vec.push must expose Copy and Drop owner-preserving impure overloads that share the same storage-checked implementation');
assert.match(vecMutationPushCode, /\bfn\s+vec_push_slot_store_initialized\s+<\.T>\s+<\(&RegionToken<\.T>,i32,\.T\)\*>unit>/, 'Vec.push must keep impure collection slot initialization in a private implementation helper');
...
assert.match(privatePushSection, /let\s+byte_off\s+<i32>\s+mul\s+v_len\s+size_of\s+%\.T[\s\S]*vec_push_slot_store_initialized<\.T>\s+&grown_region\s+byte_off\s+item[\s\S]*let\s+byte_off\s+<i32>\s+mul\s+v_len\s+size_of\s+%\.T[\s\S]*vec_push_slot_store_initialized<\.T>\s+&grown_region\s+byte_off\s+item[\s\S]*let\s+byte_off\s+<i32>\s+mul\s+v_len\s+size_of\s+%\.T[\s\S]*vec_push_slot_store_initialized<\.T>\s+&v_region\s+byte_off\s+item/, 'Vec.push must route Empty allocation, Owned grow, and in-place append stores through the same lifecycle proof helper');
assert.match(vecCode, /fn\s+vec_next_capacity\s+<\.T>\s+<\(i32\)->Result<i32,\s*StdErrorKind>>[\s\S]*size_of\s+%\.T[\s\S]*max_alloc_payload_bytes[\s\S]*StdErrorKind::CapacityExceeded[\s\S]*half_limit[\s\S]*Result::Ok\s+mul\s+cap0\s+2/, 'Vec.push growth must prove next capacity against element size and allocator payload bounds before doubling');
...
assert.match(popSection, /let\s+byte_off\s+<i32>\s+mul\s+next_len\s+size_of\s+%\.T[\s\S]*vec_pop_move_out_initialized_slot<\.T>\s+&v_region\s+byte_off[\s\S]*OwnedBuffer<\.T>\s+next_len\s+next_len\s+v_cap/, 'Vec.pop success path must move out the removed slot before updating initialized_len with len');
assert.match(popSection, /fn\s+pop\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)\*>VecPop<\.T>>[\s\S]*vec_pop_storage_checked<\.T>\s+v[\s\S]*fn\s+pop\s+<\.T:\s*Drop>\s+<\(Vec<\.T>\)\*>VecPop<\.T>>[\s\S]*vec_pop_storage_checked<\.T>\s+v/, 'Vec.pop must expose Copy and Drop impure overloads that share the same storage-checked implementation');
assert.match(popCode, /fn\s+vec_pop_move_out_initialized_slot\s+<\.T>\s+<\(&RegionToken<\.T>,i32\)\*>\.T>[\s\S]*load<\.T>\s+raw[\s\S]*#intrinsic\s+"collection_slot_move_out"\s+<\.T>\s+\(storage,\s*byte_off\)[\s\S]*loaded/, 'Vec.pop helper must pair raw load witness with single-slot MoveOut proof and return the loaded item through an impure signature');
...
assert.match(popSection, /fn\s+vec_drop_last_copy_initialized_slot\s+<\.T:\s*Copy>\s+<\(&RegionToken<\.T>,i32\)\*>unit>[\s\S]*load<\.T>\s+raw[\s\S]*#intrinsic\s+"collection_slot_drop_initialized"\s+<\.T>\s+\(storage,\s*byte_off\)/, 'Vec.drop_last Copy helper must pair raw load witness with single-slot drop proof through an impure signature');
assert.match(popSection, /fn\s+vec_drop_last_drop_initialized_slot\s+<\.T:\s*Drop>\s+<\(&RegionToken<\.T>,i32\)\*>unit>[\s\S]*load<\.T>\s+raw[\s\S]*Drop::drop\s+&loaded[\s\S]*#intrinsic\s+"collection_slot_drop_initialized"\s+<\.T>\s+\(storage,\s*byte_off\)/, 'Vec.drop_last Drop helper must pair actual Drop::drop proof with single-slot drop proof');
assert.doesNotMatch(popSection, /\bpub\s+fn\s+vec_drop_last_(?:copy|drop)_initialized_slot\b/, 'Vec.drop_last single-slot lifecycle helpers must not become public marker boundaries');
assert.match(popSection, /fn\s+drop_last\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)\*>Vec<\.T>>[\s\S]*vec_drop_last_storage_checked_copy<\.T>\s+v[\s\S]*fn\s+drop_last\s+<\.T:\s*Drop>\s+<\(Vec<\.T>\)\*>Vec<\.T>>[\s\S]*vec_drop_last_storage_checked_drop<\.T>\s+v/, 'Vec.drop_last must expose Copy and Drop impure overloads that delegate to storage-checked implementations');
assert.match(dropLastCopySection, /let\s+v_invariant\s+<VecStorageInvariant>\s+vec_buffer_current_storage_invariant<\.T>\s+v_buffer_ref[\s\S]*VecStorageInvariant::Invalid\s+_reason:[\s\S]*VecStorageInvariant::Valid:[\s\S]*vec_drop_last_copy_initialized_slot<\.T>\s+&v_region\s+byte_off[\s\S]*OwnedBuffer<\.T>\s+next_len\s+next_len\s+v_cap/, 'Vec.drop_last Copy implementation must use payload-independent storage invariant and close the removed slot before metadata decrement');
assert.match(dropLastDropSection, /let\s+v_invariant\s+<VecStorageInvariant>\s+vec_buffer_current_storage_invariant<\.T>\s+v_buffer_ref[\s\S]*VecStorageInvariant::Invalid\s+_reason:[\s\S]*VecStorageInvariant::Valid:[\s\S]*vec_drop_last_drop_initialized_slot<\.T>\s+&v_region\s+byte_off[\s\S]*OwnedBuffer<\.T>\s+next_len\s+next_len\s+v_cap/, 'Vec.drop_last Drop implementation must use payload-independent storage invariant and actual Drop proof before metadata decrement');
assert.doesNotMatch(`${dropLastCopySection}\n${dropLastDropSection}`, /\bVecCopyInvariant\b|\bvec_buffer_current_copy_invariant\b/, 'Vec.drop_last must not use Copy raw-access invariant because it relies on storage invariant plus single-slot lifecycle proof');
...
assert.match(vecCode, /fn\s+vec_pop_with\s+<\.T,\.R>\s+<\(VecPop<\.T>,\(Vec<\.T>,Option<\.T>\)\*>\.R\)\*>\.R>[\s\S]*let\s+vec\s+<Vec<\.T>>\s+field::get\s+p\s+"vec"[\s\S]*let\s+item\s+<Option<\.T>>\s+field::get\s+p\s+"item"[\s\S]*callback\s+vec\s+item/, 'Vec.pop must expose a generic owner-preserving eliminator that returns the Vec and popped Option<T> owners together');
assert.match(popSource, /struct\s+DropPayload:[\s\S]*impl\s+Drop\s+for\s+DropPayload:[\s\S]*\bpop\s+v1\b[\s\S]*\bvec_pop_with\s+p\s+@finish\b/, 'Vec.pop docs must cover Drop payload move-out through the owner-preserving eliminator');
assert.match(popSource, /diag_codes:\s*type\.overload\.no_match[\s\S]*struct\s+NonCopyPayload:[\s\S]*vec_empty<NonCopyPayload>[\s\S]*pop<NonCopyPayload>/, 'Vec.pop must reject payloads with neither Copy nor Drop capability');
assert.match(vecMutationCleanupCode, /\bfn\s+vec_cleanup_copy_initialized_prefix\s+<\.T:\s*Copy>\s+<\(&RegionToken<\.T>,i32\)\*>unit>[\s\S]*while\s+lt\s+i\s+initialized_len:[\s\S]*load<\.T>\s+raw[\s\S]*#intrinsic\s+"collection_slot_drop_traversal"\s+<\.T>\s+\(storage,\s*initialized_len\)/, 'Vec cleanup must pair Copy initialized-prefix source traversal with collection slot drop-traversal proof in a private impure helper');
assert.match(vecMutationCleanupCode, /\bfn\s+vec_cleanup_drop_initialized_prefix\s+<\.T:\s*Drop>\s+<\(&RegionToken<\.T>,i32\)\*>unit>[\s\S]*while\s+lt\s+i\s+initialized_len:[\s\S]*load<\.T>\s+raw[\s\S]*Drop::drop\s+&loaded[\s\S]*#intrinsic\s+"collection_slot_drop_traversal"\s+<\.T>\s+\(storage,\s*initialized_len\)/, 'Vec cleanup must pair Drop initialized-prefix traversal with actual Drop::drop proof before the traversal marker');
...
assert.match(vecMutationCleanupCode, /\bfn\s+vec_cleanup_release_storage\s+<\.T>\s+<\(RegionToken<\.T>\)\*>unit>[\s\S]*field::get\s+region\s+"raw"[\s\S]*field::get\s+region\s+"size"[\s\S]*allocator::dealloc_raw\s+raw\s+total_size[\s\S]*#intrinsic\s+"collection_slot_storage_dealloc"\s+<\.T>\s+\(&region\)/, 'Vec cleanup must pair raw storage release with collection slot storage dealloc proof in the same private impure helper without requiring payload Copy');
assert.match(clearSection, /fn\s+clear\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)\*>Vec<\.T>>/, 'Vec.clear Copy overload must remain available but carry an impure lifecycle-cleanup signature');
...
assert.match(freeSection, /fn\s+free\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)\*>unit>/, 'Vec.free Copy overload must remain available but carry an impure storage-release signature');
assert.match(freeSection, /fn\s+free\s+<\.T:\s*Drop>\s+<\(Vec<\.T>\)\*>unit>/, 'Vec.free Drop overload must be impure because it runs destructors and releases storage');
...
assert.match(vecMutationPushCode, /fn\s+vec_push_rejected_with\s+<\.T,\.R>\s+<\(VecPushRejected<\.T>,\(Vec<\.T>,\.T\)\*>\.R\)\*>\.R>[\s\S]*let\s+vec\s+<Vec<\.T>>\s+field::get\s+rejected\s+"vec"[\s\S]*let\s+item\s+<\.T>\s+field::get\s+rejected\s+"item"[\s\S]*callback\s+vec\s+item/, 'Vec.push rejected payload must provide a generic owner-preserving eliminator instead of forcing public field projection');
...
assert.match(vecMutationReplaceCode, /fn\s+vec_replace_copy_initialized_slot\s+<\.T:\s*Copy>\s+<\(&RegionToken<\.T>,i32,\.T\)\*>unit>[\s\S]*load<\.T>[\s\S]*store<\.T>[\s\S]*#intrinsic\s+"collection_slot_replace_drop_old"\s+<\.T,\s*\.T>/, 'Vec.replace Copy raw store must stay in a private helper that emits collection slot replacement proof');
...
assert.match(mapSection, /fn\s+map\s+<\.T:\s*Copy,\s*\.U:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->\.U\)\*>Result<Vec<\.U>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.U>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.map must be impure and return the consumed input Vec owner on output allocation failure');
assert.match(vecTransformFilterCode, /fn\s+filter\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)\*>Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.filter must be impure and return the consumed input Vec owner on output allocation failure');
assert.match(vecTransformFilterCode, /Result::Err\s+e:[\s\S]*vec_cleanup::free<\.T>\s+left0[\s\S]*(?:Result::Err<VecPartition<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.partition right allocation failure must free partial output and return the consumed input Vec owner');
...
assert.match(vecCode, /fn\s+partition\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)\*>Result<VecPartition<\.T>,\s*VecTransformError<\.T>>>/, 'Vec.partition must be impure, return named VecPartition, and return input owner on failure');
...
assert.match(vecTransformPrefixCode, /fn\s+take_while\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)\*>Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.take_while must be impure, require Copy elements, and return input owner on output allocation failure');
assert.match(vecTransformPrefixCode, /fn\s+drop_while\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)\*>Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.drop_while must be impure, require Copy elements, and return input owner on output allocation failure');
...
assert.match(sortMergeApiCode, /fn\s+sort_merge_buffer_set\s+<\.T:\s*Copy>\s+<\(MemPtr<\.T>,i32,\.T\)\*>unit>/, 'sort/merge/api.nepl must own private Copy-only scratch buffer stores as an impure write');
...
for (const [name, signature] of [
    ['sort_quick_set_data', /<\(MemPtr<\.T>,i32,\.T\)\*>unit>/],
