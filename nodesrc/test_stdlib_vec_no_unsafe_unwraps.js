...
assert.match(vecMutationPushCode, /\bfn\s+vec_push_slot_store_initialized\s+<\.T>\s+<\(&RegionToken<\.T>,i32,\.T\)->unit>/, 'Vec.push must keep collection slot initialization in a private implementation helper');
...
assert.match(popSection, /fn\s+vec_drop_last_copy_initialized_slot\s+<\.T:\s*Copy>\s+<\(&RegionToken<\.T>,i32\)->unit>[\s\S]*load<\.T>\s+raw[\s\S]*#intrinsic\s+"collection_slot_drop_initialized"\s+<\.T>\s+\(storage,\s*byte_off\)/, 'Vec.drop_last Copy helper must pair raw load witness with single-slot drop proof');
...
assert.match(popSection, /fn\s+drop_last\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)->Vec<\.T>>[\s\S]*vec_drop_last_storage_checked_copy<\.T>\s+v[\s\S]*fn\s+drop_last\s+<\.T:\s*Drop>\s+<\(Vec<\.T>\)\*>Vec<\.T>>[\s\S]*vec_drop_last_storage_checked_drop<\.T>\s+v/, 'Vec.drop_last must expose Copy and Drop overloads that delegate to storage-checked implementations');
...
assert.match(freeSection, /fn\s+free\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)->unit>/, 'Vec.free Copy overload must remain available for destructor-free payload cleanup');
assert.match(freeSection, /fn\s+free\s+<\.T:\s*Drop>\s+<\(Vec<\.T>\)\*>unit>/, 'Vec.free Drop overload must be impure because it runs destructors');
...
assert.match(mapSection, /fn\s+map\s+<\.T:\s*Copy,\s*\.U:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->\.U\)->Result<Vec<\.U>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.U>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.map must return the consumed input Vec owner on output allocation failure');
assert.match(vecTransformFilterCode, /fn\s+filter\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)->Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.filter must return the consumed input Vec owner on output allocation failure');
...
assert.match(vecTransformPrefixCode, /fn\s+take_while\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)->Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.take_while must require Copy elements and return input owner on output allocation failure');
assert.match(vecTransformPrefixCode, /fn\s+drop_while\s+<\.T:\s*Copy>\s+<\(Vec<\.T>,\s*\(\.T\)->bool\)->Result<Vec<\.T>,\s*VecTransformError<\.T>>>[\s\S]*(?:Result::Err<Vec<\.T>,\s*VecTransformError<\.T>>|Result::Err)\s+VecTransformError<\.T>\s+v\s+e/, 'Vec.drop_while must require Copy elements and return input owner on output allocation failure');
