...
assert.match(vecInvariantCode, /enum\s+VecCopyInvariantInvalid:[\s\S]*NegativeLength[\s\S]*InitializedLengthMismatch[\s\S]*LengthExceedsCapacity[\s\S]*EmptyStorageMetadataMismatch[\s\S]*OwnedStorageCapacityMismatch[\s\S]*OwnedStorageExtentMismatch/, 'Vec invariant failure reasons must be a typed enum instead of an opaque bool');
...
assert.match(vecInvariantCode, /fn\s+vec_buffer_current_copy_invariant\s+<\.T>\s+<\(&OwnedBuffer<\.T>\)->VecCopyInvariant>[\s\S]*let\s+len0\s+<i32>[\s\S]*let\s+initialized_len0\s+<i32>[\s\S]*let\s+cap0\s+<i32>[\s\S]*VecStorage::Empty:[\s\S]*VecCopyInvariant::Valid[\s\S]*VecCopyInvariant::Invalid\s+VecCopyInvariantInvalid::EmptyStorageMetadataMismatch[\s\S]*VecStorage::Owned\s+region:[\s\S]*let\s+elem_size\s+<i32>\s+size_of<\.T>[\s\S]*let\s+max_count\s+<i32>\s+div_s\s+max_alloc_payload_bytes\s+elem_size[\s\S]*let\s+expected_bytes\s+<i32>\s+mul\s+cap0\s+elem_size[\s\S]*eq\s+region_size<\.T>\s+region\s+expected_bytes[\s\S]*VecCopyInvariant::Invalid\s+VecCopyInvariantInvalid::OwnedStorageExtentMismatch/, 'Vec invariant helper must return typed proof/refutation evidence for len/initialized_len/cap/storage/extent correlation');
assert.match(vecInvariantCode, /fn\s+vec_current_copy_invariant\s+<\.T>\s+<\(&Vec<\.T>\)->VecCopyInvariant>[\s\S]*vec_buffer_current_copy_invariant<\.T>/, 'Vec invariant helper must expose a Vec facade proof observer for raw access boundaries');
assert.doesNotMatch(vecInvariantCode, /fn\s+vec_(?:buffer_)?current_copy_invariant\s+<\.T>\s+<\([^)]*\)->bool>/, 'Vec raw-boundary invariant helpers must not collapse proof evidence to bool');
...
assert.match(vecAccessDataCode, /match\s+vec_buffer_current_copy_invariant<\.T>\s+v_buffer:[\s\S]*VecCopyInvariant::Invalid\s+_reason:[\s\S]*mem_ptr_wrap\s+0[\s\S]*VecCopyInvariant::Valid:[\s\S]*match\s+v_storage:/, 'Vec.data_mem_ptr must match typed invariant proof before deriving a raw storage view');
