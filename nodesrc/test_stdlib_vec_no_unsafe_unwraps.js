...
assert.match(popSource, /struct\s+DropPayload:[\s\S]*impl\s+Drop\s+for\s+DropPayload:[\s\S]*pop<DropPayload>[\s\S]*vec_pop_with<DropPayload,i32>/, 'Vec.pop docs must cover Drop payload move-out through the owner-preserving eliminator');
