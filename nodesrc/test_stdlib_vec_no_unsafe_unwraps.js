...
assert.match(popSection, /let\s+byte_off\s+<i32>\s+mul\s+next_len\s+size_of<\.T>[\s\S]*vec_pop_copy_move_out_initialized_slot<\.T>\s+&v_region\s+byte_off[\s\S]*OwnedBuffer<\.T>\s+next_len\s+next_len\s+v_cap/, 'Vec.pop success path must move out the removed slot before updating initialized_len with len');
assert.match(popSection, /fn\s+pop\s+<\.T:\s*Copy>\s+<\(Vec<\.T>\)->VecPop<\.T>>/, 'Vec.pop must return named VecPop and remain Copy-only until non-Copy VecPop consuming accessors are designed');
assert.match(popSource, /fn\s+vec_pop_copy_move_out_initialized_slot\s+<\.T:\s*Copy>\s+<\(&RegionToken<\.T>,i32\)->\.T>[\s\S]*load<\.T>\s+raw[\s\S]*#intrinsic\s+"collection_slot_move_out"\s+<\.T>\s+\(storage,\s*byte_off\)[\s\S]*loaded/, 'Vec.pop Copy helper must pair raw load witness with single-slot MoveOut proof and return the loaded item');
assert.doesNotMatch(popSource, /\bpub\s+fn\s+vec_pop_copy_move_out_initialized_slot\b/, 'Vec.pop single-slot lifecycle helper must not become a public marker boundary');
assert.doesNotMatch(publicPopSection, /\bVecCopyInvariant\b|\bvec_buffer_current_copy_invariant\b|\bVecDataView\b|\bdata_mem_view\b|\b(?:region_ptr|mem_ptr_addr|mem_ptr_add|load<\.T>)\b/, 'Vec.pop public body must not open-code raw slot load or depend on Copy raw-access view; slot move-out must go through vec_pop_copy_move_out_initialized_slot');
