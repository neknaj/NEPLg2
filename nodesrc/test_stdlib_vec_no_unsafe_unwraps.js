...
assert.match(vecInvariantCode, /enum\s+VecCopyInvariantInvalid:[\s\S]*NegativeLength[\s\S]*InitializedLengthMismatch[\s\S]*LengthExceedsCapacity[\s\S]*EmptyStorageMetadataMismatch[\s\S]*OwnedStorageCapacityMismatch/, 'Vec invariant failure reasons must be a typed enum instead of an opaque bool');
...
assert.match(vecInvariantCode, /fn\s+vec_buffer_current_copy_invariant\s+<\.T>\s+<\(&OwnedBuffer<\.T>\)->VecCopyInvariant>[\s\S]*let\s+len0\s+<i32>[\s\S]*let\s+initialized_len0\s+<i32>[\s\S]*let\s+cap0\s+<i32>[\s\S]*VecStorage::Empty:[\s\S]*VecCopyInvariant::Valid[\s\S]*VecCopyInvariant::Invalid\s+VecCopyInvariantInvalid::EmptyStorageMetadataMismatch[\s\S]*VecStorage::Owned\s+_region:[\s\S]*VecCopyInvariant::Valid[\s\S]*VecCopyInvariant::Invalid\s+VecCopyInvariantInvalid::OwnedStorageCapacityMismatch/, 'Vec invariant helper must return typed proof/refutation evidence for len/initialized_len/cap/storage correlation');
