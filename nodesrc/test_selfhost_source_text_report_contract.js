...
const file = path.join(repoRoot, "tests", "stdlib", "neplg2_text.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [6, 5, 4, 6];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "neplg2_text doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `neplg2_text doctest#${index + 1}`;
...
console.log("selfhost source text report contract passed");
