...
const file = path.join(repoRoot, "tests", "stdlib", "neplg2_module_graph.n.md");
const parsed = parseFile(file);

const expectedCheckCounts = [9, 2, 2];

assert.equal(parsed.doctests.length, expectedCheckCounts.length, "selfhost module graph doctest count changed");
...
parsed.doctests.forEach((doctest, index) => {
    const name = `selfhost module graph doctest#${index + 1}`;
...
console.log("selfhost module graph report contract passed");
