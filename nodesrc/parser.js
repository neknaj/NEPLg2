...
exports.parseNmdAst = parseNmdAst;
...
function parseDiagSpanEntry(raw) {
    ...
    if (m)
        return { file: null, line: parseInt(m[1], 10), col: parseInt(m[2], 10) };
    m = s.match(/^(.+):(\d+):(\d+)$/);
    if (m)
        return { file: String(m[1]).trim(), line: parseInt(m[2], 10), col: parseInt(m[3], 10) };
    return null;
}
function parseDiagSpanList(raw) {
    ...
    return vals.map(v => parseDiagSpanEntry(v)).filter((x) => x !== null);
}
            ...
            if (mm) {
                ...
                if (k === "diag_id") {
                    ...
                    if (mk === "ret") {
                        ...
                        meta[mk] = parseMetaValue(mm[2]);
                    }
                }
            }
            if (/^\s*```\s*neplg2\s*$/.test(l2))
                break;
            j++;
        }
        if (j >= lines.length)
        ...
        if (m) {
            ...
            if (inDocBlock) {
                ...
                for (let j = i; j < lines.length; j++) {
                    ...
                    else if (codeLine.startsWith("struct "))
                        kind = "struct";
                    else if (codeLine.startsWith("enum "))
                        kind = "enum";
                    else if (codeLine.startsWith("trait "))
                        kind = "trait";
                    else if (codeLine.startsWith("impl "))
                        kind = "impl";
                    else if (codeLine.startsWith("let "))
                        kind = "let";
                    break;
                }
...
function splitGlossParts(inner) {
    const parts = [];
    let buf = "";
    let bracket = 0;
...
function parseInlines(text) {
    ...
    const findClose = (from, close, skipDouble) => {
        let j = from;
...
function parseNmdAstFromLines(lines) {
    const rootChildren = [];
    const stack = [{ level: 0, node: { children: rootChildren } }];
    ...
    const isBlockStarter = (ln, nextLn) => {
        if (ln.trim() === "")
            return true;
        if (isFenceStart(ln))
            return true;
        ...
        if (/^\d+\.\s/.test(ln))
            return true;
        if (isTableLine(ln) && nextLn !== undefined && isTableSeparatorLine(nextLn))
            return true;
        return false;
    };
    let i = 0;
    while (i < lines.length) {
        ...
        if (fs0) {
            ...
            if (j < lines.length && isFenceEnd(lines[j]))
                j++;
