...
fn check_kind %impure fn TestReport impure fn Option SelfhostTypeKind impure fn SelfhostTypeKind TestReport \checks\actual\expected:
    match actual:
        Option::Some kind:
                                    ...
                                    match selfhost_type_arena_add_primitive selfhost_type_arena_alloc_into_arena alloc3 SelfhostPrimitiveTypeKind::F64:
                                        Result::Ok alloc4:
                                            ...
                                            match selfhost_type_arena_add_primitive selfhost_type_arena_alloc_into_arena alloc4 SelfhostPrimitiveTypeKind::Never:
                                                ...
                                                Result::Err _e:
                                                    let checks1 checks_push checks0 Result::Err "never type allocation failed"
                                                    let shown checks_print_report checks1
                                                    checks_exit_code shown
                                        Result::Err _e:
                                            let checks1 checks_push checks0 Result::Err "f64 type allocation failed"
                                            let shown checks_print_report checks1
                                            checks_exit_code shown
                                Result::Err _e:
                                    let checks1 checks_push checks0 Result::Err "f32 type allocation failed"
                                    let shown checks_print_report checks1
                                    checks_exit_code shown
                        Result::Err _e:
...
fn check_type_id %impure fn TestReport impure fn Option SelfhostTypeId impure fn SelfhostTypeId TestReport \checks\actual\expected:
    match actual:
        Option::Some type_id:
            checks_push checks check selfhost_type_id_eq type_id expected
        Option::None:
            checks_push checks Result::Err "type id was absent"
                    ...
                    match selfhost_type_arena_add_primitive selfhost_type_arena_alloc_into_arena alloc1 SelfhostPrimitiveTypeKind::Bool:
                        Result::Ok alloc2:
                            ...
                            match params_result:
                                                ...
                                                Result::Ok params2:
                                                    match selfhost_type_arena_add_function selfhost_type_arena_alloc_into_arena alloc2 params2 bool_id:
                                                        Result::Ok alloc3:
                                                            ...
                                                            checks_exit_code shown
                                                        Result::Err _e:
                                                            let checks1 checks_push checks0 Result::Err "function type allocation failed"
                                                            let shown checks_print_report checks1
                                                            checks_exit_code shown
                                                Result::Err _e:
                                ...
                                Result::Err _e:
                                    selfhost_type_arena_free selfhost_type_arena_alloc_into_arena alloc2
                                    let checks1 checks_push checks0 Result::Err "param vector allocation failed"
                                    let shown checks_print_report checks1
                                    checks_exit_code shown
                        Result::Err _e:
                            let checks1 checks_push checks0 Result::Err "bool type allocation failed"
                                ...
                                Result::Ok alloc3:
                                    ...
                                    match add_one_arg_function selfhost_type_arena_alloc_into_arena alloc3 i32_id bool_id:
                                        Result::Ok alloc4:
                                            ...
                                            checks_exit_code shown
                                        Result::Err _e:
                                            let checks1 checks_push checks0 Result::Err "second function allocation failed"
                                            let shown checks_print_report checks1
                                            checks_exit_code shown
                                Result::Err _e:
                                    let checks1 checks_push checks0 Result::Err "first function allocation failed"
                                    let shown checks_print_report checks1
                                    checks_exit_code shown
                        Result::Err _e:
                            let checks1 checks_push checks0 Result::Err "bool type allocation failed"
                                                ...
                                                Result::Ok alloc5:
                                                    ...
                                                    match add_zero_arg_function selfhost_type_arena_alloc_into_arena alloc5 bool_id:
                                                        Result::Ok alloc6:
                                                            ...
                                                            checks_exit_code shown
                                                        Result::Err _e:
                                                            let checks1 checks_push checks0 Result::Err "arity mismatch function allocation failed"
                                                            let shown checks_print_report checks1
                                                            checks_exit_code shown
                                                Result::Err _e:
                                                    let checks1 checks_push checks0 Result::Err "result mismatch function allocation failed"
                                                    let shown checks_print_report checks1
                                                    checks_exit_code shown
                                        Result::Err _e:
                                            let checks1 checks_push checks0 Result::Err "arg mismatch function allocation failed"
                                            let shown checks_print_report checks1
                                            checks_exit_code shown
                                Result::Err _e:
                                    let checks1 checks_push checks0 Result::Err "base function allocation failed"
                                    let shown checks_print_report checks1
                                    checks_exit_code shown
                        Result::Err _e:
                            let checks1 checks_push checks0 Result::Err "bool type allocation failed"
