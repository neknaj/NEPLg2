# GUI render2d row tile RLE encode cursor doctests

このファイルは、F5ch の RGBA8888 row tile RLE encode cursor owner が payload seed から ready cursor と exact total run count を作り、encoded buffer や host present へ進まないことを固定する。

source policy coverage labels:

- render2d_row_tile_rle_encode_cursor_facade_ok
- render2d_row_tile_rle_encode_cursor_seed_to_ready_cursor_ok
- render2d_row_tile_rle_encode_cursor_total_run_count_preserved_ok
- render2d_row_tile_rle_encode_cursor_start_error_owner_recovery_ok
- render2d_row_tile_rle_encode_cursor_no_status_no_drain_no_encoded_buffer_no_platform_no_fallback

## payload seed becomes ready encode cursor

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_row_tile_rle_encode_cursor\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"ready cursor\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn fail_encode_cursor_owner %fn GuiRgba8888RowTileRleEncodeCursorOwner fn i32 i32 \owner\code:
    match gui_rgba8888_row_tile_rle_encode_cursor_owner_free owner:
        Result::Err _:
            code
        Result::Ok _:
            code
...
fn encode_cursor_success %fn GuiRgba8888RowTileRleEncodeSeedOwner i32 \seed:
    match gui_rgba8888_row_tile_rle_encode_cursor_start seed:
        ...
        Result::Ok owner:
            let total_ok %bool eq gui_rgba8888_row_tile_rle_encode_cursor_owner_total_run_count &owner 3
            let restart_ok %bool eq gui_rgba8888_row_tile_rle_encode_cursor_owner_cursor_next_pixel_index &owner 0
            let pixel_count_ok %bool eq gui_rgba8888_row_tile_rle_encode_cursor_owner_cursor_pixel_count &owner 4
            let code %i32 if and total_ok and restart_ok pixel_count_ok 0 32
            fail_encode_cursor_owner owner code
...
fn run_pipeline %fn GuiRgba8888SoftwareSurfaceOwner i32 \surface:
    ...
    match gui_rgba8888_software_surface_dirty_owner_push_region_checked dirty_owner0 dirty_region_full:
        ...
        Result::Ok dirty_owner:
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 713
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_row_tile_rle_encode_cursor"
        |> test::test_report_push test::assert_eq_i32 "ready cursor" 0 actual
    let shown test::test_report_print_stdout report
    test::test_report_exit_code shown
```

## encode cursor owner constructor is not public application surface

通常の application code は encode cursor owner を直 constructor で作れない。後続 transport は `gui_rgba8888_row_tile_rle_encode_cursor_start` を通した ready cursor owner だけを受け取る。
...
fn main %impure fn void i32 \void:
    match gui_rgba8888_software_surface_create 1 1:
        ...
        Result::Ok surface:
            ...
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 714
            match gui_rgba8888_bitmap_frame_prepare dirty_owner frame_config:
                Result::Err _:
                    0
                Result::Ok frame:
                                                                        ...
                                                                        Result::Ok payload:
                                                                            match gui_rgba8888_row_tile_rle_cursor_start payload:
                                                                                Result::Err _:
                                                                                    0
                                                                                Result::Ok cursor:
                                                                                    let owner %GuiRgba8888RowTileRleEncodeCursorOwner GuiRgba8888RowTileRleEncodeCursorOwner cursor 1
                                                                                    match gui_rgba8888_row_tile_rle_encode_cursor_owner_free owner:
                                                                                        Result::Err _:
                                                                                            0
                                                                                        Result::Ok _:
