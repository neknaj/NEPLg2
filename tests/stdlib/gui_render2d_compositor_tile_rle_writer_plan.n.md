# GUI render2d compositor tile RLE writer plan

このファイルは、F5mk の RGBA8888 compositor tile RLE writer plan bridge が F5mj ready cursor owner から lower writer capacity plan へ進み、storage / packet / present / fallback へ進まないことを固定する。

source policy coverage labels:

- render2d_compositor_tile_rle_writer_plan_facade_ok
- render2d_compositor_tile_rle_writer_plan_ready_cursor_to_capacity_ok
- render2d_compositor_tile_rle_writer_plan_encoded_byte_count_checked_ok
- render2d_compositor_tile_rle_writer_plan_metadata_ok
- render2d_compositor_tile_rle_writer_plan_finish_payload_delegation_ok
- render2d_compositor_tile_rle_writer_plan_error_recovers_ready_owner_ok
- render2d_compositor_tile_rle_writer_plan_no_storage_packet_present_no_fallback

## compositor ready cursor becomes writer capacity plan

[目的/もくてき]:
- F5mj ready cursor owner の metadata を保持したまま、lower `gui_rgba8888_row_tile_rle_writer_plan_prepare` を通すことを確認します。
- writer plan では exact total run count、encoded byte count、cursor progress、metadata が保持されることを確認します。
- writer plan owner を frame entry owner へ戻せることを確認し、storage / packet / present へ進まないことを source policy label で固定します。

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_compositor_tile_rle_writer_plan_contract\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"writer plan\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn writer_plan_success %fn GuiRgba8888CompositorTileRleEncodeCursorOwner fn bool i32 \ready\ready_ok:
    match gui_rgba8888_compositor_tile_rle_writer_plan_prepare ready:
        Result::Err error:
            fail_writer_plan_error error 44
        Result::Ok owner:
            let ok %bool and ready_ok writer_plan_owner_ok &owner 409
            match gui_rgba8888_compositor_tile_rle_writer_plan_owner_finish_entry owner:
                Result::Err error:
                    fail_payload_finish error 45
                Result::Ok entry:
                    if ok:
                        then expect_complete_entry entry 46
                        else free_entry_code entry 47
...
fn cursor_start_success %fn GuiRgba8888CompositorTileRleEncodeSeedOwner fn bool i32 \seed\status_ok:
    match gui_rgba8888_compositor_tile_rle_encode_cursor_start seed:
        ...
        Result::Ok ready:
            let ready_ok %bool and:
                status_ok
                cursor_owner_ok &ready 409
            writer_plan_success ready ready_ok
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_compositor_tile_rle_writer_plan_contract"
        |> test::test_report_push test::assert_eq_i32 "writer plan" 0 actual
