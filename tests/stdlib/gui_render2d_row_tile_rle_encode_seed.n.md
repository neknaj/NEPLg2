# GUI render2d row tile RLE encode seed doctests

このファイルは、F5cg の RGBA8888 row tile RLE encode seed owner が completed count evidence から payload owner と total run count を切り出し、encoded buffer や host present へ進まないことを固定する。

source policy coverage labels:

- render2d_row_tile_rle_encode_seed_facade_ok
- render2d_row_tile_rle_encode_seed_completed_to_seed_total_ok
- render2d_row_tile_rle_encode_seed_payload_restarts_at_zero
- render2d_row_tile_rle_encode_seed_error_recovers_completed_owner_ok
- render2d_row_tile_rle_encode_seed_no_cursor_start_no_encoded_buffer_no_platform_no_fallback

## completed count evidence becomes encode payload seed

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_row_tile_rle_encode_seed\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"seed restart\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn fail_rle_cursor %fn GuiRgba8888RowTileRleCursorOwner fn i32 i32 \cursor\code:
...
fn fail_seed_owner %fn GuiRgba8888RowTileRleEncodeSeedOwner fn i32 i32 \owner\code:
    match gui_rgba8888_row_tile_rle_encode_seed_owner_free owner:
        Result::Err _:
            code
        Result::Ok _:
            code
...
fn seed_payload_restarts %fn GuiRgba8888RowTileRleEncodeSeedOwner i32 \seed:
    let total_ok %bool eq gui_rgba8888_row_tile_rle_encode_seed_owner_total_run_count &seed 3
    ...
    match gui_rgba8888_row_tile_rle_cursor_start payload:
        Result::Err error:
            fail_rle_start error 29
        Result::Ok cursor:
            let restart_ok %bool eq gui_rgba8888_row_tile_rle_cursor_next_pixel_index &cursor 0
            let code %i32 if and total_ok restart_ok 0 30
            fail_rle_cursor cursor code
            ...
            match gui_rgba8888_row_tile_rle_count_completed_prepare owner1:
                ...
                Result::Ok completed:
                    match gui_rgba8888_row_tile_rle_encode_seed_prepare completed:
                        ...
                        Result::Ok seed:
                            if status_ok:
                                then seed_payload_restarts seed
                                else fail_seed_owner seed 24
...
fn run_pipeline %fn GuiRgba8888SoftwareSurfaceOwner i32 \surface:
    ...
    match gui_rgba8888_software_surface_dirty_owner_push_region_checked dirty_owner0 dirty_region_full:
        ...
        Result::Ok dirty_owner:
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 711
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_row_tile_rle_encode_seed"
        |> test::test_report_push test::assert_eq_i32 "seed restart" 0 actual
    let shown test::test_report_print_stdout report
    test::test_report_exit_code shown
```

## encode seed owner constructor is not public application surface

通常の application code は encode seed owner を直 constructor で作れない。後続 transport は `gui_rgba8888_row_tile_rle_encode_seed_prepare` を通した payload seed だけを受け取る。
...
fn main %impure fn void i32 \void:
    match gui_rgba8888_software_surface_create 1 1:
        ...
        Result::Ok surface:
            ...
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 712
            match gui_rgba8888_bitmap_frame_prepare dirty_owner frame_config:
                Result::Err _:
                    0
                Result::Ok frame:
                                                                ...
                                                                Result::Ok tile_owner:
                                                                    match gui_rgba8888_row_tile_payload_prepare tile_owner 0:
                                                                        Result::Err _:
                                                                            0
                                                                        Result::Ok payload:
                                                                            let seed %GuiRgba8888RowTileRleEncodeSeedOwner GuiRgba8888RowTileRleEncodeSeedOwner payload 1
