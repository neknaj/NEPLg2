...
fn sfnt_glyf_contour_edge_error_is %fn Result GuiSfntSimpleGlyphContourEdge GuiSfntParseError fn GuiSfntParseErrorKind bool \result\expected:
    match result:
        Result::Err error:
            match expected:
                GuiSfntParseErrorKind::MalformedGlyfRecord:
...
fn append_success_cases %impure fn TestReport TestReport \report0:
    ...
    let report16 %TestReport match build_topology_case_sfnt 8 34 17:
        ...
        Result::Ok bytes:
            ...
            io_bytebuf_free bytes
            next_report
    let report17 %TestReport match build_topology_case_sfnt 15 22 11:
        Result::Err _message:
        ...
        Result::Ok bytes:
            ...
            io_bytebuf_free bytes
            next_report
    let report18 %TestReport match build_topology_case_sfnt 8 34 17:
        Result::Err _message:
            test_report_push report17 assert false
        Result::Ok bytes:
            let next_report %TestReport match gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 0:
                Result::Err _error:
                    test_report_push report17 assert false
                Result::Ok edge:
                    ...
                    let r1 %TestReport test_report_push report17 assert_eq_i32 "contour edge first edge index" 0 gui_sfnt_simple_glyph_contour_edge_index &edge
                    let r2 %TestReport test_report_push r1 assert_eq_i32 "contour edge first next local index" 1 gui_sfnt_simple_glyph_contour_edge_next_local_index &edge
                    let r3 %TestReport test_report_push r2 assert_eq_i32 "contour edge first end local index" 1 gui_sfnt_simple_glyph_contour_point_local_index &end
                    let r4 %TestReport test_report_push r3 assert_eq_i32 "contour edge first start absolute index" 0 gui_sfnt_simple_glyph_point_index &start_point
                    let r5 %TestReport test_report_push r4 assert_eq_i32 "contour edge first end absolute index" 1 gui_sfnt_simple_glyph_point_index &end_point
                    test_report_push r5 assert "contour edge first end contour end" gui_sfnt_simple_glyph_point_end_of_contour &end_point
            io_bytebuf_free bytes
            next_report
    let report19 %TestReport match build_topology_case_sfnt 8 34 17:
        Result::Err _message:
            test_report_push report18 assert false
        Result::Ok bytes:
            let next_report %TestReport match gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 1 1:
                Result::Err _error:
                    test_report_push report18 assert false
                Result::Ok edge:
                    let start %GuiSfntSimpleGlyphContourPoint gui_sfnt_simple_glyph_contour_edge_start &edge
                    ...
                    let start_point %GuiSfntSimpleGlyphPoint gui_sfnt_simple_glyph_contour_point_point &start
                    let end_point %GuiSfntSimpleGlyphPoint gui_sfnt_simple_glyph_contour_point_point &end
                    let r1 %TestReport test_report_push report18 assert_eq_i32 "contour edge wrap next local index" 0 gui_sfnt_simple_glyph_contour_edge_next_local_index &edge
                    let r2 %TestReport test_report_push r1 assert_eq_i32 "contour edge wrap start absolute index" 3 gui_sfnt_simple_glyph_point_index &start_point
                    test_report_push r2 assert_eq_i32 "contour edge wrap end absolute index" 2 gui_sfnt_simple_glyph_point_index &end_point
            io_bytebuf_free bytes
            next_report
    let report20 %TestReport match build_topology_case_sfnt 10 18 9:
        Result::Err _message:
        ...
        Result::Ok bytes:
            let next_report %TestReport match gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 0:
                ...
                Result::Ok edge:
                    ...
                    let start_point %GuiSfntSimpleGlyphPoint gui_sfnt_simple_glyph_contour_point_point &start
                    let end_point %GuiSfntSimpleGlyphPoint gui_sfnt_simple_glyph_contour_point_point &end
                    let r1 %TestReport test_report_push report19 assert_eq_i32 "contour edge self wrap next local index" 0 gui_sfnt_simple_glyph_contour_edge_next_local_index &edge
                    test_report_push r1 assert "contour edge self wrap same absolute index" eq gui_sfnt_simple_glyph_point_index &start_point gui_sfnt_simple_glyph_point_index &end_point
            io_bytebuf_free bytes
            next_report
    match build_topology_case_sfnt 15 22 11:
        ...
        Result::Ok bytes:
            let next_report %TestReport match gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 1:
                Result::Err _error:
                    test_report_push report20 assert false
                Result::Ok edge:
                    let start %GuiSfntSimpleGlyphContourPoint gui_sfnt_simple_glyph_contour_edge_start &edge
                    let start_point %GuiSfntSimpleGlyphPoint gui_sfnt_simple_glyph_contour_point_point &start
                    let r1 %TestReport test_report_push report20 assert_eq_i32 "contour edge signed start absolute index" 1 gui_sfnt_simple_glyph_point_index &start_point
                    let r2 %TestReport test_report_push r1 assert_eq_i32 "contour edge signed start x" 2 gui_sfnt_simple_glyph_point_x &start_point
                    test_report_push r2 assert_eq_i32 "contour edge signed start y" -6 gui_sfnt_simple_glyph_point_y &start_point
            io_bytebuf_free bytes
            next_report
...
fn append_error_cases %impure fn TestReport TestReport \report0:
    ...
    let report31 %TestReport match build_topology_case_sfnt 13 16 8:
        Result::Err _message:
    ...
    let report32 %TestReport match build_topology_case_sfnt 8 34 17:
        Result::Err _message:
            test_report_push report31 assert false
        Result::Ok bytes:
            let ok %bool sfnt_glyf_contour_edge_error_is gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 -1 GuiSfntParseErrorKind::MissingGlyphOutline
            io_bytebuf_free bytes
            test_report_push report31 assert "contour edge negative local missing" ok
    let report33 %TestReport match build_topology_case_sfnt 8 34 17:
        Result::Err _message:
            test_report_push report32 assert false
        Result::Ok bytes:
            let ok %bool sfnt_glyf_contour_edge_error_is gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 2 GuiSfntParseErrorKind::MissingGlyphOutline
            io_bytebuf_free bytes
            test_report_push report32 assert "contour edge local count missing" ok
    match build_topology_case_sfnt 13 16 8:
        Result::Err _message:
            test_report_push report33 assert false
        Result::Ok bytes:
            let ok %bool sfnt_glyf_contour_edge_error_is gui_sfnt_lookup_simple_glyph_contour_edge &bytes none glyph1 0 0 GuiSfntParseErrorKind::MalformedGlyfRecord
            io_bytebuf_free bytes
            test_report_push report33 assert "contour edge x coordinate overrun" ok
