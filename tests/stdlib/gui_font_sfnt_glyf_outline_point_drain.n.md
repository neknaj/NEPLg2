# GUI font SFNT glyf outline point drain doctests

このファイルは、F5p の point read drain が terminal-before-budget と budget-before-F5o の順序を守り、F5o の失敗を typed sub-error として保持することを検査する。

## point read drain respects budget and terminal state
...
fn summary_cursor_is %fn &GuiSfntSimpleGlyphOutlinePointReadDrainSummary fn i32 bool \summary\expected:
    let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_drain_summary_cursor summary
    eq expected gui_sfnt_simple_glyph_outline_point_read_cursor_next_point_index &cursor

fn summary_count_is %fn &GuiSfntSimpleGlyphOutlinePointReadDrainSummary fn i32 bool \summary\expected:
    eq expected gui_sfnt_simple_glyph_outline_point_read_drain_summary_points_read summary

fn summary_last_point_index_is %fn &GuiSfntSimpleGlyphOutlinePointReadDrainSummary fn i32 bool \summary\expected:
    match gui_sfnt_simple_glyph_outline_point_read_drain_summary_last_point summary:
        Option::None:
            false
        Option::Some point:
            eq expected gui_sfnt_simple_glyph_point_index &point

fn summary_last_point_is_none %fn &GuiSfntSimpleGlyphOutlinePointReadDrainSummary bool \summary:
    match gui_sfnt_simple_glyph_outline_point_read_drain_summary_last_point summary:
        Option::None:
            true
        Option::Some point:
...
fn point_drain_full_end_ok %fn &ByteBuf fn GuiSfntTableRecord fn GuiSfntSimpleGlyphPointStream fn &GuiSfntSimpleGlyphOutlineStorage bool \bytes\glyf\stream\storage:
    let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_cursor 0
    match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget bytes glyf stream storage cursor 4:
        Result::Err _error:
            false
        Result::Ok drain:
            match drain:
                GuiSfntSimpleGlyphOutlinePointReadDrain::End summary:
                    and summary_cursor_is &summary 4 and summary_count_is &summary 4 summary_last_point_index_is &summary 3
                GuiSfntSimpleGlyphOutlinePointReadDrain::StepBudgetExhausted _summary:
                    false

fn point_drain_partial_budget_exhausted_ok %fn &ByteBuf fn GuiSfntTableRecord fn GuiSfntSimpleGlyphPointStream fn &GuiSfntSimpleGlyphOutlineStorage bool \bytes\glyf\stream\storage:
    let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_cursor 0
    match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget bytes glyf stream storage cursor 2:
        Result::Err _error:
            false
        Result::Ok drain:
            match drain:
                GuiSfntSimpleGlyphOutlinePointReadDrain::End _summary:
                    false
                GuiSfntSimpleGlyphOutlinePointReadDrain::StepBudgetExhausted summary:
                    and summary_cursor_is &summary 2 and summary_count_is &summary 2 summary_last_point_index_is &summary 1

fn point_drain_zero_budget_nonterminal_ok %fn &ByteBuf fn GuiSfntTableRecord fn GuiSfntSimpleGlyphPointStream fn &GuiSfntSimpleGlyphOutlineStorage bool \bytes\glyf\stream\storage:
    let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_cursor 0
    match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget bytes glyf stream storage cursor 0:
        Result::Err _error:
            false
        Result::Ok drain:
            match drain:
                GuiSfntSimpleGlyphOutlinePointReadDrain::End _summary:
                    false
                GuiSfntSimpleGlyphOutlinePointReadDrain::StepBudgetExhausted summary:
                    and summary_cursor_is &summary 0 and summary_count_is &summary 0 summary_last_point_is_none &summary

fn point_drain_zero_budget_terminal_ok %fn &ByteBuf fn GuiSfntTableRecord fn GuiSfntSimpleGlyphPointStream fn &GuiSfntSimpleGlyphOutlineStorage bool \bytes\glyf\stream\storage:
    ...
    match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget bytes glyf stream storage cursor 0:
        ...
        Result::Ok drain:
            match drain:
                GuiSfntSimpleGlyphOutlinePointReadDrain::End summary:
                    and summary_cursor_is &summary 4 and summary_count_is &summary 0 summary_last_point_is_none &summary
                GuiSfntSimpleGlyphOutlinePointReadDrain::StepBudgetExhausted _summary:
                    false

fn point_drain_cursor_too_far_ok %fn &ByteBuf fn GuiSfntTableRecord fn GuiSfntSimpleGlyphPointStream fn &GuiSfntSimpleGlyphOutlineStorage bool \bytes\glyf\stream\storage:
    let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_cursor 5
    match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget bytes glyf stream storage cursor 4:
        Result::Ok _drain:
            false
        Result::Err error:
            match gui_sfnt_simple_glyph_outline_point_read_drain_error_kind &error:
                GuiSfntSimpleGlyphOutlinePointReadDrainErrorKind::CursorOutOfRange:
                    true
                _:
                    false

fn point_drain_wraps_step_read_failure_ok %impure fn void bool \void:
    let glyph %GuiGlyphId unwrap_ok gui_glyph_id_result 115
            ...
            match prepare_full_storage &capacity &limit:
                ...
                Result::Ok storage:
                    match bytes2_result 9 4:
                        ...
                        Result::Ok bytes:
                            let cursor %GuiSfntSimpleGlyphOutlinePointReadCursor gui_sfnt_simple_glyph_outline_point_read_cursor 0
                            let result %bool match gui_sfnt_simple_glyph_outline_storage_read_point_drain_budget &bytes glyf stream &storage cursor 1:
                                Result::Ok _drain:
                                    false
                                Result::Err error:
                                    let kind_ok %bool match gui_sfnt_simple_glyph_outline_point_read_drain_error_kind &error:
                                        GuiSfntSimpleGlyphOutlinePointReadDrainErrorKind::StepReadFailed:
                                            true
                                        _:
                                            false
                                    let step_ok %bool match gui_sfnt_simple_glyph_outline_point_read_drain_error_step_error &error:
                                        Option::None:
                                            false
...
fn point_drain_budget_states_ok %impure fn void bool \void:
    let glyph %GuiGlyphId unwrap_ok gui_glyph_id_result 110
    let topology %GuiSfntSimpleGlyphTopology make_topology glyph 2 4
                ...
                Result::Ok storage:
                    match bytes4_result 1 0 1 0:
                        Result::Err _message:
                            ...
                            false
                        Result::Ok bytes:
                            let full_ok %bool point_drain_full_end_ok &bytes glyf stream &storage
                            let partial_ok %bool point_drain_partial_budget_exhausted_ok &bytes glyf stream &storage
                            let zero_nonterminal_ok %bool point_drain_zero_budget_nonterminal_ok &bytes glyf stream &storage
                            let zero_terminal_ok %bool point_drain_zero_budget_terminal_ok &bytes glyf stream &storage
                            let cursor_ok %bool point_drain_cursor_too_far_ok &bytes glyf stream &storage
                            io_bytebuf_free bytes
                            gui_sfnt_simple_glyph_outline_storage_free storage
...
fn main %impure fn void i32 \void:
    let budget_ok %bool point_drain_budget_states_ok
    let wrap_ok %bool point_drain_wraps_step_read_failure_ok
    test_assertion_exit_code assert "point read drain contract" and budget_ok wrap_ok
```
