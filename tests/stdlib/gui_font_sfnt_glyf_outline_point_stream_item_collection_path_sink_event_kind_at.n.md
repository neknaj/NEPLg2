# GUI font SFNT glyf outline point stream item collection path sink event kind at doctests

このファイルは、F5ac の collection-backed path sink event kind slot lookup が F5ab path sink event kind pair lookup と既存 pure typed slot projection だけを通ることを固定する。

source policy coverage labels:

- path_sink_event_kind_at_first_line_ok
- path_sink_event_kind_at_second_line_ok
- path_sink_event_kind_at_no_segment_skip_ok
- path_sink_event_kind_at_error_propagates_ok
- no_vec_no_fallback_no_sink_traversal

## point stream item collection path sink event kind at smoke
...
fn line_event_kind_slot_contract_ok %impure fn void bool \void:
    ...
    match build_line_collection &capacity:
        Result::Err _message:
            false
        Result::Ok collection:
            match gui_sfnt_simple_glyph_outline_point_stream_item_collection_path_sink_event_kind_at &collection 0 0 GuiSfntSimpleGlyphPathSinkEventSlot::First:
                Result::Err _error:
                    gui_sfnt_simple_glyph_outline_point_stream_item_collection_free collection
                    false
                Result::Ok first_kind:
                    match gui_sfnt_simple_glyph_outline_point_stream_item_collection_path_sink_event_kind_at &collection 0 0 GuiSfntSimpleGlyphPathSinkEventSlot::Second:
                        Result::Err _error:
                            gui_sfnt_simple_glyph_outline_point_stream_item_collection_free collection
                            false
                        Result::Ok second_kind:
...
fn main %impure fn void i32 \void:
    let ok1 %bool line_event_kind_slot_contract_ok
    test_assertion_exit_code assert "point stream item collection path sink event kind at smoke" ok1
```
