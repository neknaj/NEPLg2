# GUI render2d compositor tile RLE encode seed

このファイルは、F5mi の RGBA8888 compositor tile RLE encode seed bridge が F5mh completed owner から lower payload seed へ進み、cursor restart / encode cursor / writer / storage / packet / present / fallback へ進まないことを固定する。

source policy coverage labels:

- render2d_compositor_tile_rle_encode_seed_facade_ok
- render2d_compositor_tile_rle_encode_seed_completed_to_seed_total_ok
- render2d_compositor_tile_rle_encode_seed_metadata_ok
- render2d_compositor_tile_rle_encode_seed_finish_payload_delegation_ok
- render2d_compositor_tile_rle_encode_seed_error_recovers_completed_owner_ok
- render2d_compositor_tile_rle_encode_seed_no_cursor_start_no_encode_storage_packet_present_no_fallback

## compositor completed count evidence becomes encode seed

[目的/もくてき]:
- F5mh completed owner の metadata を保持したまま、lower `gui_rgba8888_row_tile_rle_encode_seed_prepare` を通すことを確認します。
- success seed では exact total run count と metadata が保持されることを確認します。
- seed owner を frame entry owner へ戻せることを確認し、cursor restart / encode / present へ進まないことを source policy label で固定します。

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_compositor_tile_rle_encode_seed_contract\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"encode seed\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn fail_seed_owner %fn GuiRgba8888CompositorTileRleEncodeSeedOwner fn i32 i32 \owner\code:
    match gui_rgba8888_compositor_tile_rle_encode_seed_owner_free owner:
        Result::Ok _:
            code
        Result::Err _:
            code
...
fn seed_owner_ok %fn &GuiRgba8888CompositorTileRleEncodeSeedOwner fn i32 bool \owner\frame_id:
    ...
    and:
        ...
        eq gui_rgba8888_compositor_tile_rle_encode_seed_owner_total_run_count owner 2
...
fn seed_prepare_success %fn GuiRgba8888CompositorTileRleCountCompletedOwner fn bool i32 \completed\status_ok:
    match gui_rgba8888_compositor_tile_rle_encode_seed_prepare completed:
        Result::Err error:
            fail_seed_error error 40
        Result::Ok seed:
            let ok %bool and:
                status_ok
                seed_owner_ok &seed 398
            match gui_rgba8888_compositor_tile_rle_encode_seed_owner_finish_entry seed:
                Result::Err error:
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_compositor_tile_rle_encode_seed_contract"
        |> test::test_report_push test::assert_eq_i32 "encode seed" 0 actual
