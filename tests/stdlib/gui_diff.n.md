...
fn main %impure fn unit i32 \unit:
    let old_root %WidgetDescriptor widget_label (widget_id 1) "old" (layout_hint_fixed 4 1)
    let new_root %WidgetDescriptor widget_label (widget_id 1) "new" (layout_hint_fixed 4 1)
    ...
    let invalidation_check match gui_invalidation_widget_id (view_tree_invalidation &old_tree &new_tree):
        Option::Some id:
            assert_eq_i32 1 widget_id_value id
        Option::None:
            assert false
    let checks checks_push (checks_push checks_new changed_check) invalidation_check
...
fn main %fn unit i32 \unit:
    let root %WidgetDescriptor widget_label (widget_id 1) "root" (layout_hint_fixed 4 1)
    let child %WidgetDescriptor widget_button (button_config (widget_id 2) "Run" (action_id 3)) (layout_hint_fixed 4 1)
...
fn main %fn unit i32 \unit:
    let root %WidgetDescriptor widget_label (widget_id 1) "root" (layout_hint_fixed 4 1)
    let child_old %WidgetDescriptor widget_button (button_config (widget_id 2) "Run" (action_id 3)) (layout_hint_fixed 4 1)
    let child_new %WidgetDescriptor widget_button (button_config (widget_id 2) "Run" (action_id 4)) (layout_hint_fixed 4 1)
    let old_tree %ViewTree unwrap_ok view_tree_add_child (view_tree_single root) child_old
    let new_tree %ViewTree unwrap_ok view_tree_add_child (view_tree_single root) child_new
    match gui_invalidation_widget_id (view_tree_invalidation &old_tree &new_tree):
        Option::Some id:
            widget_id_value id
        Option::None:
            9
