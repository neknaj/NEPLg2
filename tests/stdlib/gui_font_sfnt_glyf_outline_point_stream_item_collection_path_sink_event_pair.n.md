# GUI font SFNT glyf outline point stream item collection path sink event pair doctests

このファイルは、F5aa の collection-backed path sink event pair lookup が F5z path command pair lookup と既存 pure sink event pair projection だけを通ることを固定する。

source policy coverage labels:

- path_sink_event_pair_line_ok
- path_sink_event_pair_quadratic_ok
- path_sink_event_pair_no_segment_skip_ok
- path_sink_event_pair_curve_error_propagates_ok
- no_vec_no_fallback_no_sink_traversal

## point stream item collection path sink event pair smoke
...
fn line_event_pair_contract_ok %impure fn void bool \void:
    let glyph %GuiGlyphId unwrap_ok gui_glyph_id_result 511
    let capacity %GuiSfntSimpleGlyphOutlineStorageCapacity make_capacity glyph 1 2
    match build_line_collection &capacity:
        Result::Err _message:
            false
        Result::Ok collection:
            match gui_sfnt_simple_glyph_outline_point_stream_item_collection_path_sink_event_pair &collection 0 0:
                Result::Err _error:
                ...
                Result::Ok event_pair:
                    ...
                    let move_ok %bool match first_command:
                        ...
                        _:
                            false
                    let draw_ok %bool match second_command:
...
fn main %impure fn void i32 \void:
    let ok1 %bool line_event_pair_contract_ok
    test_assertion_exit_code assert "point stream item collection path sink event pair smoke" ok1
```
