...
fn main <()*>i32> ():
    ...
    match lex_all source:
        Result::Ok tokens:
            let token_len <i32> len_ref<SelfhostToken> &tokens
            let checks1 <Vec<Result<(),str>>> checks_push checks0 check_eq_i32 17 token_len
            let checks2 <Vec<Result<(),str>>> check_token checks1 &tokens 4 "DirWasm" "#wasm:"
            let checks3 <Vec<Result<(),str>>> check_token checks2 &tokens 6 "Indent" ""
            let checks4 <Vec<Result<(),str>>> check_token checks3 &tokens 7 "WasmText" "local.get 0"
            let checks5 <Vec<Result<(),str>>> check_token checks4 &tokens 9 "Dedent" ""
            let checks6 <Vec<Result<(),str>>> check_token checks5 &tokens 10 "DirLlvmIr" "#llvmir:"
            let checks7 <Vec<Result<(),str>>> check_token checks6 &tokens 12 "Indent" ""
            let checks8 <Vec<Result<(),str>>> check_token checks7 &tokens 13 "LlvmIrText" "ret i32 0"
            let checks9 <Vec<Result<(),str>>> check_token checks8 &tokens 15 "Dedent" ""
            let checks10 <Vec<Result<(),str>>> check_token checks9 &tokens 16 "Eof" ""
            free<SelfhostToken> tokens
            let shown <Vec<Result<(),str>>> checks_print_report checks10
            checks_exit_code shown
        Result::Err diag:
            let _msg <str> field::get diag "message"
            let checks1 <Vec<Result<(),str>>> checks_push checks0 Result<(),str>::Err "lexer returned Err"
