# GUI render2d row tile RLE writer plan doctests

このファイルは、F5ci の RGBA8888 row tile RLE writer plan owner が ready cursor owner から exact encoded byte capacity を作り、encoded buffer や host present へ進まないことを固定する。

source policy coverage labels:

- render2d_row_tile_rle_writer_plan_facade_ok
- render2d_row_tile_rle_writer_plan_ready_cursor_to_capacity_ok
- render2d_row_tile_rle_writer_plan_encoded_byte_count_checked_ok
- render2d_row_tile_rle_writer_plan_owner_recovery_ok
- render2d_row_tile_rle_writer_plan_no_status_no_drain_no_payload_read_no_encoded_buffer_no_platform_no_fallback

## ready cursor becomes writer capacity plan

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_row_tile_rle_writer_plan\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"writer plan byte count\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn writer_plan_success %fn GuiRgba8888RowTileRleEncodeCursorOwner i32 \ready:
    match gui_rgba8888_row_tile_rle_writer_plan_prepare ready:
        ...
        Result::Ok owner:
            let total_ok %bool eq gui_rgba8888_row_tile_rle_writer_plan_owner_total_run_count &owner 3
            let bytes_ok %bool eq gui_rgba8888_row_tile_rle_writer_plan_owner_encoded_byte_count &owner 36
            let cursor_ok %bool eq gui_rgba8888_row_tile_rle_writer_plan_owner_cursor_next_pixel_index &owner 0
            let pixel_count_ok %bool eq gui_rgba8888_row_tile_rle_writer_plan_owner_cursor_pixel_count &owner 4
            let code %i32 if and total_ok and bytes_ok and cursor_ok pixel_count_ok 0 42
            fail_writer_plan_owner owner code
...
fn run_pipeline %fn GuiRgba8888SoftwareSurfaceOwner i32 \surface:
    ...
    match gui_rgba8888_software_surface_dirty_owner_push_region_checked dirty_owner0 dirty_region_full:
        ...
        Result::Ok dirty_owner:
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 715
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_row_tile_rle_writer_plan"
        |> test::test_report_push test::assert_eq_i32 "writer plan byte count" 0 actual
    let shown test::test_report_print_stdout report
    test::test_report_exit_code shown
```

## writer plan owner constructor is not public application surface

通常の application code は writer plan owner を直 constructor で作れない。後続 writer は `gui_rgba8888_row_tile_rle_writer_plan_prepare` を通した capacity plan owner だけを受け取る。
...
fn main %impure fn void i32 \void:
    match gui_rgba8888_software_surface_create 1 1:
        ...
        Result::Ok surface:
            ...
            let frame_config %GuiRgba8888BitmapFrameConfig GuiRgba8888BitmapFrameConfig 716
            match gui_rgba8888_bitmap_frame_prepare dirty_owner frame_config:
                Result::Err _:
                    0
                Result::Ok frame:
                                                                        ...
                                                                        Result::Ok payload:
                                                                            match gui_rgba8888_row_tile_rle_cursor_start payload:
                                                                                Result::Err _:
                                                                                    0
                                                                                Result::Ok cursor:
                                                                                    let owner %GuiRgba8888RowTileRleWriterPlanOwner GuiRgba8888RowTileRleWriterPlanOwner cursor 1 12
                                                                                    match gui_rgba8888_row_tile_rle_writer_plan_owner_free owner:
                                                                                        Result::Err _:
                                                                                            0
                                                                                        Result::Ok _:
