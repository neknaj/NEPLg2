# GUI render2d compositor tile RLE write cursor

このファイルは、F5mm の RGBA8888 compositor tile RLE write cursor start bridge が F5ml storage owner から lower write cursor start へ進み、write step / encoded seal / packet / present / fallback へ進まないことを固定する。

source policy coverage labels:

- render2d_compositor_tile_rle_write_cursor_facade_ok
- render2d_compositor_tile_rle_write_cursor_storage_to_start_ok
- render2d_compositor_tile_rle_write_cursor_count_and_progress_ok
- render2d_compositor_tile_rle_write_cursor_written_counts_zero_ok
- render2d_compositor_tile_rle_write_cursor_metadata_ok
- render2d_compositor_tile_rle_write_cursor_finish_payload_recovery_ok
- render2d_compositor_tile_rle_write_cursor_start_error_recovery_source_policy_ok
- render2d_compositor_tile_rle_write_cursor_free_delegates_lower_write_cursor_ok
- render2d_compositor_tile_rle_write_cursor_no_step_encoded_packet_present_no_fallback

## compositor storage becomes write cursor owner

[目的/もくてき]:
- F5ml storage owner の metadata を保持したまま、lower `gui_rgba8888_row_tile_rle_write_cursor_start` を通すことを確認します。
- write cursor owner では exact total run count、encoded byte count、cursor progress、written count 0、metadata が保持されることを確認します。
- write cursor owner は payload recovery / free で閉じ、write step、encoded seal、packet、present へ進まないことを source policy label で固定します。

neplg2:test[stdio, normalize_newlines]
stdout: "test_report name=\"gui_render2d_compositor_tile_rle_write_cursor_contract\" count=1 failed=0\nassertion index=0 status=ok kind=eq_i32 label=\"write_cursor\" expected=\"0\" actual=\"0\" message=\"\"\n"
exit_code: 0
```neplg2
#entry main
#target std
...
fn write_cursor_owner_ok %fn &GuiRgba8888CompositorTileRleWriteCursorOwner fn i32 bool \owner\frame_id:
    let metadata %GuiRgba8888CompositorFrameEntryMetadata gui_rgba8888_compositor_tile_rle_write_cursor_owner_metadata owner
    and:
        metadata_ok &metadata frame_id
        and:
            eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_total_run_count owner 2
            and:
                eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_encoded_byte_count owner 24
                and:
                    eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_written_run_count owner 0
                    and:
                        eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_written_byte_count owner 0
                        and:
                            eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_cursor_next_pixel_index owner 0
                            eq gui_rgba8888_compositor_tile_rle_write_cursor_owner_cursor_pixel_count owner 2
...
fn write_cursor_finish_payload_success %fn GuiRgba8888CompositorTileRleWriteCursorOwner fn bool i32 \owner\owner_ok:
    match gui_rgba8888_compositor_tile_rle_write_cursor_owner_finish_payload owner:
        Result::Err error:
            fail_write_cursor_finish error 65
        Result::Ok payload:
            match gui_rgba8888_compositor_tile_payload_owner_finish_entry payload:
                Result::Err error:
                    fail_payload_finish error 66
                Result::Ok entry:
                    if owner_ok:
                        then expect_complete_entry entry 67
                        else free_entry_code entry 68

fn write_cursor_free_success %fn GuiRgba8888CompositorTileRleWriteCursorOwner fn bool i32 \owner\owner_ok:
    match gui_rgba8888_compositor_tile_rle_write_cursor_owner_free owner:
        Result::Err _:
            71
        Result::Ok _:
            if owner_ok:
...
fn write_cursor_start_success %fn GuiRgba8888CompositorTileRleStorageOwner fn bool fn i32 i32 \storage\storage_ok\mode:
    match gui_rgba8888_compositor_tile_rle_write_cursor_start storage:
        Result::Err error:
            fail_write_cursor_start error 64
        Result::Ok owner:
            let owner_ok %bool and storage_ok write_cursor_owner_ok &owner 509
            if eq mode 0:
                then write_cursor_finish_payload_success owner owner_ok
                else write_cursor_free_success owner owner_ok
...
fn storage_prepare_success %fn GuiRgba8888CompositorTileRleWriterPlanOwner fn bool fn i32 i32 \plan\plan_ok\mode:
    match gui_rgba8888_compositor_tile_rle_storage_prepare plan:
        ...
        Result::Ok storage:
            let storage_ok %bool and plan_ok storage_owner_ok &storage 509
            write_cursor_start_success storage storage_ok mode
...
fn writer_plan_success %fn GuiRgba8888CompositorTileRleEncodeCursorOwner fn bool fn i32 i32 \ready\ready_ok\mode:
    match gui_rgba8888_compositor_tile_rle_writer_plan_prepare ready:
        ...
        Result::Ok owner:
            storage_prepare_success owner ready_ok mode
...
fn main %impure fn void i32 \void:
    ...
    let report:
        test::test_report_new "gui_render2d_compositor_tile_rle_write_cursor_contract"
        |> test::test_report_push test::assert_eq_i32 "write_cursor" 0 free_result
    let shown test::test_report_print_stdout report
    test::test_report_exit_code shown
```

## malformed storage owner is not public application surface

通常の application code は malformed compositor storage owner を直 constructor で作れない。F5mm start error recovery の read-before-consume 順序と storage owner recovery は `nodesrc/test_web_gui_font_rendering_contract.js` の source policy で固定する。
...
fn forge_compositor_storage %fn GuiRgba8888RowTileRleStorageOwner fn GuiRgba8888CompositorFrameEntryMetadata GuiRgba8888CompositorTileRleStorageOwner \lower\metadata:
    GuiRgba8888CompositorTileRleStorageOwner lower metadata
